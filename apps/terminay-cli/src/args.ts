...
const ALIASES: Readonly<Record<string, DaemonCommand>> = Object.freeze({ 'pairing-url': 'qr-code' });
 ...
 */
const ACCEPTED: Readonly<Record<DaemonCommand, readonly string[]>> = Object.freeze({
	install: [
		'--system',
		'--user',
		'--run-as',
...
function parsePort(raw: string): number {
	if (!/^\d{1,5}$/u.test(raw)) fail('--port must be a whole number between 1 and 65535');
	const port = Number(raw);
	if (port < 1 || port > 65535) fail('--port must be a whole number between 1 and 65535');
	return port;
}

function parseMode(raw: string): PairingMode {
	if (raw !== 'hosted' && raw !== 'direct') fail('--mode must be hosted or direct');
	return raw;
}

export function parseCommandLine(argv: readonly string[]): ParsedCommandLine {
	const noOptions: DaemonOptions = Object.freeze({ allowDowngrade: false, purge: false, yes: false, wait: true });
	if (argv.length === 0 || argv[0] === '--help' || argv[0] === 'help') {
		return Object.freeze({ command: 'help', options: noOptions });
	}
	if (argv[0] !== 'daemon') fail(`unknown command: ${argv[0]}. The only command group is \`daemon\`.`);

	const rest = argv.slice(1);
	if (rest.length === 0) return Object.freeze({ command: 'help', options: noOptions });
	if (rest.includes('--help')) return Object.freeze({ command: 'help', options: noOptions });

	const requested = rest[0] as string;
	const command = ALIASES[requested] ?? (COMMANDS.includes(requested as DaemonCommand) ? (requested as DaemonCommand) : undefined);
	...
	for (let index = 1; index < rest.length; index += 1) {
		...
		if (VALUE_FLAGS.includes(name)) {
			if (!accepted.includes(name)) fail(`${name} is not accepted by \`daemon ${command}\``);
			let value: string;
			if (equals >= 0) {
	...
	if (booleans.has('--system') && booleans.has('--user')) fail('--system and --user cannot both be given');

	let ref: string | undefined;
	let approvalId: string | undefined;
	if (REF_COMMANDS.includes(command)) {
		if (positional.length > 1) fail(`\`daemon ${command}\` takes at most one version reference`);
		ref = positional[0];
	} else if (APPROVAL_ID_COMMANDS.includes(command)) {
		if (positional.length !== 1) fail(`\`daemon ${command}\` requires exactly one approval id`);
		approvalId = positional[0] as string;
		if (!APPROVAL_ID.test(approvalId)) fail('approval id is not a valid identifier');
	...
	const scope = booleans.has('--system') ? 'system' : booleans.has('--user') ? 'user' : undefined;

	const options: DaemonOptions = Object.freeze({
		...(scope === undefined ? {} : { scope }),
		...(values.has('--run-as') ? { runAs: values.get('--run-as') as string } : {}),
		...(port === undefined ? {} : { port: parsePort(port) }),
		...(values.has('--direct-origin') ? { directOrigin: values.get('--direct-origin') as string } : {}),
		...(values.has('--hosted-domain') ? { hostedDomain: values.get('--hosted-domain') as string } : {}),
		...(values.has('--expose') ? { expose: values.get('--expose') as string } : {}),
		...(values.has('--project-root') ? { projectRoot: values.get('--project-root') as string } : {}),
