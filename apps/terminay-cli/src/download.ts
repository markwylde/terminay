...
export async function downloadAsset(url: string, prefix: string, name = basename(new URL(url).pathname)): Promise<DownloadedAsset> {
	...
	try {
		const response = await openStream(url, '*/*', undefined, existing > 0 ? { range: `bytes=${existing}-` } : undefined);
		...
		if (Number.isFinite(declared) && declared + (resuming ? existing : 0) > MAX_ASSET_BYTES) {
			response.resume();
			throw new Error(`refusing to download ${url}: it is larger than the ${MAX_ASSET_BYTES}-byte limit`);
		}
		await pipeline(response, createWriteStream(target, resuming ? { flags: 'a' } : { flags: 'w', mode: 0o600 }));
		const bytes = await sizeOf(target);
		if (bytes === 0) throw new Error(`downloading ${url} produced an empty file`);
		if (bytes > MAX_ASSET_BYTES) throw new Error(`${url} is larger than the ${MAX_ASSET_BYTES}-byte limit`);
		return Object.freeze({ path: target, sha256: await hashFile(target), bytes });
	} catch (error) {
...
export async function downloadText(url: string): Promise<string> {
	...
	if (response.status !== 200) throw new Error(`downloading ${url} failed with status ${response.status}`);
