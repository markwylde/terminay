...
export function parseArchiveManifest(value: unknown): ArchiveManifest {
	if (typeof value !== 'object' || value === null)
		fail('its manifest is not an object');
	const manifest = value as Record<string, unknown>;
	if (manifest.schemaVersion !== 1)
		fail('its manifest has an unsupported schema version');
	if (manifest.artifact !== 'terminay-server')
		fail(`its manifest describes ${String(manifest.artifact)}`);
	for (const field of [
		'target',
		'revision',
		'version',
		...
		if (
			...
			(manifest[field] as string).length === 0
		)
			fail(`its manifest is missing ${field}`);
	}
	if (!/^[a-f0-9]{40}$/u.test(manifest.revision as string))
		fail('its manifest revision is not a commit sha');
	const entrypoints = manifest.entrypoints as
		| Record<string, unknown>
		| undefined;
	if (typeof entrypoints?.server !== 'string')
		fail('its manifest names no server entrypoint');
	if (!Array.isArray(manifest.files) || manifest.files.length === 0)
		fail('its manifest lists no files');
	for (const file of manifest.files) {
		const entry = file as Record<string, unknown>;
		if (typeof entry.path !== 'string' || !SAFE_PATH.test(entry.path))
			fail(`its manifest lists an unsafe path: ${String(entry.path)}`);
		if (!/^[a-f0-9]{64}$/u.test(String(entry.sha256)))
			fail(`its manifest has no digest for ${entry.path}`);
		if (!Number.isSafeInteger(entry.size))
			fail(`its manifest has no size for ${entry.path}`);
		if (!/^[0-7]{3}$/u.test(String(entry.mode)))
			fail(`its manifest has no mode for ${entry.path}`);
	}
...
export async function readArchiveManifest(
	...
	const raw = await readFile(join(root, ARCHIVE_MANIFEST), 'utf8').catch(
		() => undefined,
	);
 ...
 */
export async function validateUnpackedArchive(
	root: string,
	...
	for (const [field, wanted] of Object.entries(expected)) {
		...
		if (actual !== wanted)
			fail(
				`its manifest records ${field} ${String(actual)}, but ${String(wanted)} was expected`,
			);
	}
	for (const file of manifest.files) {
		...
		if (info.size !== file.size)
			fail(
				`${file.path} is ${info.size} bytes, but its manifest records ${file.size}`,
			);
		if ((await digest(absolute)) !== file.sha256)
			fail(`${file.path} does not match the digest its manifest records`);
	}
	const entrypoint = join(root, manifest.entrypoints.server);
	const launcher = await lstat(entrypoint).catch(() => undefined);
	if (launcher === undefined || !launcher.isFile())
		fail(`its server entrypoint ${manifest.entrypoints.server} is missing`);
	if ((launcher.mode & 0o111) === 0)
		fail(
			`its server entrypoint ${manifest.entrypoints.server} is not executable`,
		);
	return manifest;
}
