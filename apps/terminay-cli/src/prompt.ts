...
export function isInteractive(streams: PromptStreams = defaultStreams()): boolean {
...
export async function confirm(question: string, streams: PromptStreams = defaultStreams(), fallback = false): Promise<boolean> {
	const suffix = fallback ? '[Y/n]' : '[y/N]';
	const answer = (await ask(`${question} ${suffix} `, streams)).trim().toLowerCase();
