...
export function archiveName(version: string, architecture: Architecture): string {
	return `terminay-server-${version}-linux-${architecture}.tar.gz`;
}

function assetUrls(options: ResolveOptions, tag: string, version: string): ResolvedAssets {
...
function readReleaseJson(document: unknown): ReleasePayload | undefined {
	...
	return Object.freeze({
		...
		assetNames: Object.freeze(
			assets
				.map((asset) => (typeof asset === 'object' && asset !== null ? (asset as Record<string, unknown>).name : undefined))
 ...
 */
async function readReleaseHtml(options: ResolveOptions, tag: string): Promise<ReleasePayload> {
	...
	if (response.status !== 200) throw new Error(`release ${tag} was not found (GitHub returned ${response.status})`);
	...
	return Object.freeze({ tagName: tag, publishedAt: undefined, assetNames: Object.freeze([...names]) });
}

async function readRelease(options: ResolveOptions, path: string, tag: string): Promise<ReleasePayload> {
	try {
...
async function readLatestTag(options: ResolveOptions): Promise<string> {
	...
	if (match?.[1] === undefined) throw new Error('no tagged release could be resolved for this repository');
	return decodeURIComponent(match[1]);
}

function requireArchitectureAsset(payload: ReleasePayload, options: ResolveOptions, version: string): void {
	const name = archiveName(version, options.architecture);
...
async function resolveTag(options: ResolveOptions, tag: string): Promise<ResolvedRef> {
...
async function resolveSource(options: ResolveOptions, ref: string): Promise<ResolvedRef> {
	const remote = `${webBase(options)}/${repository(options)}.git`;
	if (COMMIT_SHA.test(ref)) {
		return Object.freeze({ channel: 'source' as const, version: ref.slice(0, 12), revision: ref, sourceRef: ref });
	}
	let stdout: string;
	try {
		({ stdout } = await execFileAsync('git', ['ls-remote', remote, ref], { timeout: 60_000 }));
	} catch (error) {
	...
	return Object.freeze({ channel: 'source' as const, version: revision.slice(0, 12), revision, sourceRef: ref });
}
...
export async function resolveRef(ref: string | undefined, options: ResolveOptions): Promise<ResolvedRef> {
	if (ref === undefined) return resolveTag(options, await readLatestTag(options));
	if (ref === 'latest') return resolveTag(options, await readLatestTag(options));
