...
import { UpgradeError, runUpgrade } from '../dist/commands/upgrade.js';
...
async function withMachine(options, run) {
	...
	fake.install('id', '');
	const releases = await startReleaseFixture({ directory: home, releases: options.releases });
	const health = await startFakeHealthServer(options.health ?? [{ status: 'ok', ready: true, version: '4.1.1' }]);
	const lines = [];
	try {
		await run({
			home,
...
async function pointHealthAtFixture(layout, port) {
	...
	await writeFile(recordPath, `${JSON.stringify({ ...record, healthPort: port }, null, 2)}\n`);
}

test('install resolves, verifies, unpacks, writes the unit, and starts the service', async () => {
	await withMachine(
		{ releases: [{ tag: 'v4.1.1', version: '4.1.1', revision: 'a'.repeat(40), latest: true, publishedAt: '2026-09-08T00:00:00Z' }] },
		async ({ home, fake, lines, layout, installDependencies }) => {
			const result = await runInstall(undefined, { ...OPTIONS, scope: 'user', directOrigin: 'https://box.example.test:8443' }, installDependencies);
			...
			assert.ok(invocations.includes('systemctl --user enable --now terminay-server.service'));
			assert.ok(invocations.some((line) => line.startsWith('loginctl enable-linger')), 'a user service must linger');
			...
			const environment = parseEnvironmentFile(await readFile(layout.environmentFile, 'utf8'));
			assert.equal(environment.TERMINAY_SERVER_ID, 'test-box');
			assert.equal(environment.TERMINAY_EXPOSE, 'hosted,direct');
			assert.equal(environment.TERMINAY_DIRECT_ORIGIN, 'https://box.example.test:8443');
			...
			assert.equal(await stat(join(layout.prefix, '.downloads')).catch(() => undefined), undefined);

			const output = lines.join('\n');
			assert.match(output, /Checksum and signature verified\./u);
			assert.match(output, /direct URL   https:\/\/box\.example\.test:8443/u);
...
test('an archive signed by another key is refused and nothing is installed', async () => {
	await withMachine(
		{ releases: [{ tag: 'v4.1.1', version: '4.1.1', revision: 'a'.repeat(40), latest: true }] },
		async ({ layout, installDependencies }) => {
			await assert.rejects(
				// Verified against the CLI's real embedded key, which did not sign
				// this fixture.
				() => runInstall(undefined, { ...OPTIONS, scope: 'user', directOrigin: 'https://box.example.test:8443' }, { ...installDependencies, releasePublicKeyPem: undefined }),
				/not signed by the Terminay release key/u,
			);
			assert.deepEqual(await installedVersions(layout), []);
			assert.equal(await readFile(layout.unitPath, 'utf8').catch(() => undefined), undefined, 'no unit may be written');
		},
	);
});
...
test('a reinstall keeps the server id devices are paired with', async () => {
	await withMachine(
		{ releases: [{ tag: 'v4.1.1', version: '4.1.1', revision: 'a'.repeat(40), latest: true }] },
		async ({ layout, installDependencies }) => {
			await runInstall(undefined, { ...OPTIONS, scope: 'user', directOrigin: 'https://box.example.test:8443' }, installDependencies);
			await writeFile(
				layout.environmentFile,
				(await readFile(layout.environmentFile, 'utf8')).replace('TERMINAY_SERVER_ID=test-box', 'TERMINAY_SERVER_ID=already-paired'),
			);
			await runInstall(undefined, { ...OPTIONS, scope: 'user', directOrigin: 'https://box.example.test:8443' }, { ...installDependencies, serverId: 'a-different-id' });
			const environment = parseEnvironmentFile(await readFile(layout.environmentFile, 'utf8'));
...
test('direct exposure with no derivable origin fails rather than advertising a wrong one', async () => {
	await withMachine(
		...
		async ({ installDependencies }) => {
			...
			const result = await runInstall(undefined, { ...OPTIONS, scope: 'user', expose: 'hosted' }, installDependencies);
...
test('upgrade follows the tag channel, stages beside, switches, and retains one previous', async () => {
	await withMachine(
		{
			releases: [
				{ tag: 'v4.1.0', version: '4.1.0', revision: 'a'.repeat(40), publishedAt: '2026-09-01T00:00:00Z' },
				{ tag: 'v4.1.1', version: '4.1.1', revision: 'b'.repeat(40), latest: true, publishedAt: '2026-09-08T00:00:00Z' },
			],
		},
		async ({ health, fake, layout, installDependencies, write }) => {
			await runInstall('v4.1.0', { ...OPTIONS, scope: 'user', expose: 'hosted' }, installDependencies);
			...
			assert.equal(await activeVersion(layout), '4.1.1');
			assert.deepEqual(await installedVersions(layout), ['4.1.0', '4.1.1'], 'one previous version is kept');
...
test('upgrade refuses a downgrade unless it is asked for', async () => {
	await withMachine(
		...
		async ({ health, fake, layout, installDependencies, write }) => {
			await runInstall('v4.1.1', { ...OPTIONS, scope: 'user', expose: 'hosted' }, installDependencies);
			await pointHealthAtFixture(layout, health.port);
			const dependencies = {
				apiBase: installDependencies.apiBase,
				webBase: installDependencies.webBase,
			...
			const context = { layout, record: await readInstallRecord(layout), systemd: createSystemd({ scope: 'user', env: fake.env() }), write };

			await assert.rejects(
				() => runUpgrade('v4.1.0', OPTIONS, context, dependencies),
				(error) => error instanceof UpgradeError && error.message.includes('4.1.1') && error.message.includes('4.1.0'),
			);
			assert.equal(await activeVersion(layout), '4.1.1', 'a refused downgrade must not move current');

			const allowed = await runUpgrade('v4.1.0', { ...OPTIONS, allowDowngrade: true }, context, dependencies);
...
test('an upgrade already on the newest version does nothing', async () => {
	await withMachine(
		{ releases: [{ tag: 'v4.1.1', version: '4.1.1', revision: 'a'.repeat(40), latest: true }] },
		async ({ health, fake, layout, installDependencies, write }) => {
			await runInstall(undefined, { ...OPTIONS, scope: 'user', expose: 'hosted' }, installDependencies);
			await pointHealthAtFixture(layout, health.port);
			const context = { layout, record: await readInstallRecord(layout), systemd: createSystemd({ scope: 'user', env: fake.env() }), write };
			const result = await runUpgrade(undefined, OPTIONS, context, {
				apiBase: installDependencies.apiBase,
	...
	await withMachine(
		...
		async ({ health, fake, layout, installDependencies, write, lines }) => {
			...
			await assert.rejects(
				...
				(error) => error instanceof UpgradeError && /did not become ready/u.test(error.message),
			);

			assert.equal(await activeVersion(layout), '4.1.0', 'current must point back at the working version');
			assert.equal((await readInstallRecord(layout)).version, '4.1.0', 'the record must not claim the failed version');
...
test('a source install has no channel to follow and says so', async () => {
	await withMachine(
		...
		async ({ fake, layout, installDependencies, write }) => {
			await runInstall(undefined, { ...OPTIONS, scope: 'user', expose: 'hosted' }, installDependencies);
			const record = { ...(await readInstallRecord(layout)), channel: 'source' };
			const context = { layout, record, systemd: createSystemd({ scope: 'user', env: fake.env() }), write };
			await assert.rejects(
				() => runUpgrade(undefined, OPTIONS, context, {}),
				(error) => error instanceof UpgradeError && /no channel to follow/u.test(error.message),
			);
		},
	);
});
...
test('start waits for readiness and stop stops the unit', async () => {
	await withMachine(
		...
		async ({ health, fake, layout, installDependencies, write }) => {
			...
			const invocations = fake.invocations();
			assert.ok(invocations.includes('systemctl --user start terminay-server.service'));
			assert.ok(invocations.includes('systemctl --user stop terminay-server.service'));
		},
	);
});
...
test('status reports the unit, version, channel, readiness, and exposure — and nothing else', async () => {
	await withMachine(
		...
		async ({ health, fake, layout, installDependencies, lines, write }) => {
			...
			assert.doesNotMatch(output, /\/home\/|\/var\/lib|\.local\/share/u, 'status must not print filesystem paths');
			assert.doesNotMatch(output, /passphrase|token|deviceId|device key|host key/iu);
			assert.ok(
...
test('uninstall removes the service and versions but keeps the data root', async () => {
	await withMachine(
		...
		async ({ fake, layout, installDependencies, write }) => {
			...
			assert.equal(await readFile(layout.unitPath, 'utf8').catch(() => undefined), undefined);
			assert.deepEqual(await installedVersions(layout), []);
			assert.equal(await readInstallRecord(layout), undefined);
			assert.ok((await stat(record.dataRoot)).isDirectory(), 'the data root must survive');
			assert.ok(fake.invocations().includes('systemctl --user disable terminay-server.service'));
		},
	);
});
...
test('--purge without a terminal needs --yes, and removes the data root with it', async () => {
	await withMachine(
		...
		async ({ fake, layout, installDependencies, write }) => {
			...
			await assert.rejects(() => runUninstall({ ...OPTIONS, purge: true }, context, pipe()), /needs --yes/u);
			assert.ok((await stat(record.dataRoot)).isDirectory(), 'a refused purge must not remove anything');

			const result = await runUninstall({ ...OPTIONS, purge: true, yes: true }, context, pipe());
			assert.equal(result.removedDataRoot, true);
			assert.equal(await stat(record.dataRoot).catch(() => undefined), undefined);
		},
	);
});
...
test('reset-identity refuses to unpair every device without confirmation', async () => {
	await withMachine(
		...
		async ({ fake, layout, installDependencies, write }) => {
			await runInstall(undefined, { ...OPTIONS, scope: 'user', expose: 'hosted' }, installDependencies);
			const context = { layout, record: await readInstallRecord(layout), systemd: createSystemd({ scope: 'user', env: fake.env() }), write };
			await assert.rejects(() => runResetIdentity(OPTIONS, context, pipe()), /needs --yes/u);
		},
	);
});
...
test('commands run against a machine with nothing installed say so', async () => {
	...
	try {
		await assert.rejects(
			...
			(error) => error instanceof NotInstalledError && /npx terminay daemon install/u.test(error.message),
		);
	} finally {
		await rm(home, { recursive: true, force: true });
	}
...
test('the binary never supplies its own release key', async () => {
	const source = await readFile(resolve(new URL('../src/cli.ts', import.meta.url).pathname), 'utf8');
	assert.doesNotMatch(source, /releasePublicKeyPem/u, 'the shipped entry point must verify against the embedded key only');
});
