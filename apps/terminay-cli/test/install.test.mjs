...
test('the layout puts every version under versions/ with current as the only pointer', () => {
	...
	assert.equal(
		user.unitPath,
		'/home/ada/.config/systemd/user/terminay-server.service',
	);
});
...
test('a fresh install lands under versions/<version> and current points at it', async () => {
	await withPrefix(async (layout, home) => {
		...
		assert.ok(
			(await stat(join(layout.currentLink, 'bin/terminay-server'))).isFile(),
		);
	});
});
...
test('permissions keep the installed tree readable and the launcher executable', async () => {
	await withPrefix(async (layout, home) => {
		...
		assert.ok(
			((await stat(join(installed.directory, 'bin/terminay-server'))).mode &
				0o111) !==
				0,
		);
	});
...
test('an archive whose payload does not match its manifest is refused', async () => {
	await withPrefix(async (layout, home) => {
		...
		const original = await readFile(
			join(fixture.root, 'server/dist/cli.js'),
			'utf8',
		);
		...
		await assert.rejects(
			...
			(error) =>
				...
				/does not match the digest/u.test(error.message),
		);
		assert.deepEqual(
			await installedVersions(layout),
			[],
			'a refused archive must leave nothing installed',
		);
	});
});
...
test('an archive whose release coordinates differ from what was asked for is refused', async () => {
	await withPrefix(async (layout, home) => {
		...
		await assert.rejects(
			...
			(error) =>
				error instanceof ManifestError && /architecture/u.test(error.message),
		);
	});
});
...
test('a manifest listing a symbolic link is refused', async () => {
	await withPrefix(async (layout, home) => {
		...
		await assert.rejects(
			...
			(error) =>
				error instanceof ManifestError && /symbolic link/u.test(error.message),
		);
	});
});
...
test('rolling installs are named for the commit so two builds can coexist', async () => {
	await withPrefix(async (layout, home) => {
		...
		const installedFirst = await installArchive({
			layout,
		...
		const installedSecond = await installArchive({
			layout,
...
test('an unreadable install record is refused rather than half-believed', () => {
	assert.throws(
		() => parseInstallRecord({ schemaVersion: 2 }),
		/unsupported schema version/u,
	);
	assert.throws(
		() => parseInstallRecord({ schemaVersion: 1 }),
		/missing scope/u,
	);
