...
async function withTools(present, run) {
	...
	for (const [name, script] of Object.entries(present))
		fake.install(name, script);
	...
	await withTools({ git: '', make: '' }, async (fake, env) => {
		await assert.rejects(
			...
			(error) =>
				...
				/python3/u.test(error.message) &&
				/c\+\+/u.test(error.message),
		);
		assert.ok(
...
test('a source build fails before downloading when the toolchain is incomplete', async () => {
	await withTools({ git: '' }, async (fake, env) => {
		...
		assert.deepEqual(
			fake.invocations(),
			[],
			'the preflight uses the shell builtin, not the tools themselves',
		);
	});
});
	...
	try {
		...
		assert.ok(
			invocations.some((line) =>
				line.startsWith('git clone --depth 1 --branch feature/x'),
			),
			'the ref is shallow-cloned',
		);
		assert.ok(
			invocations.some((line) => line === 'npm ci'),
			'dependencies are installed',
		);
		assert.ok(
			invocations.some((line) => line.includes('build:application-graph')),
...
test('a failed build keeps its directory and says where it is', async () => {
	...
	try {
		...
		await assert.rejects(
			...
			(error) =>
				error instanceof SourceBuildError &&
				/The build directory was kept at/u.test(error.message),
		);
		const { readdir } = await import('node:fs/promises');
		const left = await readdir(workspace);
		assert.ok(
			left.length > 0,
			'the build directory must survive a failure so it can be read',
		);
	} finally {
		await fake.close();
		await rm(workspace, { recursive: true, force: true });
...
test('the real source build runs end to end when it is opted into', {
	skip: process.env.TERMINAY_RUN_SOURCE_BUILD_E2E !== '1',
}, async () => {
