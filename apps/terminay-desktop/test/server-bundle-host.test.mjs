...
import {
	...
	LocalServerUiSession,
	migrateDesktopHostState,
} from '../dist/index.js';
			...
			tar([
				[
					...
					JSON.stringify({
						...
						bundleId,
						entryPath: 'nested/generated/server-entry.html',
						applicationProtocolVersion: '1',
					}),
				],
				[
					'nested/generated/server-entry.html',
					'<!doctype html><script type=module src=./assets/main.mjs></script>',
				],
...
test('Local UI session verifies once per window and never owns a listener or credential', async () => {
	...
	try {
		...
		await writeFile(
			join(localRoot, 'manifest.json'),
			JSON.stringify(selected.manifest),
		);
		...
		session.release(41);
		assert.equal(session.launchFor(41), undefined);
	} finally {
		await rm(root, { recursive: true, force: true });
	}
});
...
test('host-state migration discards server authority and rejects unclassified persistence', () => {
	const migrated = migrateDesktopHostState({
		workspaceSnapshot: { projects: ['secret'] },
		terminalState: ['secret'],
		credentialReferences: ['credential:one'],
		devicePreferences: { reduceMotion: true },
	});
	assert.equal('workspaceSnapshot' in migrated, false);
	assert.deepEqual(migrated.credentialReferences, ['credential:one']);
	assert.throws(
		() => migrateDesktopHostState({ mysteryFeatureState: {} }),
		/not classified/,
	);
});
...
test('Desktop has no second host action/context schema', async () => {
	const legacy = await readFile(
		new URL('../src/main/hostBridge.ts', import.meta.url),
		'utf8',
	);
	const electronContract = await readFile(
		new URL('../../../electron/serverUiHostContract.ts', import.meta.url),
		'utf8',
	);
	assert.doesNotMatch(
		legacy,
		/clipboard\.read|export interface DesktopHostContext|export type DesktopHostAction\s*=\s*\{/u,
	);
	assert.doesNotMatch(
		electronContract,
		/export type ServerUiHostContext|export type ServerUiHostAction/u,
	);
});
...
test('normal packaged Desktop startup launches the verified bundle through the canonical narrow preload', async () => {
	...
	assert.doesNotMatch(
		main,
		/localServerUiSession[\s\S]{0,500}loadFile\([\s\S]{0,100}RENDERER_DIST/u,
	);
...
test('actual Local Desktop, remote Desktop, and session browser compositions launch one server identity', async () => {
	const root = await mkdtemp(join(tmpdir(), 'terminay-four-host-paths-'));
	try {
		const selected = fixture();
		const serverId = 'server-shared';
		...
		const browserContext = (sourceId) => ({
			...
			applicationProtocolVersion: selected.manifest.protocolVersion,
			hostKind: 'browser',
			hostBridgeVersion: 1,
			byteEndpointVersion: 1,
			capabilities: { notifications: 1, clipboardWrite: 1 },
		});
		const browser = await createBrowserSessionBundleHost(
			new MemoryCacheStorage(),
		).installAndPrepare({
			expectedServerId: serverId,
			sessionOrigin: 'https://shared.example',
			context: browserContext('session-browser'),
			endpoint,
			compressedArchive: archive.bytes,
		});
		const identities = [local.context, remote.context, browser.context].map(
			({ bundleId, serverId, profileId, applicationProtocolVersion }) => ({
				bundleId,
				serverId,
				profileId,
		...
		assert.deepEqual(
			identities,
			Array.from({ length: 3 }, () => identities[0]),
		);
	} finally {
		await rm(root, { recursive: true, force: true });
	}
