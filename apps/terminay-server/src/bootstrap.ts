...
export interface EmbeddedBootstrapOptions
	extends Omit<ServerRuntimeConfig, 'runtimeMode' | 'localEndpoint'> {
...
export type EmbeddedBootstrapPhase =
	| 'created'
	| 'starting'
	| 'ready'
	| 'failed'
...
export class EmbeddedServerBootstrap {
	...
	constructor(private readonly options: EmbeddedBootstrapOptions) {
		if (!options.dataRoot || options.dataRoot.length > 4096)
			throw new TypeError('dataRoot is invalid');
		if (
			options.bootstrapCredential !== undefined &&
			!isBootstrapCredential(options.bootstrapCredential)
		) {
			throw new TypeError('embedded bootstrap credential is invalid');
		}
	}

	get phase(): EmbeddedBootstrapPhase {
	...
	async start(): Promise<EmbeddedBootstrapReady> {
		...
		if (this.phaseValue === 'stopped' || this.phaseValue === 'failed')
			this.phaseValue = 'created';
		if (this.phaseValue !== 'created')
			throw new Error(`embedded bootstrap is ${this.phaseValue}`);
	...
	private async startOnce(): Promise<EmbeddedBootstrapReady> {
		...
		try {
			...
			this.leaseHeld = true;
			candidate = validateLoopbackCandidate(
				await this.options.allocator.choose(),
			);
			candidateClaimAttempted = true;
			await this.options.allocator.claim(candidate);
			this.candidateValue = candidate;
			const credential =
				this.options.bootstrapCredential ??
				randomBytes(32).toString('base64url');
			this.credentialValue = credential;
			const {
				allocator: _allocator,
				dataRootLease: _dataRootLease,
				publishReady: _publishReady,
				bootstrapCredential: _bootstrapCredential,
				hooks,
				createUiServer,
				...config
			} = this.options;
			...
			if (
				uiServer !== undefined &&
				(typeof uiServer.start !== 'function' ||
					typeof uiServer.stop !== 'function')
			) {
				throw new TypeError(
					'embedded UI server factory returned an invalid listener',
				);
			}
			...
			if (
				uiServer !== undefined &&
				uiServer.address?.origin !== candidate.origin
			) {
				throw new Error(
					'embedded UI server did not bind the selected local endpoint',
				);
			}
			this.phaseValue = 'ready';
			...
			if (candidate !== undefined && candidateClaimAttempted)
				await Promise.resolve(this.options.allocator.release(candidate)).catch(
					() => undefined,
				);
			if (this.leaseHeld)
				await Promise.resolve(
					this.options.dataRootLease.release(this.options.dataRoot),
				).catch(() => undefined);
	...
	private async stopOnce(): Promise<void> {
		...
		try {
			if (this.candidateValue !== undefined)
				await this.options.allocator.release(this.candidateValue);
		} catch (error) {
		...
		try {
			if (this.leaseHeld)
				await this.options.dataRootLease.release(this.options.dataRoot);
		} catch (error) {
			leaseFailure = error;
		}
		this.runtimeValue = undefined;
		...
		const failures = [runtimeFailure, endpointFailure, leaseFailure].filter(
			(error): error is unknown => error !== undefined,
		);
		if (failures.length === 1) throw failures[0];
		if (failures.length > 1)
			throw new AggregateError(failures, 'embedded bootstrap shutdown failed');
	}

	private ready(
		candidate: LocalEndpointCandidate,
		credential: string,
	): EmbeddedBootstrapReady {
...
function validateLoopbackCandidate(
	candidate: LocalEndpointCandidate,
...
export function createEmbeddedBootstrap(
	options: EmbeddedBootstrapOptions,
): EmbeddedServerBootstrap {
