...
export class ServerHealthServer {
	...
	constructor(options: ServerHealthServerOptions) {
		if (typeof options.health !== 'function')
			throw new TypeError('health callback is required');
		if (
		...
		if (!Number.isSafeInteger(port) || port < 0 || port > 65535)
			throw new RangeError('health port is invalid');
	...
	get address(): ServerHealthAddress | undefined {
	...
	private handle(request: IncomingMessage, response: ServerResponse): void {
		...
		try {
			...
			if (!isSafeHealthSnapshot(snapshot))
				throw new TypeError('health snapshot is invalid');
		} catch {
...
export function createServerHealthServer(
	options: ServerHealthServerOptions,
): ServerHealthServer {
