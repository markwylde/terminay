...
export interface StandaloneServerOptions
	extends Omit<ServerRuntimeConfig, 'runtimeMode'> {
...
function createServerRuntime(
	...
	const services =
		serviceFactory === undefined
			...
			: serviceFactory.create({
					...
					paths: requirePlatformPaths(runtimeConfig),
				});
	...
	const stopComposition = async (
		deadline: number,
		services: ServerRuntimeServices | undefined,
	): Promise<void> => {
		...
		try {
			...
			if (listenerFailure !== undefined) {
				throw new AggregateError(
					[listenerFailure, error],
					'server listener and service shutdown failed',
				);
			}
			throw error;
		}
			...
			try {
				...
				try {
					await stopComposition(
						Date.now() + (runtimeConfig.shutdownTimeoutMs ?? 5_000),
						services,
					);
				} catch (cleanupError) {
					throw new AggregateError(
						[error, cleanupError],
						'server listener startup and rollback failed',
					);
				}
				throw error;
			}
	...
	return new ServerRuntime(
		{ ...runtimeConfig, ...(services === undefined ? {} : { services }) },
		composedHooks,
	);
}

function requirePlatformPaths(
	config: ServerRuntimeConfig,
): ServerPlatformPaths {
	if (config.platformPaths === undefined)
		throw new TypeError('service composition requires injected platform paths');
