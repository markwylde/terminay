...
export async function resolveTerminalProcessCwd(
	rootPid: number,
	signal?: AbortSignal,
): Promise<string | null> {
	...
	return (
		(await resolveProcessCwd(deepestPid, signal)) ??
		(deepestPid === rootPid ? null : await resolveProcessCwd(rootPid, signal))
	);
}

async function resolveDeepestProcessPid(
	rootPid: number,
	signal?: AbortSignal,
): Promise<number> {
	let current = rootPid;
...
async function childProcessIds(
	pid: number,
	signal?: AbortSignal,
): Promise<number[]> {
	try {
