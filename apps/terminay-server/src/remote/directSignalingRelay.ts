...
const DEVICE_PLANE_FRAMES = new Set([
	'device-offer',
	'device-answer',
	'device-ice',
]);
...
export function createDirectSignalingRelay(
	...
	const maxFrameBytes =
		options.maxFrameBytes ?? DIRECT_SIGNALING_MAX_FRAME_BYTES;
	const roomHandshakes =
		options.roomHandshakes ?? DIRECT_SIGNALING_ROOM_HANDSHAKES;
	const handshakeWindowMs =
		options.handshakeWindowMs ?? DIRECT_SIGNALING_HANDSHAKE_WINDOW_MS;
	...
	acceptSessionSignalingUpgrade(
		{
			host: new URL(options.sessionOrigin).host,
			upgrade: 'websocket',
			url: signalingPath,
		},
		{
	...
	function onFrame(
		...
		if (routing.type === DEVICE_HOST_FRAME) {
			registerHost(
				...
				socket,
				routing.room,
				'device-host-registered',
				'sessionId',
			);
			return;
		}
		if (
			routing.type === PAIRING_JOIN_FRAME ||
			routing.type === DEVICE_JOIN_FRAME
		) {
		...
		if (routing.type === PEER_CLOSED_FRAME) {
			if (socket === pairing.host || socket === pairing.client)
				route(pairing, socket, raw);
			if (socket === device.host || socket === device.client)
