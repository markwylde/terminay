...
export interface NodeDataChannelSignaling {
	...
	readonly encode: (
		message: NodeDataChannelSignal,
	) => unknown | Promise<unknown>;
	readonly decode: (
		message: unknown,
	) => NodeDataChannelSignal | null | Promise<NodeDataChannelSignal | null>;
}
...
export function createNodeDataChannelOpenChannels(
	...
	if (
		...
		typeof options.signaling.onMessage !== 'function'
	) {
		throw new TypeError('node-datachannel signaling transport is required');
	}
	if (
		typeof options.signaling.encode !== 'function' ||
		typeof options.signaling.decode !== 'function'
	) {
	...
	if (
		!Number.isSafeInteger(maxQueuedSignals) ||
		maxQueuedSignals <= 0 ||
		maxQueuedSignals > 1_024
	) {
		throw new RangeError('node-datachannel queued signaling limit is invalid');
	}
	const signalDecodingTimeoutMs =
		options.signalDecodingTimeoutMs ?? Math.min(timeoutMs, 10_000);
	if (
		...
		throw new RangeError(
			'node-datachannel signaling decoding timeout is invalid',
		);
	}
	...
	if (
		...
		throw new RangeError(
			'node-datachannel outbound signaling queue limit is invalid',
		);
	}
	const outboundSignalTimeoutMs =
		options.outboundSignalTimeoutMs ?? Math.min(timeoutMs, 10_000);
	if (
		...
		throw new RangeError(
			'node-datachannel outbound signaling timeout is invalid',
		);
	}
	const maxSignalBytes = options.maxSignalBytes ?? 64 * 1024;
	if (
		!Number.isSafeInteger(maxSignalBytes) ||
		maxSignalBytes <= 0 ||
		maxSignalBytes > 1024 * 1024
	) {
	...
	if (
		!Number.isSafeInteger(maxPendingCandidates) ||
		maxPendingCandidates <= 0 ||
		maxPendingCandidates > 1_024
	) {
	...
	if (
		!Number.isSafeInteger(maxRemoteCandidates) ||
		maxRemoteCandidates <= 0 ||
		maxRemoteCandidates > 4_096
	) {
	...
	return async (module, context) => {
		if (context.channels.length === 0)
			throw new TypeError('node-datachannel channel contract is empty');
		const PeerConnection = module.PeerConnection as new (
			id: string,
			configuration: Record<string, unknown>,
		) => NodeDataChannelPeerLike;
		if (typeof PeerConnection !== 'function')
			throw new TypeError('node-datachannel PeerConnection is unavailable');
		...
		const pendingCandidates: Array<
			Extract<NodeDataChannelSignal, { readonly type: 'ice' }>
		> = [];
		...
		let resolve!: (
			value: ReadonlyMap<RemoteTrafficChannel, NodeDataChannelLike>,
		) => void;
		let reject!: (reason?: unknown) => void;
		const result = new Promise<
			ReadonlyMap<RemoteTrafficChannel, NodeDataChannelLike>
			...
			if (!settled) {
				...
				reject(
					error instanceof Error
						? error
						: new Error('node-datachannel peer setup failed'),
				);
			}
		};
		const completeIfReady = (): void => {
			...
			try {
				if ([...channels.values()].some((channel) => channel.isOpen() !== true))
					return;
			} catch {
				fail(
					new Error(
						'node-datachannel channel state inspection failed during setup',
					),
				);
				return;
			}
			...
			resolve(
				new Map(
					context.channels.map((label) => [
						label,
						channels.get(label) as NodeDataChannelLike,
					]),
				),
			);
		};
		const send = async (message: NodeDataChannelSignal): Promise<void> => {
			...
			if (encoded === undefined || encoded === null)
				throw new Error(
					'node-datachannel signaling encoder rejected a message',
				);
			await runWithBudget(
		...
		const handleSignal = async (raw: unknown): Promise<void> => {
			...
			if (closed) return;
			if (message === null)
				throw new Error('node-datachannel signaling relay binding failed');
			validateSignal(message, maxSignalBytes);
			if (message.type === 'offer' || message.type === 'answer') {
				const expectedDescriptionType =
					options.role === 'offerer' ? 'answer' : 'offer';
				if (
					message.type !== expectedDescriptionType ||
					remoteDescriptionAccepted
				) {
					throw new Error(
						'node-datachannel signaling description is invalid or replayed',
					);
				}
			...
			if (message.type !== 'ice')
				throw new Error('node-datachannel signaling message type is invalid');
		...
		const enqueueOutboundSignal = (message: NodeDataChannelSignal): void => {
			...
			if (pendingOutboundSignals >= maxQueuedOutboundSignals) {
				fail(
					new Error('node-datachannel outbound signaling queue limit reached'),
				);
				return;
			}
		...
		try {
			...
			if (typeof unsubscribe !== 'function') {
				throw new TypeError(
					'node-datachannel signaling unsubscribe is invalid',
				);
			}
			...
			peer.onLocalDescription((sdp, type) => {
				...
				const expectedLocalDescriptionType =
					options.role === 'offerer' ? 'offer' : 'answer';
				if (type !== expectedLocalDescriptionType) {
					return fail(
						new Error(
							'node-datachannel produced an SDP type inconsistent with its role',
						),
					);
				}
			...
			if (closed)
				throw new Error(
					'node-datachannel peer closed during native listener registration',
				);
			peer.onLocalCandidate((candidate, mid) => {
				...
				if (
					...
					return fail(
						new Error('node-datachannel produced an invalid ICE candidate'),
					);
			...
			peer.onStateChange((state) => {
				...
				if (
					...
					fail(new Error(`node-datachannel peer is ${state}`));
			});
			...
			const watchChannel = (channel: NodeDataChannelLike): void => {
				...
				channel.onClosed(() => {
					...
					if (!connected)
						fail(
							new Error(
								`node-datachannel channel ${label} closed during setup`,
							),
						);
					...
					// the close callbacks raised while it closes the remaining channels.
						...
						fail(
							new Error(`node-datachannel channel ${label} closed after setup`),
						);
				});
			};
			peer.onDataChannel((channel) => {
				...
				try {
					const label = channel.getLabel();
					if (
						!context.channels.includes(label as RemoteTrafficChannel) ||
						channels.has(label)
					) {
						closeUntrackedChannel(channel);
						fail(
							new Error(
								'node-datachannel supplied an unexpected or duplicate channel',
							),
						);
						return;
					}
			...
			if (options.role === 'offerer') {
				if (typeof peer.createDataChannel !== 'function')
					throw new Error('node-datachannel cannot create data channels');
				for (const label of context.channels) {
					...
					try {
						...
						if (channel.getLabel() !== label)
							throw new Error(
								'node-datachannel returned a channel with the wrong label',
							);
						channels.set(label, channel);
						watchChannel(channel);
						...
						if (channel !== undefined && !channels.has(label))
							closeUntrackedChannel(channel);
						throw error;
					}
				}
			...
			const timeout = setTimeout(
				() => fail(new Error('node-datachannel peer setup timed out')),
				timeoutMs,
			);
			const abort = (): void =>
				fail(
...
function validateSignal(
	...
	if (message.type === 'offer' || message.type === 'answer') {
		...
		if (
			...
			throw new Error(
				'node-datachannel signaling description is invalid or too large',
			);
		}
		return;
	}
	if (
		...
		throw new Error(
			'node-datachannel signaling candidate is invalid or too large',
		);
	}
}
