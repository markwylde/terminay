...
export function readSctpMaxMessageBytes(channel: UiArchiveDataChannel): number {
	const raw =
		typeof channel.maxMessageSize === 'function'
			? channel.maxMessageSize()
			: channel.maxMessageSize;
	if (
		typeof raw === 'number' &&
		Number.isSafeInteger(raw) &&
		raw > UI_ARCHIVE_FRAME_HEADER_BYTES
	) {
		return raw;
	}
...
export function archiveChunkBytes(channel: UiArchiveDataChannel): number {
	const maxPayload =
		readSctpMaxMessageBytes(channel) - UI_ARCHIVE_FRAME_HEADER_BYTES;
...
export function safeChannelSend(
	...
	if (bytes > max) {
		throw new Error(
			`WebRTC data-channel message is ${bytes} bytes; negotiated SCTP max is ${max}.`,
		);
	}
...
export function encodeUiArchiveChunk(
	index: number,
	bytes: Uint8Array,
): Uint8Array {
	const frame = new Uint8Array(
		UI_ARCHIVE_FRAME_HEADER_BYTES + bytes.byteLength,
	);
...
export function sendUiArchiveError(
	...
	try {
		...
		safeChannelSend(
			channel,
			JSON.stringify({ code, id, message, type: 'asset:bundle-error' }),
		);
	} catch {
...
async function handleUiArchiveMessage(
	...
	if (request.type === 'asset:get-manifest' || request.type === 'asset:get') {
		sendUiArchiveError(
			...
			request.id,
			'invalid-request',
			'The requested UI archive format is unsupported.',
		);
		return;
	}
	...
	if (
		...
		activeIds.size >= MAX_ACTIVE_UI_ARCHIVE_REQUESTS
	) {
		sendUiArchiveError(
			channel,
			request.id,
...
export async function sendUiArchive(
	...
	if (
		archive.bytes.byteLength < 1 ||
		typeof archive.bundleId !== 'string' ||
		archive.bundleId.length < 1
	) {
	...
	if (chunks < 1 || chunks > 0xffff_ffff)
		throw new RangeError('Server UI archive chunk count is invalid.');
	...
	try {
		...
		for (let index = 0; index < chunks; index += 1) {
			...
			safeChannelSend(
				channel,
				encodeUiArchiveChunk(
					index,
					archive.bytes.subarray(offset, offset + chunkBytes),
				),
			);
		...
		if (channel.readyState === 'open') {
			safeChannelSend(
				channel,
				JSON.stringify({ id, type: 'asset:bundle-complete' }),
			);
		}
	} finally {
		transfers.delete(id);
...
function acknowledgeUiArchiveChunk(
	transfer: UiArchiveTransfer | undefined,
	indexValue: unknown,
): void {
