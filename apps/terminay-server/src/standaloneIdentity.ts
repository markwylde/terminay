 ...
 */
export async function resolveStandaloneServerIdentity(
	options: ServerCliOptions,
): Promise<ServerCliOptions> {
	...
	if (
		...
		stored !== undefined &&
		workspaceId !== stored.serverId
	) {
		throw new Error(
			'standalone workspace and server identity records disagree',
		);
	}
	...
	if (
		...
		throw new Error(
			"--server-id does not match this data root's existing server authority",
		);
	}
	...
	return Object.freeze({
		...options,
		serverId,
		...(options.remoteOriginExplicit
			? {}
			: { remoteOrigin: defaultRemoteOrigin(serverId) }),
	});
}

async function readWorkspaceServerId(
	dataRoot: string,
): Promise<string | undefined> {
	let raw: string;
	try {
		raw = await readFile(join(dataRoot, WORKSPACE_FILE), 'utf8');
	} catch (error) {
		if ((error as NodeJS.ErrnoException).code === 'ENOENT') return undefined;
		throw new Error(
			'could not read the existing standalone workspace identity',
			{ cause: error },
		);
	}
	let value: unknown;
	try {
		...
		throw new Error('existing standalone workspace identity is invalid', {
			cause: error,
		});
	}
	...
	if (!validServerId(serverId))
		throw new Error(
			'existing standalone workspace has no valid server authority',
		);
	return serverId;
}
...
async function readStoredIdentity(
	...
	let raw: string;
	try {
		raw = await readFile(join(dataRoot, IDENTITY_FILE), 'utf8');
	} catch (error) {
		if ((error as NodeJS.ErrnoException).code === 'ENOENT') return undefined;
		throw new Error('could not read the standalone server identity', {
			cause: error,
		});
	}
	let value: unknown;
	try {
		...
		throw new Error('standalone server identity is invalid', { cause: error });
	}
...
async function persistIdentity(
	...
	try {
		await writeFile(
			path,
			`${JSON.stringify({ schemaVersion: SCHEMA_VERSION, serverId })}\n`,
			{
				encoding: 'utf8',
				flag: 'wx',
				mode: 0o600,
		...
		if ((error as NodeJS.ErrnoException).code !== 'EEXIST') {
			throw new Error('could not persist the standalone server identity', {
				cause: error,
			});
		}
		const concurrent = await readStoredIdentity(dataRoot);
		if (concurrent?.serverId !== serverId)
			throw new Error(
				'standalone server identity changed while it was being resolved',
			);
	}
}

function newServerId(): string {
	return `standalone-${randomBytes(18).toString('base64url')}`;
}
