...
async function standaloneRuntimeGraph(entrypoints) {
	...
	while (queue.length > 0) {
		...
		if (
			!javascriptModule.test(modulePath) ||
				...
				if (!specifier.startsWith('.')) {
					...
					try {
						resolved = fileURLToPath(
							import.meta.resolve(specifier, pathToFileURL(modulePath).href),
						);
					} catch {
						assert.fail(
							`unable to resolve standalone runtime dependency ${specifier} from ${relative(repositoryRoot, modulePath)}: ${error.message}`,
						);
					}
				}
				if (specifier.startsWith('.')) {
					...
					assert.ok(
						...
						`unable to resolve standalone runtime dependency ${specifier} from ${relative(repositoryRoot, modulePath)}`,
					);
...
test('standalone runtime dependency graph has no Electron imports', async () => {
	...
	assert.ok(
		modules.some((modulePath) =>
			modulePath.includes('/packages/server-core/dist/'),
		),
		'expected server-core in standalone runtime graph',
	);
	assert.ok(
		modules.some((modulePath) =>
			modulePath.includes('/packages/protocol/dist/'),
		),
		'expected protocol in standalone runtime graph',
	);
});

test('standalone runtime graph rejects direct, dynamic, and CommonJS Electron imports', async () => {
	const root = await mkdtemp(
		join(tmpdir(), 'terminay-standalone-runtime-graph-'),
	);
	try {
		for (const [index, source] of [
			...
			await assert.rejects(
				() => standaloneRuntimeGraph([entrypoint]),
				/Electron import in standalone runtime module/,
			);
		}
	} finally {
