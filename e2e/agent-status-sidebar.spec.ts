 ...
 */
async function destroySettingsWindow(
	...
	await electronApp.evaluate(({ BrowserWindow }) => {
		const settingsWindow = BrowserWindow.getAllWindows().find(
			(window) =>
				!window.isDestroyed() &&
				new URL(window.webContents.getURL()).searchParams.get('view') ===
					'settings',
		);
		if (!settingsWindow) {
...
test('native agent lifecycle drives stable tabs, notifications, hierarchy, and focus', async ({
	...
	await expect(
		mainWindow.locator('.terminal-activity-pill--attention'),
	).toHaveText('1');
	...
	const agentTerminalInput = mainWindow
		.locator(
			`.terminal-panel[data-terminay-terminal-session-id="${agentTerminalSessionId}"]`,
		)
...
test('real Codex SubagentStart transcript metadata supplies the task name', async ({
	...
	try {
		...
		await expect(
			mainWindow.getByText('Subagent 1', { exact: true }),
		).toHaveCount(0);
	} finally {
