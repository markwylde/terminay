...
test('folder panel aggregates markdown tasks recursively', async ({ createWorkspace, mainWindow }) => {
  ...
  await expect(mainWindow.locator('.file-tasks__label').filter({ hasText: 'Root todo' })).toBeVisible()
  await expect(mainWindow.locator('.file-tasks__label').filter({ hasText: 'Root done' })).toHaveCount(0)
  await mainWindow.getByLabel('Search tasks').fill('Nested todo')
  await expect(mainWindow.locator('.file-tasks__label').filter({ hasText: 'Nested todo' })).toBeVisible()
  await expect(mainWindow.locator('.file-tasks__label').filter({ hasText: 'Root todo' })).toHaveCount(0)
