...
function folderViewButton(
  page: Parameters<typeof test>[0]["mainWindow"],
  name: string,
) {
  return page
    .locator(".folder-viewer__view-button")
    .filter({ hasText: name })
    .first();
}

async function ignoreMissing<T>(
  operation: () => Promise<T>,
): Promise<T | undefined> {
  try {
...
test("folder panel supports view modes navigation and refresh", async ({
  createWorkspace,
  mainWindow,
}) => {
  ...
  const innerRow = mainWindow
    .locator(".folder-viewer__list-row")
    .filter({ hasText: "inner.txt" });
  ...
  const workspaceName = workspace.rootDir
    .split("/")
    .filter(Boolean)
    .pop() as string;
  await expect(mainWindow.locator(".folder-viewer__title")).toHaveText(
  ...
  await mainWindow
    .locator(".folder-viewer__list-sort")
    .filter({ hasText: "Name" })
    .click();
  ...
  await mainWindow
    .locator(".folder-viewer__list-row")
    .filter({ hasText: "fresh.txt" })
    .dblclick();
  await expect(mainWindow.locator(".file-preview-text")).toContainText(
    "after refresh",
  );
});

test("folder panel aggregates markdown tasks recursively", async ({
  createWorkspace,
  mainWindow,
}) => {
  const workspace = await createWorkspace({
  ...
  await expect(
    mainWindow
      .locator(".folder-tasks__file-header")
  ...
  await expect(mainWindow.locator(".folder-tasks")).not.toContainText(
    "Ignored todo",
  );

  await mainWindow.getByRole("tab", { name: "Remaining" }).click();
  await expect(
    mainWindow.locator(".file-tasks__label").filter({ hasText: "Root todo" }),
  ).toBeVisible();
  await expect(
    mainWindow.locator(".file-tasks__label").filter({ hasText: "Root done" }),
  ).toHaveCount(0);
  await mainWindow.getByLabel("Search tasks").fill("Nested todo");
  await expect(
    mainWindow.locator(".file-tasks__label").filter({ hasText: "Nested todo" }),
  ).toBeVisible();
  await expect(
    mainWindow.locator(".file-tasks__label").filter({ hasText: "Root todo" }),
  ).toHaveCount(0);
  await mainWindow.getByLabel("Clear search").click();
  await mainWindow.getByRole("tab", { name: "Kanban" }).click();
  await expect(mainWindow.locator(".file-kanban__board")).toBeVisible();
  await mainWindow.getByRole("tab", { name: "List" }).click();
  await mainWindow.getByRole("tab", { name: "All" }).click();
  ...
  await expect(
    contextMenuItem(mainWindow, "Open shell in folder"),
  ).toBeVisible();
  await expect(contextMenuItem(mainWindow, "Reveal in OS")).toBeVisible();
  await mainWindow.keyboard.press("Escape");

  await workspace.writeText(
    "work/nested/fresh.md",
    ["# Fresh", "", "- [ ] Watched nested task", ""].join("\n"),
  );
  await expect(mainWindow.locator(".file-tasks__summary")).toContainText(
    "3 remaining",
  );
  ...
  await mainWindow
    ...
    .dblclick();
  await expect(
    mainWindow.locator(".file-mode-switcher__button--active"),
  ).toHaveText("Tasks");
  await expect(mainWindow.locator(".file-tasks__summary")).toContainText(
    "1 done",
  );
...
test("folder task groups can be sorted recursively", async ({
  createWorkspace,
  mainWindow,
}) => {
  const workspace = await createWorkspace({
  ...
  expect(sectionTitles.indexOf("Zebra")).toBeLessThan(
    sectionTitles.indexOf("Alpha"),
  );

  await mainWindow.getByLabel("Sort task groups").selectOption("name");
  sectionTitles = await titles.allTextContents();
  expect(sectionTitles.indexOf("Alpha")).toBeLessThan(
    sectionTitles.indexOf("Zebra"),
  );
});

test("folder panel context menu mirrors file operations", async ({
  createWorkspace,
  mainWindow,
}) => {
  const workspace = await createWorkspace({
  ...
  const actionsDirSummary = mainWindow
    .locator(".folder-viewer__tree-summary")
    .filter({ hasText: "actions-dir" });
  const refreshButton = mainWindow
    .locator(".folder-viewer__action")
  ...
  await submitFileExplorerNameModal(
    mainWindow,
    "File name",
    "panel-created.txt",
  );
  await expect
    .poll(() =>
      ignoreMissing(() =>
        workspace.readText("ops-folder/actions-dir/panel-created.txt"),
      ),
    )
    .toBe("");
  ...
  await expect
    .poll(async () =>
      (
        await ignoreMissing(() =>
          stat(
            workspace.path("ops-folder", "actions-dir", "panel-created-folder"),
          ),
        )
      )?.isDirectory(),
    )
  ...
  await mainWindow
    .locator(".folder-viewer__tree-file")
    .filter({ hasText: "folder-file.txt" })
    .click({ button: "right" });
  ...
  await submitFileExplorerNameModal(
    mainWindow,
    "Name",
    "folder-file-renamed.txt",
  );
  await expect
    .poll(() =>
      ignoreMissing(() =>
        workspace.readText("ops-folder/folder-file-renamed.txt"),
      ),
    )
    .toBe("folder file\n");
  ...
  await expect(
    mainWindow
      .locator(".folder-viewer__tree-file")
      .filter({ hasText: "folder-file-renamed.txt" }),
  ).toBeVisible();
