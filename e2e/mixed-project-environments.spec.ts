...
): Promise<{ pairing: { pairingUrl: string }; serverId: string }> {
	child.stdout.setEncoding('utf8');
	let output = '';
	return new Promise((resolve, reject) => {
		const timer = setTimeout(
			() => reject(new Error('standalone server readiness timed out')),
			15_000,
		);
		child.stdout.on('data', (chunk: string) => {
			...
			clearTimeout(timer);
			resolve(
				JSON.parse(output.slice(0, newline)) as {
...
test('Desktop selects a standalone server and its canonical session survives renderer reconnect and server restart', async ({
	mainWindow,
	tempDir,
}) => {
	...
	try {
		...
		await expect(
			mainWindow.locator(
				`.terminal-panel[data-terminay-terminal-session-id="${before}"]`,
			),
		...
		await stop(server);
		await writeFile(
			path.join(tempDir, 'standalone-mixed-proof.txt'),
			'standalone restart exercised\n',
		);
	}
});
