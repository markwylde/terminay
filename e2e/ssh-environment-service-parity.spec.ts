...
test('Docker: generic SSH and composed Puzed projects keep exact provider bindings', async () => {
  ...
  const runtime = new ExtensionProjectEnvironmentRuntime(providerId, capabilities, { async invokeProvider(call: Record<string, any>) { calls.push(call); return { accepted: true } } }, () => state)
  ...
  expect(calls.map(call => [call.request.environmentId, call.request.providerState])).toEqual([
...
test('Docker: helper absent, incompatible, crashed and upgraded stay explicit and fail closed', async () => {
  ...
  let state: any = { schemaVersion: 2, serverId: 'server-e2e', revision: 1, cursor: '1', profiles: {}, operations: {}, environments: { [current.id]: current } }
  let helper: 'absent' | 'incompatible' | 'available' | 'crashed' = 'absent'
  const runtime = new ExtensionProjectEnvironmentRuntime(providerId, capabilities, { async invokeProvider(call: Record<string, any>) {
    if (call.request.capability !== 'process-observation') return { accepted: true }
    if (call.request.operation === 'observe') return helper === 'available'
    ...
    if (call.request.operation === 'poll') return helper === 'available'
    ...
    return { observationId: String(call.request.input.observationId), stopped: true }
  } }, () => state)

  for (const unavailable of ['absent', 'incompatible', 'crashed'] as const) {
    helper = unavailable
    const result: any = await runtime.invoke('process-observation', 'observe', { sessionId: 'exact-session' }, context(current.id, 4))
    expect(result).toMatchObject({ state: 'unavailable', reason: unavailable })
  }
  helper = 'available'
  const started: any = await runtime.invoke('process-observation', 'observe', { sessionId: 'exact-session' }, context(current.id, 4))
  ...
  const upgraded: any = await runtime.invoke('process-observation', 'observe', { sessionId: 'new-exact-session' }, context(current.id, 5))
