...
async function readConfig(path: string): Promise<{ config: GeminiConfig } | { error: string }> {
  let raw: string
  try {
...
function expectedEntry(server: McpServerCommand): GeminiServerEntry {
  const entry: GeminiServerEntry = { command: server.command, args: server.args }
...
export async function isGeminiInstalled(homeDirectory?: string): Promise<boolean> {
