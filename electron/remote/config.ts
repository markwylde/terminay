...
export function readRemoteAccessConfig(
	...
	return {
		...
		pinFailureLimit: settings.pinFailureLimit,
		pairingPinHash: settings.pairingPinHash,
		tlsCertPath: settings.tlsCertPath,
		tlsKeyPath: settings.tlsKeyPath,
		webRtcHostedDomain: settings.webRtcHostedDomain,
		webRtcIceServers: settings.webRtcIceServers,
	};
}

export function resolveRemoteAccessConfig(
	config: RemoteAccessConfig,
): ResolvedRemoteAccessConfig {
	...
	if (url.pathname !== '/' || url.search || url.hash) {
		throw new Error(
			'Remote access origin must be a clean origin with no path, query, or hash.',
		);
	}
	...
	return {
		...
		pinFailureLimit: config.pinFailureLimit,
		pairingPinHash: config.pairingPinHash,
		port: url.port ? Number.parseInt(url.port, 10) : 443,
		remoteAppOrigin: url.origin,
		tlsCertPath: config.tlsCertPath.trim()
			? path.resolve(config.tlsCertPath)
			: '',
		tlsKeyPath: config.tlsKeyPath.trim() ? path.resolve(config.tlsKeyPath) : '',
		webRtcHostedDomain: config.webRtcHostedDomain,
		webRtcIceServers: config.webRtcIceServers,
	};
}
