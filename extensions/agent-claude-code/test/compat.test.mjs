...
test('Claude Code mapping v0.1 remains compatible with the captured project-session fixture', async () => {
	const records = (
		await readFile(
			new URL('../fixtures/project-session-v01.jsonl', import.meta.url),
			'utf8',
		)
	)
		.trim()
	...
	try {
		...
		assert.equal(
			events.some(
				(event) =>
					...
					event.title === 'Investigate the parser',
			),
			true,
		);
		assert.equal(
			events.some(
				(event) => event.kind === 'tool.started' && event.name === 'Bash',
			),
			true,
		);
		assert.equal(
		...
		assert.equal(
			events.some(
				(event) =>
					event.kind === 'tool.finished' && event.toolId === 'toolu-shell',
			),
			true,
		);
		...
		assert.equal(
			...
			false,
			'tool input/output must remain provider-private',
		);
	} finally {
