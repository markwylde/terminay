...
test('the Claude Code extension packs as a self-contained public package', async () => {
	const destination = await mkdtemp(
		join(tmpdir(), 'terminay-claude-code-pack-'),
	);
	try {
		...
		const item = Array.isArray(packed) ? packed[0] : Object.values(packed)[0];
		assert.equal(typeof item?.filename, 'string');
		execFileSync('tar', [
			'-xzf',
			join(destination, item.filename),
			'-C',
			destination,
		]);
		const packageJson = JSON.parse(
			await readFile(join(destination, 'package/package.json'), 'utf8'),
		);
		...
		const source = await readFile(
			join(destination, 'package/dist/provider.js'),
			'utf8',
		);
		...
		await symlink(
			new URL('../../../packages/extension-api', import.meta.url),
			join(apiScope, 'extension-api'),
			'dir',
		);
		...
		await loaded.default.activate({
			...
			agents: {
				registerProvider(providerId, runtime) {
					providers.push({ providerId, runtime });
