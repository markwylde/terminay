...
test('v0.1 is deliberately forward-compatible and malformed/content records fail closed', async () => {
	...
	const publish = new Proxy(
		{},
		{
			get: (_target, name) => (event) => events.push({ kind: name, ...event }),
		},
	);
	const context = {
		binding: { providerSessionId: 'fixture-session-v01' },
		publish,
	};
	...
	mapCodexRecord(
		{
			type: 'event_msg',
			payload: { type: 'unknown_future_event', output: 'private' },
		},
		context,
	);
	mapCodexRecord(
		{ type: 'response_item', payload: { type: 'message', content: 'private' } },
		context,
	);
	mapCodexRecord(
		{
			type: 'session_meta',
			payload: { id: 'wrong', originator: 'codex-tui', source: 'cli' },
		},
		context,
	);
