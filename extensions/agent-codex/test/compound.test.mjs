...
const root = (id, model = 'gpt-5.6-codex') => ({
	type: 'session_meta',
	payload: { id, originator: 'codex-tui', source: 'cli', model },
});
...
function terminalFor(files) {
	const handles = new Map(
		Object.keys(files).map((path) => [path, { id: path }]),
	);
	let bindingRequest;
	return {
		...
		observation: {
			processes: {
				async descendants() {
					return [{ handle: { id: 'codex' }, executableName: 'codex' }];
				},
				async openFiles() {
					return [...handles].map(([path, handle]) => ({
						path,
						handle,
						access: 'writable',
					}));
			...
			files: {
				async canonicalFile(handle) {
					return handles.get(handle.id);
				},
				async resolveHomeDirectory(relativePath) {
					const root = `/home/test/${relativePath}`;
					return [...handles.keys()].some((path) => path.startsWith(`${root}/`))
						? { id: root }
						: undefined;
				},
				...
				async listDirectory(directory, options) {
					...
					const entries = [];
					let bytes = 0;
					for (const [path, handle] of [...handles].sort(([left], [right]) =>
						...
						const size = new TextEncoder().encode(
							record.records.map(JSON.stringify).join('\n'),
						).byteLength;
						if (
							!relativePath ||
							relativePath.split('/').length - 1 > options.maxDepth ||
				...
				async readJsonLine(handle) {
					return files[handle.id].records[0];
				},
				async stat(handle) {
					return {
						handle,
						kind: 'file',
						size: 1,
						modifiedAt: files[handle.id].modifiedAt,
					};
				},
				async follow(handle) {
					const configured = files[handle.id].chunks;
					const chunks = configured ?? [
						{
							type: 'append',
							bytes: new TextEncoder().encode(
								`${files[handle.id].records.map(JSON.stringify).join('\n')}\n`,
							),
						},
					];
					return {
...
test('new, resumed and branched roots rebind only to the newest exact writable Codex root', async () => {
	...
	const terminal = terminalFor({
		[oldPath]: {
			...
			records: [root('old-root')],
		},
		[resumedPath]: {
			modifiedAt: '2026-08-24T10:01:00.000Z',
			records: [root('resumed-root')],
		},
	});
...
test('separate Codex child rollouts attach only through their native parent_thread_id', async () => {
	...
	const unrelatedPath =
		'/home/test/.codex/sessions/2026/08/rollout-unrelated-child.jsonl';
	...
	try {
		await harness.observe(
			fixtureTerminal({
				...
				files: {
					...
					[unrelatedPath]: [
						child('other-child', 'other-root', 'Unrelated'),
						{
							type: 'event_msg',
		...
		assert.deepEqual(
			...
			['session.started', 'subagent.started', 'turn.started', 'agent.done'],
		);
...
test('a late Codex child rollout is attached through public directory discovery without rebinding its root', async () => {
	const rootPath = '/home/test/.codex/sessions/2026/08/rollout-root.jsonl';
	const childPath =
		'/home/test/.codex/sessions/2026/08/rollout-late-child.jsonl';
	const unrelatedPath =
		'/home/test/.codex/sessions/2026/08/rollout-unrelated.jsonl';
	const child = (id, parent, nickname) => ({
		type: 'session_meta',
		payload: {
			id,
			source: {
				subagent: {
					thread_spawn: { parent_thread_id: parent, agent_nickname: nickname },
				},
			},
			agent_nickname: nickname,
			model_provider: 'gpt-5.6-terra',
	...
	const terminal = terminalFor({
		...
		[childPath]: {
			modifiedAt: '2026-08-24T10:02:00.000Z',
			records: [
				child('late-child', 'root', 'Late Scout'),
				{
					type: 'event_msg',
		...
		[unrelatedPath]: {
			modifiedAt: '2026-08-24T10:03:00.000Z',
			records: [child('unrelated', 'wrong-root', 'Unrelated')],
		},
	});
	const directory = { id: '/home/test/.codex/sessions' };
	const list = terminal.observation.files.listDirectory.bind(
		terminal.observation.files,
	);
	const all = await list(directory, {
		extensions: ['.jsonl'],
		maxDepth: 4,
		maxEntries: 256,
	...
	const initial = {
		entries: all.entries.filter((entry) => entry.handle.id === rootPath),
		truncated: false,
	};
	...
	const discovered = [];
	for await (const source of result.childSourceDiscovery)
		discovered.push(source);
	assert.deepEqual(
		discovered.map((source) => source.childId),
		['late-child'],
	);
	...
	const context = {
		...
		journal: { role: 'child', childId: 'late-child' },
...
test('an exact Codex session-index title follows initial, repeated, replacement and resumed updates without replaying lifecycle', async () => {
	...
	const terminal = terminalFor({
		...
		[index]: {
			...
			chunks: [
				chunk('append', [
					{ id: 'other-terminal', thread_name: 'Other terminal' },
					{ id: 'resumed-root', thread_name: 'Initial title' },
				]),
				chunk('append', [
					{ id: 'resumed-root', thread_name: 'Renamed once' },
					{ id: 'resumed-root', thread_name: 'Renamed twice' },
				]),
				chunk('replace', [
					{ id: 'resumed-root', thread_name: 'Recovered after replacement' },
				]),
				chunk('truncate', [
					{ id: 'resumed-root', thread_name: 'Recovered after truncation' },
				]),
			],
		},
	});
	...
	assert.deepEqual(
		events.slice(3).map((event) => event.title),
		[
			'Initial title',
			'Renamed once',
			'Renamed twice',
			'Recovered after replacement',
			'Recovered after truncation',
		],
	);
	assert.equal(
		events.some((event) => event.title === 'Other terminal'),
		false,
	);
	assert.equal(
	...
	assert.equal(
		events.filter((event) => event.kind === 'turn.started').length,
		1,
	);
...
test('mapping variants, malformed and oversized native records fail closed', () => {
	const events = [];
	const context = {
		binding: { providerSessionId: 'root' },
		publish: publisher(events),
	};
	...
	mapCodexRecord(
		{
			type: 'event_msg',
			payload: { type: 'turn_complete', status: 'success' },
		},
		context,
	);
	mapCodexRecord(
	...
	for (const record of [
		...
		{
			type: 'event_msg',
			payload: { type: 'user_message', message: 'x'.repeat(4_001) },
		},
		{ type: 'event_msg', payload: { type: 'error', output: 'private' } },
	])
		mapCodexRecord(record, context);
	assert.deepEqual(
		events.slice(0, 4).map((event) => event.kind),
		['session.started', 'turn.started', 'agent.done', 'session.stopped'],
	);
...
test('collaboration lifecycle exposes bounded identity but never results, reasoning or tool payloads', () => {
	...
	mapCodexRecord(
		{
			...
			payload: {
				...
				new_thread_id: 'child',
				new_agent_nickname: 'Research',
				prompt: 'bounded task',
				model: 'gpt-5.6-terra',
				reasoning_effort: 'high',
				result: 'PRIVATE RESULT',
				reasoning: 'PRIVATE REASONING',
				tool_output: 'PRIVATE TOOL',
			},
		},
		context,
	);
	mapCodexRecord(
		{
			...
			payload: {
				...
				status: { completed: 'PRIVATE RESULT' },
			},
		},
		context,
	);
	mapCodexRecord(
		{
			type: 'event_msg',
			payload: { type: 'collab_close_end', receiver_thread_id: 'child' },
		},
		context,
	);
	assert.deepEqual(
		events.map((event) => event.kind),
		['subagent.started', 'subagent.done', 'subagent.done'],
	);
	assert.deepEqual(events[0].model, {
		id: 'gpt-5.6-terra',
...
test('every supported approval and elicitation variant publishes one bounded wait and closes on progress', () => {
	...
	for (const [eventType, waitId, idField] of cases) {
		...
		mapCodexRecord(
			{
				...
				payload: {
					...
					questions: [
						{ question: 'PRIVATE QUESTION', options: ['PRIVATE OPTION'] },
					],
		...
		mapCodexRecord(
			{
				type: 'event_msg',
				payload: { type: 'task_started', turn_id: `turn-${waitId}` },
			},
			context,
			state,
		);
...
test('malformed, unknown and oversized approval-like records cannot create a wait or leak payloads', () => {
	...
	for (const record of [
		...
		},
		{
			type: 'event_msg',
			payload: {
				type: 'exec_approval_request'.repeat(20),
				request_id: 'oversized-type',
				command: 'PRIVATE',
			},
		},
		{
	...
	assert.deepEqual(events, [
		{
			kind: 'wait.started',
			waitId: 'codex:request_user_input',
			state: 'waiting',
...
test('the bounded decoder rejects an oversized physical record before mapping', () => {
	...
	const records = decoder.push(
		new TextEncoder().encode(
			`${JSON.stringify({ type: 'event_msg', payload: { type: 'user_message', message: 'secret'.repeat(100) } })}\n`,
		),
	);
