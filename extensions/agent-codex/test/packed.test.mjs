...
test("the public package packs compiled code, fixtures and no workspace source", async () => {
  const destination = await mkdtemp(join(tmpdir(), "terminay-agent-codex-pack-"));
  const repository = new URL("../../../", import.meta.url);
  const packed = spawnSync("npm", ["pack", "--workspace", "terminay-agent-codex", "--pack-destination", destination, "--json"], { cwd: repository, encoding: "utf8" });
  ...
  assert.equal(manifest.terminay.contributes.agentProviders[0].requiredEnvironmentVariables.includes("CODEX_HOME"), true);
});
