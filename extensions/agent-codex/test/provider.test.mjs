...
test('does not bind a Codex-looking record outside a rollout writer', async () => {
	...
	try {
		await harness.observe(
			fixtureTerminal({
				foregroundExecutable: 'codex',
				files: { '/tmp/not-a-rollout.jsonl': input },
			}),
		);
...
test('maps titles, collaboration children, completion and privacy allowlists', () => {
	...
	const publish = {
		...
		subagentStarted: (event) =>
			events.push({ kind: 'subagent.started', ...event }),
		sessionStopped: (event) =>
			events.push({ kind: 'session.stopped', ...event }),
	...
	mapCodexRecord(
		{
			...
			payload: {
				...
				item: {
					type: 'UserMessage',
					content: [
						{ type: 'text', text: 'Inspect the parser' },
						{ type: 'image', image_url: 'private' },
					],
				},
			},
		},
	...
	mapCodexRecord(
		{
			...
			payload: {
				...
				item: {
					...
					kind: 'started',
					agent_thread_id: 'child-live',
					agent_path: '/root/multiply',
				},
			},
		},
		context,
	...
	mapCodexRecord(
		{
			type: 'response_item',
			payload: { type: 'message', content: 'private assistant output' },
		},
		context,
	);
	assert.deepEqual(
		events.map((event) => event.kind),
		['agent.metadata', 'subagent.started', 'subagent.done', 'subagent.started'],
	);
	assert.equal(events[0]?.promptText, 'Inspect the parser');
	assert.equal(events[1]?.title, 'Ada');
	assert.deepEqual(events[1]?.model, {
		id: 'gpt-5.6-terra',
...
test('uses a stable root prompt, ignores abort guidance, and recognizes Codex executables', () => {
	...
	const publish = Object.fromEntries([
		['publish', (event) => events.push(event)],
		['sessionStarted', (event) => events.push(event)],
		['metadataChanged', (event) => events.push(event)],
		['turnStarted', (event) => events.push(event)],
		['toolStarted', (event) => events.push(event)],
		['toolFinished', (event) => events.push(event)],
		['waitStarted', (event) => events.push(event)],
		['waitFinished', (event) => events.push(event)],
		['done', (event) => events.push(event)],
		['exited', (event) => events.push(event)],
		['subagentStarted', (event) => events.push(event)],
		['subagentDone', (event) => events.push(event)],
	]);
	const context = { binding: { providerSessionId: 'root' }, publish };
	...
	mapCodexRecord(
		{
			...
			payload: {
				type: 'user_message',
				message: '<turn_aborted>private</turn_aborted>',
			},
		},
		context,
		state,
	...
	assert.equal(
		codexAgentProvider.matchesForeground({ executableName: 'codex' }),
		true,
	);
	assert.equal(
		codexAgentProvider.matchesForeground({ executableName: 'bash' }),
		false,
	);
	assert.match(
		effectiveCodexHome({ HOME: '/home/ignored', CODEX_HOME: '/custom/codex' }),
		/\/custom\/codex$/u,
	);
});
