		...
		await Promise.all([
			left.observe(
				fixtureTerminal({
					...
					files: {
						[`/home/test/.grok/sessions/%2Fleft/${firstId}/events.jsonl`]: root(
							firstId,
							0,
						),
					},
			...
			right.observe(
				fixtureTerminal({
					foregroundExecutable: 'grok',
					files: {
						[`/home/test/.grok/sessions/%2Fright/${secondId}/events.jsonl`]:
							root(secondId, 1),
					},
				}),
			),
...
test('new, resumed and branched roots rebind only to the newest exact writable Grok root', async () => {
	...
	const terminal = {
		...
		observation: {
			processes: {
				async descendants() {
					return [{ handle: { id: 'grok' }, executableName: 'grok' }];
				},
				async openFiles() {
			...
			files: {
				...
				async homeRelativePath(handle, options) {
					...
					return handle.id.startsWith(prefix)
				...
				async read(handle) {
					return new TextEncoder().encode(
						`${files[handle.id].map(JSON.stringify).join('\n')}\n`,
					);
				},
				async stat(handle) {
					return {
...
test('IPC number-array follow chunks still replay turn_ended', async () => {
	...
	const terminal = {
		...
		observation: {
			processes: {
				...
				async openFiles() {
					return [{ path, handle, access: 'writable' }];
				},
				async environment() {
					return {};
				},
			...
			files: {
				...
				async stat() {
					return { handle, kind: 'file', size: encoded.byteLength };
				},
				async follow() {
					return {
						async *[Symbol.asyncIterator]() {
	...
	for await (const chunk of result.source) {
		...
		for (const line of text.split('\n')) {
			if (line)
				result.mapRecord(JSON.parse(line), {
...
test('binds a writer-held events journal when the shell cannot expose HOME', async () => {
	...
	const terminal = {
		...
		observation: {
			...
			files: {
				...
				async read() {
					return new TextEncoder().encode(
						`${JSON.stringify({ type: 'turn_started', session_id: firstId, turn_number: 0, session_relationship: 'primary' })}\n`,
					);
				},
				async stat() {
					return {
	...
	assert.equal(
		bindingRequest.fingerprint.kind,
		'writable-file-below-terminal-process',
	);
});
		...
		[registryPath]: [
			{
				...
				pid: 4242,
				cwd,
				opened_at: '2026-08-05T10:00:00.000Z',
			},
		],
		[journalPath]: [
			{
	...
	const terminal = {
		...
		observation: {
			processes: {
				async descendants() {
					return [
						{ handle: { id: 'grok' }, executableName: 'grok', pid: 4242, cwd },
					];
				},
				async openFiles() {
					return [];
			...
			files: {
				...
				async resolveHomeRelative(relative) {
					...
					if (relative === `.grok/sessions/${encoded}/${firstId}/events.jsonl`)
						return journal;
					return undefined;
				},
				async resolveRelativeToEnvironment() {
				...
				async stat() {
					return { handle: journal, kind: 'file', size: 1 };
				},
				async follow() {
					const bytes = new TextEncoder().encode(
						`${files[journalPath].map(JSON.stringify).join('\n')}\n`,
					);
					return {
						async *[Symbol.asyncIterator]() {
							yield { type: 'append', bytes };
...
test('a later summary.json rewrite updates the bound root title without stalling turn_ended', {
	timeout: 5_000,
}, async () => {
	...
	const firstSummary = new TextEncoder().encode(
		`${JSON.stringify({
			info: { id: firstId },
	...
	const renamedSummary = new TextEncoder().encode(
		`${JSON.stringify({
			info: { id: firstId },
	...
	const terminal = {
		...
		observation: {
			processes: {
				...
				async openFiles() {
					return [
						{ path: eventsPath, handle: eventsHandle, access: 'writable' },
					];
				},
				async environment() {
					return {};
			...
			files: {
				async canonicalFile(file) {
					return file === eventsHandle ? file : undefined;
				},
				...
				async read(handle) {
					return handle === summaryHandle ? summaryBytes : encodedEvents;
				},
				async stat(handle) {
					return {
						handle,
	...
	for await (const chunk of result.source) {
		...
		for (const line of text.split('\n')) {
			...
			if (record.type === 'terminay.grok_metadata' && record.title)
				titles.push(record.title);
		}
		if (
			titles.includes('Renamed Grok session') &&
			events.some((event) => event.kind === 'agent.done')
		)
			break;
	}
	...
	assert.equal(
		events.find((event) => event.kind === 'agent.done')?.outcome,
		'success',
	);
	assert.equal(
		events.find(
			(event) =>
				...
				event.title === 'Renamed Grok session',
		)?.title,
		'Renamed Grok session',
	);
...
test('malformed, unknown and oversized records cannot create a wait or leak payloads', () => {
	...
	const context = {
		binding: { providerSessionId: firstId },
		journal: { role: 'root' },
		publish,
	};
	...
	assert.deepEqual(
		events.map((event) => event.kind),
		['sessionStarted', 'turnStarted', 'toolStarted', 'toolFinished'],
	);
