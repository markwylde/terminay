...
export type McpInstallProvider = Readonly<{
	id: McpAgentId;
	label: string;
	...
	uninstall(
		server: McpServerCommand | undefined,
		homeDirectory?: string,
	): Promise<McpInstallActionResult>;
}>;
...
export async function getMcpInstallStatus(
	...
	const agents = await Promise.all(
		MCP_INSTALL_PROVIDERS.map(async (provider) => {
			const registrationPromise: Promise<McpRegistrationInspection> =
				server === undefined
			...
			return {
				...
				configPath,
				...(registration.message === undefined
					? {}
...
function unknownAgent(agent: unknown): McpInstallActionResult {
	return {
		ok: false,
		installed: false,
		error: `Unknown agent: ${String(agent)}`,
	};
}
