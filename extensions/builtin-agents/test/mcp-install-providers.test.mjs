...
test('Claude Code install preserves unrelated configuration and uninstall removes only Terminay', async () => {
	...
	assert.equal(
		status.agents.find((agent) => agent.id === 'claudeCode')?.installed,
		true,
	);

	const uninstalled = await uninstallMcpAgent(
		'claudeCode',
		server,
		installOptions,
	);
	...
	assert.deepEqual(
		afterUninstall.mcpServers.existing,
...
test('Codex and Claude Code registrations launch the same MCP command contract', async () => {
	...
	assert.equal(
		(await installMcpAgent('codex', server, installOptions)).ok,
		true,
	);
	assert.equal(
		(await installMcpAgent('claudeCode', server, installOptions)).ok,
		true,
	);
	...
	assert.match(
		codexConfig,
		new RegExp(`command = ${escapeRegex(JSON.stringify(server.command))}`),
	);
	assert.match(
		codexConfig,
		new RegExp(`args = \\[${escapeRegex(JSON.stringify(server.args[0]))}\\]`),
	);
	assert.match(
		codexConfig,
	...
	const claudeConfig = JSON.parse(
		await readFile(join(testHome, '.claude.json'), 'utf8'),
	);
	...
	assert.equal(
		(await uninstallMcpAgent('codex', server, installOptions)).ok,
		true,
	);
	const codexAfterUninstall = await readFile(
		join(testHome, '.codex', 'config.toml'),
		'utf8',
	);
...
test('provider installs preserve config modes and identical entries are idempotent', async () => {
	...
	assert.equal(
		(await uninstallMcpAgent('claudeCode', server, options)).ok,
		true,
	);
...
test('provider registry detects and routes all supported registrations independently', async () => {
	...
	assert.deepEqual(
		MCP_INSTALL_PROVIDERS.map(({ id }) => id),
		agentIds,
	);
	assert.deepEqual(
		(await getMcpInstallStatus(server, options)).agents.map(
			({ id, state }) => ({ id, state }),
		),
	...
	assert.deepEqual(
		(await getMcpInstallStatus(server, options)).agents.map(
			({ id, state, installed }) => ({ id, state, installed }),
		),
