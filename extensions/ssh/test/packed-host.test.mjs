...
test("packed package activates and all public provider callbacks cross the real host IPC", async () => {
  ...
  const packageRoot = join(root, "package"); const installed = spawnSync("npm", ["install", "--ignore-scripts", "--omit=dev", join(root, packedFilename(api.stdout))], { cwd: packageRoot, encoding: "utf8", env: npmEnvironment }); assert.equal(installed.status, 0, installed.stderr);
