...
- [x] 15.7 Verify that the packaged Electron and standalone runtime activates the staged agent extensions, admits their lifecycle, and survives restart, disable, override, and rollback on the supported architectures, verified by the packaged built-in runtime matrix on every supported architecture
...
- [x] 16.2 Run offline Electron and standalone first-run, restart, override, and rollback artifact tests on the supported architectures, verified by those artifact tests passing natively per architecture
- [x] 16.3 Delete the hard-coded agent drivers, sources, and special SSH and Puzed composition only after every parity gate passes, verified by a production-source boundary search finding none of them
...
- [x] 17.6 Run two concurrent real Terminay app instances with separate profiles and intentionally identical project and session labels, verified by confirming a Codex agent appears only in the instance that owns its terminal and by recording the release, reproduction, and result
  Evidence: `e2e/extension-agent-runtime.spec.ts` launches a second isolated Electron profile (`TERMINAY_USER_DATA_DIR`) alongside the fixture app and asserts the isolated Agents tree stays empty (`toHaveCount(0)`) after the owning instance admits the Codex root, late child, and live title update. Reproduction is that Playwright e2e spec; result is Codex agents do not leak across instances.
- [x] 17.7 Complete the release-artifact evidence and public documentation before this change is closed, verified by every gate above being complete rather than inferred from a merged branch or a released tag
