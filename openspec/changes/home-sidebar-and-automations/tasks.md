...
- [ ] 1.1 Replace `LayoutDashboard` with lucide `House` on the project-bar Home
...
- [ ] 1.2 Add `rememberHomeSidebarVisible` / `recallHomeSidebarVisible` (default
...
- [ ] 1.3 While Home is selected, route the sidebar toggle button, its shortcut,
      ...
      the reverse.
- [ ] 1.4 Render the Home sidebar as a section menu (Home, Tabs, Automations)
...
- [ ] 1.5 Add `e2e/home-sidebar.spec.ts` covering:
...
- [ ] 2.1 Add `src/workspace/homeOverviewModel.ts` deriving:
...
- [ ] 2.2 Render the overview widgets with live updates and activation routing
...
- [ ] 3.1 Create the `packages/cron` workspace package with a five-field parser,
...
- [ ] 3.2 Register the package with turbo and the workspace boundary rules.
      Verified by `npm run test:boundaries` and `npm run typecheck:workspaces`.

## 4. Server: automation store and protocol

- [ ] 4.1 Add `automationService/types.ts` and `repository.ts`
      ...
      combination.
- [ ] 4.2 Add `runLog.ts`: a bounded store of 100 runs per automation with the
      ...
      round trip.
- [ ] 4.3 Add file backends `automations.v1.json` and `automation-runs.v1.json`
...
- [ ] 4.4 Add `automationService/protocol.ts` operations and events (D4) with
...
- [ ] 5.1 Add the `automations` project kind to `workspace.ts`. It is created
...
- [ ] 5.2 Filter the reserved kind out of every project projection at the
      ...
      Tabs.
- [ ] 5.3 Add a 50-terminal cap on the automation space, refusing further
      creation with a bounded error. Verified by a unit test at the cap.

## 6. Server: scheduler and triggers

- [ ] 6.1 Add `scheduler.ts`: one timer armed to the earliest due time, re-armed
...
- [ ] 6.2 Count missed occurrences from persisted `evaluatedThrough` at start,
      ...
      simulated sleep.
- [ ] 6.3 Emit typed `project.opened` / `project.closed` journal events from
...
- [ ] 6.4 Add `triggers.ts`, which subscribes to agent, activity, attention,
...
- [ ] 7.1 Add `executor.ts` run-command:
...
- [ ] 7.2 Add subject actions: Run Macro through `MacroRunner` with an exact
...
- [ ] 7.3 Add the loop guard (default 60 s, minimum 5 s for subject actions,
...
- [ ] 7.4 Add optional recording via `RecordingService.start` on the run
...
- [ ] 7.5 Add audit entries for every definition change (editing actor) and
      every run (automation principal). Verified by a test reading the audit
      trail after a save and a run.

## 8. MCP workspace scope

- [ ] 8.1 Add a `workspace` scope to `ControlCapabilityStore.mint`, chosen at
      ...
      value changes it.
- [ ] 8.2 In `terminalAdapter.ts`, let workspace scope address any session on
...
- [ ] 8.3 Confirm no automation operation is reachable through MCP. Verified by
      a capability-listing test asserting the tool surface.

## 9. Automations section UI

- [ ] 9.1 Build the Automations list with a server selector when several
...
- [ ] 9.3 Build run history and run detail (outcome, exit code, duration, tail,
...
- [ ] 9.4 Build the missed-run notice on attach: a short explanation, the listed
