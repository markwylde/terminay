...
- [x] 1.1 Add an optional `canonicalRoot` to `CanonicalProjectPathOptions` in
...
- [x] 1.2 Have `FileCatalog.list()` call `resolver.root()` once and pass it to
  ...
  listing output.
- [x] 1.3 Confirm no canonical root is held across operations. Verified by
...
- [x] 2.1 In `describe()`, trust `raw.isSymbolicLink` when it is a boolean and
...
- [x] 2.2 Have `canonicalTarget()` return the stat it already took and have
...
- [x] 3.1 Add `packages/server-core/test/file-catalog-path-budget.test.mjs`
...
- [x] 3.2 Record the before/after counts measured from the same harness in
...
- [x] 4.1 In `electron/main.ts`, split the disk read out of
...
- [x] 4.2 Arm an `fs.watch` on the user-data directory that invalidates the
  ...
  passing.
- [x] 4.3 Invalidate the cache inside `writeTerminalSettings` and
