...
export function jsonlSession(
	options: AgentJsonlSessionOptions,
): AgentJsonlSession {
	...
	if (
		options.childSourceDiscovery !== undefined &&
		typeof (options.childSourceDiscovery as AsyncIterable<unknown>)[
			Symbol.asyncIterator
		] !== 'function' &&
		!(options.childSourceDiscovery instanceof Promise)
	) {
		throw new Error(
			'childSourceDiscovery: expected an async iterable or promise',
		);
	}
...
export function safeAgentString(
	value: unknown,
	maximum = EXTENSION_LIMITS.stringLength,
): string | undefined {
...
export function selectAgentMapping<T>(
	mappings: readonly AgentMapping<T>[],
	providerVersion: string,
): T | undefined {
	if (mappings.length === 0) return undefined;
	const ordered = [...mappings].sort((left, right) =>
		compareVersions(left.providerVersion, right.providerVersion),
	);
	...
	for (const candidate of ordered) {
		...
		if (
			...
			compareParsedVersions(candidateVersion, requested) <= 0
		)
			selected = candidate;
	}
...
export function createJsonlRecordDecoder(
	maxRecordBytes = EXTENSION_LIMITS.agentRecordBytes,
): JsonlRecordDecoder {
	const maximum = Math.max(
		1,
		Math.min(maxRecordBytes, EXTENSION_LIMITS.agentRecordBytes),
	);
				...
				if (byte === 0x0a) {
					if (!discarding && pending.length > 0) {
						...
						try {
							...
							/* provider-private malformed input */
						}
					}
					decoder = new TextDecoder();
					pending = [];
...
export function createAgentLifecyclePublisher(
	...
	const emit = (event: AgentLifecycleEvent): void | Promise<void> =>
		sink(assertAgentLifecycleEvent(event));
	return {
