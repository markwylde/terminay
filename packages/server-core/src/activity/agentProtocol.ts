 ...
 */
export function createAgentOperationRegistry(
	options: AgentOperationRegistryOptions,
): AgentOperationRegistry {
	if (!(options.service instanceof AgentStatusService))
		throw new TypeError('agent status service is required');
	...
	return {
		operations: {
			queries: {
				[AGENT_OPERATIONS.snapshot]: (request: QueryRequest) =>
					snapshot(request),
			},
			commands: {
				[AGENT_OPERATIONS.acknowledge]: (request: CommandRequest) =>
					acknowledge(request),
			},
			policies: {
	...
	function snapshot(request: QueryRequest): JsonValue {
		...
		return asSnapshot(
			options.service.getSnapshotForProject(projectClaim(request)),
		);
	}
	...
	function acknowledge(request: CommandRequest): JsonValue {
		...
		return {
			...
			cursor: String(current.revision),
		} as JsonValue;
	}
}

function asSnapshot(
	snapshot: ReturnType<AgentStatusService['getSnapshot']>,
): JsonValue {
...
function assertReadable(request: QueryRequest): void {
	if (request.context.authScope === 'none')
		throw protocolError('forbidden', 'agents require read access');
}

function objectPayload(value: JsonValue): Record<string, JsonValue> {
	if (typeof value !== 'object' || value === null || Array.isArray(value))
		throw protocolError('validation', 'agent payload must be an object');
...
function id(value: JsonValue | undefined, name: string): string {
	if (
		typeof value !== 'string' ||
		!/^[A-Za-z0-9][A-Za-z0-9._:%-]{0,4095}$/u.test(value)
	) {
...
export function createAgentEventProjector(
	...
	return (event, client) => {
		...
		if (
			...
			return undefined;
		const scoped = service.filterSnapshotForProject(
			{
