...
import {
	AgentStatusService,
	type ExtensionLifecycleIngestResult,
...
export function createExtensionAgentBroker(
	...
	const publications = new Map<
		string,
		Map<string, ExtensionLifecycleIngestResult>
	>();
	...
	return {
		observe:
			options.observe ??
			(async () => {
				throw new Error('agent observation routing is unavailable');
			}),
		async publish(request, signal) {
			if (!isNamespacedId(request.providerId, request.extensionId)) {
				return Object.freeze({
					acceptedEventCount: 0,
					rejectedEventCount: request.events.length,
					failure: 'extension agent provider is outside its namespace',
				});
			}
			const identity = identityFor(request.terminal);
			if (retiredContexts.has(request.terminal.contextId))
				return Object.freeze({
					acceptedEventCount: 0,
					rejectedEventCount: request.events.length,
					failure: 'extension lifecycle scope is retired',
				});
			const prior = publications
				.get(request.terminal.contextId)
				?.get(request.publicationId);
			...
			if (count >= maximumQueued)
				return Object.freeze({
					acceptedEventCount: 0,
					rejectedEventCount: request.events.length,
					failure: 'extension lifecycle publication queue is full',
				});
			const execution = (async (): Promise<ExtensionLifecycleIngestResult> => {
				...
				tails.set(
					contextId,
					priorTail.then(() => ownTail),
				);
				let result: ExtensionLifecycleIngestResult;
				try {
					await waitForTurn(priorTail, signal, acknowledgementDeadlineMs);
					if (signal.aborted)
						throw new Error('extension lifecycle publication cancelled');
					result = await withDeadline(
						service.ingestExtensionLifecycle(
							identity,
							request.providerId,
					...
					if (remaining <= 0) {
						queued.delete(contextId);
						if (tails.get(contextId) !== undefined) tails.delete(contextId);
					} else queued.set(contextId, remaining);
				}
				const byContext =
					publications.get(request.terminal.contextId) ??
					new Map<string, ExtensionLifecycleIngestResult>();
				publications.set(request.terminal.contextId, byContext);
				if (byContext.size >= 128)
			...
			try {
				...
				if (inFlight.get(publicationKey) === execution)
					inFlight.delete(publicationKey);
			}
		},
		terminalCancelled(request) {
			if (retiredContexts.has(request.terminal.contextId)) return;
			if (retiredContexts.size >= 4_096)
				retiredContexts.delete(retiredContexts.values().next().value!);
			...
			for (const key of inFlight.keys())
				if (key.startsWith(`${request.terminal.contextId}\0`))
					inFlight.delete(key);
			const identity = identityFor(request.terminal);
			try {
				service.releaseExtensionProvider(identity, request.providerId);
			} catch {
				/* terminal teardown is idempotent */
			}
			options.onTerminalCancelled?.(
				request.terminal.contextId,
				request.providerId,
			);
		},
	};
...
async function withDeadline<T>(
	...
	if (
		...
		throw new RangeError(
			'extension lifecycle acknowledgement deadline is invalid',
		);
	if (signal.aborted)
		throw new Error('extension lifecycle publication cancelled');
	...
	const bounded = new Promise<never>((_resolve, reject) => {
		timer = setTimeout(
			() => reject(new Error('extension lifecycle acknowledgement timed out')),
			deadlineMs,
		);
		abort = () =>
			reject(new Error('extension lifecycle publication cancelled'));
