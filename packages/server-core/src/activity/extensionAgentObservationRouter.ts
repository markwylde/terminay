...
export function createExtensionAgentObservationRouter(
	options: ExtensionAgentObservationRouterOptions,
) {
	return async (
		request: Readonly<{
		...
		if (
			binding === undefined ||
			binding.serverId !== request.terminal.serverId ||
			binding.projectId !== request.terminal.projectId ||
			binding.projectEnvironmentId !== request.terminal.projectEnvironmentId
		)
			throw new Error('agent observation environment binding is unavailable');
		if (binding.projectEnvironmentId === THIS_SERVER_ENVIRONMENT_ID)
			return options.local.observe(
				request.terminal,
				request.operation,
				request.payload,
				signal,
		...
		return options.router.invokeBound<JsonValue>(
			binding,
			capability,
			`agent-observation.${request.operation}`,
			Object.freeze({
...
function observationCapability(
	...
	if (operation.startsWith('process.') || operation === 'terminal.tty')
		return 'process-observation';
	if (
		operation === 'filesystem.follow' ||
		operation === 'filesystem.unfollow' ||
		operation === 'filesystem.read' ||
		operation === 'filesystem.watch-directory' ||
		operation === 'filesystem.unwatch-directory'
	)
