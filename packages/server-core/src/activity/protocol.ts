 ...
 */
export function createActivityOperationRegistry(
	options: ActivityOperationRegistryOptions,
): ActivityOperationRegistry {
	if (!(options.service instanceof TerminalActivityService))
		throw new TypeError('activity service is required');
	const unsubscribe = options.service.subscribe((event) => {
		options.eventJournal.append(
			ACTIVITY_OPERATIONS.event,
	...
	return {
		operations: {
			queries: {
				[ACTIVITY_OPERATIONS.snapshot]: (request: QueryRequest) =>
					snapshot(request),
				[ACTIVITY_OPERATIONS.delta]: (request: QueryRequest) => delta(request),
				[ACTIVITY_OPERATIONS.closePreflight]: (request: QueryRequest) =>
					closePreflight(request),
			},
			commands: {
				[ACTIVITY_OPERATIONS.acknowledge]: (request: CommandRequest) =>
					acknowledge(request),
			},
			policies: {
	...
	function snapshot(request: QueryRequest): JsonValue {
		...
		return options.service.snapshotForProject(
	...
	async function closePreflight(request: QueryRequest): Promise<JsonValue> {
		...
		const sessionId =
			payload.sessionId === undefined
				? undefined
				: id(payload.sessionId, 'sessionId');
		const claimedProjectId = projectClaim(request);
		if (claimedProjectId !== undefined && claimedProjectId !== projectId) {
			throw protocolError(
				'forbidden',
				'activity close preflight is outside the authenticated project scope',
			);
		}
		const registeredProject =
			sessionId === undefined
				? undefined
				: options.service.projectIdForSession(sessionId);
		if (registeredProject !== undefined && registeredProject !== projectId) {
			throw protocolError(
				'forbidden',
				'activity close preflight is outside the session project',
			);
		}
		...
		const sessions: ActivityClosePreflightSession[] = observed
			? observations.map((observation) => {
					const committed = options.service.snapshotForProject(
	...
	function delta(request: QueryRequest): JsonValue {
		...
		if (cursor !== String(revision))
			throw protocolError(
				'validation',
				'activity cursor does not match revision',
			);
		return options.service.replayForProject(
			revision,
			projectClaim(request),
		) as unknown as JsonValue;
	}
	...
	function acknowledge(request: CommandRequest): JsonValue {
		...
		if (claimedProjectId !== undefined && claimedProjectId !== projectId) {
			throw protocolError(
				'forbidden',
				'activity acknowledgement is outside the authenticated project scope',
			);
		}
...
function assertReadable(request: QueryRequest): void {
	if (request.context.authScope === 'none')
		throw protocolError('forbidden', 'activity requires read access');
}

function objectPayload(value: JsonValue): Record<string, JsonValue> {
	if (typeof value !== 'object' || value === null || Array.isArray(value))
		throw protocolError('validation', 'activity payload must be an object');
...
export function createActivityEventProjector(
	...
	return (event, client) => {
		...
		if (
			...
			service.projectIdForSession(sessionId) !== projectId
		)
			return undefined;
		...
		if (
			...
			(typeof snapshot !== 'object' ||
				...
				(snapshot as Record<string, unknown>).projectId !== projectId)
		)
			return undefined;
		return event;
	};
...
function fallbackLimitedSessions(
	...
	return Object.values(service.snapshotForProject(projectId).sessions).map(
		(session) =>
			Object.freeze({
