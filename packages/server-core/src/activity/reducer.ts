...
export class TerminalActivityReducer {
	...
	constructor(options: TerminalActivityReducerOptions = {}) {
		this.maxEvents = normalizePositive(
			options.maxEvents,
			DEFAULT_MAX_EVENTS,
			'maxEvents',
		);
		this.rawActivityMs = normalizeMs(
			options.rawActivityMs,
			DEFAULT_RAW_ACTIVITY_MS,
			'rawActivityMs',
		);
		this.progressStaleMs = normalizeMs(
			options.progressStaleMs,
			DEFAULT_PROGRESS_STALE_MS,
			'progressStaleMs',
		);
		this.now = options.now ?? (() => Date.now());
		this.currentRevision = normalizeRevision(
			options.initialRevision ?? 0,
			'initialRevision',
		);
		for (const seed of options.initialSessions ?? [])
			this.register(seed.sessionId, seed.projectId, seed.now);
	}
	...
	register(
		...
		projectId?: string,
		at = this.now(),
	): TerminalActivitySessionSnapshot | undefined {
		...
		if (existing) {
			...
			if (projectId !== undefined && existing.projectId === undefined)
	...
	applySignal(
		...
		const existing = this.sessionsById.get(sessionId);
		if (!matchesProject(existing, options.projectId) || isStale(existing, at))
			return undefined;
			...
			case 'command':
				...
				if (signal.phase === 'executing') {
					...
					if (
						...
						(session.settled && !recentInput(session, at))
					) {
						session.acknowledged = false;
					}
	...
	applyRawOutput(
		...
		if (session.settled && !recentInput(session, at))
			session.acknowledged = false;
	...
	applyProviderActivity(
		...
		if (session.provider !== undefined && session.provider !== update.provider)
			return undefined;
		if (
			session.agentId !== undefined &&
			update.agentId !== undefined &&
			session.agentId !== update.agentId
		)
			return undefined;
		if (
			session.providerSequence !== undefined &&
			update.sequence !== undefined &&
			update.sequence <= session.providerSequence
		)
			return undefined;
		if (session.providerSequence !== undefined && update.sequence === undefined)
			return undefined;

		const before = snapshotOf(session);
		const state =
			update.state !== undefined || update.status !== undefined
		...
		session.source = update.source
			? boundSource(update.source)
			: `provider:${update.provider}`;
		session.status = state === 'working' ? 'working' : 'idle';
		session.attention =
			update.attention ?? (state === 'waiting' || state === 'blocked');
	...
	acknowledge(
		...
		const at = normalizeTime(options.now ?? this.now());
		if (!matchesProject(session, options.projectId) || isStale(session, at))
			return undefined;
		this.expire(session, at);
	...
	markTerminalExit(
		sessionId: string,
		options: { readonly now?: number; readonly projectId?: string } = {},
	): ActivityEvent | undefined {
		assertSessionId(sessionId);
		if (this.exited.has(sessionId)) return undefined;
		const session = this.sessionsById.get(sessionId);
		if (
			...
			(session === undefined || !matchesProject(session, options.projectId))
		)
			return undefined;
		if (!session) {
	...
	tick(at = this.now()): readonly ActivityEvent[] {
		...
		for (const session of this.sessionsById.values()) {
			...
			if (session.provider === undefined)
				derive(session, time, this.rawActivityMs);
	...
	nextDeadline(): number | null {
		...
		for (const session of this.sessionsById.values()) {
			if (session.progressDeadline !== undefined)
				deadline = minDefined(deadline, session.progressDeadline);
			if (session.rawActivityAt !== undefined)
				deadline = minDefined(
					deadline,
					session.rawActivityAt + this.rawActivityMs,
				);
			if (session.inputQuietDeadline !== undefined)
				deadline = minDefined(deadline, session.inputQuietDeadline);
		}
	...
	snapshot(): ActivitySnapshot {
		...
		for (const [id, value] of this.sessionsById)
			sessions[id] = snapshotOf(value);
		return Object.freeze({
			revision: this.currentRevision,
			cursor: String(this.currentRevision),
	...
	replay(afterRevision = 0): ActivityReplay {
		...
		if (afterRevision > this.currentRevision)
			throw new RangeError('afterRevision is ahead of current revision');
	...
	subscribe(listener: ActivityListener): () => void {
		if (typeof listener !== 'function')
			throw new TypeError('activity listener must be a function');
	...
	private expire(session: MutableSession, at: number): void {
		...
		if (
			session.rawActivityAt !== undefined &&
			at >= session.rawActivityAt + this.rawActivityMs
		) {
			session.rawActivityAt = undefined;
		}
		if (
			session.inputQuietDeadline !== undefined &&
			at >= session.inputQuietDeadline
		) {
			session.inputQuietDeadline = undefined;
			if (
				session.provider === undefined &&
	...
	private updateFallbackEvidence(
		...
		if (signal.kind === 'progress') {
			...
			session.progressDeadline =
				signal.state === 0 ? undefined : at + this.progressStaleMs;
		}
	...
	private nextRevision(): number {
		if (this.currentRevision === Number.MAX_SAFE_INTEGER)
			throw new RangeError('activity revision exhausted');
...
export function createTerminalActivityReducer(
	options: TerminalActivityReducerOptions = {},
...
function derive(
	...
	const rawWorking =
		!session.explicitSeen &&
		session.rawActivityAt !== undefined &&
		at < session.rawActivityAt + rawActivityMs;
	const working =
		session.progressBusy ||
		session.commandExecuting ||
		(session.foregroundBusy && !session.explicitSeen) ||
		rawWorking;
...
function normalizeProviderState(
	value: ProviderActivityUpdate['state'] | ProviderActivityUpdate['status'],
): ProviderActivityState {
...
function assertProviderUpdate(update: ProviderActivityUpdate): void {
	if (
		...
		update.provider.length === 0 ||
		update.provider.length > 64 ||
		update.provider.includes('\0')
	)
		throw new TypeError('provider update is invalid');
	if (
		update.sequence !== undefined &&
		(!Number.isSafeInteger(update.sequence) || update.sequence < 0)
	)
		throw new TypeError('provider sequence is invalid');
	if (
		update.agentId !== undefined &&
		(typeof update.agentId !== 'string' ||
			update.agentId.length === 0 ||
			update.agentId.length > 256)
	)
		throw new TypeError('provider agent id is invalid');
}
...
function normalizeTime(value: number): number {
	if (!Number.isFinite(value))
		throw new RangeError('activity time must be finite');
	return value;
}
...
function recentInput(session: MutableSession, at: number): boolean {
	return (
		...
		at - session.lastUserInputAt < DEFAULT_RAW_ACTIVITY_MS
	);
}
