...
export function trimChars(
	value: string,
	maxChars: number,
): { readonly text: string; readonly truncated: boolean } {
	if (!Number.isSafeInteger(maxChars) || maxChars < 0)
		throw new RangeError('maxChars must be non-negative');
...
export function safeProviderError(
	...
	if (error instanceof AiServiceError && !isProviderErrorCode(error.code))
		return error;
	...
	return new AiServiceError(
		safeCode,
		sanitized || fallback,
		safeCode === 'provider_timeout' || safeCode === 'provider_cancelled',
	);
}
...
export function throwIfAborted(
	signal: AbortSignal,
	code: AiServiceErrorCode = 'cancelled',
): void {
	if (signal.aborted)
