...
import { type AiProviderCredentialResolver, AiServiceError } from './types.js';
 ...
 */
export class VaultProviderCredentialResolver
	implements AiProviderCredentialResolver
{
	...
	constructor(options: VaultProviderCredentialResolverOptions) {
		if (!options?.vault || typeof options.vault.withSecret !== 'function')
			throw new TypeError('a server vault is required');
		if (!Array.isArray(options.bindings))
			throw new TypeError('provider credential bindings are required');
		const bindings = new Map<string, ProtocolId>();
		for (const binding of options.bindings) {
			if (!binding || typeof binding.provider !== 'string')
				throw new TypeError('provider credential binding is invalid');
			const provider = normalizeProvider(binding.provider);
			if (provider === 'disabled')
				throw new TypeError(
					'disabled provider cannot have a credential binding',
				);
			if (
				typeof binding.secretId !== 'string' ||
				!VAULT_ID_PATTERN.test(binding.secretId)
			)
				throw new TypeError('provider credential secret id is invalid');
			if (bindings.has(provider))
				throw new TypeError('provider credential binding is duplicated');
	...
	withCredential<T>(
		...
		if (typeof callback !== 'function')
			return Promise.reject(new TypeError('credential callback is required'));
		let provider: string;
		try {
		...
		return this.vault
			.withSecret(secretId, async (secret) => {
				try {
