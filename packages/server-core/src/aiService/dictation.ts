...
export class DictationService {
	...
	constructor(options: DictationServiceOptions) {
		...
		this.limits = {
			...
			maxAudioDurationMs:
				options.limits?.maxAudioDurationMs ?? DEFAULT_MAX_AUDIO_DURATION_MS,
			maxAudioUploadMs:
				options.limits?.maxAudioUploadMs ?? DEFAULT_MAX_AUDIO_UPLOAD_MS,
			maxProviderOutputBytes:
				options.limits?.maxProviderOutputBytes ??
				DEFAULT_MAX_PROVIDER_OUTPUT_BYTES,
			maxTranscriptChars:
				options.limits?.maxTranscriptChars ?? DEFAULT_MAX_TRANSCRIPT_CHARS,
			allowedAudioMimeTypes:
				options.limits?.allowedAudioMimeTypes ??
				DEFAULT_ALLOWED_AUDIO_MIME_TYPES,
			maxStatusSnapshots: options.limits?.maxStatusSnapshots ?? 256,
		};
		this.now = options.now ?? Date.now;
		if (
			!Number.isSafeInteger(this.limits.maxAudioBytes) ||
			this.limits.maxAudioBytes <= 0
		)
			throw new RangeError('maxAudioBytes must be positive');
		if (
			!Number.isSafeInteger(this.limits.maxAudioDurationMs) ||
			this.limits.maxAudioDurationMs <= 0
		)
			throw new RangeError('maxAudioDurationMs must be positive');
		if (
			!Number.isSafeInteger(this.limits.maxAudioUploadMs) ||
			this.limits.maxAudioUploadMs <= 0
		)
			throw new RangeError('maxAudioUploadMs must be positive');
		if (
			!Number.isSafeInteger(this.limits.maxProviderOutputBytes) ||
			this.limits.maxProviderOutputBytes <= 0
		)
			throw new RangeError('maxProviderOutputBytes must be positive');
		if (
			!Number.isSafeInteger(this.limits.maxTranscriptChars) ||
			this.limits.maxTranscriptChars <= 0
		)
			throw new RangeError('maxTranscriptChars must be positive');
	}
	...
	async transcribe(
		...
		if (this.running.has(request.requestId))
			throw new AiServiceError('invalid_request', 'request is already running');
		const before = await this.authorizeAndState(
			request.clientId,
			request.target,
		);
		const settings = await this.readSettings();
		if (settings.enabled === false || settings.provider === 'disabled')
			throw new AiServiceError(
				'provider_disabled',
				'dictation provider is disabled.',
			);
		const model = (
			request.model ??
			settings.model ??
		...
		if (!model)
			throw new AiServiceError(
				'invalid_request',
				'dictation model is invalid.',
			);

		const startedAt = this.now();
		const controller = new AbortController();
		const snapshot: AiRequestStatusSnapshot = {
		...
		this.log(snapshot);
		const bounded = deadlineSignal(
			request.signal,
			request.deadlineMs ?? this.limits.maxAudioUploadMs,
		);
		...
		try {
			...
			const audio = await raceAbort(
				...
				() =>
					new AiServiceError(
						bounded.timedOut() ? 'audio_timeout' : 'provider_cancelled',
						bounded.timedOut()
							? 'dictation upload timed out.'
							: 'dictation upload was cancelled.',
						true,
					),
			);
			if (audio.byteLength === 0)
				throw new AiServiceError('audio_empty', 'dictation audio is empty.');
			if (
				request.peakLevel !== undefined &&
				(!Number.isFinite(request.peakLevel) || request.peakLevel < 0)
			)
				throw new AiServiceError(
					'invalid_request',
					'dictation audio level is invalid.',
				);
			if (request.peakLevel === 0)
				throw new AiServiceError(
					'audio_inaudible',
					'dictation audio contains no audible speech.',
				);
			const elapsed = Math.max(0, this.now() - startedAt);
			if (
				request.durationMs !== undefined &&
				request.durationMs > this.limits.maxAudioDurationMs
			)
				throw new AiServiceError(
					'audio_duration_exceeded',
					'dictation recording exceeds the duration limit.',
				);
			if (elapsed > this.limits.maxAudioDurationMs)
				throw new AiServiceError(
					'audio_duration_exceeded',
					'dictation recording exceeds the duration limit.',
				);
			throwIfAborted(bounded.signal);
			const language =
				(request.language ?? settings.language)?.trim() || undefined;
			const prompt = request.prompt ?? settings.prompt;
			const withCredential =
				this.options.credentialResolver === undefined
					? undefined
					: <T>(callback: (secret: Uint8Array) => T | Promise<T>): Promise<T> =>
							this.options.credentialResolver!.withCredential(
								settings.provider ?? '',
								callback,
							);
			const pending = Promise.resolve(
				this.options.provider.transcribe({
					model,
					language,
					prompt,
					...
					...(withCredential === undefined ? {} : { withCredential }),
				}),
			);
			...
			const text = normalizeTranscript(
				raw,
				this.limits.maxProviderOutputBytes,
				this.limits.maxTranscriptChars,
			);
			if (!text)
				throw new AiServiceError(
					'empty_output',
					'dictation provider returned an empty transcript.',
				);
			const after = await this.authorizeAndState(
				request.clientId,
				request.target,
			);
			if (
				...
				throw new AiServiceError(
					'target_exited',
					'terminal target changed or has exited.',
				);
			if (this.options.authority.writeInput === undefined)
				throw new AiServiceError(
					'target_unavailable',
			...
			const appendNewline =
				request.appendNewline ?? settings.appendNewline ?? false;
			await this.options.authority.writeInput(
				request.target,
				appendNewline ? `${text}\n` : text,
			);
			...
			this.finish(
				request.requestId,
				mapped.code === 'cancelled' || mapped.code === 'provider_cancelled'
					? 'cancelled'
					: 'failed',
				mapped.code,
			);
	...
	snapshots(): readonly AiRequestStatusSnapshot[] {
		return [...this.history.values()].map((value) => ({
			...value,
			target: { ...value.target },
		}));
	}
	...
	private async authorizeAndState(
		clientId: ProtocolId,
		...
		if (!state.live)
			throw new AiServiceError('target_exited', 'terminal target has exited.');
		if (
			this.options.authority.authorize !== undefined &&
			!(await this.options.authority.authorize(clientId, target))
		)
			throw new AiServiceError(
				'not_authorized',
				'client is not authorized for this terminal.',
			);
		return state;
	}
	...
	private assertRequest(request: DictationTranscribeRequest): void {
		...
		if (
			!this.limits.allowedAudioMimeTypes
				.map((value) => value.toLowerCase())
				.includes(mime)
		)
			throw new AiServiceError(
				'audio_type_unsupported',
				'dictation audio MIME type is unsupported.',
			);
		if (
			request.durationMs !== undefined &&
	...
	private assertTarget(target: TerminalTarget): void {
		if (!target || typeof target !== 'object')
			throw new AiServiceError(
				'invalid_request',
				'dictation target is required.',
			);
		for (const [name, value] of Object.entries(target))
			this.assertId(value as string, `target.${name}`);
		if (target.serverId !== this.options.serverId)
			throw new AiServiceError(
				'target_unavailable',
	...
	private mapError(error: unknown, timedOut: boolean): AiServiceError {
		...
		if (timedOut)
			return new AiServiceError(
				'audio_timeout',
				'dictation request timed out.',
				true,
			);
		if (isAbortError(error))
			return new AiServiceError(
				'provider_cancelled',
				'dictation request was cancelled.',
				true,
			);
	...
	private finish(
		...
		const next: AiRequestStatusSnapshot = {
			...current,
			status,
			finishedAt: this.now(),
	...
	private log(snapshot: AiRequestStatusSnapshot): void {
		this.options.logger?.status?.({
			requestId: snapshot.requestId,
			kind: snapshot.kind,
			status: snapshot.status,
			targetSessionId: snapshot.target.sessionId,
	...
	private assertId(value: string, name: string): void {
		if (typeof value !== 'string' || !idPattern.test(value))
			throw new AiServiceError('invalid_request', `${name} is invalid.`);
	}
}
...
function normalizeMime(value: string): string {
	if (typeof value !== 'string')
		throw new AiServiceError(
			'invalid_request',
			'dictation audio MIME type is required.',
		);
...
async function collectAudio(
	...
	const append = (chunk: Uint8Array): void => {
		if (!(chunk instanceof Uint8Array))
			throw new AiServiceError(
				'invalid_request',
				'dictation audio chunk is invalid.',
			);
		total += chunk.byteLength;
		if (total > maxBytes)
			throw new AiServiceError(
				'audio_too_large',
				'dictation audio exceeds the upload limit.',
			);
...
function normalizeTranscript(
	value: DictationProviderResult | string,
	...
	const raw =
		typeof value === 'string'
			...
			: value && typeof value === 'object' && typeof value.text === 'string'
				? value.text
				: '';
	if (utf8ByteLength(raw) > maxOutputBytes)
		throw new AiServiceError(
			'audio_output_too_large',
			'dictation provider returned an oversized transcript.',
		);
	const normalized = stripTerminalControls(raw)
		.replace(/^```(?:text|txt|markdown|md)?\s*/i, '')
		.replace(/```\s*$/i, '')
		.replace(/^(?:transcript|text|answer)\s*:\s*/i, '')
		.trim();
