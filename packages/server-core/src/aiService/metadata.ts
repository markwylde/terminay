...
export class AiMetadataService {
	...
	constructor(options: AiMetadataServiceOptions) {
		...
		this.limits = {
			maxContextBytes:
				options.limits?.maxContextBytes ?? DEFAULT_MAX_CONTEXT_BYTES,
			maxContextChars:
				options.limits?.maxContextChars ?? DEFAULT_MAX_CONTEXT_CHARS,
			maxProviderOutputBytes:
			...
			providerTimeoutMs:
				options.limits?.providerTimeoutMs ?? DEFAULT_PROVIDER_TIMEOUT_MS,
			modelListTimeoutMs:
				options.limits?.modelListTimeoutMs ?? DEFAULT_MODEL_LIST_TIMEOUT_MS,
	...
	async context(target: TerminalTarget): Promise<AiMetadataContext> {
		...
		this.assertExactTarget(
			target,
			state?.serverId === this.options.serverId ? state : undefined,
		);
		const currentTitle = trimUtf8(
			trimChars(
				stripTerminalControls(state?.title ?? 'Terminal'),
				this.limits.maxTitleChars,
			).text,
			this.limits.maxTitleChars * 4,
		).text;
		const existingNote = trimUtf8(
			trimChars(
				stripTerminalControls(state?.note ?? ''),
				this.limits.maxNoteChars,
			).text,
			this.limits.maxNoteChars * 4,
		).text;
		const metadataBytes =
			utf8ByteLength(currentTitle) + utf8ByteLength(existingNote);
	...
	async listModels(
		...
		if (provider === 'disabled')
			throw new AiServiceError('provider_disabled', 'AI provider is disabled.');
		...
		try {
			...
			const raw = await raceAbort(
				...
				() =>
					new AiServiceError(
						bounded.timedOut() ? 'provider_timeout' : 'provider_cancelled',
						bounded.timedOut()
							? 'AI model discovery timed out.'
							: 'AI model discovery was cancelled.',
						true,
					),
			);
			if (!Array.isArray(raw))
				throw new AiServiceError(
					'provider_unavailable',
					`${providerLabel(provider)} returned an invalid model list.`,
				);
			let serializedBytes = 0;
			try {
				serializedBytes = utf8ByteLength(JSON.stringify(raw));
			} catch {
				throw new AiServiceError(
					'provider_unavailable',
					`${providerLabel(provider)} returned an invalid model list.`,
			...
			if (serializedBytes > this.limits.maxProviderOutputBytes)
				throw new AiServiceError(
					'provider_output_too_large',
					`${providerLabel(provider)} returned an oversized model list.`,
				);
			...
			for (const item of raw) {
				...
				const label =
					typeof candidate.label === 'string' && candidate.label.trim()
						? candidate.label.trim().slice(0, 256)
						: id;
				seen.add(id);
			...
			throw safeProviderError(
				error,
				`Unable to list ${providerLabel(provider)} models.`,
			);
		} finally {
			bounded.cancel();
		}
	...
	async generate(request: AiMetadataRequest): Promise<AiMetadataResult> {
		...
		const state = await this.authorizeAndState(
			request.clientId,
			request.target,
		);
		const expectedRevision = request.expectedRevision ?? state.metadataRevision;
		if (expectedRevision !== state.metadataRevision)
			throw new AiServiceError(
				'revision_conflict',
				'terminal metadata revision is stale.',
				true,
			);
		...
		if (
			...
			provider !== normalizeProviderName(settings.provider)
		)
			throw new AiServiceError(
				'invalid_request',
				'requested provider does not match server settings.',
			);
		if (provider === 'disabled')
			throw new AiServiceError(
				'provider_disabled',
				`AI ${request.targetType} generation is disabled.`,
			);
		const model = (request.model ?? settings.model).trim();
		if (!model)
			throw new AiServiceError(
				'invalid_request',
				'Choose an AI model before generating tab metadata.',
			);
		const adapter = this.providers[provider];
		...
		const bounded = deadlineSignal(
			request.signal,
			request.deadlineMs ?? this.limits.providerTimeoutMs,
		);
		...
		try {
			...
			const rawPending = Promise.resolve(
				adapter.generate({
					provider,
					model,
					target: request.targetType,
					context,
			...
			const raw = await raceAbort(
				...
				() =>
					new AiServiceError(
						bounded.timedOut() ? 'provider_timeout' : 'provider_cancelled',
						bounded.timedOut()
							? `${providerLabel(provider)} timed out.`
			...
			if (
				typeof raw !== 'string' ||
				utf8ByteLength(raw) > this.limits.maxProviderOutputBytes
			)
				throw new AiServiceError(
					'provider_output_too_large',
					`${providerLabel(provider)} returned an oversized result.`,
				);
			const text = normalizeMetadataText(
				request.targetType,
				raw,
				request.targetType === 'title'
					? this.limits.maxTitleChars
					: this.limits.maxNoteChars,
			);
			if (!text)
				throw new AiServiceError(
					'empty_output',
					`${providerLabel(provider)} returned an empty result.`,
				);
			...
			if (!after.live)
				throw new AiServiceError(
					'target_exited',
					'terminal target has exited.',
				);
			if (after.metadataRevision !== expectedRevision)
				throw new AiServiceError(
					'revision_conflict',
					'terminal metadata changed while AI was running.',
					true,
				);
			if (this.options.authority.applyMetadata === undefined)
				throw new AiServiceError(
					'target_unavailable',
					'terminal metadata mutation is unavailable.',
					true,
				);
			const applied = await this.options.authority.applyMetadata(
				request.target,
				request.targetType,
				text,
				expectedRevision,
			);
			const result: AiMetadataResult = {
				requestId: request.requestId,
				target: { ...request.target },
	...
	private async readSettings(target: 'title' | 'note') {
		...
		if (source === undefined)
			return { provider: 'disabled' as const, model: '' };
	...
	private assertRequest(request: AiMetadataRequest): void {
		if (!request || typeof request !== 'object')
			throw new AiServiceError(
				'invalid_request',
				'AI metadata request is required.',
			);
		...
		if (
			...
			throw new AiServiceError(
				'invalid_request',
				'expected metadata revision is invalid.',
			);
		if (
			request.deadlineMs !== undefined &&
			(!Number.isSafeInteger(request.deadlineMs) || request.deadlineMs <= 0)
		)
			throw new AiServiceError('invalid_request', 'AI deadline is invalid.');
	}
...
export function normalizeMetadataText(
	target: 'title' | 'note',
	raw: string,
	maxChars: number,
): string {
