...
import type {
	DictationProviderAdapter,
	DictationProviderRequest,
} from './types.js';
 ...
 */
export class ServerParakeetDictationProvider
	implements DictationProviderAdapter
{
	constructor(
	...
	async transcribe(request: DictationProviderRequest): Promise<string> {
		if (request.model !== SERVER_PARAKEET_MODEL) {
			throw new AiServiceError(
				'invalid_request',
				'The Parakeet model is not the pinned server model.',
			);
		}
		if (request.signal.aborted) {
			throw new AiServiceError(
				'provider_cancelled',
				'Parakeet transcription was cancelled.',
				true,
			);
		}
		await mkdir(this.temporaryRoot, { recursive: true, mode: 0o700 });
		const directory = await mkdtemp(
			path.join(this.temporaryRoot, 'terminay-parakeet-input-'),
		);
		const inputPath = path.join(
			directory,
			`capture.${extensionForMime(request.mimeType)}`,
		);
		try {
	...
	async status(): Promise<ServerParakeetRuntimeStatus> {
		if (this.runtime.getStatus === undefined)
			throw new AiServiceError(
				'provider_unavailable',
				'Parakeet runtime status is unavailable.',
				true,
			);
	...
	async install(): Promise<ServerParakeetRuntimeStatus> {
		if (this.runtime.install === undefined)
			throw new AiServiceError(
				'provider_unavailable',
				'Parakeet runtime installation is unavailable.',
				true,
			);
...
function normalizeStatus(input: unknown): ServerParakeetRuntimeStatus {
	if (typeof input !== 'object' || input === null || Array.isArray(input))
		throw new AiServiceError(
			'provider_unavailable',
			'Parakeet runtime returned invalid status.',
			true,
		);
	...
	if (
		...
		value.model !== SERVER_PARAKEET_MODEL
	)
		throw new AiServiceError(
			'provider_unavailable',
			'Parakeet runtime returned invalid status.',
	...
	const engine =
		...
		['package', 'version', 'license'].every(
			(key) =>
				typeof engineValue[key] === 'string' &&
				(engineValue[key] as string).length <= 64,
		)
			? {
			...
			: undefined;
	const safe = (field: unknown, max = 128) =>
		typeof field === 'string' &&
		field.length > 0 &&
		field.length <= max &&
		!/[\0\r\n/\\]/u.test(field)
	...
	return {
		state: value.state as ServerParakeetRuntimeStatus['state'],
		model: SERVER_PARAKEET_MODEL,
		...(message === undefined ? {} : { message }),
		...(progress === undefined ? {} : { progress }),
		...(engine === undefined ? {} : { engine }),
		...(modelRevision === undefined ? {} : { modelRevision }),
		...(modelLicense === undefined ? {} : { modelLicense }),
		...(audioFormat === undefined ? {} : { audioFormat }),
	};
}
