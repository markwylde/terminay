...
export type ParakeetRuntimeDisclosure = {
	...
	engine: {
		license: typeof PARAKEET_MLX_LICENSE;
		package: 'parakeet-mlx';
		version: typeof PARAKEET_MLX_VERSION;
	};
...
export function parakeetFfmpegArguments(
	inputPath: string,
	outputPath: string,
): string[] {
	return [
...
export class ParakeetRuntime {
	...
	constructor(options: ParakeetRuntimeOptions) {
		...
		this.spawnProcess =
			options.spawnProcess ?? (spawn as unknown as SpawnProcess);
	}
	...
	async getStatus(): Promise<ParakeetRuntimeStatus> {
		if (!this.isSupported()) {
			return this.status(
				'unsupported',
				'On-device Parakeet requires an Apple Silicon Mac.',
			);
		}
		if (this.installPromise !== null)
			return this.installStatus ?? this.status('installing', undefined, 0);
		...
		try {
			...
			if (!(await this.findFfmpeg()))
				return this.status(
					'error',
					'Audio conversion is unavailable. Install ffmpeg with “brew install ffmpeg” and try again.',
				);
			return this.status('ready');
		} catch {
			return this.status(
				'not-installed',
				'Install the on-device engine and model to use Parakeet.',
			);
		}
	}
	...
	async transcribe(audioPath: string): Promise<string> {
		...
		if (status.state !== 'ready')
			throw new Error(
				status.message ?? 'The on-device Parakeet engine is not ready.',
			);
		...
		if (child === null)
			throw new Error('The on-device Parakeet worker is unavailable.');
		const id = `dictation-${Date.now()}-${++this.sequence}`;
		const conversionDirectory = await mkdtemp(
			path.join(this.rootDirectory, 'audio-'),
		);
	...
	private venvDirectory(): string {
		return path.join(this.rootDirectory, 'venv');
	}
	private pythonPath(): string {
		return path.join(this.venvDirectory(), 'bin', 'python');
	}
	private workerPath(): string {
		return path.join(this.rootDirectory, 'worker.py');
	}
	private markerPath(): string {
		return path.join(
			this.rootDirectory,
			`parakeet-mlx-${PARAKEET_MLX_VERSION}.installed`,
		);
	}
	...
	private async installInternal(): Promise<ParakeetRuntimeStatus> {
		...
		try {
			...
			if (!uv)
				throw new Error(
					'The uv runtime manager is required. Install it from https://docs.astral.sh/uv/ and try again.',
				);
			const ffmpeg = await this.findFfmpeg();
			if (!ffmpeg)
				throw new Error(
					'ffmpeg is required to decode microphone audio. Install it with “brew install ffmpeg” and try again.',
				);
			await mkdir(this.rootDirectory, { recursive: true, mode: 0o700 });
			await writeFile(this.workerPath(), WORKER_SOURCE, {
				encoding: 'utf8',
				mode: 0o600,
			});
			...
			this.updateInstallStatus(
				0.7,
				'Downloading and loading the Parakeet model. The first setup may take several minutes…',
			);
	...
	private status(
		state: ParakeetRuntimeStatus['state'],
		message?: string,
		progress?: number,
	): ParakeetRuntimeStatus & ParakeetRuntimeDisclosure {
		return {
			audioFormat: PARAKEET_AUDIO_FORMAT,
			engine: {
				license: PARAKEET_MLX_LICENSE,
				package: 'parakeet-mlx',
				version: PARAKEET_MLX_VERSION,
			},
	...
	private async ensureWorker(
		...
		this.startPromise = new Promise<void>((resolve, reject) => {
			...
			child.stderr.on('data', () => {
				/* diagnostics are intentionally bounded and discarded */
			});
			let settled = false;
			const startupTimer = setTimeout(
				() => fail(new Error('Timed out while loading the Parakeet model.')),
				startupTimeoutMs,
			);
			...
			lines.on('line', (line) => {
				if (Buffer.byteLength(line) > MAX_WORKER_LINE_BYTES)
					return fail(new Error('Parakeet returned an oversized response.'));
				let message: Record<string, unknown>;
				try {
					message = JSON.parse(line) as Record<string, unknown>;
				} catch {
					return fail(new Error('Parakeet returned an invalid response.'));
				}
			...
			child.once('exit', (code) =>
				code === 0
					...
					: reject(
							new Error(
								stderr.trim() ||
									`Runtime setup failed with exit code ${code ?? 'unknown'}.`,
							),
						),
			);
		});
