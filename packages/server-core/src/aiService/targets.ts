...
export class ExactTerminalTargetRegistry implements AiTargetAuthority {
	...
	register(
		...
		if (this.targets.has(key))
			throw new AiServiceError(
				'invalid_request',
				'terminal target is already registered',
			);
		if (
			options.metadataRevision !== undefined &&
			(!Number.isSafeInteger(options.metadataRevision) ||
				options.metadataRevision < 0)
		)
			throw new TypeError('metadataRevision is invalid');
		const mutable: MutableTarget = {
			...
			note: options.note ?? '',
			...(options.authorizedClients === undefined
				? {}
				: { authorizedClients: new Set(options.authorizedClients) }),
			...(options.writeInput === undefined
				? {}
	...
	authorize(clientId: ProtocolId, target: TerminalTarget): boolean {
		...
		return (
			value.authorizedClients === undefined ||
			value.authorizedClients.has(clientId)
		);
	}

	setAuthorizedClients(
		target: TerminalTarget,
		clients: readonly ProtocolId[] | undefined,
	): void {
		const value = this.require(target);
		value.authorizedClients =
			clients === undefined ? undefined : new Set(clients);
	}
	...
	writeInput(target: TerminalTarget, input: string): void | Promise<void> {
		const current = this.requireLive(target);
		if (current.writeInput === undefined)
			throw new AiServiceError(
				'target_unavailable',
	...
	markExited(targetOrSessionId: TerminalTarget | ProtocolId): boolean {
		if (typeof targetOrSessionId === 'string') {
			...
			for (const value of this.targets.values())
				if (value.sessionId === targetOrSessionId && value.live) {
					value.live = false;
					changed = true;
				}
			return changed;
