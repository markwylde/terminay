...
export function validateIdentity(
	...
	if (typeof clientId !== 'string' || !idPattern.test(clientId))
		throw new TypeError('invalid authenticated client id');
	if (!(authScope in scopeRank))
		throw new TypeError('invalid authenticated scope');
}
...
export function forbiddenError(
	...
	return {
		code: 'forbidden',
		message: `operation ${operation} requires ${required} scope`,
		retryable: false,
	};
}
