...
export class ServerConnection implements ServerConnectionLike {
	...
	private readonly transportAuthenticatedClient:
		| AuthenticatedClient
		| undefined;
	...
	constructor(
		...
		this.authenticate =
			...
			((context) => ({ clientId: context.hello.clientId, authScope: 'none' }));
	...
	get client(): AuthenticatedClient | undefined {
	...
	async start(signal?: AbortSignal): Promise<void> {
		...
		try {
			...
			const reason = {
				...
				message: 'transport failed to open',
				cause,
			};
	...
	async process(frame: Uint8Array, body = new Uint8Array()): Promise<void> {
		...
		await this.processEnvelope(
			decoded.envelope,
			body.byteLength === 0 ? decoded.body : body,
		);
	}
	...
	async close(reason?: ReturnType<typeof protocolError>): Promise<void> {
		...
		this.abortController.abort(
			reason === undefined ? undefined : new Error(reason.message),
		);
		this.cleanupConnection();
	...
	private async processEnvelope(
		...
		if (this.currentState === 'handshaking') {
			if (envelope.type !== 'client_hello') {
				await this.send({
					type: 'error',
					error: protocolError('unauthorized', 'client hello required'),
				});
				return;
			}
		...
		if (envelope.type === 'query') await this.handleQuery(envelope, body);
			...
			await this.handleCommand(envelope, body);
	...
	private async handleHello(hello: ClientHello): Promise<void> {
		...
		try {
			auth =
				this.transportAuthenticatedClient ??
		...
		if ('code' in auth) {
			await this.send({ type: 'error', error: auth });
			await this.close(auth);
			return;
		}
		...
		this.heartbeatTimer = globalThis.setTimeout(() => {
			...
			void this.failConnection(
				new OutboundDeliveryError({
					code: 'timeout',
					message: 'application heartbeat timed out',
				}),
			).catch(() => undefined);
		}, this.heartbeatTimeoutMs);
	...
	private async handleQuery(
		...
		try {
			const result = await this.dispatcher.query({
				envelope: query,
				body,
				context: this.context(query, request.controller.signal),
			});
			...
			await this.send({
				...
				queryId: query.queryId,
				ok: false,
				error: responseFailureError(error),
			}).catch(() => undefined);
		} finally {
	...
	private async handleCommand(
		...
		try {
			...
			const result = await this.dispatcher.command({
				envelope: command,
				body,
				context: this.context(command, request.controller.signal),
			});
	...
	private beginRequest(correlationId: string): InFlightRequest | undefined {
		...
		this.abortController.signal.addEventListener('abort', onConnectionAbort, {
			once: true,
		});
	...
	private cancelRequest(cancel: CancelEnvelope): void {
		this.inFlightRequests
			.get(cancel.correlationId)
			?.controller.abort(new Error('operation cancelled'));
	}

	private async validateEventSubscription(
		command: CommandEnvelope,
	): Promise<CommandResultEnvelope> {
		if (this.authenticatedClient?.authScope === 'none')
			return commandError(
				command,
				'forbidden',
				'event subscription requires read access',
			);
		...
		if (!this.clientCapabilities.has('events.resync'))
			return commandError(
				command,
				'validation',
				'event subscription requires events.resync capability',
			);
		const payload = objectPayload(command.payload);
		const subscriptionId = payload.subscriptionId;
		if (!isSafeId(subscriptionId))
			return commandError(
				command,
				'validation',
				'event subscription id is invalid',
			);
		const event =
			payload.event === null || payload.event === undefined
				? undefined
				: payload.event;
		if (
			event !== undefined &&
			(typeof event !== 'string' || event.length === 0 || event.length > 256)
		)
			return commandError(command, 'validation', 'event name is invalid');
		if (
			...
			return commandError(
				command,
				'validation',
				'event subscription selector is invalid',
			);
		}
		const fromRevision =
			payload.fromRevision === undefined ? 0 : payload.fromRevision;
		if (!Number.isSafeInteger(fromRevision) || (fromRevision as number) < 0)
			return commandError(command, 'validation', 'event revision is invalid');
		return {
			type: 'command_result',
			commandId: command.commandId,
			correlationId: command.correlationId,
			...
			result: {
				...
				fromRevision: fromRevision as number,
			},
		};
	}

	private unsubscribeEventSubscription(
		command: CommandEnvelope,
	...
	private async activateEventSubscription(
		command: CommandEnvelope,
	): Promise<void> {
		if (this.currentState !== 'open') return;
		const payload = objectPayload(command.payload);
		...
		const listener = (value: OrderedEvent): void => {
			...
			if (replaying) pending.push(projected);
				...
				void this.sendEvent(subscriptionId, projected).catch(() => undefined);
		};
		...
		if (replay.kind === 'resync') {
			...
			if (this.clientCapabilities.has('events.resync')) {
				...
				await this.send({
					type: 'event_resync',
					subscriptionId,
					revision: snapshot?.revision ?? fromRevision,
					cursor: snapshot?.cursor ?? String(fromRevision),
				});
			}
			replaying = false;
			for (const value of pending) {
				if (terminalOutputMetadata(value) !== undefined)
		...
		for (const value of pending) {
			if (
				terminalOutputMetadata(value) !== undefined ||
				(value.revision > fromRevision &&
					!replayedRevisions.has(value.revision))
			) {
				await this.sendEvent(subscriptionId, value);
			}
		}
	...
	private context(
		request: QueryEnvelope | CommandEnvelope,
		...
		return {
			...
			...(this.authenticatedClient?.permissions === undefined
				? {}
				: { permissions: this.authenticatedClient.permissions }),
			...
			...(request.type === 'command' && request.expectedRevision === undefined
				? {}
	...
	private applyTerminalDeliveryCommand(
		...
		if (!result.ok) return;
		if (
			command.operation === 'terminal.attach' ||
			command.operation === 'terminal.resume'
		) {
	...
	private async failConnection(error: OutboundDeliveryError): Promise<void> {
		if (this.closeTask !== undefined || this.currentState === 'closed')
			return this.closeTask;
		this.currentState = 'closing';
	...
	private reportDeliveryDiagnostic(
		diagnostic: Parameters<
			NonNullable<ConnectionOptions['onDeliveryDiagnostic']>
		>[0],
	): void {
		try {
			this.onDeliveryDiagnostic?.(diagnostic);
		} catch {
			/* Diagnostic sinks cannot affect connection lifecycle. */
		}
	}
	...
	private cleanupConnection(): void {
		...
		const clientId = this.authenticatedClient?.clientId;
		if (clientId !== undefined)
			this.options.onConnectionClosed?.(this.connectionId, clientId);
		this.onClosed?.();
	}
}
...
function duplicateRequestError() {
	return {
		...
		message: 'request correlation id is already in flight',
		retryable: false,
	};
}
...
function responseFailureError(error: unknown) {
	if (
		...
		/(?:header|body|frame).*limit|frame exceeds|body exceeds|header exceeds/u.test(
			error.message,
		)
	) {
		return protocolError('resource', 'response exceeded protocol limits', {
			retryable: false,
		});
	}
	if (error instanceof Error && error.name === 'AbortError')
		return protocolError('cancelled', 'operation cancelled');
...
function terminalOutputMetadata(
	event: OrderedEvent,
): TerminalOutputMetadata | undefined {
