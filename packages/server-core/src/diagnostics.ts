...
export function createSupportBundle(
	...
	const logs =
		options.includeLogs === true
			? Object.freeze(
					(input.logs ?? [])
						.slice(0, maxLogLines)
						.map((line) => redactText(line).slice(0, 4096)),
				)
			: undefined;
	...
	if (new TextEncoder().encode(encoded).byteLength > maxBytes)
		throw new RangeError('support bundle exceeds configured size');
	return bundle;
}
...
function redactObject(
	...
	for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
		...
		if (typeof child === 'string')
			result[key] = redactText(child).slice(0, 2048);
		else if (
			typeof child === 'number' ||
			...
			result[key] = child
				.slice(0, 32)
				.map((item) =>
					typeof item === 'string' ? redactText(item).slice(0, 512) : null,
				);
...
function isSecretKey(key: string): boolean {
	return /(?:token|secret|password|passphrase|private[_-]?key|api[_-]?key|grant|proof|credential)/iu.test(
		key,
	);
}
