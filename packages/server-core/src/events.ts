...
export class OrderedEventJournal implements OrderedEventJournalLike {
	...
	private readonly snapshotProvider:
		| (() => ResyncSnapshot | Promise<ResyncSnapshot>)
		| undefined;
	...
	constructor(options: EventJournalOptions = {}) {
		...
		if (!Number.isSafeInteger(this.nextRevision) || this.nextRevision < 0)
			throw new RangeError('initialRevision must be non-negative');
	...
	append(event: string, payload: JsonValue): OrderedEvent {
		...
		if (this.nextRevision === Number.MAX_SAFE_INTEGER)
			throw new RangeError('event revision exhausted');
		const nextRevision = this.nextRevision + 1;
		const next: OrderedEvent = Object.freeze({
			revision: nextRevision,
			cursor: String(nextRevision),
			event,
			payload,
		});
	...
	replay(afterRevision = 0): EventReplay | Promise<EventReplay> {
		if (!Number.isSafeInteger(afterRevision) || afterRevision < 0)
			throw new RangeError('afterRevision must be non-negative');
		...
		if (oldest !== undefined && afterRevision < oldest - 1) {
			...
			return Promise.resolve(this.snapshotProvider()).then((snapshot) => {
				if (
					...
					throw new RangeError('snapshot revision is invalid');
	...
	private publish(event: OrderedEvent): void {
		for (const listener of this.listeners) {
			try {
				...
				/* listeners are observers, not commit participants */
			}
		}
	}
}
...
function validateEvent(event: string, payload: JsonValue): void {
	if (typeof event !== 'string' || event.length === 0 || event.length > 256)
