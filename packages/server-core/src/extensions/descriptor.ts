...
export interface ExtensionCompatibilityContext {
	...
	readonly installedExtensions?: ReadonlyMap<
		string,
...
export async function extensionLaunchDescriptor(
	input: ExtensionPackageLaunchInput,
): Promise<{
	...
	assertExtensionCompatible(
		manifest,
		input.compatibility ?? { terminayVersion: '1.0.0' },
	);
...
export function assertExtensionCompatible(
	...
	if (!satisfiesVersion(EXTENSION_API_VERSION, manifest.api))
		throw new Error(
			`extension API ${manifest.api} is incompatible with host ${EXTENSION_API_VERSION}`,
		);
	if (!satisfiesVersion(context.terminayVersion, manifest.engines.terminay))
		throw new Error(`extension requires Terminay ${manifest.engines.terminay}`);
	if (!satisfiesVersion(nodeVersion, manifest.engines.node))
		throw new Error(`extension requires Node ${manifest.engines.node}`);
	if (
		manifest.platforms !== undefined &&
		!manifest.platforms.includes(platform as 'darwin' | 'linux' | 'win32')
	)
		throw new Error(`extension does not support platform ${platform}`);
	for (const dependency of manifest.extensionDependencies ?? []) {
		...
		if (installed === undefined) {
			if (!dependency.optional)
				throw new Error(
					`required extension dependency ${dependency.extensionId} is unavailable`,
				);
		} else if (!satisfiesVersion(installed.apiVersion, dependency.apiRange)) {
			throw new Error(
				`extension dependency ${dependency.extensionId} has incompatible API ${installed.apiVersion}`,
			);
		}
	}
}
...
function satisfiesVersion(version: string, range: string): boolean {
	...
	if (trimmed.startsWith('>='))
		return compare(parsed, versionTuple(trimmed.slice(2))) >= 0;
	if (trimmed.startsWith('^')) {
...
export async function validateExtensionLaunchDescriptor(
	descriptor: ExtensionLaunchDescriptor,
): Promise<ExtensionLaunchDescriptor> {
	if (!EXTENSION_ID.test(descriptor.extensionId))
		throw new TypeError('invalid extension id');
	if (isAbsolute(descriptor.entrypoint) || descriptor.entrypoint.includes('\0'))
		throw new TypeError('extension entrypoint must be relative');
	...
	if (
		...
		unresolvedContainment.startsWith('..') ||
		isAbsolute(unresolvedContainment)
	)
		throw new TypeError('extension entrypoint escapes its package slot');
	...
	if (!entryStat.isFile())
		throw new TypeError(
			'extension entrypoint must be a regular non-symlink file',
		);
	if (
		!canonicalEntrypoint.endsWith('.js') &&
		!canonicalEntrypoint.endsWith('.mjs')
	)
		throw new TypeError('extension entrypoint must be ESM JavaScript');
	for (const path of [
		descriptor.configDirectory,
