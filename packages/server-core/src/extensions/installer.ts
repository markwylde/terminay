...
import {
	inspectNpmPackArchive,
	MAX_EXTENSION_ARCHIVE_BYTES,
} from './npmPackArchive.js';
import { validateMaterializedExtension } from './packageValidation.js';

const WARNING =
	'This is third-party trusted code. It runs on the selected Terminay Server and can access files and networks available to that server account.';
const EMPTY: ExtensionRegistrySnapshot = Object.freeze({
	schemaVersion: 1,
	revision: 0,
...
export class ExtensionInstaller {
	...
	constructor(private readonly options: ExtensionInstallerOptions) {
		this.root = join(options.dataRoot, 'extensions');
	}

	async initialize(): Promise<ExtensionRegistrySnapshot> {
		await Promise.all([
			mkdir(join(this.root, 'packages'), { recursive: true }),
			mkdir(join(this.root, 'staging'), { recursive: true }),
			mkdir(join(this.root, 'uploads'), { recursive: true }),
			mkdir(join(this.root, 'data'), { recursive: true }),
					...
					state = await this.commit(
						...
						existing === undefined
							? {
									...
									enabled: true,
									slots: {},
									failureClass: safeFailure(error),
								}
							: {
									...existing,
									state: 'failed',
						...
						'extension.built_in_failed',
					);
					await this.options.audit?.({
						kind: 'extension.built_in_failed',
						extensionId: artifact.extensionId,
						packageName: artifact.packageName,
	...
	async previewArchive(
		...
		if (!/^[^/\\\0]{1,200}\.tgz$/iu.test(filename))
			throw new Error('choose an npm pack .tgz package file');
		if (bytes.byteLength > MAX_EXTENSION_ARCHIVE_BYTES)
			throw new Error('extension package file exceeds the 12 MiB limit');
		const inspected = await inspectNpmPackArchive(bytes);
		const packageName = inspected.packageJson.name;
		const version = inspected.packageJson.version;
		if (
		...
		const resolution: RegistryPackageResolution = Object.freeze({
			...
			archivePath,
			manifestMetadata: inspected.packageJson.terminay,
			provenance: 'unverified',
			dependencyCount:
				typeof inspected.packageJson.dependencies === 'object' &&
				inspected.packageJson.dependencies !== null
					? Object.keys(inspected.packageJson.dependencies).length
					: 0,
		});
	...
	async preview(
		...
		const resolution = await this.options.registryClient.resolve(
			packageName,
			selector,
			signal,
		);
		...
		const official = OFFICIAL_EXTENSION_CATALOGUE.some(
			(item) => item.packageName === packageName,
		);
		const expiresAt = (this.options.now ?? Date.now)() + 10 * 60_000;
		const previewDigest = digest(
			canonicalJson({ resolution, manifest, expiresAt }),
		);
		const preview = Object.freeze({
			...resolution,
			previewDigest,
	...
	confirm(
		...
		return this.serial(async () => {
			...
			const timer = this.previewTimers.get(previewDigest);
			if (timer !== undefined) clearTimeout(timer);
			this.previewTimers.delete(previewDigest);
			if (
				preview === undefined ||
				preview.expiresAt < (this.options.now ?? Date.now)()
			) {
				await cleanupArchive(preview);
				throw new Error('extension install preview expired or changed');
			}
			try {
				return await this.installExact(preview, signal);
			} finally {
				await cleanupArchive(preview);
			}
		});
	...
	confirmedPreview(previewDigest: string): ExtensionInstallPreview | undefined {
		return this.previews.get(previewDigest);
	}

	disable(extensionId: string): Promise<ExtensionRegistrySnapshot> {
		return this.serial(async () => {
			const state = await this.snapshot();
			const current = required(state, extensionId);
			const refs = await this.references(extensionId);
			if ((refs.activeUses ?? 0) > 0)
				throw new Error(
					'extension is in active use and must be drained before disabling',
				);
			return this.commit(
				state,
				{ ...current, enabled: false, state: 'disabled' },
				'extension.disabled',
			);
		});
	}

	enable(extensionId: string): Promise<ExtensionRegistrySnapshot> {
		return this.serial(async () => {
			const state = await this.snapshot();
			const current = required(state, extensionId);
			if (current.activeSlotId === undefined)
				throw new Error('extension has no active slot');
			const slot = current.slots[current.activeSlotId];
			if (slot === undefined)
				throw new Error('active extension slot is missing');
	...
	rollback(extensionId: string): Promise<ExtensionRegistrySnapshot> {
		return this.serial(async () => {
			const state = await this.snapshot();
			const current = required(state, extensionId);
			if (current.previousSlotId === undefined)
				throw new Error('no retained known-good slot is available');
			const refs = await this.references(extensionId);
			if ((refs.activeUses ?? 0) > 0)
				throw new Error(
					'extension is in active use and must be drained before rollback',
				);
			const slot = current.slots[current.previousSlotId];
			if (slot === undefined)
				throw new Error('retained extension slot is missing');
			await this.probe(slot.receipt, this.slotPackageRoot(slot));
			const next = {
				...current,
				activeSlotId: slot.slotId,
				previousSlotId: current.activeSlotId,
				pendingSlotId: undefined,
				state: current.enabled ? ('installed' as const) : ('disabled' as const),
			};
			return this.commit(state, next, 'extension.rolled_back');
		});
	}

	activatePending(extensionId: string): Promise<ExtensionRegistrySnapshot> {
		return this.serial(async () => {
			const state = await this.snapshot();
			const current = required(state, extensionId);
			if (current.pendingSlotId === undefined)
				throw new Error('extension has no pending update');
			const refs = await this.references(extensionId);
			if ((refs.activeUses ?? 0) > 0)
				throw new Error(
					'extension is in active use and must be drained before activation',
				);
			const slot = current.slots[current.pendingSlotId];
			if (slot === undefined)
				throw new Error('pending extension slot is missing');
			await this.probe(slot.receipt, this.slotPackageRoot(slot));
			const active =
				current.activeSlotId === undefined
					? undefined
					: current.slots[current.activeSlotId];
			if (active !== undefined && active.version !== slot.version)
			...
			return this.commit(
				state,
				{
					...
					previousSlotId: current.activeSlotId,
					pendingSlotId: undefined,
					state: current.enabled ? 'installed' : 'disabled',
				},
				'extension.updated',
			);
		});
	...
	setFailureState(
		extensionId: string,
		stateValue: 'failed' | 'quarantined' | 'incompatible',
		failureClass: string,
	): Promise<ExtensionRegistrySnapshot> {
		return this.serial(async () => {
			const state = await this.snapshot();
	...
	async diagnostics(): Promise<readonly Readonly<Record<string, unknown>>[]> {
		...
		return Object.freeze(
			Object.values(state.extensions).map((record) =>
				Object.freeze({
					extensionId: record.extensionId,
					...
					activeVersion:
						record.activeSlotId === undefined
							? undefined
							: record.slots[record.activeSlotId]?.version,
	...
	> {
		...
		return Object.freeze({
			...
			dataRoot: join(this.root, 'data'),
			receipts: Object.values(state.extensions)
				.flatMap((record) =>
					Object.values(record.slots).map((slot) =>
						join(this.root, 'packages', slot.slotId, 'terminay-receipt.json'),
					),
				)
				.sort(),
		});
	...
	remove(extensionId: string): Promise<ExtensionRegistrySnapshot> {
		return this.serial(async () => {
			const state = await this.snapshot();
			const current = required(state, extensionId);
			const refs = await this.references(extensionId);
			...
			if (reasons.length > 0)
				throw new Error(
					`extension removal is blocked by ${reasons.join(', ')}`,
				);
			const extensions = { ...state.extensions };
	...
	> {
		const state = await this.snapshot();
		const current = required(state, extensionId);
		if (!current.enabled || current.activeSlotId === undefined)
			throw new Error('extension is not enabled with an active slot');
		const slot = current.slots[current.activeSlotId];
	...
	async enabledExtensionIds(): Promise<readonly string[]> {
		const state = await this.snapshot();
		return Object.freeze(
			Object.values(state.extensions)
				.filter(
					(record) =>
						record.enabled &&
						record.activeSlotId !== undefined &&
	...
	private async installExact(
		resolution: ExtensionInstallPreview,
		signal?: AbortSignal,
	): Promise<ExtensionRegistrySnapshot> {
		return this.installResolved(resolution, (staging) =>
			this.options.materializer.materialize(resolution, staging, signal),
		);
	}

	private async installBuiltInExact(
		artifact: BuiltInExtensionArtifact,
		state: ExtensionRegistrySnapshot,
		signal?: AbortSignal,
	): Promise<ExtensionRegistrySnapshot> {
		if (artifact.source !== 'built-in')
			throw new Error('built-in artifact source is invalid');
		return this.installResolved(
			artifact,
			(staging) => {
				if (this.options.builtIns === undefined)
					return Promise.reject(
						new Error('built-in extension source is unavailable'),
					);
	...
	private async installResolved(
		resolution: RegistryPackageResolution,
		materialize: (staging: string) => Promise<void>,
		suppliedState?: ExtensionRegistrySnapshot,
	): Promise<ExtensionRegistrySnapshot> {
		...
		try {
			...
			const installedExtensions = new Map(
				Object.values(state.extensions).flatMap((record) => {
					const slot =
						record.activeSlotId === undefined
							? undefined
							: record.slots[record.activeSlotId];
			...
			const validated = await validateMaterializedExtension(
				staging,
				resolution,
				installedExtensions,
			);
			if (
				isBuiltInResolution(resolution) &&
				(validated.inventoryHash !== resolution.inventoryHash ||
					validated.lockHash !== resolution.lockHash)
			)
				throw new Error(
					'built-in extension artifact digest differs from release inventory',
				);
			const previewManifest = parseExtensionManifest(
				resolution.manifestMetadata,
			);
			if (canonicalJson(previewManifest) !== canonicalJson(validated.manifest))
				throw new Error(
					'materialized extension manifest differs from the confirmed preview',
				);
			const slotId =
				`${validated.manifest.id}-${resolution.version}-${validated.inventoryHash.slice(0, 16)}`.replace(
					/[^a-zA-Z0-9._-]/gu,
					'_',
				);
			const receipt: ExtensionReceipt = Object.freeze({
				schemaVersion: 1,
				...
				source: resolution.source ?? 'npmjs',
				...(resolution.uploadedFilename
					? { uploadedFilename: resolution.uploadedFilename }
					: {}),
				...(resolution.tarballUrl ? { tarballUrl: resolution.tarballUrl } : {}),
				installedAt: new Date((this.options.now ?? Date.now)()).toISOString(),
				npmVersion: this.options.materializer.npmVersion,
				lockHash: validated.lockHash,
				inventoryHash: validated.inventoryHash,
				permissions: Object.freeze([...validated.manifest.permissions]),
				manifest: validated.manifest,
				...(resolution.provenance ? { provenance: resolution.provenance } : {}),
				...(resolution.audit ? { audit: resolution.audit } : {}),
			});
			await writeFile(
				join(staging, 'terminay-receipt.json'),
			...
			if (
				persisted !== undefined &&
				persisted.packageName !== receipt.packageName &&
				resolution.source !== 'built-in'
			)
				throw new Error(
					'extension identity is already owned by another package',
				);
			...
			const replacingLegacyBuiltInConflict =
				resolution.source === 'built-in' &&
				persisted !== undefined &&
				persisted.packageName !== receipt.packageName;
			const previous = replacingLegacyBuiltInConflict ? undefined : persisted;
			const activeSlot =
				previous?.activeSlotId === undefined
					? undefined
					: previous.slots[previous.activeSlotId];
			const preserveSelection =
				resolution.source === 'built-in' &&
				activeSlot !== undefined &&
				activeSlot.receipt.source !== 'built-in';
			...
			if (
				resolution.source === 'built-in' &&
				previous?.slots[slotId] !== undefined &&
			...
			const next: InstalledExtensionRecord = Object.freeze({
				extensionId: receipt.extensionId,
				packageName: receipt.packageName,
				state:
					enabled === false
						? 'disabled'
						: activeUses > 0 && previous?.activeSlotId && !preserveSelection
							? 'pending'
							: 'installed',
				enabled,
				activeSlotId:
					preserveSelection ||
					(activeUses > 0 && previous?.activeSlotId !== undefined)
						? (previous?.activeSlotId ?? slotId)
						: slotId,
				...(previous?.activeSlotId &&
				previous.activeSlotId !== slotId &&
				!preserveSelection
					? { previousSlotId: previous.activeSlotId }
					: {}),
				...(activeUses > 0 && previous?.activeSlotId && !preserveSelection
	...
	private async removeOverride(
		state: ExtensionRegistrySnapshot,
		current: InstalledExtensionRecord,
		bundled: readonly import('./installerTypes.js').ExtensionSlotRecord[],
		refs: ExtensionReferences,
	): Promise<ExtensionRegistrySnapshot> {
		const active =
			current.activeSlotId === undefined
				? undefined
		...
		if (reasons.length > 0)
			throw new Error(
				`extension override removal is blocked by ${reasons.join(', ')}`,
			);
		const floor = newestBundledSlot(bundled);
		await this.probe(floor.receipt, this.slotPackageRoot(floor));
		if (active !== undefined && active.version !== floor.version)
			await this.migrateData(
				current,
				active.version,
				floor.version,
		...
		const slots = Object.freeze(
			Object.fromEntries(bundled.map((slot) => [slot.slotId, slot])),
		);
		const next = await this.commit(
			state,
			{
				...current,
				slots,
				activeSlotId: floor.slotId,
				previousSlotId: undefined,
				pendingSlotId: undefined,
				state: current.enabled ? 'installed' : 'disabled',
			},
			'extension.override_removed',
		);
		for (const slot of Object.values(current.slots))
			if (slot.receipt.source !== 'built-in')
				await rm(join(this.root, 'packages', slot.slotId), {
					recursive: true,
					force: true,
				});
	...
	private async commit(
		...
		const next = await this.write({
			schemaVersion: 1,
			revision: state.revision + 1,
			extensions: { ...state.extensions, [record.extensionId]: record },
		});
		await this.options.audit?.({
			kind,
			extensionId: record.extensionId,
			revision: next.revision,
			version: record.activeSlotId
				? record.slots[record.activeSlotId]?.version
				: undefined,
		});
		return next;
	...
	private async write(
		...
		return validateRegistry(state);
	}
	private async recoverStaging(): Promise<void> {
		for (const name of ['staging', 'uploads']) {
	...
	private async migrateData(
		...
		try {
			await this.options.migrateData?.({
	...
	private async references(extensionId: string): Promise<ExtensionReferences> {
		return this.options.references?.(extensionId) ?? {};
	}
	private async probe(
	...
	private expirePreview(
		...
		const timer = setTimeout(
			() => {
				if (this.previews.get(digestValue) !== preview) return;
				this.previews.delete(digestValue);
				this.previewTimers.delete(digestValue);
				void cleanupArchive(preview);
			},
			Math.max(1, preview.expiresAt - (this.options.now ?? Date.now)()),
		);
		timer.unref?.();
		this.previewTimers.set(digestValue, timer);
	}
	private slotPackageRoot(slot: { receipt: ExtensionReceipt }): string {
		return join(
			this.root,
			'packages',
			slot.receipt.slotId,
			'node_modules',
			...slot.receipt.packageName.split('/'),
		);
	}
	private registryPath(): string {
		return join(this.root, 'registry.v1.json');
	}
...
function orderBuiltIns(
	artifacts: readonly BuiltInExtensionArtifact[],
): readonly BuiltInExtensionArtifact[] {
	const remaining = new Map(
		artifacts.map((artifact) => [artifact.extensionId, artifact]),
	);
	const ordered: BuiltInExtensionArtifact[] = [];
	while (remaining.size > 0) {
		const ready = [...remaining.values()].filter((artifact) =>
			builtInDependencies(artifact).every((id) => !remaining.has(id)),
		);
		const next = (ready.length > 0 ? ready : [...remaining.values()]).sort(
			(left, right) => left.extensionId.localeCompare(right.extensionId),
		)[0];
...
function builtInDependencies(
	...
	if (
		...
		return [];
	const dependencies = (metadata as { extensionDependencies?: unknown })
	...
	return dependencies.flatMap((dependency) =>
		...
		typeof (dependency as { extensionId?: unknown }).extensionId === 'string'
			? [(dependency as { extensionId: string }).extensionId]
			: [],
	);
}

function validateResolution(
	value: RegistryPackageResolution,
	expectedName: string,
): void {
	if (
		value.packageName !== expectedName ||
		!/^\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/u.test(value.version) ||
		!/^(?:sha512|sha256)-[A-Za-z0-9+/=]+$/u.test(value.integrity)
	)
		throw new Error('npmjs resolution is not exact or lacks integrity');
	if (
		value.tarballUrl !== undefined &&
		!value.tarballUrl.startsWith('https://registry.npmjs.org/')
	)
		throw new Error('resolved tarball is not hosted by public npmjs');
}
async function cleanupArchive(
	preview: ExtensionInstallPreview | undefined,
): Promise<void> {
	if (preview?.source === 'uploaded' && preview.archivePath !== undefined)
		await rm(preview.archivePath, { force: true });
}
function validateRegistry(value: unknown): ExtensionRegistrySnapshot {
	if (
		...
		throw new Error('extension registry is invalid');
	return structuredClone(value) as ExtensionRegistrySnapshot;
}
function required(
	state: ExtensionRegistrySnapshot,
	id: string,
): InstalledExtensionRecord {
	const value = state.extensions[id];
	if (value === undefined) throw new Error('extension is not installed');
	return value;
}
function isBuiltInResolution(
	value: RegistryPackageResolution,
): value is BuiltInExtensionArtifact {
	return (
		value.source === 'built-in' &&
		typeof (value as Partial<BuiltInExtensionArtifact>).extensionId ===
			'string' &&
		typeof (value as Partial<BuiltInExtensionArtifact>).inventoryHash ===
			'string' &&
		typeof (value as Partial<BuiltInExtensionArtifact>).lockHash === 'string'
	);
}
function bundledSlots(
	record: InstalledExtensionRecord,
): readonly import('./installerTypes.js').ExtensionSlotRecord[] {
	return Object.values(record.slots).filter(
		(slot) => slot.receipt.source === 'built-in',
	);
}
function newestBundledSlot(
	slots: readonly import('./installerTypes.js').ExtensionSlotRecord[],
): import('./installerTypes.js').ExtensionSlotRecord {
	const value = [...slots].sort((left, right) =>
		compareVersion(right.version, left.version),
	)[0];
	if (value === undefined)
		throw new Error('built-in extension rollback floor is missing');
	return value;
}
function compareVersion(left: string, right: string): number {
	const parse = (value: string) =>
		value.split(/[.+-]/u, 3).map((part) => Number(part));
