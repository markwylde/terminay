...
interface TerminalState {
	...
	readonly files: Map<string, FileHandle>;
	readonly directories: Map<string, DirectoryHandle>;
	readonly watchers: Map<string, Watcher>;
	readonly directoryWatchers: Map<string, DirectoryWatcher>;
	nextId: number;
...
export class ThisServerAgentObservationAdapter {
	...
	constructor(
		...
		this.homeDirectory =
			options.homeDirectory === undefined
				? undefined
				: safePath(options.homeDirectory);
		this.maximumReadBytes = positiveBound(
			options.maximumReadBytes,
			MAX_READ_BYTES,
		);
		this.maximumFollowChunkBytes = positiveBound(
			options.maximumFollowChunkBytes,
			MAX_FOLLOW_CHUNK_BYTES,
		);
	}
	...
	async observe(
		...
		if (state.homeDirectory === undefined)
			state.homeDirectory = await this.homeDirectoryFor(local, signal);
		switch (operation) {
			case 'process.foreground':
				return this.foreground(local, signal);
			case 'process.descendants':
				return this.descendants(local, state, signal);
			case 'process.open-files':
				return this.openFiles(state, payload, signal);
			case 'process.environment':
				return this.environment(local, payload, signal);
			case 'terminal.tty':
				return this.tty(local, signal);
			case 'filesystem.resolve-home-directory':
				return this.resolveHomeDirectory(state, payload, signal);
			case 'filesystem.resolve-directory-relative-to-environment':
				return this.resolveDirectoryRelativeToEnvironment(
					local,
					state,
					payload,
					signal,
			...
			case 'filesystem.unwatch-directory':
				return this.unwatchDirectory(state, payload);
			case 'filesystem.resolve-home-relative':
				return this.resolveHomeRelative(state, payload, signal);
			case 'filesystem.resolve-path-under-home':
				return this.resolvePathUnderHome(state, payload, signal);
			case 'filesystem.home-relative-path':
				return this.homeRelativePath(state, payload, signal);
			case 'filesystem.resolve-relative-to-environment':
				return this.resolveRelativeToEnvironment(local, state, payload, signal);
			case 'filesystem.resolve-path-under-environment':
				return this.resolvePathUnderEnvironment(local, state, payload, signal);
			case 'filesystem.environment-relative-path':
				return this.environmentRelativePath(local, state, payload, signal);
			case 'filesystem.realpath':
				return this.canonicalFile(state, payload, signal);
			case 'filesystem.stat':
				return this.fileStat(state, payload, signal);
			case 'filesystem.read':
				return this.read(state, payload, signal);
			case 'filesystem.follow':
				return this.follow(state, payload, signal);
			case 'filesystem.unfollow':
				return this.unfollow(state, payload);
			default:
	...
	disposeTerminal(contextId: string): void {
	...
	private async foreground(
		...
		if (terminal.foreground !== undefined)
			return foregroundValue(terminal.foreground);
	...
	private async descendants(
		...
		const processes = (
			await this.system.descendants(requiredPid(terminal), signal)
		).slice(0, MAX_PROCESSES);
		const result = processes.flatMap((process) => {
			if (!validPid(process.pid) || !safeText(process.executableName, 512))
				return [];
			const handle = this.registerProcess(state, process.pid);
			const startedAt = safeText(process.startedAt, 128)
				? process.startedAt
				: undefined;
	...
	private async openFiles(
		...
		const supplied = Array.isArray(request?.processes)
			? request.processes
			: undefined;
		...
		if (
			supplied === undefined ||
			supplied.length > MAX_PROCESSES ||
			(access !== 'writable' && access !== 'readable')
		)
			throw new Error('agent open-file request is invalid');
		if (supplied.length === 0) return [];
		const pids = supplied.map((value) => this.processFor(state, value).pid);
		const files = selectObservedOpenFiles(
			await this.system.openFiles([...new Set(pids)], access, signal),
		);
	...
	private async environment(
		...
		if (
			...
			names.length === 0 ||
			names.length > 16 ||
			names.some((name) => !environmentName(name))
		)
		...
		for (const name of safeNames) {
			...
			if (safeText(value, 4_096)) entries.push([name, value]);
		}
	...
	private async tty(
		...
		const path =
			terminal.ttyPath ??
			(await this.system.tty(requiredPid(terminal), signal));
		if (path === undefined) return null;
		if (!safePath(path)?.startsWith('/dev/'))
			throw new Error('agent terminal TTY observation is malformed');
		return {
			path,
			terminalId: path.slice('/dev/'.length).replaceAll('/', '-'),
		};
	}

	private async canonicalFile(
		state: TerminalState,
		payload: JsonValue,
		signal: AbortSignal,
	): Promise<JsonValue> {
		const request = record(payload);
		const file = this.fileFor(state, request?.handle);
		const canonical = await this.system.realpath(file.path, signal);
		if (canonical === undefined || safePath(canonical) === undefined)
			return null;
		const options = record(request?.options);
		if (!this.matchesFileConstraint(canonical, options, state.homeDirectory))
			return null;
		file.path = canonical;
		return { id: file.id };
	}

	private async resolveHomeDirectory(
		state: TerminalState,
		payload: JsonValue,
		signal: AbortSignal,
	): Promise<JsonValue> {
		...
		const canonical = await this.system.realpath(
			resolve(homeDirectory, relativePath),
			signal,
		);
		if (
			canonical === undefined ||
			safePath(canonical) === undefined ||
			!this.withinHomeConstraint(canonical, request, true, homeDirectory)
		)
			return null;
	...
	private async resolveDirectoryRelativeToEnvironment(
		...
		if (typeof relativePath !== 'string' || !safeRelativePath(relativePath))
			return null;
		const root = await this.environmentRoot(
			terminal,
		...
		const beneath =
			typeof request?.beneathRelative === 'string' &&
	...
	private async listDirectory(
		...
		const request = record(payload);
		const root = this.directoryFor(state, request?.root);
		const options = record(request?.options);
	...
	private async watchDirectory(
		...
		if (typeof request?.watcherId === 'string') {
			...
			if (watcher === undefined)
				throw new Error('agent directory watcher is unavailable');
			const snapshot = await this.directoryListing(
				state,
				watcher.directory,
				record(watcher.options),
				signal,
			);
		...
		if (state.directoryWatchers.size >= MAX_WATCHERS_PER_TERMINAL)
			throw new Error('agent directory watch exceeds its limit');
		const root = this.directoryFor(state, request?.root);
		const options = record(request?.options);
		const snapshot = await this.directoryListing(state, root, options, signal);
		const watcher: DirectoryWatcher = {
			id: `directory-watch-${++state.nextId}`,
			directory: root,
	...
	private unwatchDirectory(
		...
		if (
			...
			!state.directoryWatchers.delete(watcherId)
		)
			throw new Error('agent directory watcher is unavailable');
		return { stopped: true };
	}

	private async directoryListing(
		state: TerminalState,
		...
		if (
			...
			throw new Error('agent directory list request is invalid');
		...
		const visit = async (directory: string, depth: number): Promise<void> => {
			...
			for (const child of [...children].sort((left, right) =>
				...
				if (canonical === undefined || !contained(root.path, canonical))
					continue;
				...
				if (
					...
					continue;
				if (entries.length >= maxEntries || bytes + details.size > maxBytes) {
					truncated = true;
					return;
				}
				...
				entries.push({
					...
					relativePath,
					size: details.size,
					...(safeText(details.modifiedAt, 128)
						? { modifiedAt: details.modifiedAt }
						: {}),
					...(safeText(details.createdAt, 128)
	...
	private async resolveHomeRelative(
		state: TerminalState,
		payload: JsonValue,
		signal: AbortSignal,
	): Promise<JsonValue> {
	...
	private async resolvePathUnderHome(
		state: TerminalState,
		payload: JsonValue,
		signal: AbortSignal,
	): Promise<JsonValue> {
		const request = record(payload);
		const providerPath = request?.providerPath;
		if (
			typeof providerPath !== 'string' ||
			safePath(providerPath) === undefined ||
			record(request?.beneath)?.homeRelative === undefined
		)
			return null;
	...
	private async homeRelativePath(
		state: TerminalState,
		payload: JsonValue,
		signal: AbortSignal,
	): Promise<JsonValue> {
		...
		if (
			canonical === undefined ||
			safePath(canonical) === undefined ||
			!this.withinHomeConstraint(canonical, request, false, homeDirectory)
		)
			return null;
		const details = await this.system.stat(canonical, signal);
		const beneath = record(request?.beneath);
		const homeRelative = beneath?.homeRelative;
		if (
			details?.kind !== 'file' ||
			typeof homeRelative !== 'string' ||
			homeDirectory === undefined ||
			!safeRelativePath(homeRelative)
		)
			return null;
	...
	private async resolveRelativeToEnvironment(
		...
		if (
			...
			!matchesExtension(canonical, request?.extension)
		)
			return null;
		if ((await this.system.stat(canonical, signal))?.kind !== 'file')
			return null;
		return { id: this.registerFile(state, canonical).id };
	}
	...
	private async environmentRelativePath(
		...
		const beneath =
			...
			safeRelativePath(request.beneathRelative)
				...
				: undefined;
		const containedRoot =
			root === undefined ? undefined : resolve(root, beneath ?? '.');
		const canonical =
			containedRoot === undefined
				? undefined
				: await this.system.realpath(file.path, signal);
		if (
			root === undefined ||
			containedRoot === undefined ||
			canonical === undefined ||
			!contained(containedRoot, canonical)
		)
			return null;
		if ((await this.system.stat(canonical, signal))?.kind !== 'file')
			return null;
	...
	private withinHomeConstraint(
		...
		if (homeRelative === undefined)
			return (
				defaultHome &&
				homeDirectory !== undefined &&
				contained(resolve(homeDirectory), path)
			);
	...
	private matchesFileConstraint(
		...
		if (
			...
			return false;
		const beneath = record(options?.beneath);
		const homeRelative = beneath?.homeRelative;
		...
		return (
			pathRelative !== '' &&
			pathRelative !== '..' &&
			!pathRelative.startsWith(`..${pathSeparator()}`) &&
	...
	private async fileStat(
		...
		if (
			...
			return null;
		const modifiedAt = safeText(value.modifiedAt, 128)
			? value.modifiedAt
			: undefined;
		const createdAt = safeText(value.createdAt, 128)
			? value.createdAt
			: undefined;
		return {
			handle: { id: file.id },
	...
	private async follow(
		...
		const request = record(payload);
		if (typeof request?.watcherId === 'string')
			return this.pollWatcher(state, request.watcherId, signal);
		const file = this.fileFor(state, request?.handle);
		const options = record(request?.options);
	...
	private async pollWatcher(
		...
		else if (changedIdentity || changedMetadata) {
			watcher.offset = 0;
			type = 'replace';
		}
		watcher.modifiedAt = details.modifiedAt;
		...
		const bytes =
			(await this.system.read(
				watcher.path,
				watcher.offset,
				watcher.maximumChunkBytes,
				signal,
			)) ?? new Uint8Array();
		watcher.offset += bytes.byteLength;
		return {
			events: bytes.byteLength === 0 ? [] : [{ type, bytes: [...bytes] }],
			closed: false,
		};
	}
	...
	private registerDirectory(
		...
		for (const handle of state.directories.values())
			if (handle.path === path) return handle;
		const handle = { id: `directory-${++state.nextId}`, path };
		state.directories.set(handle.id, handle);
		return handle;
	}
	private directoryFor(
		...
		const id = nested?.id ?? source?.id;
		const handle =
			typeof id === 'string' ? state.directories.get(id) : undefined;
		if (handle === undefined)
			throw new Error('agent directory handle is unavailable');
		return handle;
	}
	private fileFor(
		state: TerminalState,
		...
		const handle = typeof id === 'string' ? state.files.get(id) : undefined;
		if (handle === undefined)
			throw new Error('agent file handle is unavailable');
		return handle;
	}
}
		...
		return {
			kind: value.isFile()
				...
				: value.isDirectory()
					...
					: 'other',
			size: value.size,
			modifiedAt: Number.isFinite(value.mtimeMs)
				? new Date(value.mtimeMs).toISOString()
				: undefined,
			createdAt:
				Number.isFinite(value.birthtimeMs) && value.birthtimeMs > 0
					? new Date(value.birthtimeMs).toISOString()
					: undefined,
			identity: fileIdentity(value),
		};
	},
	readDirectory: async (path, signal) => {
		throwIfAborted(signal);
		const values = await readdir(path, { withFileTypes: true }).catch(
			() => undefined,
		);
		throwIfAborted(signal);
...
async function linuxDescendants(
	...
	for (const entry of entries.split(/\r?\n/u)) {
		const pid = Number(entry.slice('/proc/'.length));
		if (!validPid(pid)) continue;
		const raw = await readFile(`/proc/${pid}/stat`, 'utf8').catch(() => '');
		const close = raw.lastIndexOf(')');
		const parent = Number(raw.slice(close + 2).split(/\s+/u)[1]);
...
async function nodeOpenFiles(
	...
	const files = accumulateObservedOpenFiles();
	let current: 'readable' | 'writable' | 'read-write' | undefined;
	for (const line of output.split(/\r?\n/u)) {
		if (line.startsWith('p')) current = undefined;
			...
			line.startsWith('n') &&
			current !== undefined &&
			(access === 'readable' || current !== 'readable')
		) {
...
async function linuxOpenFiles(
	...
	for (const pid of processIds) {
		...
		for (const fd of entries.split(/\r?\n/u)) {
			if (!fd) continue;
			const path = await readlink(fd).catch(() => undefined);
			const safe = safePath(path);
			if (safe === undefined) continue;
			const flags = await readFile(
				`/proc/${pid}/fdinfo/${fd.slice(fd.lastIndexOf('/') + 1)}`,
				'utf8',
			).catch(() => '');
			const octal = /^flags:\s*(0[0-7]+)/mu.exec(flags)?.[1];
			const mode =
				octal === undefined ? undefined : Number.parseInt(octal, 8) & 3;
			const fileAccess =
				mode === 1
					? 'writable'
					: mode === 2
			...
			if (
				...
				(access === 'readable' || fileAccess !== 'readable')
			)
				result.add({ path: safe, access: fileAccess });
		}
	}
	return result.snapshot();
}

async function nodeTty(
	shellPid: number,
	signal: AbortSignal,
): Promise<string | undefined> {
	throwIfAborted(signal);
	if (process.platform === 'linux')
		return readlink(`/proc/${shellPid}/fd/0`).catch(() => undefined);
	const output = await commandText(
		unixTool('lsof'),
		['-a', '-p', String(shellPid), '-d', '0', '-Fn'],
		64 * 1024,
		signal,
		true,
	);
...
async function nodeForeground(
	shellPid: number,
	signal: AbortSignal,
): Promise<
	| Readonly<{ executableName: string; arguments?: readonly string[] }>
	| undefined
> {
	const output = await commandText(
		unixTool('ps'),
		['-p', String(shellPid), '-o', 'comm='],
		64 * 1024,
		signal,
		true,
	);
	const executableName = basenameSafe(output.trim());
	return executableName ? { executableName } : undefined;
}
...
async function nodeEnvironment(
	...
	throwIfAborted(signal);
	if (process.platform === 'linux') {
		const raw = await readFile(`/proc/${shellPid}/environ`).catch(
			() => undefined,
		);
		...
		const values = new Map(
			raw
				...
				.flatMap((entry) => {
					...
					return equals <= 0
						? []
						: [[entry.slice(0, equals), entry.slice(equals + 1)]];
				}),
		);
	...
	const output = await commandText(
		unixTool('ps'),
		['e', '-p', String(shellPid), '-o', 'command='],
		256 * 1024,
		signal,
		true,
	);
	const values = new Map<string, string>();
	for (const token of output.split(/\s+/u)) {
		const equals = token.indexOf('=');
		if (equals > 0) values.set(token.slice(0, equals), token.slice(equals + 1));
	}
	return Object.fromEntries(
		names.flatMap((name) =>
			values.has(name) ? [[name, values.get(name)!]] : [],
...
async function nodeRead(
	...
	try {
		...
		const read = await file.read(buffer, 0, maximum, position);
		throwIfAborted(signal);
		return new Uint8Array(read.buffer.subarray(0, read.bytesRead));
	} finally {
...
function unixTool(name: 'ps' | 'lsof' | 'sh'): string {
	if (process.platform === 'darwin')
		return name === 'lsof' ? '/usr/sbin/lsof' : `/bin/${name}`;
	if (process.platform === 'linux')
		return name === 'lsof' ? '/usr/bin/lsof' : `/bin/${name}`;
	return name;
}
...
function observationSpawnEnv(): NodeJS.ProcessEnv {
	...
	return {
		...process.env,
		PATH: path && path.length > 0 ? `${path}:${fallback}` : fallback,
	};
}
...
function commandText(
	...
	return new Promise((resolvePromise, reject) => {
		...
		const abort = () => {
			child.kill('SIGTERM');
			finish(() => reject(new Error('agent observation cancelled')));
		};
...
function sessionProcesses(
	...
	return pids.flatMap((pid) => {
		const executableName = basenameSafe(
			names.get(pid) ?? (pid === shellPid ? 'shell' : ''),
		);
...
function lsofAccess(
	value: string,
): 'readable' | 'writable' | 'read-write' | undefined {
	const read = value.includes('r');
	const write = value.includes('w') || value.includes('u');
	return read && write
		? 'read-write'
		: write
			? 'writable'
...
function foregroundValue(
	...
	if (!safeText(value.executableName, 512))
		throw new Error('agent foreground observation is malformed');
	const argumentsSafe = value.arguments
		?.filter((item) => safeText(item, 4_096) !== undefined)
		.slice(0, 64);
	const startedAt = safeText(value.startedAt, 128)
		? value.startedAt
		: undefined;
	return {
		executableName: value.executableName,
		...(argumentsSafe === undefined ? {} : { arguments: argumentsSafe }),
		...(startedAt === undefined ? {} : { startedAt }),
	};
}
...
function safePath(value: unknown): string | undefined {
	return safeText(value, MAX_PATH_LENGTH) && isAbsolute(value)
		? value
		: undefined;
}
...
function safeDirectoryEntryName(value: string): boolean {
	return (
...
function isOpenAccess(
	value: unknown,
): value is 'readable' | 'writable' | 'read-write' {
	return value === 'readable' || value === 'writable' || value === 'read-write';
}
function isAgentJournalPath(path: string): boolean {
...
function accumulateObservedOpenFiles(): {
	...
	return {
		...
		snapshot() {
			return journals.length > 0 ? journals : others;
		},
	};
}
...
function boundedBytes(value: unknown, maximum: number): number {
	if (
		typeof value !== 'number' ||
		!Number.isSafeInteger(value) ||
		value <= 0 ||
		...
		throw new Error('agent file byte limit is invalid');
	return value;
}
...
function positiveBound(value: number | undefined, fallback: number): number {
	...
	if (!Number.isSafeInteger(value) || value <= 0 || value > fallback)
		throw new RangeError('invalid agent observation byte limit');
	return value;
}
...
function throwIfAborted(signal: AbortSignal): void {
	if (signal.aborted) throw new Error('agent observation cancelled');
}
