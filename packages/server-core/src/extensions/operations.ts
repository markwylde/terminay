...
export function createExtensionOperationHandlers(options: ExtensionOperationOptions): OperationRegistries {
  ...
  const queries: Record<string, QueryHandler> = {
    ...
    "extensions.previewInstall": async (request) => { permission(request.context, "extensions:manage"); const payload = exact(request.envelope.payload, ["spec"]); return previewDto(await options.installer.preview(text(payload, "spec"), request.context.signal)); },
    "extensions.previewUpdate": async (request) => { permission(request.context, "extensions:manage"); const payload = exact(request.envelope.payload, ["extensionId", "spec"]); const state = await options.installer.snapshot(); const value = state.extensions[text(payload, "extensionId")]; if (value === undefined) throw failure("not_found", "extension is not installed"); const spec = payload.spec === undefined ? `${value.packageName}@latest` : text(payload, "spec"); const preview = await options.installer.preview(spec, request.context.signal); if (preview.packageName !== value.packageName) throw failure("validation", "an update cannot change package identity"); return previewDto(preview); },
  };
