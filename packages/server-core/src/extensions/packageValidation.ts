...
import {
	...
	type TerminayExtensionManifest,
...
export async function validateMaterializedExtension(
	stagingRoot: string,
	resolution: RegistryPackageResolution,
	installedExtensions?: ReadonlyMap<string, { readonly apiVersion: string }>,
): Promise<ValidatedExtensionTree> {
	...
	if (outside(canonicalStaging, canonicalPackage))
		throw new Error('extension package root escapes staging slot');
	const packageJson = parseObject(
		await readFile(join(packageRoot, 'package.json')),
		'extension package.json',
	);
	if (
		packageJson.name !== resolution.packageName ||
		packageJson.version !== resolution.version
	)
		throw new Error('materialized package identity differs from preview');
	...
	assertExtensionCompatible(manifest, {
		terminayVersion: '1.0.0',
		installedExtensions,
	});
...
export function validateNpmLockfile(
	lock: unknown,
	resolution: RegistryPackageResolution,
): void {
	if (typeof lock !== 'object' || lock === null || Array.isArray(lock))
		throw new Error('exact npm lockfile v3 is required');
	const lockRecord = lock as Record<string, unknown>;
	if (
		lockRecord.lockfileVersion !== 3 ||
		typeof lockRecord.packages !== 'object' ||
		lockRecord.packages === null ||
	...
	if (
		...
		target === null ||
		(target as Record<string, unknown>).version !== resolution.version ||
		(target as Record<string, unknown>).integrity !== resolution.integrity
	)
		throw new Error('lockfile does not bind the confirmed package integrity');
	for (const [path, value] of Object.entries(packages)) {
		...
		if (
			...
			throw new Error('lockfile contains an invalid package record');
		const record = value as Record<string, unknown>;
		if (typeof record.integrity !== 'string' || record.integrity.length < 20)
			throw new Error('every dependency requires registry integrity');
		if (
			...
			const localRoot =
				path === `node_modules/${resolution.packageName}` &&
				(resolution.source === 'uploaded' ||
					resolution.source === 'built-in') &&
				record.resolved.startsWith('file:');
			const localBuiltInDependency =
				resolution.source === 'built-in' &&
				isAllowedBuiltInLocalDependency(resolution, path) &&
				record.resolved.startsWith('file:');
			if (!localRoot && !localBuiltInDependency)
				throw new Error('dependency did not resolve from public npmjs');
		}
		if (record.link === true)
		...
		for (const field of [
			...
			if (
				...
				for (const spec of Object.values(dependencies))
					if (
						typeof spec !== 'string' ||
						/^(?:git|https?|file|link|npm|workspace):/iu.test(spec)
					)
						throw new Error(
							'non-registry dependency specifications are unsupported',
						);
			}
		}
	}
...
function isAllowedBuiltInLocalDependency(
	resolution: RegistryPackageResolution,
	path: string,
): boolean {
	const allowed = (
		resolution as import('./installerTypes.js').BuiltInExtensionArtifact
	).localDependencies;
	const packageName = path.slice('node_modules/'.length);
	return (
		Array.isArray(allowed) &&
		allowed.includes(packageName) &&
		packageName === '@terminay/extension-api'
	);
}
	...
	async function walk(directory: string): Promise<void> {
		for (const entry of await readdir(directory, { withFileTypes: true })) {
			...
			if (entry.isSymbolicLink())
				throw new Error('extension trees cannot contain symbolic links');
			...
			if (output.length > MAX_FILES || bytes > MAX_BYTES)
				throw new Error('extension package tree exceeds v1 limits');
...
function outside(root: string, candidate: string): boolean {
	...
	return value === '' ? false : value.startsWith('..') || isAbsolute(value);
}
