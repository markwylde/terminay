...
import type {
	ProjectEnvironmentInvocationContext,
	ProjectEnvironmentRegistry,
	ProjectEnvironmentRuntime,
} from '../projectEnvironment/registry.js';
import type {
	ProjectEnvironmentCapability,
	ProjectEnvironmentState,
} from '../projectEnvironment/types.js';
...
const OPERATIONS: Readonly<
	Record<ProjectEnvironmentCapability, ReadonlySet<string>>
> = Object.freeze({
	terminal: new Set([
		'resolve-launch',
		'spawn',
	...
	'filesystem-observation': new Set([
		'observe',
		'poll',
		'stop',
		'manualRefresh',
...
export class ExtensionProjectEnvironmentRuntime
	implements ProjectEnvironmentRuntime
{
	...
	constructor(
		...
		this.remoteFiles = new RemoteFileProtocol(
			(operation, input, context) =>
				this.invokeService('filesystem', operation, input, context),
			projectRoot,
		);
	}
	...
	async invoke(
		...
		if (!this.capabilities.includes(capability))
			throw new Error('provider service operation is unavailable');
		if (capability === 'filesystem' && operation === 'prepare-project-root')
			return this.prepareProjectRoot(input, context);
		if (capability === 'filesystem' && operation === 'commit-project-root')
			return this.commitProjectRoot(input, context);
		if (
			capability === 'filesystem' &&
			(operation.startsWith('files.') || operation.startsWith('file.'))
		)
			return this.remoteFiles.invoke(operation, input, context);
		if (!OPERATIONS[capability].has(operation))
			throw new Error('provider service operation is unavailable');
		if (capability === 'git')
			input = withProjectRoot(input, this.projectRoot?.(context.projectId));
		if (capability === 'terminal' && operation === 'resolve-launch')
			return this.resolveLaunch(input, context);
		if (capability === 'terminal' && operation === 'spawn')
			return this.spawn(input, context);
	...
	private resolveLaunch(
		...
		if (!isLaunchInput(input))
			throw new Error('remote terminal launch input is invalid');
		...
		if (typeof cwd !== 'string' || cwd.length === 0)
			throw new Error('project environment has no default root');
	...
	private environment(context: ProjectEnvironmentInvocationContext) {
		const environment =
			this.snapshot().environments[context.projectEnvironmentId];
		if (
			environment === undefined ||
			environment.providerId !== this.providerId ||
			environment.pinnedRevision !== context.environmentRevision ||
			environment.status !== 'ready' ||
			environment.archived
		)
			throw new Error('project environment binding changed');
		return environment;
	}
	private async prepareProjectRoot(
		input: unknown,
		context: ProjectEnvironmentInvocationContext,
	): Promise<{
	...
	private commitProjectRoot(
		input: unknown,
		context: ProjectEnvironmentInvocationContext,
	): null {
		if (
			!isPlainObject(input) ||
			Object.keys(input).some((key) => key !== 'preparationId')
		)
			throw new TypeError('provider service input contains unknown fields');
		const preparationId = input.preparationId;
		if (
			typeof preparationId !== 'string' ||
			preparationId.length === 0 ||
			preparationId.length > 128
		)
			throw new TypeError('project root preparation is invalid');
	...
	private async spawn(
		...
		if (created.sessionId !== sessionId)
			throw new Error('remote terminal identity mismatch');
...
export function registerActivatedExtensionProjectEnvironmentRuntimes(
	options: Readonly<{
		...
		workspaceSnapshot?: () => {
			readonly projects: Readonly<Record<string, { readonly root?: string }>>;
		};
	}>,
): { dispose(): void } {
	...
	const reconcile = (): void => {
		const contributions =
			options.hosts.activatedProjectEnvironmentContributions();
			...
			options.registry.register(
				new ExtensionProjectEnvironmentRuntime(
					...
					options.workspaceSnapshot === undefined
						? undefined
						: (projectId) =>
								options.workspaceSnapshot!().projects[projectId]?.root,
				),
			);
...
function isLaunchInput(value: unknown): value is Record<string, unknown> {
	...
	return (
		...
		Number(source.cols) >= 1 &&
		Number(source.cols) <= 1000 &&
		Number.isInteger(source.rows) &&
		Number(source.rows) >= 1 &&
		Number(source.rows) <= 1000 &&
		(source.cwd === undefined ||
			(typeof source.cwd === 'string' &&
				source.cwd.length > 0 &&
				source.cwd.length <= 4096)) &&
		(source.profileId === undefined ||
			(typeof source.profileId === 'string' &&
				source.profileId.length > 0 &&
				source.profileId.length <= 256)) &&
		(source.activePanelId === undefined ||
			(typeof source.activePanelId === 'string' &&
				source.activePanelId.length > 0 &&
				source.activePanelId.length <= 256))
	);
}

class ProviderPty implements PtyProcess {
	private data = new Set<(bytes: Uint8Array) => void>();
	private exits = new Set<
		(exit: { exitCode?: number | null; signal?: number | null }) => void
	>();
	private foreground = new Set<
		(event: { processName: string; shellForeground: boolean }) => void
	>();
	private timer: ReturnType<typeof setTimeout> | undefined;
	private processTimer: ReturnType<typeof setTimeout> | undefined;
	private processObservationId: string | undefined;
	private lastProcess: string | null | undefined;
	private stopped = false;
	private readonly streamAbort = new AbortController();
	constructor(
		private runtime: ExtensionProjectEnvironmentRuntime,
		private context: ProjectEnvironmentInvocationContext,
		private id: string,
	) {
		this.poll();
	}
	...
	resize = (dimensions: { cols: number; rows: number }) =>
		this.call('resize', { sessionId: this.id, ...dimensions }).then(
			() => undefined,
		);
	...
	dispose = async () => {
		...
		try {
			if (this.processObservationId)
				await this.runtime
					.invoke(
						'process-observation',
						'stop',
						{ observationId: this.processObservationId, sessionId: this.id },
						this.streamContext(),
					)
					.catch(() => undefined);
			await this.call('dispose', { sessionId: this.id });
		} finally {
	...
	getCwd = async (signal?: AbortSignal) => {
		const state = await this.observeProcess(signal);
		return state.state === 'available' ? state.cwd : null;
	};
	onForegroundProcess = (
		listener: (event: {
			processName: string;
		...
		if (this.foreground.size === 1) this.pollProcess();
		return () => {
			this.foreground.delete(listener);
			if (this.foreground.size === 0 && this.processTimer) {
				clearTimeout(this.processTimer);
				this.processTimer = undefined;
			}
		};
	};
	refreshForegroundProcess = async (signal?: AbortSignal) => {
		if (this.stopped) return;
		try {
			const state = await this.observeProcess();
			if (signal?.aborted || this.stopped) return;
			if (state.state !== 'available') return;
			if (state.foregroundProcess === this.lastProcess) return;
			this.lastProcess = state.foregroundProcess;
			const event = {
				processName: state.foregroundProcess ?? '',
				shellForeground: state.foregroundProcess === null,
			};
			for (const listener of this.foreground) listener(event);
		} catch {
			/* close observation reports limited from the last committed sample */
		}
	};
	 ...
	 * out. */
	private streamContext(): ProjectEnvironmentInvocationContext {
		return {
			...this.context,
	...
	private poll = async () => {
		...
		try {
			...
			if (bytes.length) for (const listener of this.data) listener(bytes);
			if (result.exit) {
				this.stopped = true;
				for (const listener of this.exits)
					listener({ exitCode: result.exit.code, signal: null });
				return;
			}
		} catch {
			this.stopped = true;
		...
		this.timer = setTimeout(this.poll, 15);
	};
	private observeProcess = async (signal?: AbortSignal) => {
		const context =
			signal === undefined
				? this.streamContext()
				: { ...this.streamContext(), signal };
		if (!this.processObservationId) {
			const started = (await this.runtime.invoke(
				'process-observation',
				'observe',
				{ sessionId: this.id },
				context,
			)) as Record<string, unknown>;
			if (typeof started.observationId !== 'string')
				throw new TypeError('invalid process observation identity');
			this.processObservationId = started.observationId;
		}
		return this.runtime.invoke(
			'process-observation',
			'poll',
	...
	private pollProcess = async () => {
		if (this.stopped || this.foreground.size === 0) return;
		try {
			const state = await this.observeProcess();
			if (
				state.state === 'available' &&
				state.foregroundProcess !== this.lastProcess
			) {
				this.lastProcess = state.foregroundProcess;
				const event = {
					processName: state.foregroundProcess ?? '',
			...
			/* unavailable observation preserves terminal output fallback */
		}
...
function validateInput(
	...
	if (input === null || typeof input !== 'object' || Array.isArray(input))
		throw new TypeError('provider service input is invalid');
	const value = input as Record<string, unknown>;
	const allowed =
		capability === 'terminal'
			? (
					{
						create: ['sessionId', 'term', 'rows', 'cols', 'environment'],
				...
				? (
						{
							...
							createDirectory: ['path', 'mode', 'root'],
							rename: ['path', 'destination', 'root'],
							remove: ['path', 'root'],
						} as Record<string, string[]>
					)[operation]
				: capability === 'filesystem-observation'
					...
					: capability === 'process-observation'
						? (
								{
									observe: ['sessionId'],
									poll: ['observationId', 'sessionId'],
									stop: ['observationId', 'sessionId'],
								} as Record<string, string[]>
							)[operation]
						: capability === 'git'
	...
	if (
		...
		throw new TypeError('terminal session identity is required');
	if (
		operation === 'input' &&
		(typeof value.data !== 'string' ||
			Buffer.byteLength(value.data) > 64 * 1024)
	)
	...
	if (
		...
		(typeof value.data !== 'string' ||
			...
			![undefined, 'utf8', 'base64'].includes(value.encoding as never))
	)
		throw new TypeError('filesystem write input is invalid');
	for (const key of ['observationId', 'proof', 'cwd', 'foregroundProcess'])
		if (
			value[key] !== undefined &&
			(typeof value[key] !== 'string' ||
				String(value[key]).length > 4096 ||
	...
	if (
		['poll', 'stop', 'manualRefresh'].includes(operation) &&
		capability.includes('observation') &&
	...
	if (capability === 'git') {
		if (
			...
			throw new TypeError('Git protocol payload is invalid');
		if (
			value.body !== undefined &&
			(typeof value.body !== 'string' || value.body.length > 1_400_000)
		)
			throw new TypeError('Git protocol body is invalid');
		...
		if (
			...
			throw new TypeError('Git protocol request is invalid');
		const metadata = request as Record<string, unknown>;
		if (
			Object.keys(metadata).some(
				(key) => !['clientId', 'authScope', 'expectedRevision'].includes(key),
			) ||
			typeof metadata.clientId !== 'string' ||
			typeof metadata.authScope !== 'string' ||
...
function withProjectRoot(input: unknown, root: string | undefined): unknown {
	if (typeof root !== 'string' || root.length === 0 || !isPlainObject(input))
		return input;
	return { ...input, root };
}
function requiredRoot(input: unknown): string {
	if (!isPlainObject(input) || Object.keys(input).some((key) => key !== 'root'))
		throw new TypeError('provider service input contains unknown fields');
...
function asRecord(value: unknown): Record<string, unknown> {
	if (!isPlainObject(value))
		throw new TypeError('provider service result is invalid');
	return value;
}
...
function toJson(value: unknown): JsonValue {
	if (containsNonJson(value))
		throw new TypeError('provider service input is invalid or too large');
