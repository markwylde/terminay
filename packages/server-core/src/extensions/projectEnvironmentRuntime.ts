...
class ProviderPty implements PtyProcess {
  ...
  dispose=async()=>{this.stopped=true;if(this.timer)clearTimeout(this.timer);if(this.processTimer)clearTimeout(this.processTimer);if(this.processObservationId)await this.runtime.invoke("process-observation","stop",{observationId:this.processObservationId,sessionId:this.id},this.context).catch(()=>undefined);await this.call("dispose",{sessionId:this.id});};
