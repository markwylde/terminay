...
export interface FileAdapterOptions {
	readonly serverId: string;
	readonly projects:
		| ReadonlyMap<string, FileProjectContext>
		| Readonly<Record<string, FileProjectContext>>;
...
export class ServerFileAdapter {
	...
	constructor(options: FileAdapterOptions) {
		if (typeof options?.serverId !== 'string' || !validId(options.serverId))
			throw new TypeError('file server id is invalid');
		if (
			options.projects === undefined ||
			typeof options.projects !== 'object' ||
			options.projects === null
		)
			throw new TypeError('file projects are required');
		this.serverId = options.serverId;
		this.options = options;
		this.maxSessions = positive(options.maxSessions ?? 1024, 'maxSessions');
		this.maxDraftBytes = optionalPositive(
			options.maxDraftBytes,
			'maxDraftBytes',
		);
		this.maxRangeBytes = optionalPositive(
	...
	async open(request: FileOpenRequest): Promise<FileOpenResult> {
		...
		const authorization = this.requireProject(
		...
		if (this.sessions.size >= this.maxSessions)
			throw new FileServiceError(
				'draft_too_large',
				'file session limit reached',
				{ max: this.maxSessions },
			);
		const metadata =
			context.storage.stat === undefined
				? undefined
				: await context.storage.stat(canonicalPath);
		const sessionOptions: FileSessionOptions = {
			...(metadata === undefined ? {} : { initialMetadata: metadata }),
			...(this.maxDraftBytes === undefined
				? {}
				: { maxDraftBytes: this.maxDraftBytes }),
			...(this.maxRangeBytes === undefined
				? {}
				: { maxRangeBytes: this.maxRangeBytes }),
		};
	...
	readRange(request: FileReadRangeRequest): Promise<FileReadRange> {
		const record = this.authorizedSession(request, 'read');
		return this.assertCanonical(record).then(() =>
			record.session.readRange(request.offset, request.length, request.signal),
		);
	}
	...
	async readText(request: FileReadRangeRequest): Promise<FileTextRange> {
		...
		try {
			text = new TextDecoder('utf-8', { fatal: true }).decode(range.bytes);
		} catch {
			text = new TextDecoder('utf-8').decode(range.bytes);
	...
	): Promise<FileMutationResult<FileSessionState>> {
		const record = this.authorizedSession(request, 'write');
		await this.assertCanonical(record);
		return record.session.applyDraft(request.bytes, {
	...
	save(
		...
		return this.assertCanonical(record).then(() =>
			record.session.save({
				expectedDiskRevision: request.expectedDiskRevision,
				expectedDraftRevision: request.expectedDraftRevision,
			}),
		);
	}
	...
	async close(
		...
		const record = this.authorizedSession(request, 'write');
		await this.assertCanonical(record);
		if (record.session.dirty && request.confirmDirty !== true)
			return {
				ok: false,
				error: new FileServiceError(
					'save_precondition',
					'dirty file requires explicit close confirmation',
				),
			};
	...
	disconnect(): readonly FileOpenResult[] {
		return [...this.sessions.values()].map((record) => this.openResult(record));
	}

	operations(): FileOperationHandlers {
		return {
			queries: {
				[FILE_OPERATIONS.open]: (request) =>
					asJson(this.open(this.openRequest(request))),
				[FILE_OPERATIONS.metadata]: async (request) =>
					asJson(await this.metadata(this.sessionRequest(request))),
				[FILE_OPERATIONS.readRange]: async (request) =>
					asJson(
						serializeRange(await this.readRange(this.rangeRequest(request))),
					),
				[FILE_OPERATIONS.readText]: async (request) =>
					asJson(
						serializeText(await this.readText(this.rangeRequest(request))),
					),
			},
			commands: {
				[FILE_OPERATIONS.edit]: async (request) =>
					serializeMutation(await this.edit(this.editRequest(request))),
				[FILE_OPERATIONS.save]: async (request) =>
					serializeMutation(await this.save(this.saveRequest(request))),
				[FILE_OPERATIONS.reload]: async (request) =>
					serializeMutation(await this.reload(this.reloadRequest(request))),
				[FILE_OPERATIONS.keepLocal]: async (request) =>
					serializeMutation(await this.keepLocal(this.sessionRequest(request))),
				[FILE_OPERATIONS.close]: async (request) =>
					serializeMutation(await this.close(this.closeRequest(request))),
			},
		};
	}
	...
	private project(projectId: string): FileProjectContext {
		const project =
			typeof (this.options.projects as ReadonlyMap<string, FileProjectContext>)
				.get === 'function'
				? (
						this.options.projects as ReadonlyMap<string, FileProjectContext>
					).get(projectId)
				: (
		...
		if (!(project.resolver instanceof CanonicalProjectPathResolver))
			throw new TypeError('project resolver is invalid');
		if (typeof project.storage.atomicWrite !== 'function')
			throw new TypeError('project storage cannot save files');
		return project;
	}
	...
	private requireProject(
		...
		if (!scopeAllows(authorization.scope, required))
			throw new FileServiceError(
				'path_escape',
				`file operation requires ${required} scope`,
			);
		if (
			...
			throw new FileServiceError(
				'path_escape',
				'file project identity is missing',
			);
		if (
			authorization.projectId !== undefined &&
		...
		if (this.options.authorizeProject?.(authorization, projectId) === false)
			throw new FileServiceError(
				'path_escape',
				'file is outside the authorized project',
			);
	...
	private authorizedSession(
		...
		if (!scopeAllows(request.authorization.scope, required))
			throw new FileServiceError(
				'path_escape',
				`file operation requires ${required} scope`,
			);
		...
		if (record === undefined)
			throw new FileServiceError('path_missing', 'file session not found', {
				canonical: request.sessionId,
			});
	...
	private requireSession(
		...
		if (
			...
			throw new FileServiceError(
				'path_escape',
				'file session identity mismatch',
			);
		if (
	...
	private async assertCanonical(record: SessionRecord): Promise<void> {
		...
		if (canonical !== record.canonicalPath)
			throw new FileServiceError(
				'revision_conflict',
				'file canonical identity changed',
	...
	private makeSessionId(projectId: string, canonicalPath: string): string {
		const generated =
			this.options.generateSessionId?.(projectId, canonicalPath) ??
			`file-${(++this.sessionCounter).toString(36)}`;
		if (!validId(generated) || this.sessions.has(generated))
			throw new FileServiceError(
				'path_missing',
				'file session id is unavailable',
			);
		return generated;
	}
	...
	private remove(record: SessionRecord): void {
		...
		this.sessionsByPath.delete(
			`${record.projectId}\u0000${record.canonicalPath}`,
		);
	}
	...
	private sessionRequest(
		...
		return {
			...
			sessionId: stringField(
				objectPayload(request.envelope.payload),
				'sessionId',
			),
		};
	}
	private rangeRequest(request: QueryRequest): FileReadRangeRequest {
		const payload = objectPayload(request.envelope.payload);
		return {
			authorization: this.authorization(request),
			sessionId: stringField(payload, 'sessionId'),
			offset: uintField(payload, 'offset'),
			length: uintField(payload, 'length'),
			signal: request.context.signal,
		};
	}
	private editRequest(request: CommandRequest): FileEditRequest {
		const payload = objectPayload(request.envelope.payload);
		return {
			authorization: this.authorization(request),
			sessionId: stringField(payload, 'sessionId'),
			bytes:
				request.body.byteLength > 0
					...
					: textBytes(typeof payload.text === 'string' ? payload.text : ''),
			...(optionalUInt(payload.expectedDraftRevision) === undefined
				? {}
				: {
						expectedDraftRevision: optionalUInt(payload.expectedDraftRevision),
					}),
		};
	}
	private saveRequest(request: CommandRequest): FileSaveRequest {
		const payload = objectPayload(request.envelope.payload);
		return {
			authorization: this.authorization(request),
			sessionId: stringField(payload, 'sessionId'),
			...(optionalUInt(payload.expectedDiskRevision) === undefined
				? {}
				: { expectedDiskRevision: optionalUInt(payload.expectedDiskRevision) }),
			...(optionalUInt(payload.expectedDraftRevision) === undefined
				? {}
				: {
						expectedDraftRevision: optionalUInt(payload.expectedDraftRevision),
	...
	private reloadRequest(request: CommandRequest): FileReloadRequest {
		const payload = objectPayload(request.envelope.payload);
		return {
			authorization: this.authorization(request),
			sessionId: stringField(payload, 'sessionId'),
			...(typeof payload.confirm === 'boolean'
	...
	private closeRequest(request: CommandRequest): FileCloseRequest {
		const payload = objectPayload(request.envelope.payload);
		return {
			authorization: this.authorization(request),
			sessionId: stringField(payload, 'sessionId'),
			...(typeof payload.confirmDirty === 'boolean'
	...
	private authorization(
		...
		return {
			...
			...(claimsProject(request.context.claims) === undefined
				? {}
				: { projectId: claimsProject(request.context.claims) }),
			...(claimsSession(request.context.claims) === undefined
				? {}
				: { sessionId: claimsSession(request.context.claims) }),
		};
	}
}
...
function objectPayload(value: unknown): Record<string, unknown> {
	if (typeof value !== 'object' || value === null || Array.isArray(value))
		throw invalidRequest('file payload must be an object');
...
function optionalString(value: unknown): string | undefined {
	...
	if (
		...
		throw invalidRequest('string field is invalid');
	return value;
}
...
function uintField(value: Record<string, unknown>, key: string): number {
	const candidate = value[key];
...
function normalizeRelative(value: string): string {
	if (
		...
		throw new FileServiceError('invalid_path', 'file path is invalid');
	const parts = value.split('/');
	if (parts.some((part) => part.length === 0 || part === '.' || part === '..'))
		throw new FileServiceError('invalid_path', 'file path is not canonical');
	return parts.join('/');
}
function claimsProject(value: unknown): string | undefined {
	return claimsString(value, 'projectId');
}
function claimsSession(value: unknown): string | undefined {
	return claimsString(value, 'sessionId');
}
function claimsString(value: unknown, key: string): string | undefined {
	if (typeof value !== 'object' || value === null || Array.isArray(value))
		return undefined;
...
function serializeRange(range: FileReadRange): JsonValue {
	return {
		...range,
		bytes: bytesToBase64(range.bytes),
	} as unknown as JsonValue;
}
function serializeText(range: FileTextRange): JsonValue {
	return {
		...(serializeRange(range) as Record<string, unknown>),
		text: range.text,
		invalidEncoding: range.invalidEncoding,
	} as unknown as JsonValue;
}
...
function invalidRequest(message: string): FileServiceError {
	return new FileServiceError('invalid_path', message);
}
