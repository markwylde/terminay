...
import {
	FileCatalog,
	type FileCatalogListOptions,
	type FileCatalogPreviewOptions,
	type FileCatalogSearchOptions,
...
export interface FileCatalogAdapterOptions {
	readonly serverId: string;
	readonly projects:
		| ReadonlyMap<string, FileCatalogProjectContext>
		| Readonly<Record<string, FileCatalogProjectContext>>;
...
export class ServerFileCatalogAdapter {
	...
	constructor(options: FileCatalogAdapterOptions) {
		if (typeof options?.serverId !== 'string' || !validId(options.serverId))
			throw new TypeError('file catalog server id is invalid');
		if (
			options.projects === undefined ||
			typeof options.projects !== 'object' ||
			options.projects === null
		)
			throw new TypeError('file catalog projects are required');
	...
	async list(
		...
		return asJson(
			await catalog.list(
				request.path ?? '.',
				withSignal(request.options, request.signal),
			),
		);
	}
	...
	operations(): {
		...
		readonly commands: Readonly<
			Record<
				string,
				(request: CommandRequest) => JsonValue | Promise<JsonValue>
			>
		>;
	} {
		return {
			queries: {
				[FILE_CATALOG_OPERATIONS.list]: (request) =>
					this.observed(FILE_CATALOG_OPERATIONS.list, () =>
						this.list(this.listRequest(request)),
					),
				[FILE_CATALOG_OPERATIONS.search]: (request) =>
					this.observed(FILE_CATALOG_OPERATIONS.search, () =>
						this.search(this.searchRequest(request)),
					),
				[FILE_CATALOG_OPERATIONS.size]: (request) =>
					this.observed(FILE_CATALOG_OPERATIONS.size, () =>
						this.size(
							this.optionsRequest(request) as FileCatalogRequest & {
								readonly options?: FileCatalogSizeOptions;
							},
						),
					),
				[FILE_CATALOG_OPERATIONS.previewMetadata]: (request) =>
					this.observed(FILE_CATALOG_OPERATIONS.previewMetadata, () =>
				...
				[FILE_CATALOG_OPERATIONS.tasks]: (request) =>
					this.observed(FILE_CATALOG_OPERATIONS.tasks, () =>
						this.tasks(
							this.optionsRequest(request) as FileCatalogRequest & {
								readonly options?: MarkdownTaskAggregationOptions;
							},
			...
			commands: {
				[FILE_CATALOG_OPERATIONS.createFile]: (request) =>
					this.observed(FILE_CATALOG_OPERATIONS.createFile, () =>
						this.createFile(this.createFileRequest(request)),
					),
				[FILE_CATALOG_OPERATIONS.createDirectory]: (request) =>
					this.observed(FILE_CATALOG_OPERATIONS.createDirectory, () =>
						this.createDirectory(this.pathRequest(request)),
					),
				[FILE_CATALOG_OPERATIONS.rename]: (request) =>
					this.observed(FILE_CATALOG_OPERATIONS.rename, () =>
						this.rename(this.renameRequest(request)),
					),
				[FILE_CATALOG_OPERATIONS.delete]: (request) =>
					this.observed(FILE_CATALOG_OPERATIONS.delete, () =>
			...
			typeof (
				...
				: (
						this.options.projects as Readonly<
							Record<string, FileCatalogProjectContext>
						>
					)[projectId];
		if (
			project === undefined ||
			project.projectId !== projectId ||
			!(project.catalog instanceof FileCatalog)
		)
	...
	private payload(
		...
		if (typeof value !== 'object' || value === null || Array.isArray(value))
			throw invalidRequest('file catalog payload must be an object');
	...
	): FileCatalogRequest & { readonly options?: Record<string, unknown> } {
		const payload = this.payload(request);
	...
	private listRequest(
		request: QueryRequest,
	): FileCatalogRequest & { readonly options?: FileCatalogListOptions } {
		return this.optionsRequest(request) as FileCatalogRequest & {
			readonly options?: FileCatalogListOptions;
		};
	}
		...
		return {
			...
			query:
				typeof payload.query === 'string'
					? payload.query
					: (invalidRequest('search query is invalid') as never),
		} as FileCatalogRequest & {
			readonly query: string;
			readonly options?: FileCatalogSearchOptions;
		};
	}
	...
	): FileCatalogRequest & { readonly bytes: Uint8Array } {
		const payload = this.payload(request);
		const bytes =
			request.body.byteLength > 0
				? new Uint8Array(request.body)
				: decodeBase64(payload.bytesBase64);
		return { ...this.pathRequest(request), bytes };
	}
	private renameRequest(
		request: CommandRequest,
	): FileCatalogRequest & { readonly destination: string } {
		const payload = this.payload(request);
		return {
			...this.pathRequest(request),
	...
	private deleteRequest(
		request: CommandRequest,
	): FileCatalogRequest & { readonly recursive?: boolean } {
		const payload = this.payload(request);
		return {
			...this.pathRequest(request),
...
function requiredPath(value: string | undefined): string {
	if (
		...
		throw invalidRequest('file path is invalid');
	return value;
}
...
function decodeBase64(value: unknown): Uint8Array {
	if (
		...
		throw invalidRequest('file bytes are invalid');
...
function compactTaskOptions(
	options: MarkdownTaskAggregationOptions | undefined,
): MarkdownTaskAggregationOptions {
	return {
		...(options ?? {}),
		maxBytes: Math.min(
			positiveOption(
				options?.maxBytes ?? PROTOCOL_TASK_LIMITS.maxBytes,
				'maxBytes',
			),
			PROTOCOL_TASK_LIMITS.maxBytes,
		),
		maxFiles: Math.min(
			positiveOption(
				options?.maxFiles ?? PROTOCOL_TASK_LIMITS.maxFiles,
				'maxFiles',
			),
			PROTOCOL_TASK_LIMITS.maxFiles,
		),
		maxTasks: Math.min(
			positiveOption(
				options?.maxTasks ?? PROTOCOL_TASK_LIMITS.maxTasks,
				'maxTasks',
			),
			PROTOCOL_TASK_LIMITS.maxTasks,
		),
		maxTaskLabelLength: Math.min(
			positiveOption(
				options?.maxTaskLabelLength ?? PROTOCOL_TASK_LIMITS.maxTaskLabelLength,
				'maxTaskLabelLength',
			),
			PROTOCOL_TASK_LIMITS.maxTaskLabelLength,
		),
	};
}
