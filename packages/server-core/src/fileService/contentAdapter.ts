...
export interface FileContentAdapterOptions {
	readonly serverId: string;
	readonly projects:
		| ReadonlyMap<string, FileContentProjectContext>
		| Readonly<Record<string, FileContentProjectContext>>;
...
export class ServerFileContentAdapter {
	...
	constructor(options: FileContentAdapterOptions) {
		if (typeof options?.serverId !== 'string' || !validId(options.serverId))
			throw new TypeError('file content server id is invalid');
		if (
			options.projects === undefined ||
			typeof options.projects !== 'object' ||
			options.projects === null
		)
			throw new TypeError('file content projects are required');
	...
	operations(): {
		...
		return {
			queries: {
				[FILE_CONTENT_OPERATIONS.capabilities]: (request) =>
					this.capabilities(this.pathRequest(request)),
				[FILE_CONTENT_OPERATIONS.readRange]: (request) =>
					this.readRange(this.rangeRequest(request)),
				[FILE_CONTENT_OPERATIONS.readText]: (request) =>
					this.readText(this.rangeRequest(request)),
				[FILE_CONTENT_OPERATIONS.readHex]: (request) =>
					this.readHex(this.hexRequest(request)),
				[FILE_CONTENT_OPERATIONS.readPreview]: (request) =>
					this.readPreview(this.pathRequest(request)),
				[FILE_CONTENT_OPERATIONS.textMetadata]: (request) =>
					this.textMetadata(this.pathRequest(request)),
				[FILE_CONTENT_OPERATIONS.textLines]: (request) =>
					this.textLines(this.textLinesRequest(request)),
			},
			commands: {},
		};
	}
			...
			typeof (
				...
				: (
						this.options.projects as Readonly<
							Record<string, FileContentProjectContext>
						>
					)[projectId];
		if (
			project === undefined ||
			project.projectId !== projectId ||
			!(project.content instanceof FileContentStreamService)
		)
			throw new FileContentError(
				'storage_unavailable',
				'file project is unavailable',
			);
	...
	private payload(
		...
		if (typeof value !== 'object' || value === null || Array.isArray(value))
			throw invalidRequest('file content payload must be an object');
	...
	): FileContentRequest & { readonly offset: number; readonly length: number } {
		const payload = this.payload(request);
		return {
			...this.pathRequest(request),
			offset: uintField(payload.offset, 'offset'),
			length: uintField(payload.length, 'length'),
		};
	}
	...
	private hexRequest(
		...
		const payload = this.payload(request);
		const bytesPerRow =
			payload.bytesPerRow === undefined
				? undefined
				: uintField(payload.bytesPerRow, 'bytesPerRow');
		return {
	...
	private textLinesRequest(
		...
		return {
			...this.pathRequest(request),
			startLine: uintField(payload.startLine, 'startLine'),
			lineCount: positiveUintField(payload.lineCount, 'lineCount', 512),
		};
	}
}

function serializeRangeMetadata({
	bytes: _bytes,
	...value
}: FileContentRange): JsonValue {
	return asJson({ ...value, bodyLength: _bytes.byteLength });
}
function serializeText(value: FileContentTextRange): JsonValue {
	return asJson({
		...value,
...
function serializeHex(value: FileContentHexRange): JsonValue {
	return asJson({
		...value,
		bytes: bytesToBase64(value.bytes),
		rows: value.rows,
...
function serializePreview(value: FileContentPreview): JsonValue {
	return asJson({ ...value, bytes: bytesToBase64(value.bytes) });
}
...
function uintField(value: unknown, name: string): number {
	if (!Number.isSafeInteger(value) || (value as number) < 0)
		throw invalidRequest(`${name} is invalid`);
	return value as number;
}
function positiveUintField(
	value: unknown,
	name: string,
	maximum: number,
): number {
	const parsed = uintField(value, name);
...
function invalidRequest(message: string): FileContentError {
	return new FileContentError('invalid_path', message);
}
