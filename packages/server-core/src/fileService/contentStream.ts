...
export class FileContentError extends Error {
	...
	constructor(code: FileContentErrorCode, message: string) {
		super(message);
...
export class FileContentStreamService {
	...
	constructor(
		...
		options: FileContentStreamOptions = {},
	) {
		if (storage.readRange === undefined && storage.readFile === undefined)
			throw new TypeError(
				'file content storage must provide readRange or readFile',
			);
		this.resolver = resolver;
		this.storage = storage;
		this.maxRangeBytes = positive(
		...
		this.maxTextBytes = positive(
			options.maxTextBytes ?? DEFAULT_MAX_TEXT_BYTES,
			'maxTextBytes',
		);
		this.maxHexRows = positive(
			options.maxHexRows ?? DEFAULT_MAX_HEX_ROWS,
			'maxHexRows',
		);
		this.maxConcurrentReads = positive(
			options.maxConcurrentReads ?? DEFAULT_MAX_CONCURRENT_READS,
			'maxConcurrentReads',
		);
		this.largeFileBytes = positive(
			options.largeFileBytes ?? DEFAULT_LARGE_FILE_BYTES,
			'largeFileBytes',
		);
		this.maxDecodedImagePixels = positive(
			options.maxDecodedImagePixels ?? DEFAULT_MAX_DECODED_IMAGE_PIXELS,
	...
	async capabilities(
		...
		signal?: AbortSignal,
	): Promise<FileContentCapabilities> {
		...
		const sample =
			size === 0
				...
				: await this.readCanonical(
						canonical,
						0,
						Math.min(size, this.maxRangeBytes),
						signal,
						size,
					);
		...
		return Object.freeze({
			...
			isLarge: size > this.largeFileBytes,
			canPreview:
				detected.kind === 'markdown' ||
				detected.kind === 'image' ||
				detected.kind === 'pdf',
			canText: detected.kind === 'text' || detected.kind === 'markdown',
			canHex: true,
			maxDecodedImagePixels: this.maxDecodedImagePixels,
		});
	}
	...
	async readRange(
		...
		length: number,
		signal?: AbortSignal,
	): Promise<FileContentRange> {
		const relativePath = normalizeRelative(requestedPath);
		const canonical = await this.resolveFile(relativePath);
		const stat = await this.storage.stat(canonical);
		const totalSize = safeSize(stat.size);
		const bytes = await this.readCanonical(
			canonical,
			offset,
			length,
			signal,
	...
	async readText(
		...
		length: number,
		signal?: AbortSignal,
	): Promise<FileContentTextRange> {
		if (
			!Number.isSafeInteger(length) ||
			length < 0 ||
			length > this.maxTextBytes
		)
			throw new FileContentError(
				'range_too_large',
				'text range exceeds the configured limit',
			);
		const range = await this.readRange(requestedPath, offset, length, signal);
		if (range.kind !== 'text' && range.kind !== 'markdown')
			throw new FileContentError(
				'unsupported_preview',
				'binary content is not available in text mode',
			);
		let text: string;
	...
	async readHex(
		...
		bytesPerRow = 16,
		signal?: AbortSignal,
	): Promise<FileContentHexRange> {
		if (
			!Number.isSafeInteger(bytesPerRow) ||
			bytesPerRow < 1 ||
			bytesPerRow > 64
		)
			throw new FileContentError('invalid_range', 'bytesPerRow is invalid');
		const range = await this.readRange(requestedPath, offset, length, signal);
		const rows: FileContentHexRow[] = [];
		for (
			let index = 0;
			index < range.bytes.byteLength && rows.length < this.maxHexRows;
			index += bytesPerRow
		) {
			const row = range.bytes.subarray(
				index,
				Math.min(index + bytesPerRow, range.bytes.byteLength),
			);
			rows.push(
				Object.freeze({
					offset: offset + index,
	...
	async readPreview(
		...
		if (
			...
			(capabilities.kind !== 'markdown' &&
				capabilities.kind !== 'image' &&
				capabilities.kind !== 'pdf')
		)
			throw new FileContentError(
				'unsupported_preview',
				'file has no preview source',
			);
		if (capabilities.size > this.maxPreviewBytes)
			throw new FileContentError(
				'preview_too_large',
				'preview exceeds the configured limit',
			);
		const relativePath = normalizeRelative(requestedPath);
		const canonical = await this.resolveFile(relativePath);
		const rangeBytes = await this.readCanonical(
			canonical,
			0,
			capabilities.size,
			signal,
		...
		const range: FileContentRange = {
			relativePath,
			kind: capabilities.kind,
			contentType: capabilities.contentType,
			offset: 0,
			requestedLength: capabilities.size,
			bytes: rangeBytes,
			totalSize: capabilities.size,
			truncated: rangeBytes.byteLength < capabilities.size,
		};
		return Object.freeze({
			...range,
			kind: capabilities.kind,
			decodedImagePixelLimit: this.maxDecodedImagePixels,
		});
	}

	private async resolveFile(relativePath: string): Promise<string> {
		if (relativePath.length === 0)
			throw new FileContentError('invalid_path', 'a file path is required');
		try {
			return await this.resolver.resolve(relativePath, { requireFile: true });
		} catch (error) {
			if (
				error instanceof Error &&
				'code' in error &&
				(error as { readonly code?: unknown }).code === 'not_file'
			)
				throw new FileContentError('not_file', 'path is not a file');
			throw error;
		}
	}
	...
	private async readCanonical(
		...
		totalSize: number,
		allowedLength = this.maxRangeBytes,
	): Promise<Uint8Array> {
		...
		if (this.activeReads >= this.maxConcurrentReads)
			throw new FileContentError(
				'concurrency_limit',
				'concurrent file reads are bounded',
			);
		this.activeReads += 1;
		try {
			...
			if (this.storage.readRange !== undefined)
				bytes = await this.storage.readRange(
					canonical,
					offset,
					boundedLength,
					signal,
				);
			else if (this.storage.readFile !== undefined)
				bytes = (await this.storage.readFile(canonical, signal)).slice(
					offset,
				...
				throw new FileContentError(
					'storage_unavailable',
					'file storage cannot read content',
				);
			throwIfAborted(signal);
			if (!(bytes instanceof Uint8Array))
				throw new FileContentError(
					'storage_unavailable',
					'file storage returned invalid bytes',
				);
...
function detectKind(
	relativePath: string,
	sample: Uint8Array,
): { readonly kind: FileContentKind; readonly contentType: string } {
	const extension = extensionOf(relativePath);
	if (startsWithAscii(sample, '%PDF-') || extension === 'pdf')
		return { kind: 'pdf', contentType: 'application/pdf' };
	if (isImage(sample, extension))
		return { kind: 'image', contentType: imageType(extension, sample) };
	if (
		extension === 'md' ||
		extension === 'markdown' ||
		extension === 'mdown' ||
		...
		return {
			kind: looksUtf8(sample) ? 'markdown' : 'binary',
...
function isImage(sample: Uint8Array, extension: string): boolean {
	return (
		startsWithBytes(sample, [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]) ||
		startsWithBytes(sample, [0xff, 0xd8, 0xff]) ||
		startsWithAscii(sample, 'GIF87a') ||
		startsWithAscii(sample, 'GIF89a') ||
		[
			'png',
			'apng',
			'jpg',
...
function imageType(extension: string, sample: Uint8Array): string {
	if (startsWithBytes(sample, [0x89, 0x50, 0x4e, 0x47])) return 'image/png';
	if (startsWithBytes(sample, [0xff, 0xd8, 0xff])) return 'image/jpeg';
	if (startsWithAscii(sample, 'GIF')) return 'image/gif';
	return (
		(
			{
				png: 'image/png',
...
function textType(extension: string): string {
	return (
		(
			{
				md: 'text/markdown',
...
function extensionOf(value: string): string {
	const name = value.slice(value.lastIndexOf('/') + 1).toLocaleLowerCase();
...
function looksBinary(bytes: Uint8Array): boolean {
	return bytes.some(
		(byte) => byte === 0 || byte < 7 || (byte > 14 && byte < 32 && byte !== 27),
	);
}
function normalizeRelative(value: string): string {
	if (
		...
		throw new FileContentError('invalid_path', 'file path is invalid');
	const parts = value.split('/');
	if (parts.some((part) => part.length === 0 || part === '.' || part === '..'))
		throw new FileContentError('invalid_path', 'file path is not canonical');
	return parts.join('/');
}
...
function validateRange(offset: number, length: number, max: number): void {
	if (
		...
		throw new FileContentError('invalid_range', 'file range is invalid');
	if (length > max)
		throw new FileContentError(
			'range_too_large',
			'file range exceeds the configured limit',
		);
}
