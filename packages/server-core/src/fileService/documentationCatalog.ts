...
export class DocumentationCatalog {
	...
	constructor(
		...
		this.maxEntries = bounded(
			options.maxEntries,
			DOCUMENTATION_CATALOG_LIMITS.maxEntries,
		);
		this.maxFiles = bounded(
			options.maxFiles,
			DOCUMENTATION_CATALOG_LIMITS.maxFiles,
		);
		this.maxDepth = bounded(
			options.maxDepth,
			DOCUMENTATION_CATALOG_LIMITS.maxDepth,
		);
	...
	async catalog(
		signalOrOptions?: AbortSignal | DocumentationCatalogScanOptions,
	): Promise<DocumentationCatalogResult> {
		...
		const visit = async (relative: string, depth: number): Promise<void> => {
			...
			try {
				directory = await this.resolver.resolve(relative || root, {
			...
			for (const entry of entries) {
				...
				documents.push(
					await this.metadata(canonical, child, extensionValue, options.signal),
				);
		...
		if (
			pageDocuments.length + pageFolders.length >
				DOCUMENTATION_CATALOG_LIMITS.maxPageRecords ||
			...
			while (
				...
				pageDocuments = pageDocuments.slice(
					0,
					Math.max(1, Math.floor(pageDocuments.length / 2)),
				);
				const kept = new Set<string>();
				for (const document of pageDocuments)
					for (const folder of parentFolders(document.relativePath))
						kept.add(folder);
				pageFolders = orderedFolders.filter((folder) =>
					kept.has(folder.relativePath),
				);
	...
	private async metadata(
		...
		const parsed = frontmatterTitle(
			text,
			bytes.byteLength === this.maxInspectionBytes,
		);
		return {
			kind: 'document',
			relativePath,
	...
	try {
		...
		if (
			document.warnings.some((warning) =>
				/alias|anchor|merge/iu.test(warning.message),
			)
		)
		...
		if (
			...
			DOCUMENTATION_CATALOG_LIMITS.maxTitleBytes
		) {
			return {
				diagnostic: 'Frontmatter title exceeds the display-title limit.',
			};
		}
		return { title };
	} catch {
...
function documentationExtension(
	path: string,
): DocumentationExtension | undefined {
...
function bounded(value: number | undefined, fallback: number): number {
	...
	if (!Number.isSafeInteger(result) || result < 1)
		throw new RangeError(
			'documentation catalog limit must be a positive integer',
		);
	return result;
}
...
function compareDocument(
	a: DocumentationDocument,
	b: DocumentationDocument,
): number {
	return (
...
function boundDiagnostic(value: string): string {
	...
	if (bytes.byteLength <= DOCUMENTATION_CATALOG_LIMITS.maxDiagnosticBytes)
		return value;
	return new TextDecoder().decode(
		bytes.slice(0, DOCUMENTATION_CATALOG_LIMITS.maxDiagnosticBytes),
	);
}
