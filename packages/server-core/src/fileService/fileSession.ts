import {
	type ExternalDiskChange,
	type FileMetadata,
	type FileMutationResult,
	type FileReadRange,
	FileServiceError,
	type FileSessionMetadata,
	type FileSessionOptions,
...
export class FileSession {
	...
	constructor(
		...
		if (
			...
			throw new FileServiceError(
				'invalid_path',
				'canonical file path is invalid',
			);
		if (typeof storage.atomicWrite !== 'function')
			throw new TypeError('file session storage must provide atomicWrite');
		this.canonicalPath = canonicalPath;
		this.storage = storage;
		this.maxRangeBytes = boundedLimit(
			options.maxRangeBytes ?? DEFAULT_MAX_RANGE_BYTES,
			'maxRangeBytes',
		);
		this.maxDraftBytes = boundedLimit(
			options.maxDraftBytes ?? DEFAULT_MAX_DRAFT_BYTES,
			'maxDraftBytes',
		);
		this.diskRevisionValue = boundedRevision(
			options.diskRevision ?? 1,
			'diskRevision',
		);
		...
		if (options.initialBytes !== undefined) {
			...
			if (initial.byteLength > this.maxDraftBytes)
				throw new FileServiceError(
					'draft_too_large',
					'initial file exceeds draft limit',
					{ max: this.maxDraftBytes },
				);
	...
	static async open(
		canonicalPath: string,
		storage: FileSessionStorage,
		options: FileSessionOpenOptions = {},
	): Promise<FileSession> {
		const metadata =
			options.initialMetadata ?? (await storage.stat?.(canonicalPath));
		...
		if (initialBytes === undefined && options.loadInitialBytes === true) {
			if (storage.readFile === undefined)
				throw new FileServiceError(
					'write_failed',
					'storage cannot load initial bytes',
				);
			initialBytes = await storage.readFile(canonicalPath);
		}
		return new FileSession(canonicalPath, storage, {
			...options,
			...(metadata === undefined ? {} : { initialMetadata: metadata }),
			...(initialBytes === undefined ? {} : { initialBytes }),
		});
	}

	get state(): FileSessionState {
		return this.snapshot();
	}
	get diskRevision(): number {
		return this.diskRevisionValue;
	}
	get draftRevision(): number {
		return this.draftRevisionValue;
	}
	get dirty(): boolean {
	...
	applyDraft(
		bytes: Uint8Array,
		options: ApplyDraftOptions | number = {},
	): FileMutationResult<FileSessionState> {
		const checked = this.ensureOpen();
		if (!checked.ok) return checked;
		if (!(bytes instanceof Uint8Array))
			throw new TypeError('draft must be a Uint8Array');
		const expected =
			typeof options === 'number' ? options : options.expectedDraftRevision;
		const revision = expected ?? this.draftRevisionValue;
		if (!Number.isSafeInteger(revision) || revision < 0)
			throw new RangeError(
				'expectedDraftRevision must be a non-negative safe integer',
			);
		if (revision !== this.draftRevisionValue)
			return this.revisionFailure(
				'draft revision is stale',
				revision,
				this.draftRevisionValue,
			);
		if (bytes.byteLength > this.maxDraftBytes)
			return {
				ok: false,
				error: new FileServiceError(
					'draft_too_large',
					'draft exceeds the configured limit',
					{ max: this.maxDraftBytes },
				),
			};
	...
	async applyDraftPatch(
		bytes: Uint8Array,
		options: ApplyDraftPatchOptions | number,
	): Promise<FileMutationResult<FileSessionState>> {
		const checked = this.ensureOpen();
		if (!checked.ok) return checked;
		if (!(bytes instanceof Uint8Array))
			throw new TypeError('draft patch must be a Uint8Array');
		const offset = typeof options === 'number' ? options : options.offset;
		const expected =
			typeof options === 'number' ? undefined : options.expectedDraftRevision;
		if (!Number.isSafeInteger(offset) || offset < 0)
			throw new FileServiceError(
				'invalid_range',
				'draft patch offset is invalid',
			);
		const current =
			this.draftBytes === undefined
				? await this.readWholeForDraft()
				: copyBytes(this.draftBytes);
		if (offset + bytes.byteLength > this.maxDraftBytes)
			return {
				ok: false,
				error: new FileServiceError(
					'draft_too_large',
		...
		if (nextLength > this.maxDraftBytes)
			return {
				ok: false,
				error: new FileServiceError(
					'draft_too_large',
	...
	async readRange(
		...
		if (source !== undefined) {
			...
			bytes = source.slice(
				offset,
				Math.min(source.byteLength, offset + length),
			);
		} else {
			if (
				this.storage.readRange === undefined &&
				this.storage.readFile === undefined
			)
				throw new Error('storage cannot read file content');
			bytes =
				this.storage.readRange !== undefined
					...
					: (await this.storage.readFile!(this.canonicalPath, signal)).slice(
							offset,
							offset + length,
						);
	...
	observeDiskChange(change: ExternalDiskChange = {}): FileSessionState {
		...
		if (
			...
			nextRevision <= this.diskRevisionValue
		)
			throw new FileServiceError(
				'revision_conflict',
				'disk revision is stale',
				{ expected: this.diskRevisionValue + 1, actual: nextRevision },
			);
	...
	async reload(
		...
		const checked = this.ensureOpen();
		if (!checked.ok) return checked;
		if (this.dirtyValue && options.confirm !== true)
			return {
				ok: false,
				error: new FileServiceError(
					'confirmation_required',
					'reloading would discard local edits',
				),
			};
		const revisionFailure = this.checkExpected(
			options.expectedDiskRevision,
			this.diskRevisionValue,
			'disk revision is stale',
		);
		if (revisionFailure !== undefined) return revisionFailure;
		const draftFailure = this.checkExpected(
		...
		if (this.storage.stat !== undefined)
			metadata = await this.storage.stat(this.canonicalPath);
		this.diskBytes = undefined;
		this.draftBytes = undefined;
	...
	async save(
		options: SaveOptions | number = {},
	): Promise<FileMutationResult<FileSessionState>> {
		const checked = this.ensureOpen();
		if (!checked.ok) return checked;
		const normalized: SaveOptions =
			typeof options === 'number' ? { expectedDiskRevision: options } : options;
		const diskFailure = this.checkExpected(
			normalized.expectedDiskRevision,
			this.diskRevisionValue,
			'disk revision save precondition failed',
		);
		if (diskFailure !== undefined)
			return {
				...
				error: new FileServiceError(
					'save_precondition',
					diskFailure.error.message,
					diskFailure.error.details,
				),
			};
		const draftFailure = this.checkExpected(
			normalized.expectedDraftRevision,
			this.draftRevisionValue,
			'draft revision save precondition failed',
		);
		if (draftFailure !== undefined)
			return {
				...
				error: new FileServiceError(
					'save_precondition',
					draftFailure.error.message,
					draftFailure.error.details,
				),
			};
		if (!this.dirtyValue || this.draftBytes === undefined)
			return {
				ok: false,
				error: new FileServiceError('not_dirty', 'file has no local edits'),
			};
		if (this.conflictValue)
			return {
				ok: false,
				error: new FileServiceError(
					'save_precondition',
					'file has an unresolved external conflict',
				),
			};
		...
		try {
			...
			const message =
				error instanceof Error ? error.message : 'atomic save failed';
			return {
				ok: false,
				error: new FileServiceError('write_failed', message),
			};
		}
	...
	close(): FileMutationResult<FileSessionState> {
		if (this.watchStateValue === 'closed')
			return { ok: true, value: this.snapshot() };
		if (this.dirtyValue)
			return {
				ok: false,
				error: new FileServiceError(
					'save_precondition',
					'dirty file requires explicit save, reload, or keep-local before close',
				),
			};
	...
	private ensureOpen(): FileMutationResult<FileSessionState> {
		if (this.watchStateValue === 'closed')
			return {
				ok: false,
				error: new FileServiceError('session_closed', 'file session is closed'),
			};
		return { ok: true, value: this.snapshot() };
	}
	...
	private checkExpected(
		...
		actual: number,
		message: string,
	): FileMutationFailureOrUndefined {
		...
		if (expected !== actual)
			return this.revisionFailure(message, expected, actual);
		return undefined;
	}

	private revisionFailure(
		message: string,
		expected: number,
		actual: number,
	): FileMutationFailure {
		return {
			ok: false,
			error: new FileServiceError('revision_conflict', message, {
				expected,
				actual,
			}),
		};
	...
	private setDiskMetadata(metadata: FileMetadata | undefined): void {
		...
		if (!Number.isSafeInteger(metadata.size) || metadata.size < 0)
			throw new TypeError('file metadata size is invalid');
		if (
			metadata.mtimeMs !== undefined &&
			(!Number.isFinite(metadata.mtimeMs) || metadata.mtimeMs < 0)
		)
			throw new TypeError('file metadata mtime is invalid');
		if (
			metadata.mode !== undefined &&
			(!Number.isSafeInteger(metadata.mode) || metadata.mode < 0)
		)
			throw new TypeError('file metadata mode is invalid');
		if (
			metadata.identity !== undefined &&
			(typeof metadata.identity !== 'string' ||
				metadata.identity.length > 256 ||
				metadata.identity.includes('\0'))
		)
			throw new TypeError('file metadata identity is invalid');
	...
	private async readWholeForDraft(): Promise<Uint8Array> {
		...
		if (this.storage.readFile !== undefined) {
			...
			if (bytes.byteLength > this.maxDraftBytes)
				throw new FileServiceError(
					'draft_too_large',
					'file exceeds the draft limit',
					{ max: this.maxDraftBytes },
				);
			return copyBytes(bytes);
		}
		if (this.storage.readRange === undefined)
			throw new Error('storage cannot read complete file for draft patch');
		const size = this.sizeValue;
		if (size === undefined || size > this.maxDraftBytes)
			throw new FileServiceError(
				'draft_too_large',
				'file exceeds the draft limit',
				{ max: this.maxDraftBytes },
...
type FileMutationFailure = {
	readonly ok: false;
	readonly error: FileServiceError;
};
...
export class FileSessionRegistry {
	...
	get(canonicalPath: string): FileSession | undefined {
		return this.sessions.get(canonicalPath);
	}
	values(): readonly FileSession[] {
	...
	release(
		...
		if (session === undefined)
			return {
				ok: false,
				error: new FileServiceError('path_missing', 'file session not found', {
		...
		if (session.dirty && confirmDirty) {
			...
			if (reloaded instanceof Promise)
				return {
					ok: false,
					error: new FileServiceError(
						'save_precondition',
						'asynchronous close requires releaseAsync',
					),
				};
		}
	...
	disconnect(): readonly FileSession[] {
		return this.values();
	}
}
...
function validateRange(offset: number, length: number, max: number): void {
	if (
		...
		throw new FileServiceError('invalid_range', 'byte range is invalid');
	if (length > max)
		throw new FileServiceError(
			'range_too_large',
			'byte range exceeds the configured limit',
			{ requested: String(length), max },
		);
}
