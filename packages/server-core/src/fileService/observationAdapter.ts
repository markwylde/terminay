...
export class ServerFileObservationAdapter {
	...
	constructor(private readonly options: FileObservationAdapterOptions) {
		...
		if (
			typeof options.host?.watch !== 'function' ||
			typeof options.host?.calculateFolderSize !== 'function'
		)
			throw new TypeError('file observation host is invalid');
		this.maxFolderSizeJobs = positive(
			options.maxFolderSizeJobs ?? 128,
			'maxFolderSizeJobs',
		);
		this.watches = new FileWatchRegistry({
			serverId: options.serverId,
			maxSubscriptions: options.maxWatches ?? 1024,
		});
		this.operations = {
			queries: {
				[FILE_OBSERVATION_OPERATIONS.watchRead]: (request) =>
					this.read(request),
			},
			commands: {
				[FILE_OBSERVATION_OPERATIONS.watchStart]: (request) =>
					this.startWatch(request),
				[FILE_OBSERVATION_OPERATIONS.watchStop]: (request) =>
					this.stopWatch(request),
				[FILE_OBSERVATION_OPERATIONS.folderSizeStart]: (request) =>
					this.startFolderSize(request),
				[FILE_OBSERVATION_OPERATIONS.folderSizeCancel]: (request) =>
					this.cancelFolderSize(request),
			},
			policies: {
				...
				publish: (event) => {
					...
					for (const matched of result.subscriptions) {
						this.options.eventJournal.append(
							FILE_OBSERVATION_OPERATIONS.watchEvent,
							{
								...
								...(event.relatedResource === undefined
									? {}
	...
	private async read(request: QueryRequest): Promise<JsonValue> {
		...
		return (await this.watches.read(id, {
	...
	private stopWatch(request: CommandRequest): JsonValue {
		...
		const state = this.requireWatch(id, request.context.clientId);
		this.project(request, state.projectId);
		if (removeConsumer(state, request.context.connectionId) === 0)
	...
	private startFolderSize(request: CommandRequest): JsonValue {
		...
		const emit = (
			phase: 'progress' | 'completed' | 'cancelled' | 'failed',
			value?: {
				readonly bytes: number;
				readonly files: number;
				readonly directories: number;
			...
			this.options.eventJournal.append(
				FILE_OBSERVATION_OPERATIONS.folderSizeEvent,
				{
					jobId,
					clientId: request.context.clientId,
					projectId,
		...
		void Promise.resolve()
			.then(() =>
				this.options.host.calculateFolderSize({
					...
					signal: controller.signal,
					progress: (value) => {
						if (!controller.signal.aborted) emit('progress', value);
					},
				}),
			)
			.then((value) => {
				if (!controller.signal.aborted) emit('completed', value);
			})
	...
	private cancelFolderSize(request: CommandRequest): JsonValue {
		...
		if (job === undefined || job.clientId !== request.context.clientId)
			return null;
	...
	private project(
		...
		if (!scopeAllows(request.context.authScope, 'read'))
			throw new Error('file observation requires read scope');
		...
		const claim =
			...
			!Array.isArray(request.context.claims)
				? (request.context.claims as Record<string, unknown>).projectId
				: undefined;
		if (typeof claim === 'string' && claim !== projectId)
			throw new Error(
				'file observation is outside authenticated project scope',
			);
		return projectId;
	}
	private requireWatch(id: string, clientId: string): WatchState {
		...
		if (state === undefined || state.clientId !== clientId)
			throw new Error('file watch subscription is unavailable');
		return state;
	}
	private closeWatch(id: string, state: WatchState): void {
		state.controller.abort();
		this.watches.unsubscribe(id);
		this.watchStates.delete(id);
	}
	private closeJob(id: string, job: SizeJob): void {
...
function addConsumer(state: WatchState, connectionId: string): void {
	state.consumersByConnection.set(
		connectionId,
		(state.consumersByConnection.get(connectionId) ?? 0) + 1,
	);
}
...
function objectPayload(value: unknown): Record<string, unknown> {
	if (typeof value !== 'object' || value === null || Array.isArray(value))
		throw new TypeError('file observation payload is invalid');
...
export function createFileObservationEventProjector(
	...
	if (
		event.event !== FILE_OBSERVATION_OPERATIONS.watchEvent &&
		event.event !== FILE_OBSERVATION_OPERATIONS.folderSizeEvent
	)
		return event;
	if (
		client === undefined ||
	...
	const claim =
		typeof client.claims === 'object' &&
		client.claims !== null &&
		!Array.isArray(client.claims)
			? (client.claims as Record<string, unknown>).projectId
			: undefined;
	return typeof claim === 'string' && payload.projectId !== claim
		? undefined
		: event;
}

function relativeResource(value: unknown): string {
	if (value === '') return '';
	const result = text(value, 'resource', 4096);
	if (
		result.startsWith('/') ||
		result.includes('\\') ||
		result
		...
		throw new TypeError('resource must be project-relative');
	return result;
}
...
function counters(value: {
	...
	for (const number of [value.bytes, value.files, value.directories])
		if (!Number.isSafeInteger(number) || number < 0)
			throw new RangeError('folder-size counters are invalid');
	return {
		bytes: value.bytes,
		files: value.files,
