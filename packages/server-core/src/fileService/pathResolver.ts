...
export interface CanonicalProjectPathResolverOptions
	extends CanonicalProjectPathOptions {
...
export class CanonicalProjectPathResolver {
	...
	constructor(
		...
		if (
			...
			this.projectRoot.length === 0 ||
			this.projectRoot.includes('\0')
		) {
			throw new FileServiceError('invalid_path', 'project root is invalid');
		}
		if (
			this.adapter === undefined ||
			typeof this.adapter.realpath !== 'function' ||
			typeof this.adapter.stat !== 'function'
		) {
			throw new TypeError(
				'canonical path adapter must provide realpath and stat',
			);
		}
		if (
			this.options.maxPathLength !== undefined &&
			(!Number.isSafeInteger(this.options.maxPathLength) ||
				this.options.maxPathLength <= 0)
		) {
	...
	async resolve(
		...
		const maxPathLength =
			options.maxPathLength ?? this.options.maxPathLength ?? 4096;
		if (
			typeof requestedPath !== 'string' ||
			requestedPath.length === 0 ||
			requestedPath.length > maxPathLength ||
		...
		if (
			...
			if (options.requireFile ?? false)
				throw new FileServiceError('not_file', 'path is not a file', {
					canonical: root,
				});
			return root;
		}
		...
		if (!isWithin(root, canonical))
			throw new FileServiceError(
				...
				{ requested: requestedPath, canonical },
			);
		...
		try {
			...
			if (options.allowMissing ?? this.options.allowMissing ?? false)
				return canonical;
			throw new FileServiceError('path_missing', 'path does not exist', {
				requested: requestedPath,
				canonical,
			});
		}
		if ((options.requireFile ?? false) && stat.isFile === false)
			throw new FileServiceError('not_file', 'path is not a file', {
				canonical,
			});
		if ((options.requireDirectory ?? false) && stat.isDirectory === false)
			throw new FileServiceError('not_directory', 'path is not a directory', {
				canonical,
			});
		return canonical;
	}
	...
	private async canonicalExisting(
		...
		try {
			...
			if (!missingAllowed || !isMissingError(error))
				throw new FileServiceError('path_missing', 'path does not exist', {
					requested: path,
				});
			...
			if (this.adapter.lstat !== undefined) {
				try {
					...
					if (leaf.isSymbolicLink === true)
						throw new FileServiceError(
							'path_escape',
							'dangling symlink cannot be authorized',
							{ requested: path },
						);
				} catch (leafError) {
				...
				if (this.adapter.lstat !== undefined) {
					try {
						...
						if (stat.isSymbolicLink === true)
							throw new FileServiceError(
								'path_escape',
								'symlink target cannot be authorized',
								{ canonical },
							);
					} catch (error) {
						if (error instanceof FileServiceError) throw error;
...
export async function resolveCanonicalProjectPath(
	...
	return new CanonicalProjectPathResolver(
		projectRoot,
		adapter,
		options,
	).resolve(requestedPath, options);
}
...
function isWithin(root: string, candidate: string): boolean {
	...
	return (
		...
		comparisonCandidate.startsWith(
			`${comparisonRoot}${separatorFor(normalizedRoot)}`,
		)
	);
}
...
function isMissingError(error: unknown): boolean {
	if (typeof error !== 'object' || error === null)
		return (
			...
			/not found|no such file|enoent|enotdir/iu.test(error.message)
		);
	...
	return (
		...
		(typeof candidate.message === 'string' &&
			/not found|no such file|enoent|enotdir/iu.test(candidate.message))
	);
}
...
function normalizePath(path: string): string {
	...
	for (const part of parts) {
		...
		if (part === '..' && result.length > 0 && result.at(-1) !== '..') {
			result.pop();
			continue;
		}
	...
	const prefix =
		drive.length > 0 ? `${drive}${rooted ? '/' : ''}` : rooted ? '/' : '';
