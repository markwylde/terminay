...
export async function aggregateMarkdownTasks(
	...
	const maxTaskLabelLength = positive(
		options.maxTaskLabelLength ?? DEFAULT_MAX_TASK_LABEL_LENGTH,
		'maxTaskLabelLength',
	);
	const ignored = (
		options.ignoredDirectories ?? DEFAULT_IGNORED_DIRECTORIES
	).map(validIgnorePattern);
	...
	if (rootStat.isFile === true) {
		if (isMarkdownPath(root)) {
			...
			const parsed = await readMarkdownFile(
				...
				canonical,
				storage,
				Math.min(maxFileBytes, maxBytes),
				maxTaskLabelLength,
				signal,
			);
		...
		for (const raw of orderedEntries) {
			...
			try {
				...
				if (
					...
					(error.code === 'path_escape' || error.code === 'path_missing')
				)
					continue;
				throw error;
			}
			...
			const file = makeFile(
				...
				stat.size,
				stat.mtimeMs,
				parsed.bytesRead < safeSize(stat.size),
			);
			if (file.tasks.length > 0) files.push(file);
			discoveredTasks += file.tasks.length;
...
async function readMarkdownFile(
	...
	if (storage.readRange === undefined)
		throw new FileServiceError(
			'write_failed',
			'markdown aggregation requires ranged file reads',
		);
	...
	if (!(bytes instanceof Uint8Array))
		throw new FileServiceError(
			'write_failed',
			'markdown range read returned invalid bytes',
		);
	const bounded =
		bytes.byteLength > requested ? bytes.slice(0, requested) : bytes;
	let text: string;
	...
	const parsed = invalidEncoding
		...
		: parseMarkdown(relativePath, text, maxTaskLabelLength);
	return {
		bytesRead: bounded.byteLength,
		parsed: { ...parsed, invalidEncoding },
		truncated:
			parsed.truncated ||
			invalidEncoding ||
			bounded.byteLength < safeSize(stat.size),
	};
}
...
function parseMarkdown(
	...
	const root: MutableSection = {
		id: `${relativePath}:section-root`,
		title: null,
		level: 0,
		tasks: [],
		children: [],
	...
	for (let index = 0; index < lines.length; index += 1) {
		...
		while (indentStack.length > 0 && (indentStack.at(-1) ?? 0) >= width)
			indentStack.pop();
		const sectionPath = sectionStack
			.slice(1)
			.flatMap((section) => (section.title === null ? [] : [section.title]));
		const task: MarkdownTaskItem = Object.freeze({
			id: `${relativePath}:task-${index}`,
			relativePath,
			lineNumber: index + 1,
			label,
		...
		if (
			label.length === maxTaskLabelLength &&
			rawLabel.trim().length > maxTaskLabelLength
		)
			truncated = true;
	}
...
function freezeSection(section: MutableSection): MarkdownTaskSection {
	return Object.freeze({
		...
		title: section.title,
		level: section.level,
		tasks: Object.freeze([...section.tasks]),
		children: Object.freeze(section.children.map(freezeSection)),
	});
}

function flattenSections(
	sections: readonly MarkdownTaskSection[],
	output: MarkdownTaskItem[] = [],
): MarkdownTaskItem[] {
	for (const section of sections) {
		output.push(...section.tasks);
...
function makeFile(
	...
	return Object.freeze({
		relativePath,
		size: safeSize(size),
		...(finite(mtimeMs) ? { mtimeMs } : {}),
		sections: parsed.parsed.sections,
		tasks,
		stats: statsForTasks(tasks),
		truncated: truncated || parsed.truncated,
		invalidEncoding: parsed.parsed.invalidEncoding === true,
	});
}

function limitFilesToTasks(
	files: readonly MarkdownTaskFile[],
	maxTasks: number,
): {
	readonly files: readonly MarkdownTaskFile[];
	readonly truncated: boolean;
...
): readonly MarkdownTaskSection[] {
	const trimmed: MarkdownTaskSection[] = [];
	for (const section of sections) {
		const tasks = Object.freeze(
			section.tasks.filter((task) => taskIds.has(task.id)),
		);
...
function buildDirectoryTree(
	...
	const rootNode: MutableDirectory = {
	...
	for (const file of files) {
		const relative =
			root === '.'
				? file.relativePath
				: file.relativePath.slice(root.length + 1);
		const parts = relative.split('/');
		let node = rootNode;
		for (let index = 0; index < parts.length - 1; index += 1) {
			const part = parts[index] ?? '';
			const path =
				node.relativePath === '.' ? part : `${node.relativePath}/${part}`;
			const existing = node.directories.get(part);
			const child = existing ?? {
				name: part,
				relativePath: path,
				directories: new Map<string, MutableDirectory>(),
				files: [],
			};
...
function statsForTasks(tasks: readonly MarkdownTaskItem[]): MarkdownTaskStats {
	const completed = tasks.reduce(
		(count, task) => count + (task.checked ? 1 : 0),
		0,
	);
	return Object.freeze({
		total: tasks.length,
...
function directStats(tasks: readonly MarkdownTaskItem[]): MarkdownTaskStats {
	return statsForTasks(tasks);
}
function statsForStats(
	direct: MarkdownTaskStats,
	children: readonly MarkdownTaskStats[],
): MarkdownTaskStats {
	const total =
		direct.total + children.reduce((sum, stats) => sum + stats.total, 0);
	const completed =
		direct.completed +
		children.reduce((sum, stats) => sum + stats.completed, 0);
