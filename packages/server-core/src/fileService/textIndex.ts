...
export class ServerTextIndex {
	...
	constructor(
		private readonly content: FileContentStreamService,
		options: ServerTextIndexOptions = {},
	) {
		this.chunkBytes = positive(options.chunkBytes ?? 256 * 1024, 'chunkBytes');
		this.maxBytesPerRequest = positive(
			options.maxBytesPerRequest ?? 1024 * 1024,
			'maxBytesPerRequest',
		);
		this.maxWindowBytes = positive(
			options.maxWindowBytes ?? 1024 * 1024,
			'maxWindowBytes',
		);
	}
	...
	async lines(
		...
		lineCount: number,
		signal?: AbortSignal,
	): Promise<ServerTextWindow> {
		if (!Number.isSafeInteger(startLine) || startLine < 0)
			throw new RangeError('startLine must be a non-negative safe integer');
		if (!Number.isSafeInteger(lineCount) || lineCount < 1 || lineCount > 512)
		...
		const completeEnd = indexComplete
			? availableLineCount
			: Math.max(0, availableLineCount - 1);
		...
		if (startLine < endLine) {
			...
			while (
				boundedEndLine > startLine &&
				boundary(boundedEndLine) - start > this.maxWindowBytes
			)
				boundedEndLine -= 1;
			if (boundedEndLine === startLine)
				throw new FileContentError(
					'range_too_large',
					'text line exceeds the bounded window limit',
				);
			const end = boundary(boundedEndLine);
			const range = await this.content.readRange(
				path,
				start,
				end - start,
				signal,
			...
			for (
				...
				const contentLength =
					byteLength - (hasNewline ? 1 : 0) - (hasCarriageReturn ? 1 : 0);
				let text: string;
				try {
					text = new TextDecoder('utf-8', {
						fatal: true,
						ignoreBOM: true,
					}).decode(bytes.subarray(0, contentLength));
				} catch {
					throw new FileContentError(
						'invalid_encoding',
						`line ${lineNumber + 1} contains invalid UTF-8`,
					);
				}
				lines.push(
					Object.freeze({
		...
		return Object.freeze({
			...
			startLine,
			windowComplete:
				deliveredEnd >= requestedEnd ||
				(indexComplete && deliveredEnd >= availableLineCount),
		});
	}
	...
	private async advance(
		path: string,
		signal?: AbortSignal,
	): Promise<{ canonical: string; entry: IndexEntry }> {
		const canonical = await this.content.resolver.resolve(path, {
			requireFile: true,
		});
		...
		while (entry.indexedByteLength < size && budget > 0) {
			const length = Math.min(
				this.chunkBytes,
				budget,
				size - entry.indexedByteLength,
			);
			const range = await this.content.readRange(
				path,
				entry.indexedByteLength,
				length,
				signal,
			);
	...
	private metadataResult(
		...
		return Object.freeze({
			indexedByteLength: entry.indexedByteLength,
			ino: stableInode(canonical),
			isComplete: entry.indexedByteLength >= entry.size,
			lineCount: entry.lineCount,
...
function safeSize(value: number | undefined): number {
	if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0)
		throw new FileContentError('storage_unavailable', 'file size is invalid');
	return value;
}
...
function stableInode(path: string): number {
