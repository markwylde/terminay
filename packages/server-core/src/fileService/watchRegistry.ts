 ...
 * watch event. */
export type FileWatchEventKind =
	| 'created'
	| 'changed'
	| 'deleted'
	| 'renamed'
...
const EVENT_KINDS: readonly FileWatchEventKind[] = [
	'created',
	'changed',
	'deleted',
	'renamed',
...
export class FileWatchRegistry {
	...
	constructor(options: FileWatchRegistryOptions) {
		...
		this.maxSubscriptions = positive(
			options.maxSubscriptions ?? DEFAULT_MAX_SUBSCRIPTIONS,
			'maxSubscriptions',
		);
		this.maxQueueEvents = positive(
			options.maxQueueEvents ?? DEFAULT_MAX_QUEUE_EVENTS,
			'maxQueueEvents',
		);
		this.maxBatchEvents = positive(
			options.maxBatchEvents ?? DEFAULT_MAX_BATCH_EVENTS,
			'maxBatchEvents',
		);
		this.maxResourceLength = positive(
			options.maxResourceLength ?? DEFAULT_MAX_RESOURCE_LENGTH,
			'maxResourceLength',
		);
	...
	subscribe(options: FileWatchSubscriptionOptions): FileWatchSubscription {
		...
		if (
			...
			afterSequence < 0 ||
			afterSequence > this.sequenceValue
		)
		...
		if (this.subscriptions.size >= this.maxSubscriptions)
			throw new Error('file watch subscription limit reached');

		if (this.subscriptionSequence >= Number.MAX_SAFE_INTEGER)
			throw new Error('file watch subscription sequence exhausted');
	...
	unsubscribe(subscriptionId: string): boolean {
		...
		this.subscriptionsByKey.delete(
			subscriptionKey(
				state.key.serverId,
				state.key.projectId,
				state.key.resource,
	...
	publish(input: FileWatchEventInput): FileWatchPublishResult {
		...
		const relatedResource =
			input.relatedResource === undefined
				? undefined
				: validResource(input.relatedResource, this.maxResourceLength);
		const revision = input.revision;
		if (
			revision !== undefined &&
			(!Number.isSafeInteger(revision) || revision < 0)
		)
			throw new RangeError('file watch revision is invalid');
		...
		if (this.sequenceValue >= Number.MAX_SAFE_INTEGER)
			throw new Error('file watch sequence exhausted');
		...
		while (this.eventHistory.length > this.maxQueueEvents * 2)
	...
	async read(
		...
		if (
			...
			limit <= 0 ||
			limit > this.maxBatchEvents
		)
			throw new RangeError('watch batch limit is invalid');
		const events = state.queue.splice(0, limit);
		if (events.length > 0)
			state.cursor = events[events.length - 1]?.sequence ?? state.cursor;
		const resyncRequired = state.resyncRequired;
		if (resyncRequired && events.some((event) => event.kind === 'resync'))
	...
	pending(subscriptionId: string): number {
		return this.requireSubscription(subscriptionId).queue.length;
	}
	...
	private replay(
		...
		key: FileWatchKey,
	): { readonly events: FileWatchEvent[]; readonly resyncRequired: boolean } {
		if (afterSequence === this.sequenceValue || this.eventHistory.length === 0)
			return { events: [], resyncRequired: false };
		const oldest = this.eventHistory[0]?.sequence ?? this.sequenceValue;
		if (afterSequence < oldest - 1)
			return {
				events: [resyncEvent(this.serverId, key, this.sequenceValue)],
				resyncRequired: true,
			};
	...
	private requireSubscription(subscriptionId: string): SubscriptionState {
		const state = this.subscriptions.get(subscriptionId);
		if (state === undefined)
			throw new Error('file watch subscription is unknown');
		return state;
	}
...
function resourceMatches(
	...
	return (
		subscriptionResource.length === 0 ||
		subscriptionResource === changedResource ||
		changedResource.startsWith(`${subscriptionResource}/`)
	);
}
...
function validResource(value: string, maxLength: number): string {
	if (
		...
		throw new TypeError('watch resource must be project-relative');
	if (value === '' || value === '.') return '';
	const parts = value.split('/');
	if (parts.some((part) => part.length === 0 || part === '.' || part === '..'))
		throw new TypeError('watch resource is not canonical');
	return value;
}
...
function normalizeMetadata(
	...
	if (!Number.isSafeInteger(metadata.size) || metadata.size < 0)
		throw new RangeError('watch metadata size is invalid');
	if (
		metadata.mtimeMs !== undefined &&
		(!Number.isFinite(metadata.mtimeMs) || metadata.mtimeMs < 0)
	)
		throw new RangeError('watch metadata mtime is invalid');
	if (
		metadata.mode !== undefined &&
		(!Number.isSafeInteger(metadata.mode) || metadata.mode < 0)
	)
		throw new RangeError('watch metadata mode is invalid');
	if (
		...
		throw new TypeError('watch metadata identity is invalid');
	return Object.freeze({
		size: metadata.size,
