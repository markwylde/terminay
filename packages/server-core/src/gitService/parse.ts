...
export function parseStatus(output: string, maxEntries: number): ParsedStatus {
	...
	const branch = parseBranchHeader(
		branchField.startsWith('## ') ? branchField.slice(3) : branchField,
	);
	...
	for (let index = 0; index < fields.length; index += 1) {
		...
		if (
			...
			if (nextPath !== undefined && nextPath.length > 0) {
				...
				entries.push(
					makeStatusEntry(firstPath, nextPath, indexStatus, worktreeStatus),
				);
			} else {
				entries.push(
					makeStatusEntry(firstPath, null, indexStatus, worktreeStatus),
				);
			}
		} else {
			entries.push(
...
export function parseBranchHeader(header: string): GitBranchStatus {
	...
	if (
		/^HEAD \(no branch\)$/u.test(trimmed) ||
		/^HEAD \(detached/u.test(trimmed)
	) {
		detached = true;
	} else {
		...
		const branchName =
			separator < 0 ? branchAndRemote : branchAndRemote.slice(0, separator);
		const remoteName =
			separator < 0 ? null : branchAndRemote.slice(separator + 3);
		if (noCommit !== null) {
			...
			ahead =
				aheadMatch === null ? null : Number.parseInt(aheadMatch[1] ?? '', 10);
			behind =
				behindMatch === null ? null : Number.parseInt(behindMatch[1] ?? '', 10);
...
export function parseDiff(
	patch: string,
	limits: { maxHunks: number; maxLines: number; maxLineBytes: number },
): ParsedDiff {
	...
	for (const line of patch.split(/\r?\n/u)) {
		if (line.startsWith('diff --git ')) {
			...
			currentFile = {
				path: names.path,
				previousPath: names.previousPath,
				additions: 0,
				deletions: 0,
				binary: false,
			};
...
function parseDiffNames(value: string): {
	path: string;
	previousPath: string | null;
} {
	const separator = value.indexOf(' b/');
	if (separator < 0)
		return { path: value.replace(/^b\//u, ''), previousPath: null };
