...
export function boundGitQueryResult(
	...
	if (Array.isArray(result.worktrees))
		return boundWorktreeList(result, limits.maxHeaderBytes);
	if (Array.isArray(result.entries))
		return boundStatusEntries(result, limits.maxHeaderBytes);
	return result;
}
...
function boundWorktreeList(
	...
	const cloned = worktrees.map((worktree) => {
		if (!isRecord(worktree) || !Array.isArray(worktree.entries))
			return worktree;
	...
	if (!gitQueryResultFits(candidate, maxHeaderBytes)) {
		const kept = cloned.map((worktree) =>
			isRecord(worktree) ? { ...worktree, entries: [] } : worktree,
		);
		while (kept.length > 1) {
			kept.pop();
...
function boundStatusEntries(
	...
	if (!gitQueryResultFits(withCap(0, true), maxHeaderBytes))
		return withCap(0, true);
	let lo = 0;
	let hi = entries.length;
	while (lo < hi) {
		const mid = Math.ceil((lo + hi) / 2);
		if (gitQueryResultFits(withCap(mid, mid < entries.length), maxHeaderBytes))
			lo = mid;
		else hi = mid - 1;
	}
