...
export class GitQuickPushService {
	...
	async propose(
		request: GitQuickPushProposalRequest,
	): Promise<GitQuickPushProposal> {
		validateProposalRequest(request);
		const context = this.boundContext(
			await this.snapshot(request, request.signal),
		);
	...
	async approve(
	...
	): Promise<GitQuickPushApprovalResult> {
		const stored = this.proposals.get(request.proposalId);
		if (stored === undefined)
			throw new GitServiceError(
				'proposal-not-found',
				'Quick Push proposal was not found',
			);
		if (stored.consumed)
			throw new GitServiceError(
				'proposal-replayed',
				'Quick Push approval has already been consumed',
			);
		if (this.options.now() >= stored.proposal.expiresAt) {
			stored.consumed = true;
			throw new GitServiceError(
				'proposal-stale',
				'Quick Push proposal has expired',
			);
		}
		if (
			request.actionDigest !== stored.proposal.actionDigest ||
			!sameRevision(request.revision, stored.proposal.revision)
		) {
			throw new GitServiceError(
				'proposal-stale',
				'Quick Push approval does not match the reviewed proposal',
			);
		}
		...
		if (!sameRevision(makeRevision(context), stored.proposal.revision)) {
			throw new GitServiceError(
				'proposal-stale',
				'repository state changed after Quick Push review',
			);
		}
		...
		for (let index = 0; index < stored.proposal.actions.length; index += 1) {
			...
			if (!sameRevision(current, baseline)) {
				return {
					...
					error: {
						code: 'proposal-stale',
						message:
							'repository state changed before the next Quick Push action',
			...
			const actionResult: GitQuickPushActionResult = {
				...
				...(execution.detail === undefined
					? {}
					: {
							detail: redactQuickPushText(
								truncateUtf8(execution.detail, 2048).value,
							),
						}),
			};
			...
			if (!execution.applied) {
				return {
					...
					error: {
						code: 'action-failed',
						message: 'Quick Push action was not applied',
						operation: `quick-push.${action.kind}`,
					},
				};
			}
	...
	private async snapshot(
		...
		if (
			status.state !== 'ready' ||
			status.repositoryId !== request.repositoryId ||
			status.worktreeId !== request.worktreeId
		) {
			throw new GitServiceError(
				'repository-mismatch',
				'Git target is no longer bound to this project',
			);
		}
		...
		if (
			diff.state !== 'ready' ||
			diff.repositoryId !== request.repositoryId ||
			diff.worktreeId !== request.worktreeId
		) {
			throw new GitServiceError(
				'command-error',
				'Git diff could not be captured for Quick Push',
			);
		}
		return {
			projectId: request.projectId,
	...
	private pruneProposals(): void {
		...
		for (const [id, stored] of this.proposals)
			if (stored.proposal.expiresAt <= now || stored.consumed)
...
function validateProposalRequest(request: GitQuickPushProposalRequest): void {
	if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/u.test(request.projectId))
		throw new GitServiceError(
			'invalid-proposal',
			'Quick Push project ID is invalid',
		);
	if (
		!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/u.test(request.repositoryId) ||
		!/^worktree-[a-f0-9]{32}$/u.test(request.worktreeId)
	)
		throw new GitServiceError(
			'invalid-proposal',
			'Quick Push target identity is invalid',
		);
	if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/u.test(request.provider))
		throw new GitServiceError(
			'invalid-proposal',
			'Quick Push provider is invalid',
		);
	validateBranch(request.targetBranch);
}

function validatePlan(
	plan: GitQuickPushPlan,
	targetBranch: string,
	maxActions: number,
): readonly GitQuickPushAction[] {
	if (
		plan === null ||
		typeof plan !== 'object' ||
		!Array.isArray(plan.actions) ||
		plan.actions.length === 0 ||
		plan.actions.length > maxActions
	)
		throw new GitServiceError(
			'invalid-proposal',
			'Quick Push plan is empty or exceeds the action limit',
		);
	...
	const actions = plan.actions.map((action) => {
		if (
			...
			!(action.kind in order) ||
			typeof action.target !== 'string' ||
			typeof action.summary !== 'string' ||
			typeof action.mutatesRevision !== 'boolean'
		)
			throw new GitServiceError(
				'invalid-proposal',
				'Quick Push plan contains an invalid action',
			);
		...
		if (current === undefined || current < previous)
			throw new GitServiceError(
				'invalid-proposal',
				'Quick Push actions are out of order',
			);
		previous = current;
		if (
			...
			action.summary.length > 512 ||
			hasControlCharacter(action.target) ||
			hasControlCharacter(action.summary)
		)
			throw new GitServiceError(
				'invalid-proposal',
				'Quick Push action text is invalid or too long',
			);
		if ((kind === 'commit') !== action.mutatesRevision)
			throw new GitServiceError(
				'invalid-proposal',
				'Quick Push commit mutation flag is invalid',
			);
		if (
			(kind === 'push' || kind === 'pull-request') &&
			action.target !== targetBranch
		)
			throw new GitServiceError(
				'invalid-proposal',
				'Quick Push action target differs from the reviewed branch',
			);
		return Object.freeze({
			kind,
			target: action.target,
			summary: redactQuickPushText(action.summary),
			mutatesRevision: action.mutatesRevision,
		});
	});
...
function validateBranch(value: string): void {
	if (
		...
		value.includes('..') ||
		value.includes('@{') ||
		hasBranchControlCharacter(value) ||
		[...value].some((character) => '~^:?*[\\]'.includes(character))
	)
		throw new GitServiceError(
			'invalid-proposal',
			'Quick Push branch target is invalid',
		);
}
...
function sameRevision(
	first: GitQuickPushRevision,
	second: GitQuickPushRevision,
): boolean {
	return (
		...
		first.statusDigest === second.statusDigest
	);
}
...
function redactQuickPushText(value: string): string {
	return value
		.replace(
			/((?:token|secret|password|passphrase|private[_-]?key|api[_-]?key|grant|proof)[=:]\s*)[^\s,;]+/giu,
			'$1[redacted]',
		)
		.replace(/Bearer\s+[A-Za-z0-9._~+/=-]+/gu, 'Bearer [redacted]')
		.replace(
			/\b(?:gh[pousr]_[A-Za-z0-9_]{20,}|sk-[A-Za-z0-9_-]{20,})\b/gu,
			'[redacted]',
		);
}
...
function validateOptions(options: Required<GitQuickPushServiceOptions>): void {
	if (
		!Number.isSafeInteger(options.maxActions) ||
		options.maxActions <= 0 ||
		!Number.isSafeInteger(options.maxContextBytes) ||
		options.maxContextBytes <= 0 ||
		!Number.isSafeInteger(options.plannerTimeoutMs) ||
		options.plannerTimeoutMs <= 0 ||
		!Number.isSafeInteger(options.executorTimeoutMs) ||
		options.executorTimeoutMs <= 0 ||
		!Number.isSafeInteger(options.proposalTtlMs) ||
		options.proposalTtlMs <= 0 ||
		!Number.isSafeInteger(options.maxProposals) ||
		options.maxProposals <= 0
	)
		throw new RangeError('Quick Push limits must be positive safe integers');
}
...
async function runBoundedProvider<T>(
	...
	if (parentSignal?.aborted === true)
		throw new GitServiceError('cancelled', `${label} was cancelled`);
	...
	const timeoutPromise = new Promise<never>((_, reject) => {
		timer = setTimeout(() => {
			...
			reject(
				new GitServiceError(
					'provider-timeout',
					`${label} exceeded its server deadline`,
				),
			);
		}, timeoutMs);
	});
...
function providerFailureInfo(
	error: unknown,
	actionKind: string,
): {
	readonly code: 'provider-timeout' | 'cancelled' | 'action-failed';
	readonly message: string;
} {
	if (
		error instanceof GitServiceError &&
		(error.code === 'provider-timeout' || error.code === 'cancelled')
	) {
		return {
			code: error.code,
			message: redactQuickPushText(truncateUtf8(error.message, 2048).value),
		};
	}
	return {
		code: 'action-failed',
		message: redactQuickPushText(
			truncateUtf8(
				error instanceof Error
					? error.message
					: `Quick Push ${actionKind} failed`,
				2048,
			).value,
		),
	};
	...
	return new GitServiceError(
		...
		redactQuickPushText(
			truncateUtf8(
				error instanceof Error
					? error.message
					: 'Quick Push provider planning failed',
				2048,
			).value,
		),
	);
