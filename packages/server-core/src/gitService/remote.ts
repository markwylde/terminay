...
import {
	GitQuickPushService,
	type GitQuickPushServiceOptions,
} from './quickPush.js';
...
export interface GitProviderPlanner {
	readonly plan: (
		...
		discovery: GitProviderDiscoveryResult,
		signal?: AbortSignal,
...
export interface GitProviderCredentialResolver {
	...
	readonly withCredential: <T>(
		provider: string,
		callback: (secret: Uint8Array) => T | PromiseLike<T>,
	) => Promise<T>;
}
...
export interface GitProviderServiceOptions {
	...
	readonly maxOutputBytes?: number;
	readonly pullRequestCommands?: Partial<
		Record<Exclude<GitHostingProvider, 'unknown'>, ProviderCommand>
	>;
}
...
export class GitProviderService {
	...
	private readonly pullRequestCommands: Partial<
		Record<Exclude<GitHostingProvider, 'unknown'>, ProviderCommand>
	>;

	constructor(
		private readonly service: GitService,
		options: GitProviderServiceOptions = {},
	) {
		...
		if (!Number.isSafeInteger(this.maxOutputBytes) || this.maxOutputBytes <= 0)
			throw new RangeError('provider output limit must be positive');
		this.pullRequestCommands = options.pullRequestCommands ?? {
			github: {
				command: 'gh',
				args: ({ targetBranch, sourceBranch, title }) => [
					'pr',
					'create',
					'--base',
					targetBranch,
					...(sourceBranch === null ? [] : ['--head', sourceBranch]),
					'--title',
					title,
				],
			},
	...
	async discover(
		request: GitProviderDiscoveryRequest,
	): Promise<GitProviderDiscoveryResult> {
		...
		const result = await this.git.run(
			...
			target.path,
			{ signal: request.signal, maxOutputBytes: this.maxOutputBytes },
		);
		...
		return {
			...
			pullRequestSupported:
				...
				this.pullRequestCommands[remote.provider] !== undefined,
		};
	}

	createPlanner(planner: GitProviderPlanner): GitQuickPushPlanner {
		if (typeof planner?.plan !== 'function')
			throw new TypeError('provider planner is required');
		return {
			plan: async (context, signal) =>
				planner.plan(context, await this.discover(context), signal),
		};
	}

	createExecutor(): GitQuickPushExecutor {
		return {
			execute: (action, context, signal) =>
				this.execute(action, context, signal),
		};
	}

	/** Compose the transport-neutral review coordinator with these server-only
	 * discovery and fixed-command adapters. */
	createQuickPushService(
		planner: GitProviderPlanner,
		options: GitQuickPushServiceOptions = {},
	): GitQuickPushService {
		return new GitQuickPushService(
			this.service,
			this.createPlanner(planner),
			this.createExecutor(),
			options,
		);
	}
	...
	async execute(
		...
		context: GitQuickPushContext,
		signal?: AbortSignal,
	): Promise<GitQuickPushExecutionResult> {
		...
		if (action.kind === 'commit') {
			...
			const result = await this.git.run(
				['add', '--all', '--', '.'],
				target.path,
				{ signal, maxOutputBytes: this.maxOutputBytes },
			);
			if (result.exitCode !== 0 || result.truncated)
				return failure('commit', result);
			const commit = await this.git.run(
				['commit', '-m', message],
				target.path,
				{ signal, maxOutputBytes: this.maxOutputBytes },
			);
			return commit.exitCode === 0 && !commit.truncated
				? { applied: true, detail: 'commit created' }
				: failure('commit', commit);
		}
		if (action.kind === 'push') {
			const branch = boundedBranch(action.target);
			const push = await this.git.run(
				['push', 'origin', `HEAD:refs/heads/${branch}`],
				target.path,
				{ signal, maxOutputBytes: this.maxOutputBytes },
			);
			return push.exitCode === 0 && !push.truncated
				? { applied: true, detail: 'remote branch updated' }
				: failure('push', push);
		}
		const discovery = await this.discover(context);
		if (
			discovery.remote === null ||
			!discovery.pullRequestSupported ||
			discovery.remote.provider === 'unknown'
		)
			return {
				applied: false,
				detail: 'pull-request provider is unavailable for this remote',
			};
		...
		const run = (credential: Uint8Array | undefined) =>
			this.provider.run(
				command.command,
				command.args({
					targetBranch: boundedBranch(action.target),
					sourceBranch: null,
					title,
				}),
				target.path,
				{
					...
					...(command.credentialEnvironmentVariable === undefined
						? {}
						: {
								credentialEnvironmentVariable:
									command.credentialEnvironmentVariable,
							}),
				},
			);
		const result =
		...
		return result.exitCode === 0 && !result.truncated
			? {
					applied: true,
					detail: redactProviderOutput(result.stdout || result.stderr),
				}
			: failure('pull-request', result);
	}

	private async resolveTarget(
		request: GitProviderDiscoveryRequest,
	): Promise<{ readonly path: string }> {
		const listing = await this.service.worktrees({
			projectId: request.projectId,
			repositoryId: request.repositoryId,
		...
		if (
			...
			throw new GitServiceError(
				'repository-mismatch',
				'Git provider target is no longer bound to this project',
			);
		...
		if (worktree === undefined || worktree.isBare || worktree.isPrunable)
			throw new GitServiceError(
				'worktree-not-found',
				'Git provider target worktree is unavailable',
			);
...
export function parseGitRemote(name: string, url: string): GitRemoteInfo {
	...
	if (
		...
		throw new GitServiceError('invalid-project', 'Git remote URL is invalid');
	...
	try {
		...
		/* malformed remotes remain discoverable as unknown */
	}
	const parts =
		path
			?.replace(/\.git$/u, '')
	...
	const provider: GitHostingProvider =
		host === 'github.com'
			? 'github'
			: host !== null && /(?:gitea|gitlab|forgejo)/iu.test(host)
				? 'gitea'
				: 'unknown';
	const webUrl =
		host !== null &&
...
function assertRemoteName(value: string): void {
	if (
		...
		throw new GitServiceError('invalid-project', 'Git remote name is invalid');
}
function boundedText(value: string, max: number): string {
	if (
		...
		throw new GitServiceError(
			'invalid-proposal',
			'Git provider action text is invalid',
		);
	return value;
}
function boundedBranch(value: string): string {
	if (
		typeof value !== 'string' ||
		value.length === 0 ||
		value.length > 256 ||
		...
		throw new GitServiceError(
			'invalid-proposal',
			'Git provider branch is invalid',
		);
	return value;
}
function failure(
	operation: string,
	result: ProviderProcessResult | GitCommandResult,
): GitQuickPushExecutionResult {
	return {
		applied: false,
		detail: redactProviderOutput(
			`${operation} failed${result.stderr.length === 0 ? '' : `: ${result.stderr}`}`,
		),
	};
}
function redactProviderOutput(value: string): string {
	return value
		.replace(
			/((?:token|secret|password|passphrase|private[_-]?key|api[_-]?key)[=:]\s*)[^\s,;]+/giu,
			'$1[redacted]',
		)
...
function runProcess(
	...
	if (!Number.isSafeInteger(maxOutputBytes) || maxOutputBytes <= 0)
		return Promise.reject(
			new RangeError('provider output limit must be positive'),
		);
	return new Promise((resolve, reject) => {
		...
		if (options.credential !== undefined) {
			secretName =
				options.credentialEnvironmentVariable ?? 'TERMINAY_PROVIDER_CREDENTIAL';
		...
		child.once('close', (exitCode) => {
			...
			resolve({
				stdout: new TextDecoder().decode(Buffer.concat(stdout)),
				stderr: new TextDecoder().decode(Buffer.concat(stderr)),
