...
export function normalizeLimits(
	input: MacroLimits = {},
): Required<MacroLimits> {
	return {
		maxSteps: boundedLimit(
			input.maxSteps,
			DEFAULT_MACRO_LIMITS.maxSteps,
			1,
			4096,
		),
		maxFields: boundedLimit(
			input.maxFields,
			DEFAULT_MACRO_LIMITS.maxFields,
			1,
			256,
		),
		maxStringBytes: boundedLimit(
			input.maxStringBytes,
			DEFAULT_MACRO_LIMITS.maxStringBytes,
			256,
			1_048_576,
		),
		maxOutputBytes: boundedLimit(
			input.maxOutputBytes,
			DEFAULT_MACRO_LIMITS.maxOutputBytes,
			1024,
			16_777_216,
		),
		maxDelayMs: boundedLimit(
			input.maxDelayMs,
			DEFAULT_MACRO_LIMITS.maxDelayMs,
			0,
			86_400_000,
		),
		maxConcurrentRuns: boundedLimit(
			input.maxConcurrentRuns,
			DEFAULT_MACRO_LIMITS.maxConcurrentRuns,
			1,
			64,
		),
	};
...
export function normalizeMacroState(
	input: unknown,
	limits: MacroLimits = {},
): MacroState {
	...
	const rawMacros = Array.isArray(record?.macros)
		? record.macros
		: Array.isArray(input)
			? input
			: [];
	if (rawMacros.length > 4096)
		throw new MacroServiceError(
			'limit',
			'macro definition count exceeds the limit',
		);
	const macros = rawMacros.map((value, index) =>
		normalizeMacro(value, index, normalizedLimits),
	);
	const ids = new Set<string>();
	for (const macro of macros) {
		if (ids.has(macro.id))
			throw new MacroServiceError('invalid_macro', 'macro ids must be unique', {
				id: macro.id,
			});
		ids.add(macro.id);
	}
...
export function normalizeMacro(
	...
	const title = boundedString(
		record.title,
		`Macro ${index + 1}`,
		normalizedLimits.maxStringBytes,
	);
	...
	const rawSteps = Array.isArray(record.steps)
		...
		: typeof record.template === 'string'
			...
			: [];
	if (rawSteps.length > normalizedLimits.maxSteps)
		throw new MacroServiceError('limit', 'macro step count exceeds the limit', {
			id,
		});
	const steps = rawSteps.map((step, stepIndex) =>
		normalizeStep(step, stepIndex, normalizedLimits),
	);
	const explicitFields = Array.isArray(record.fields)
		? uniqueFieldsByName(
				record.fields.map((field, fieldIndex) =>
					normalizeField(field, fieldIndex, normalizedLimits),
				),
			)
		: [];
	if (explicitFields.length > normalizedLimits.maxFields)
		throw new MacroServiceError(
			'limit',
			'macro field count exceeds the limit',
			{ id },
		);
	const fields = mergeFieldsWithSteps(
		steps,
		explicitFields,
...
export function renderMacroTemplate(
	...
	maxBytes = DEFAULT_MACRO_LIMITS.maxStringBytes,
): string {
	const etaRendered = renderSafeEtaTemplate(template, values);
	const rendered = etaRendered.replace(
		PLACEHOLDER_PATTERN,
		(
			_match,
	...
	if (new TextEncoder().encode(rendered).byteLength > maxBytes)
		throw new MacroServiceError(
			'limit',
			'rendered macro step exceeds the string limit',
		);
	return rendered;
}
 ...
 */
function renderSafeEtaTemplate(
	template: string,
	values: Readonly<Record<string, MacroFieldValue>>,
): string {
	...
	for (const match of template.matchAll(tagPattern)) {
		...
		if (marker === '=' || marker === '~') {
			...
			if (branch === undefined)
				throw new MacroServiceError(
					'invalid_macro',
					'template has an unmatched else branch',
				);
			...
			if (branch === undefined)
				throw new MacroServiceError(
					'invalid_macro',
					'template has an unmatched closing branch',
				);
			active = branch.parentActive;
		} else if (code.length > 0) {
			throw new MacroServiceError(
				'invalid_macro',
				'template expression is not allowed on the server',
			);
		}
	...
	if (stack.length > 0)
		throw new MacroServiceError(
			'invalid_macro',
			'template has an unterminated branch',
		);
	return output;
}

function renderSafeEtaExpression(
	expression: string,
	values: Readonly<Record<string, MacroFieldValue>>,
): string {
	...
	if (name === undefined)
		throw new MacroServiceError(
			'invalid_macro',
			'template interpolation is not allowed on the server',
		);
...
function evaluateSafeEtaCondition(
	code: string,
	values: Readonly<Record<string, MacroFieldValue>>,
): boolean {
	const match =
		/^if\s*\(\s*([A-Za-z_$][\w$]*|it\.[A-Za-z_$][\w$]*)\s*(===|!==|==|!=)\s*(true|false|null|-?\d+(?:\.\d+)?|'(?:\\.|[^'])*'|"(?:\\.|[^"])*")\s*\)\s*\{?$/u.exec(
			code,
		);
	if (match === null)
		throw new MacroServiceError(
			'invalid_macro',
			'template condition is not allowed on the server',
		);
	const name = safeEtaName(match[1] as string);
	if (name === undefined)
		throw new MacroServiceError(
			'invalid_macro',
			'template condition field is invalid',
		);
	const actual = values[name];
	const expectedToken = match[3] as string;
	const expected: MacroFieldValue | null =
		expectedToken === 'true'
			? true
			: expectedToken === 'false'
				? false
	...
	switch (type) {
		case 'type':
			return {
				id,
				type,
				content: boundedString(record.content, '', limits.maxStringBytes),
			};
		case 'key':
			return {
				id,
...
function normalizeField(
	...
	const options = rawOptions
		.slice(0, 256)
		.flatMap((option, optionIndex): MacroFieldOption[] => {
			const value = asRecord(option);
			if (value === undefined) return [];
			const optionValue = boundedString(
				value.value,
				boundedString(
					value.label,
			...
			return [
				{
					label: boundedString(value.label, optionValue, limits.maxStringBytes),
					value: optionValue,
				},
			];
		});
...
function mergeFieldsWithSteps(
	steps: readonly MacroStep[],
	explicit: readonly MacroFieldDefinition[],
	maxFields: number,
): readonly MacroFieldDefinition[] {
...
function normalizeDuration(
	...
	fallback: string,
	maxBytes: number,
	fromMilliseconds = false,
): string {
	const number =
		typeof value === 'number' && Number.isFinite(value)
			? value / (fromMilliseconds ? 1000 : 1)
			: undefined;
	const stringValue =
		typeof value === 'string' && fromMilliseconds
			? Number(value) / 1000
			: value;
	const result =
