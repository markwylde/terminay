...
export function createMacroOperationRegistry(
	options: MacroOperationRegistryOptions,
): MacroOperationRegistry {
	if (!(options.repository instanceof MacroRepository))
		throw new TypeError('macro repository is required');
	...
	return {
		...
		closeConnection: (connectionId) =>
			runner.launcherDisconnected(connectionId),
	};
	...
	async function upsert(
		...
		return apply(() => {
			const payload = objectPayload(request.envelope.payload);
			if (payload.macro === undefined)
				throw macroError('invalid_macro', 'macro upsert payload is invalid');
	...
	async function remove(
		...
		return apply(() => {
			const macroId = boundedId(
				objectPayload(request.envelope.payload).macroId,
				'macro id',
			);
	...
	async function apply(
		operation: () => Promise<Awaited<ReturnType<MacroRepository['replace']>>>,
	) {
		try {
	...
	async function run(request: CommandRequest): Promise<JsonValue> {
		try {
			...
			if (macro === undefined)
				throw macroError('macro_not_found', 'macro is unavailable');
			const values = parseValues(payload.values);
			const disconnectPolicy =
				payload.disconnectPolicy === undefined
					? 'cancel'
					: parseDisconnectPolicy(payload.disconnectPolicy);
			const environment = options.environmentFor(request, target);
			const handle = runner.start(macro, environment, {
				authorization: {
					target,
					scope: request.context.authScope === 'admin' ? 'admin' : 'write',
				},
				values,
	...
	async function cancel(request: CommandRequest): Promise<JsonValue> {
		try {
			...
			if (snapshot === undefined || !sameTarget(snapshot.target, target))
			...
			if (
				...
				throw macroError(
					'unauthorized_target',
					'macro run belongs to another client',
				);
...
function parseTarget(
	...
	if (target.serverId !== serverId)
		throw macroError(
			'unauthorized_target',
			'macro target is outside this server',
		);
	return {
		serverId,
		projectId: boundedId(target.projectId, 'project id'),
		sessionId: boundedId(target.sessionId, 'session id'),
	};
}
...
function parseValues(
	...
	for (const [name, candidate] of Object.entries(values)) {
		if (
			...
			throw macroError('invalid_macro', 'macro field value is invalid');
		result[name] = candidate;
	}
	return result;
}

function parseDisconnectPolicy(value: JsonValue): 'cancel' | 'continue' {
	if (value !== 'cancel' && value !== 'continue')
		throw macroError('invalid_macro', 'macro disconnect policy is invalid');
	return value;
}
function boundedId(value: JsonValue | undefined, name: string): string {
	if (
		...
		throw macroError('invalid_macro', `${name} is invalid`);
	return value;
}
...
function macroError(
	code: MacroServiceError['code'],
	message: string,
): MacroServiceError {
	return new MacroServiceError(code, message);
}
