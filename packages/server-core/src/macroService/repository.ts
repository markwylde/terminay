...
export class MacroRepository {
	...
	async load(): Promise<MacroState> {
		...
		if (raw !== undefined && !sameJson(raw, state)) {
			if (this.backend.backup !== undefined)
				await this.backend.backup(cloneState(state));
	...
	get state(): MacroState {
		if (!this.loaded || this.current === undefined)
			throw new Error('macro repository is not loaded');
		return cloneState(this.current);
	}
	...
	async apply(envelope: MacroCommandEnvelope): Promise<MacroApplyResult> {
		...
		switch (command.type) {
			...
			case 'upsert': {
				...
				macros = [
					...current.macros.filter((item) => item.id !== macro.id),
					macro,
				];
				break;
			}
	...
	async remove(
		...
		return this.apply({
			commandId,
			expectedRevision,
			command: { type: 'remove', macroId },
		});
	}

	async reset(
		options: MacroResetOptions | number = {},
	): Promise<MacroApplyResult> {
		const normalized =
