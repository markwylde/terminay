...
export class MacroRunner {
	...
	list(): readonly MacroRunSnapshot[] {
		return [...this.activeRuns.values()].map(snapshotOf);
	}
	...
	launcherDisconnected(launcherId: string): void {
		for (const run of this.activeRuns.values()) {
			if (run.launcherId === launcherId && run.disconnectPolicy === 'cancel')
				run.controller.abort();
		}
	}
	...
	start(
		...
		if (this.activeRuns.size >= this.limits.maxConcurrentRuns)
			throw new MacroServiceError(
				'limit',
				'macro run concurrency limit reached',
			);
		...
		try {
			...
			for (let index = 0; index < run.macro.steps.length; index += 1) {
				...
				await this.executeStep(
					run,
					run.macro.steps[index] as MacroStep,
					values,
					environment,
				);
			}
			...
			if (
				...
				(error instanceof MacroServiceError && error.code === 'canceled')
			) {
				run.status = 'canceled';
				...
				run.status = 'failed';
				run.errorCode =
					error instanceof MacroServiceError ? error.code : 'execution_failed';
			}
		} finally {
	...
	private async executeStep(
		...
		switch (step.type) {
			case 'type': {
				const rendered = renderMacroTemplate(
					step.content,
					values,
					this.limits.maxStringBytes,
				);
			...
			case 'key':
				if (environment.key === undefined)
					throw new MacroServiceError(
						'execution_failed',
						'key output is unavailable at the PTY boundary',
					);
			...
			case 'wait_inactivity': {
				...
				if (environment.waitForInactivity === undefined) {
					...
					await environment.waitForInactivity(
						run.target,
						milliseconds,
						run.controller.signal,
					);
	...
	private async write(
		...
		assertNotCanceled(run.controller.signal);
		if (run.bytesWritten + bytes.byteLength > this.limits.maxOutputBytes)
			throw new MacroServiceError('limit', 'macro output exceeds the limit');
...
function resolveValues(
	macro: MacroDefinition,
	supplied: Readonly<Record<string, MacroFieldValue>>,
	...
	for (const field of macro.fields) {
		...
		if (field.required && (value === '' || value === undefined))
			throw new MacroServiceError(
				'invalid_macro',
				`required macro field is missing: ${field.name}`,
			);
		if (
			typeof value === 'string' &&
			new TextEncoder().encode(value).byteLength > maxStringBytes
		)
			throw new MacroServiceError(
				'limit',
				'macro field exceeds the string limit',
			);
		if (field.type === 'number' && typeof value !== 'number')
			throw new MacroServiceError(
				'invalid_macro',
				`macro field has an invalid value: ${field.name}`,
			);
...
function parseDelay(
	...
	if (!Number.isFinite(seconds) || seconds < 0)
		throw new MacroServiceError(
			'invalid_macro',
			'macro wait duration must be non-negative',
		);
	const milliseconds = Math.round(seconds * 1000);
	if (milliseconds > maxDelayMs)
		throw new MacroServiceError(
			'limit',
			'macro wait duration exceeds the limit',
		);
	return milliseconds;
}
...
function assertAuthorization(
	environment: MacroExecutionEnvironment,
	authorization: MacroRunAuthorization,
): void {
	if (
		!sameTarget(environment.target, authorization.target) ||
	...
	if (
		environment.authorize !== undefined &&
		!environment.authorize(authorization.target)
	)
		throw new MacroServiceError(
			'unauthorized_target',
			'macro target authorization was rejected',
		);
}
...
function freezeTarget(target: MacroTarget): MacroTarget {
	return Object.freeze({
...
function assertNotCanceled(signal: AbortSignal): void {
	if (signal.aborted)
		throw new MacroServiceError('canceled', 'macro run was canceled');
}
