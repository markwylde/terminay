...
export type CompatibilityMatrix = Readonly<
	Partial<Record<CompatibilityComponent, CompatibilityRequirement>>
>;
export type CompatibilityVersions = Readonly<
	Partial<Record<CompatibilityComponent, string>>
>;
...
export class CompatibilityError extends Error {
	...
	constructor(failure: CompatibilityFailure) {
		const bound =
			failure.code === 'below_minimum'
				? `requires >= ${failure.minimum}`
				: failure.maximum === undefined
					? 'has no compatible version'
					: `supports <= ${failure.maximum}`;
		super(
			`${failure.component} version ${failure.actual ?? 'is missing'}; ${bound}`,
		);
...
const COMPONENT_ORDER: readonly CompatibilityComponent[] = [
	'desktop',
	'server',
	'ui',
	'bootstrap',
...
export function checkCompatibilityMatrix(
	...
	requirements: CompatibilityMatrix,
	...
	for (const component of COMPONENT_ORDER) {
		...
		const maximum =
			requirement.maximum === undefined
				? undefined
				: normalizeVersion(requirement.maximum, `${component} maximum version`);
		if (maximum !== undefined && compareVersions(minimum, maximum) > 0)
			throw new RangeError(`${component} compatibility range is invalid`);
		const raw = versions[component];
		const actual =
			raw === undefined ? null : normalizeVersion(raw, `${component} version`);
		if (actual === null) {
			failures.push({
				component,
				actual,
