...
export interface MigrationInventoryOptions {
	...
	readonly pathProbe?: (
		path: string,
	) => MigrationPathState | Promise<MigrationPathState>;
}
 ...
 */
export async function inspectLegacyMigration(
	sourceInput: unknown,
	...
	const sourceSchemaVersion = validSchemaVersion(options.sourceSchemaVersion)
		? options.sourceSchemaVersion!
		: 0;
	const sourceVersion = readSourceVersion(source);
	const pathProbe = options.pathProbe ?? probePath;

	const projectCandidates = collectPathCandidates(
		source.projects,
		maxPathReferences,
	);
	const recordingCandidates = collectPathCandidates(
		source.recordings,
		maxPathReferences,
	);
	const projectPaths = await inspectPaths(
		projectCandidates,
		'project',
		pathProbe,
	);
	const recordingPaths = await inspectPaths(
		recordingCandidates,
		'recording',
		pathProbe,
	);
	...
	return {
		...
		sourceVersion,
		stores: STORE_KEYS.map(([name, keys]) =>
			summarizeStore(name, source, keys),
		),
		storeVersions: STORE_KEYS.map(([name, keys]) =>
			summarizeStoreVersion(name, source, keys),
		),
		projectPaths,
		recordingPaths,
		rendererLayout: {
...
function validateInventorySource(
	value: unknown,
): LegacyMigrationSource & Record<string, unknown> {
	if (typeof value !== 'object' || value === null || Array.isArray(value))
		throw new TypeError('legacy migration source is not an object');
...
function boundedPathLimit(value: number | undefined): number {
	...
	if (!Number.isSafeInteger(value) || value < 1 || value > 1024)
		throw new RangeError('maxPathReferences must be between 1 and 1024');
	return value;
}
...
function summarizeStore(
	name: MigrationStoreInventory['name'],
	source: Record<string, unknown>,
	keys: readonly string[],
): MigrationStoreInventory {
...
function summarizeStoreVersion(
	name: MigrationStoreInventory['name'],
	source: Record<string, unknown>,
	keys: readonly string[],
): MigrationStoreVersion {
	const key = keys.find((candidate) => source[candidate] !== undefined);
	if (key === undefined)
		return { name, format: 'missing', schemaVersion: null, version: null };
	const value = source[key];
	const record = isRecord(value) ? value : undefined;
	const schemaVersion = readBoundedVersion(record?.schemaVersion);
	const version = readVersionString(
		record?.version ?? record?.sourceVersion ?? record?.desktopVersion,
	);
	return {
		name,
...
function boundedEntryCount(value: unknown): number {
	...
	if (typeof value === 'object')
		return Math.min(Object.keys(value as Record<string, unknown>).length, 1024);
	return 1;
}
...
async function inspectPaths(
	...
	pathProbe: (path: string) => MigrationPathState | Promise<MigrationPathState>,
): Promise<readonly MigrationPathReference[]> {
	const result: MigrationPathReference[] = [];
	for (const path of paths) {
		const state = isAbsoluteLegacyPath(path)
			? await pathProbe(path)
			: 'invalid';
		result.push({
			kind,
			path,
...
function pathReason(
	path: string,
	state: MigrationPathState,
): MigrationPathReference['reason'] {
	if (state === 'missing') return 'not-found';
	if (state === 'inaccessible') return 'permission-denied';
	if (state === 'invalid')
		return isAbsoluteLegacyPath(path) ? 'invalid' : 'not-absolute';
	return undefined;
}
