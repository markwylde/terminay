...
export class MigrationRunner {
	...
	constructor(
		...
		if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/u.test(options.migrationId))
	...
	async run(sourceInput: unknown): Promise<MigrationRunResult> {
		...
		if (
			this.options.expectedServerId !== undefined &&
			source.serverId !== undefined &&
			source.serverId !== this.options.expectedServerId
		) {
			throw new MigrationError(
				'identity_collision',
				'legacy source belongs to a different server identity',
			);
		}
		const existing = await this.backend.loadMarker();
		if (existing?.status === 'complete')
			return { marker: existing, resumed: true };
		if (existing?.rollbackState === 'recovery-required') {
			throw new MigrationError(
				'rollback_requires_backup_recovery',
				'recover the backup before retrying this migration',
			);
		}
		if (
			existing?.serverCommitState === 'unknown' ||
			existing?.serverCommitState === 'committed'
		) {
			throw new MigrationError(
				'backup_recovery_unavailable',
				'server-only migration commit is committed or uncertain; recover the backup before retrying',
			);
		}
		...
		if (marker.migrationId !== this.options.migrationId)
			throw new MigrationError(
				'invalid_source',
				'migration marker belongs to another migration',
			);
		if (marker.schemaVersion !== MIGRATION_SCHEMA_VERSION)
			throw new MigrationError(
				'invalid_source',
				'unsupported migration marker schema',
			);
		if (marker.backupId === undefined) {
			...
			marker = {
				...
				...(this.backend.commitServerOnlyMutations === undefined
					? {}
					: { serverCommitState: 'uncommitted' as const }),
			};
		...
		if (
			this.backend.captureElectronState !== undefined &&
			marker.electronStateBackupId === undefined
		) {
		...
		try {
			...
			const failure =
				error instanceof MigrationError
					...
					: new MigrationError(
							'step_failed',
							`server-only transaction setup failed: ${safeFailureMessage(error)}`,
							undefined,
							error,
						);
			marker = {
			...
			try {
				...
				const failure =
					error instanceof MigrationError
						...
						: new MigrationError(
								'step_failed',
								`migration step failed: ${safeFailureMessage(error)}`,
								step,
								error,
							);
				marker = {
		...
		try {
			...
			const failure =
				error instanceof MigrationError
					...
					: new MigrationError(
							'step_failed',
							`server-only commit failed: ${safeFailureMessage(error)}`,
							undefined,
							error,
						);
		...
		marker = {
			...
			...(this.backend.commitServerOnlyMutations === undefined
				? {}
				: {
						serverCommitState: 'committed' as const,
						serverCommitAt: this.now(),
					}),
		};
		await this.backend.saveMarker(marker);
	...
	async rollback(): Promise<MigrationRollbackResult> {
		...
		if (marker === undefined || marker.backupId === undefined) {
			throw new MigrationError(
				...
				'migration backup is unavailable',
			);
		}
		if (
			...
			throw new MigrationError(
				'rollback_requires_backup_recovery',
				'server-only migration mutations are committed or uncertain; use explicit backup recovery',
			);
		}
		if (this.backend.restoreBackup === undefined) {
			throw new MigrationError(
				'rollback_unavailable',
				'migration backend does not support rollback',
			);
		}
		if (
			marker.electronStateBackupId !== undefined &&
			this.backend.restoreElectronState === undefined
		) {
			throw new MigrationError(
				'rollback_unavailable',
				'migration backend cannot restore pre-migration Electron state',
			);
		}
	...
	async recoverBackup(): Promise<MigrationBackupRecoveryResult> {
		...
		if (
			...
			throw new MigrationError(
				'backup_recovery_unavailable',
				'explicit backup recovery is available only after server-only commit',
			);
		}
		if (this.backend.restoreBackup === undefined) {
			throw new MigrationError(
				'backup_recovery_unavailable',
				'migration backend does not support backup recovery',
			);
		}
		await this.backend.restoreBackup(marker.backupId);
		const recovered = {
		...
		return {
			...
			...(marker.electronStateBackupId === undefined
				? {}
				: { electronStateBackupId: marker.electronStateBackupId }),
		};
	}
	...
	private async runStep(
		...
		switch (step) {
			case 'settings':
				return this.backend.importSettings(source.settings);
			case 'macros':
				return this.backend.importMacros(source.macros);
			case 'connectionProfiles':
				return this.backend.importConnectionProfiles(source.connectionProfiles);
			case 'projects':
				return this.backend.importProjects(source.projects);
			case 'recordings':
				return this.backend.importRecordings(source.recordings);
			case 'secrets': {
				for (const [id, value] of Object.entries(source.secrets ?? {}))
					await this.backend.importSecret(id, value);
				return;
			}
		}
	}
...
function readCompatibilityVersions(
	source: LegacyMigrationSource,
...
function safeFailureMessage(error: unknown): string {
	...
	const sanitized = raw
		.replace(
			/(?:password|passphrase|secret|token|credential|api[_-]?key)\s*[:=]\s*[^\s,;]+/giu,
			'$1: [redacted]',
		)
		.replace(/\s+/gu, ' ')
		.trim();
...
function validateSource(value: unknown): LegacyMigrationSource {
	if (typeof value !== 'object' || value === null || Array.isArray(value))
		throw new MigrationError(
			'invalid_source',
			'legacy migration source is not an object',
		);
	const source = value as LegacyMigrationSource;
	if (
		source.serverId !== undefined &&
		(typeof source.serverId !== 'string' || source.serverId.length > 128)
	)
		throw new MigrationError(
			'invalid_source',
			'legacy server identity is invalid',
		);
	if (
		...
		throw new MigrationError('invalid_source', 'legacy secrets are invalid');
	return source;
}
...
function isSecretKey(key: string): boolean {
	return /(?:secret|token|password|passphrase|credential|privatekey|api[_-]?key|pairing|grant|proof)/iu.test(
		key,
	);
}
