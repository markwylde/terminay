...
export function validateServerPlatformPaths(
	paths: ServerPlatformPaths,
	expectedDataRoot?: string,
): ServerPlatformPaths {
	if (typeof paths !== 'object' || paths === null)
		throw new TypeError('server platform paths are required');
	const checked = {
		...
		...(paths.configRoot === undefined
			? {}
			: { configRoot: checkedPath(paths.configRoot, 'config path') }),
		...(paths.cacheRoot === undefined
			? {}
			: { cacheRoot: checkedPath(paths.cacheRoot, 'cache path') }),
		...(paths.logRoot === undefined
			? {}
			: { logRoot: checkedPath(paths.logRoot, 'log path') }),
	} satisfies ServerPlatformPaths;
	if (expectedDataRoot !== undefined && checked.dataRoot !== expectedDataRoot)
		throw new TypeError('platform data root does not match runtime data root');
