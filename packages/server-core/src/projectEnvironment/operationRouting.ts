...
export type ProjectOperationClassifier = (
	operation: string,
) => ProjectOperationRoute | undefined;
...
export function routeProjectOperationRegistries(
	...
	return {
		...(operations.queries === undefined
			? {}
			: { queries: routeQueries(operations.queries, router, classify) }),
		...(operations.commands === undefined
			? {}
			: { commands: routeCommands(operations.commands, router, classify) }),
		...(operations.policies === undefined
			? {}
			: { policies: operations.policies }),
	};
}

export function classifyProjectOperation(
	operation: string,
): ProjectOperationRoute | undefined {
	...
	if (operation.startsWith('files.') || operation.startsWith('file.'))
	...
	if (operation.startsWith('git.'))
		return {
			capability: 'git',
			operation: GIT_SERVICE_OPERATIONS[operation] ?? 'unsupported',
		};
	if (operation.startsWith('agents.') || operation.startsWith('agent.'))
		return { capability: 'agent-journal' };
	if (operation.startsWith('shell-profiles.'))
		return { capability: 'shell-discovery' };
	return undefined;
}
...
function routeQueries(
	source: NonNullable<OperationRegistries['queries']>,
	router: ProjectEnvironmentRouter,
	classify: ProjectOperationClassifier,
): ReadonlyMap<string, QueryHandler> {
	return new Map(
		entries(source).map(([operation, handler]) => [
			operation,
...
function routeCommands(
	source: NonNullable<OperationRegistries['commands']>,
	router: ProjectEnvironmentRouter,
	classify: ProjectOperationClassifier,
...
function requestOptions(request: QueryRequest | CommandRequest) {
	const remaining =
		request.context.deadline === undefined
			? undefined
			: Math.max(1, request.context.deadline - Date.now());
	return {
