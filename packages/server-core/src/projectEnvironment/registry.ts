...
export class ThisServerEnvironmentRuntime implements ProjectEnvironmentRuntime {
	...
	readonly capabilities: readonly ProjectEnvironmentCapability[];
	constructor(private readonly services: ThisServerServices) {
		this.capabilities = Object.freeze(
			Object.keys(services.capabilities) as ProjectEnvironmentCapability[],
		);
	}
	async invoke(
		capability: ProjectEnvironmentCapability,
		...
		if (
			...
			context.environmentRevision !== 1
		)
			throw new Error('This server invocation has invalid environment binding');
		if (context.signal.aborted || Date.now() > context.deadline)
			throw new Error('project environment operation cancelled or expired');
...
export class ProjectEnvironmentRegistry {
	...
	register(runtime: ProjectEnvironmentRuntime): void {
		if (this.runtimes.has(runtime.providerId))
			throw new Error(
				`project environment provider already registered: ${runtime.providerId}`,
			);
		this.runtimes.set(runtime.providerId, runtime);
	}
	unregister(providerId: ProtocolId): void {
		if (providerId === THIS_SERVER_PROVIDER_ID)
			throw new Error('This server provider cannot be unregistered');
		this.runtimes.delete(providerId);
	}
	get(providerId: ProtocolId): ProjectEnvironmentRuntime | undefined {
	...
	resolve(
		...
		if (environment.archived || environment.status !== 'ready')
			throw new Error(
				`project environment is unavailable: ${environment.status}`,
			);
	...
	summary(environment: ProjectEnvironmentRecord): ProjectEnvironmentSummary {
		return toProjectEnvironmentSummary(environment);
	}
}

export class ProjectEnvironmentCapabilityError extends Error {
	readonly code = 'capability-unavailable';
	constructor(readonly capability: ProjectEnvironmentCapability) {
		super(`project environment capability is unavailable: ${capability}`);
		this.name = 'ProjectEnvironmentCapabilityError';
	}
}
