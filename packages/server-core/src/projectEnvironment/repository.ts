...
import {
	canonicalizeProjectEnvironmentState,
	projectEnvironmentRegistryHasOrphanedProfileRecords,
	projectEnvironmentRegistryHasUnknownCapabilities,
} from './validation.js';
...
export class ProjectEnvironmentRepository {
	...
	constructor(
		...
		initialState?: ProjectEnvironmentState,
	) {
		if (initialState !== undefined)
			this.current = canonicalizeProjectEnvironmentState(
				initialState,
				serverId,
			);
	}
	...
	async load(): Promise<ProjectEnvironmentState> {
		...
		const state =
			raw === undefined
				? createInitialProjectEnvironmentState(this.serverId)
				: migrateProjectEnvironmentState(raw, this.serverId);
		if (
			raw === undefined ||
			(raw as { schemaVersion?: unknown }).schemaVersion !==
				PROJECT_ENVIRONMENT_SCHEMA_VERSION ||
			projectEnvironmentRegistryHasUnknownCapabilities(raw) ||
			projectEnvironmentRegistryHasOrphanedProfileRecords(raw)
		)
	...
	async commit(
		...
		update: (state: ProjectEnvironmentState) => ProjectEnvironmentState,
	): Promise<ProjectEnvironmentState> {
		const before = this.current ?? (await this.load());
		if (before.revision !== expectedRevision)
			throw new ProjectEnvironmentConflictError(before.revision);
		const proposed = update(structuredClone(before));
		const next = canonicalizeProjectEnvironmentState(
			{
				...proposed,
				serverId: before.serverId,
				schemaVersion: before.schemaVersion,
				revision: before.revision + 1,
				cursor: String(before.revision + 1),
			},
			before.serverId,
		);
	...
	get state(): ProjectEnvironmentState {
		if (this.current === undefined)
			throw new Error('project environment repository is not loaded');
		return structuredClone(this.current);
	}
}

export function migrateProjectEnvironmentState(
	input: unknown,
	serverId: string,
	...
	if (value.schemaVersion === PROJECT_ENVIRONMENT_SCHEMA_VERSION)
		return canonicalizeProjectEnvironmentState(
			dropOrphanedProfileRecords(value),
			serverId,
		);
	if (
		value.schemaVersion !== 1 ||
		typeof value.environments !== 'object' ||
		value.environments === null
	)
		throw new TypeError('unsupported project environment registry schema');
	const environments = Object.fromEntries(
		Object.entries(
			value.environments as Record<string, Record<string, unknown>>,
		).map(([id, environment]) => [
			id,
			{ ...environment, providerState: null, providerRevision: 1 },
		]),
	);
	return canonicalizeProjectEnvironmentState(
		{
...
function dropOrphanedProfileRecords(
	...
	const orphanedEnvironmentIds = new Set(
		Object.entries(environments)
			.filter(([, environment]) => {
				...
				return (
					typeof profileId === 'string' && profiles[profileId] === undefined
				);
			})
			.map(([id]) => id),
	);
	const operations =
		typeof input.operations === 'object' &&
	...
	return {
		...
		environments: Object.fromEntries(
			Object.entries(environments).filter(
				([id]) => !orphanedEnvironmentIds.has(id),
			),
		),
		operations:
...
export class ProjectEnvironmentConflictError extends Error {
	...
	constructor(readonly currentRevision: number) {
		super('project environment registry revision is stale');
		this.name = 'ProjectEnvironmentConflictError';
	}
}

export class FileProjectEnvironmentStateBackend
	implements ProjectEnvironmentStateBackend
{
	constructor(private readonly filePath: string) {
		if (filePath.length === 0)
			throw new TypeError('project environment state path is required');
	}
	...
	async commit(state: ProjectEnvironmentState): Promise<void> {
