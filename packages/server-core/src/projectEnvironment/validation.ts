...
export function canonicalizeProjectEnvironmentState(input: unknown, expectedServerId: string): ProjectEnvironmentState {
	...
	if (input.schemaVersion !== PROJECT_ENVIRONMENT_SCHEMA_VERSION || input.serverId !== expectedServerId || !revision(input.revision) || input.cursor !== String(input.revision) || !record(input.profiles) || !record(input.environments))
	...
	return structuredClone({ schemaVersion: PROJECT_ENVIRONMENT_SCHEMA_VERSION, serverId: expectedServerId, revision: input.revision, cursor: input.cursor, profiles, environments });
}
...
function profile(id: string, input: unknown): EnvironmentProfile {
	if (!record(input) || input.id !== id || !identifier(id) || !identifier(input.providerId) || !text(input.name, 256) || !text(input.endpointSummary, 512) || !positiveRevision(input.activeRevision) || !positiveRevision(input.recommendedRevision) || input.recommendedRevision < input.activeRevision || !record(input.revisions) || typeof input.archived !== 'boolean') throw new TypeError(`invalid environment profile ${id}`);
...
function environment(id: string, input: unknown, profiles: Readonly<Record<string, EnvironmentProfile>>): ProjectEnvironmentRecord {
	if (!record(input) || input.id !== id || !identifier(id) || !identifier(input.providerId) || !positiveRevision(input.pinnedRevision) || !text(input.name, 256) || !text(input.endpointSummary, 512) || !capabilities(input.declaredCapabilities) || !capabilities(input.availableCapabilities) || input.availableCapabilities.some((value) => !input.declaredCapabilities.includes(value)) || !PROJECT_ENVIRONMENT_STATUSES.includes(input.status as never) || !stringList(input.operationReferences, 128) || !revision(input.projectReferenceCount) || typeof input.archived !== 'boolean' || typeof input.builtIn !== 'boolean') throw new TypeError(`invalid project environment ${id}`);
