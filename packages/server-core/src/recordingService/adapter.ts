...
export class ServerRecordingAdapter implements RecordingAdapter {
	...
	constructor(service: RecordingService, options: RecordingAdapterOptions) {
		if (!(service instanceof RecordingService))
			throw new TypeError('recording service is required');
		if (typeof options?.serverId !== 'string' || options.serverId.length === 0)
			throw new TypeError('recording server id is required');
	...
	list(request: RecordingListRequest): RecordingListResult {
		...
		const items = result.items.filter(
			(item) =>
				item.serverId === null || item.serverId === this.options.serverId,
		);
	...
	replay(request: RecordingReplayRequest): RecordingChunk {
		this.requireScope(request.authorization, 'read');
		const item = this.findAuthorizedItem(
			request.authorization,
			request.recordingId,
		);
		return this.service.readRecordingChunk({
	...
	start(request: RecordingStartRequest): RecordingState {
		...
		const projectId =
			...
			request.authorization.projectId;
		this.requireProject(request.authorization, projectId ?? null);
		if (this.options.resolveSessionProject !== undefined) {
			...
			if (projectId !== undefined && owner !== projectId)
				throw forbidden('recording is outside the terminal project');
		}
	...
	stop(request: RecordingStopRequest): RecordingState {
		...
		if (scope.serverId !== null && scope.serverId !== this.options.serverId)
			throw forbidden('recording belongs to another server');
	...
	delete(request: RecordingDeleteRequest): void {
		...
		this.service.delete(request.recordingId, {
			...(request.stopFirst === undefined
				? {}
				: { stopFirst: request.stopFirst }),
		});
	}

	async reveal(
		request: RecordingRevealRequest,
	): Promise<RecordingRevealResult> {
		this.requireScope(request.authorization, 'read');
		const item = this.findAuthorizedItem(
			request.authorization,
			request.recordingId,
		...
		const unavailable: RecordingRevealResult = {
			...
			available: false,
			guidance:
				'Reveal is available only from a client representing the server host; use replay or copy the recording id instead.',
		};
		if (!item.castAvailable)
			return {
				...unavailable,
				guidance:
					'The recording cast is unavailable on the server; use its timeline metadata or retry when the configured root is mounted.',
			};
		if (this.options.hasHostRevealCapability?.(request.authorization) !== true)
			return unavailable;
		if (this.options.revealOnHost === undefined)
			return {
				...unavailable,
				guidance:
					'The server host cannot reveal recordings in this runtime; use bounded replay instead.',
			};
		...
		return {
			recordingId: item.recordingId,
			available: true,
			guidance: 'The recording was revealed on the server host.',
		};
	}
	...
	operations(): RecordingOperationHandlers {
		return {
			queries: {
				[RECORDING_OPERATIONS.list]: (request) =>
					asJson(this.list(this.listRequest(request))),
				[RECORDING_OPERATIONS.replay]: (request) =>
					asJson(this.replay(this.replayRequest(request))),
			},
			commands: {
				[RECORDING_OPERATIONS.start]: (request) =>
					asJson(this.start(this.startRequest(request))),
				[RECORDING_OPERATIONS.stop]: (request) =>
					asJson(this.stop(this.stopRequest(request))),
				[RECORDING_OPERATIONS.delete]: (request) => {
					this.delete(this.deleteRequest(request));
					return null;
				},
				[RECORDING_OPERATIONS.reveal]: async (request) =>
					asJson(await this.reveal(this.revealRequest(request))),
			},
			policies: {
	...
	private findAuthorizedItem(
		authorization: RecordingAuthorization,
		recordingId: string,
	) {
		let item: RecordingListItem;
		try {
			item = this.service.getRecording(recordingId);
		} catch {
			throw new RecordingServiceError('not_found', 'Recording does not exist.');
		}
		if (item.serverId !== null && item.serverId !== this.options.serverId)
			throw new RecordingServiceError('not_found', 'Recording does not exist.');
	...
	private requireServer(authorization: RecordingAuthorization): void {
		if (authorization.serverId !== this.options.serverId)
			throw forbidden('recording belongs to another server');
	}
	...
	private requireScope(
		...
		if (!scopeAllows(authorization.scope, required))
			throw forbidden(`recording operation requires ${required} scope`);
	}
	...
	private requireProject(
		...
		if (
			...
			throw forbidden('recording is outside the authorized project');
		if (
			projectId !== null &&
	...
	private listRequest(request: QueryRequest): RecordingListRequest {
		return {
			...
			options: asListOptions(request.envelope.payload),
		};
	}
	...
	private startRequest(request: CommandRequest): RecordingStartRequest {
		...
		const metadata =
			payload.metadata === undefined
				? undefined
				: asStartOptions(payload.metadata);
		return {
			authorization: this.authorization(request),
			sessionId,
			...(projectId === undefined ? {} : { projectId }),
	...
	private stopRequest(request: CommandRequest): RecordingStopRequest {
		const payload = objectPayload(request.envelope.payload);
		return {
			authorization: this.authorization(request),
	...
	private deleteRequest(request: CommandRequest): RecordingDeleteRequest {
		const payload = objectPayload(request.envelope.payload);
		return {
			authorization: this.authorization(request),
			recordingId: stringField(payload, 'recordingId'),
			...(typeof payload.stopFirst === 'boolean'
				? { stopFirst: payload.stopFirst }
				: {}),
		};
	}
	private revealRequest(request: CommandRequest): RecordingRevealRequest {
		const payload = objectPayload(request.envelope.payload);
		return {
			authorization: this.authorization(request),
			recordingId: stringField(payload, 'recordingId'),
	...
	private authorization(
		...
		return {
			...
			request.context.claims !== null &&
			!Array.isArray(request.context.claims) &&
			typeof request.context.claims.projectId === 'string'
				? { projectId: request.context.claims.projectId }
				: {}),
		};
	}
}

function forbidden(message: string): RecordingServiceError {
	return new RecordingServiceError('forbidden', message);
}
...
function objectPayload(value: unknown): Record<string, unknown> {
	if (typeof value !== 'object' || value === null || Array.isArray(value))
		throw new RecordingServiceError(
			'invalid_request',
			'recording payload must be an object',
		);
...
function stringField(value: Record<string, unknown>, key: string): string {
	...
	if (
		...
		throw new RecordingServiceError('invalid_request', `${key} is invalid`);
	return candidate;
}
...
function optionalUInt(value: unknown, name: string): number | undefined {
	if (value === undefined) return undefined;
	if (!Number.isSafeInteger(value) || (value as number) < 0)
		throw new RecordingServiceError('invalid_request', `${name} is invalid`);
	return value as number;
}
function normalizeListOptions(
	value: RecordingListOptions | undefined,
): RecordingListOptions {
	if (value === undefined) return {};
	const payload = objectPayload(value);
	return asListOptions(payload);
}
function asListOptions(value: unknown): RecordingListOptions {
	...
	if (
		...
		throw new RecordingServiceError(
			'invalid_request',
			'recording state filter is invalid',
		);
	if (
		payload.inputPresent !== undefined &&
		typeof payload.inputPresent !== 'boolean'
	)
		throw new RecordingServiceError(
			'invalid_request',
			'input-present filter is invalid',
		);
...
function asStartOptions(value: unknown): RecordingStartOptions {
	const payload = objectPayload(value);
	return {
