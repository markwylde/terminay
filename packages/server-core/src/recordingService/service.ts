...
function errorMessage(error: unknown): string {
	...
	return error instanceof RecordingServiceError &&
...
function asLifecycle(
	value: RecordingLifecycle | undefined,
): RecordingLifecycle | undefined {
	return value;
}
...
export class RecordingService {
	...
	constructor(options: import('./types.js').RecordingServiceOptions = {}) {
		const home =
			...
			process.cwd();
		const root =
			options.recordingRoot ?? options.directory ?? DEFAULT_RECORDING_ROOT;
		const index = options.libraryIndexPath ?? options.getLibraryIndexPath?.();
		this.storage =
			options.storage ??
			new NodeRecordingStorage({
				recordingRoot: root,
				homeDirectory: home,
				...(index === undefined ? {} : { libraryIndexPath: index }),
				...(options.migrateStoredMetadata === undefined
					? {}
					: { migrateStoredMetadata: options.migrateStoredMetadata }),
			});
		this.options = {
			...
			maxEventBytes: boundedLimit(
				options.maxEventBytes ?? MAX_RECORDING_EVENT_BYTES,
				MAX_RECORDING_EVENT_BYTES,
				'maxEventBytes',
			),
			maxChunkBytes: boundedLimit(
				options.maxChunkBytes ?? MAX_RECORDING_CHUNK_BYTES,
				MAX_RECORDING_CHUNK_BYTES,
				'maxChunkBytes',
			),
		...
		if (this.options.onStateChanged !== undefined)
			this.stateListeners.add(this.options.onStateChanged);
	...
	subscribe(listener: RecordingStateListener): () => void {
		if (typeof listener !== 'function')
			throw new TypeError('recording listener is required');
	...
	get recordingRoots(): readonly string[] {
		return this.storage.roots;
	}
	...
	importRecordingRoot(root: string): RecordingRootReference {
		if (
			...
			throw new RecordingServiceError(
				'invalid_root',
				'Recording root is invalid.',
			);
		const describe = this.storage.describeRoot;
		if (describe === undefined)
			throw new RecordingServiceError(
				'storage_failed',
				'Recording storage cannot import roots.',
			);
		return describe.call(this.storage, root, false);
	}

	importLegacyRoot(root: string): RecordingRootReference {
		return this.importRecordingRoot(root);
	}
	...
	getSessionScope(sessionId: string): RecordingSessionScope | undefined {
		...
		return {
			sessionId,
			serverId: (metadata.serverId ?? this.options.serverId) || null,
	...
	start(
		...
		if (current !== undefined) {
			...
			if (options.captureInput !== undefined)
				current.captureInput = options.captureInput;
		...
		if (!RECORDING_ID_PATTERN.test(recordingId))
			throw new RecordingServiceError('invalid_id', 'Recording id is invalid.');
		...
		const sensitivePolicy =
			options.sensitiveInputPolicy ?? this.resolveSensitivePolicy();
		...
		const metadata: RecordingMetadata = {
			...
			cwd:
				typeof merged.cwd === 'string' ? boundedText(merged.cwd, 2048) : null,
			shell:
				typeof merged.shell === 'string'
					? boundedText(merged.shell, 2048)
					: null,
			cols,
			rows,
			finalCols: null,
		...
		const environment =
			merged.environment === undefined
				? {}
				: safeEnvironment(merged.environment);
		...
		const header = JSON.stringify({
			...
			env: {
				TERM: 'xterm-256color',
				...(metadata.shell === null ? {} : { SHELL: metadata.shell }),
				...environment,
			},
		});
	...
	startIfEnabled(
		sessionId: string,
		metadata: RecordingStartOptions = {},
	): RecordingState {
	...
	updateSessionMetadata(
		...
		const next = {
			...
			...(metadata.environment === undefined
				? {}
				: { environment: safeEnvironment(metadata.environment) }),
		};
		...
		active.metadata = {
			...active.metadata,
			...(metadata.serverId === undefined
				? {}
				: { serverId: boundedText(metadata.serverId) }),
			...(metadata.projectId === undefined
				? {}
				: { projectId: boundedText(metadata.projectId) }),
			...(metadata.projectName === undefined
				? {}
				: { projectName: boundedText(metadata.projectName) }),
			...(metadata.projectRoot === undefined
				? {}
				: { projectRoot: boundedText(metadata.projectRoot, 2048) }),
			...(metadata.title === undefined
				? {}
				: { title: boundedText(metadata.title, 512) || 'Terminal' }),
			...(metadata.note === undefined
				? {}
				: { note: boundedText(metadata.note, 4096) }),
			...(metadata.color === undefined
				? {}
				: { color: boundedText(metadata.color, 128) }),
			...(metadata.emoji === undefined
				? {}
				: { emoji: boundedText(metadata.emoji, 32) }),
			...(metadata.cwd === undefined
				? {}
				: {
						cwd: metadata.cwd === null ? null : boundedText(metadata.cwd, 2048),
					}),
			...(metadata.shell === undefined
				? {}
				: {
						shell:
							metadata.shell === null
								? null
								: boundedText(metadata.shell, 2048),
					}),
		};
	...
	appendOutput(sessionId: string, data: string): void {
		this.appendData(sessionId, 'o', data);
	}

	appendInput(sessionId: string, data: string): void {
		const active = this.active.get(sessionId);
		if (active === undefined || active.metadata.recordingState !== 'recording')
			return;
		if (typeof this.options.captureInput === 'function')
			active.captureInput = this.options.captureInput();
		if (!active.captureInput) return;
		if (typeof this.options.sensitiveInputPolicy === 'function')
			active.metadata = {
				...active.metadata,
				sensitiveInputPolicy: this.resolveSensitivePolicy(),
			};
		active.metadata = {
			...active.metadata,
			capturedInput: true,
			inputPolicy: 'record-with-sensitive-filter',
		};
	...
	appendResize(sessionId: string, cols: number, rows: number): void {
		...
		if (active === undefined || active.metadata.recordingState !== 'recording')
			return;
		const nextCols = numberDimension(
			cols,
			active.metadata.finalCols ?? active.metadata.cols,
			2,
		);
		const nextRows = numberDimension(
			rows,
			active.metadata.finalRows ?? active.metadata.rows,
			1,
		);
	...
	finalize(
		...
		const active = this.active.get(sessionId);
		if (active === undefined) return this.getState(sessionId);
		if (active.metadata.recordingState !== 'recording')
			return this.toState(active, 'failed');
		...
		active.metadata = {
			...
			durationMs: Math.max(
				0,
				ended.getTime() - Date.parse(active.metadata.startedAt),
			),
			exitCode: Number.isSafeInteger(exitCode) ? exitCode : null,
			signal:
				typeof signal === 'number' && Number.isSafeInteger(signal) && signal > 0
					? signal
					: null,
			recordingState: lifecycle,
			bytesWritten: this.castSize(active.entry.castPath),
			castSize: this.castSize(active.entry.castPath),
		};
		try {
			this.storage.writeMetadata(active.entry, active.metadata);
		} catch {
			return this.markFailed(active);
		}
	...
	listRecordings(
		...
		return entries
			...
			.map((entry) => this.toListItem(entry.metadata, entry.castPath));
	}
	...
	searchRecordings(
		...
		return this.listRecordings(options);
	}
	...
	groupRecordings(
		...
		const entries = this.filteredEntries(options).slice(
			boundedOffset(options.offset),
			boundedOffset(options.offset) + boundedListLimit(options.limit),
		);
		const grouped = new Map<string, RecordingListItem[]>();
		for (const entry of entries) {
			const key =
				by === 'date'
					? entry.metadata.startedAt.slice(0, 10)
					: (entry.metadata.projectId ?? 'ungrouped');
		...
		return [...grouped.entries()].map(([key, items]) => ({
			key,
			label: by === 'project' && key === 'ungrouped' ? 'No project' : key,
			items,
		}));
	}

	readRecordingChunk(
		request: import('./types.js').RecordingChunkRequest,
	): RecordingChunk {
	...
	readChunk(
		...
		return this.readRecordingChunk(request);
	}
	...
	): AsyncGenerator<RecordingChunk> {
		let start = options.start ?? 0;
		for (;;) {
			if (options.signal?.aborted)
				throw new RecordingServiceError(
					'aborted',
					'Recording read was canceled.',
				);
			const chunk = this.readRecordingChunk({
				recordingId,
				start,
	...
	): RecordingReplayRange {
		const entry = this.resolveEntry(recordingId);
		const chunk = this.storage.readChunk(
			entry,
			{
				recordingId,
		...
		for (const line of chunk.content.split('\n')) {
			...
			try {
				...
				const event = {
					...
					code,
					data: value[2],
				} as import('./types.js').RecordingReplayEvent;
				if (time >= startTime && (endTime === null || time <= endTime))
					events.push(event);
				offset += utf8ByteLength(line) + 1;
			} catch {
				offset += utf8ByteLength(line) + 1;
	...
	deleteRecordingById(recordingId: string): void {
		const entry = this.resolveEntry(recordingId);
		if (
			this.active.get(entry.metadata.sessionId)?.entry.castPath ===
				entry.castPath ||
			entry.metadata.recordingState === 'recording'
		)
			throw new RecordingServiceError(
				'active_recording',
	...
	private appendData(
		...
		if (code === 'o' && SENSITIVE_OUTPUT_PATTERN.test(data))
			active.sensitiveInputUntilMs = this.options.now().getTime() + 120_000;
		...
		const pieces = splitUtf8(
			...
			Math.min(
				...
				Math.max(1, this.options.maxChunkBytes - 1024),
			),
		);
		for (const piece of pieces) {
			...
			const raw = Math.max(
				0,
				now - active.lastEventAtMs + active.roundingCarryMs,
			);
			...
			try {
				...
				active.metadata = {
					...active.metadata,
					eventCount: active.metadata.eventCount + 1,
					bytesWritten: active.metadata.bytesWritten + bytes,
					castSize: active.metadata.castSize + bytes,
				};
				this.persistMetadata(active);
				this.emitState(active);
			} catch {
				this.markFailed(active);
				return;
			}
		}
	}
	...
	private filterInput(active: ActiveRecording, data: string): string {
		if (this.options.now().getTime() > active.sensitiveInputUntilMs)
			return data;
		if (active.metadata.sensitiveInputPolicy === 'drop')
			return data.includes('\r') || data.includes('\n')
				? data.replace(/[^\r\n]/g, '')
				: '';
		return data.replace(/[ -~]/g, (character) =>
	...
	private markFailed(active: ActiveRecording): RecordingState {
		const state: RecordingState = {
			...
			recordingId: active.metadata.recordingId,
			status: 'failed',
			bytesWritten: active.metadata.bytesWritten,
			eventCount: active.metadata.eventCount,
			startedAt: active.metadata.startedAt,
			errorMessage: 'The recording could not be written.',
		};
		active.metadata = {
			...active.metadata,
			...
			durationMs: Math.max(
				0,
				this.options.now().getTime() - Date.parse(active.metadata.startedAt),
			),
		...
		try {
			...
			/* failure state remains in memory */
		}
	...
	private filteredEntries(
		options: RecordingFilter,
	): readonly RecordingStorageEntry[] {
		const entries = this.storage.list().filter((entry) => {
			...
			if (
				...
				return false;
			if (
				asLifecycle(options.state) !== undefined &&
				metadata.recordingState !== options.state
			)
				return false;
			if (
				options.inputPresent !== undefined &&
				metadata.capturedInput !== options.inputPresent
			)
				return false;
			if (
				options.startedAfter !== undefined &&
				Date.parse(metadata.startedAt) <
					Date.parse(asDate(options.startedAfter, new Date(0)).toISOString())
			)
				return false;
			if (
				options.startedBefore !== undefined &&
				Date.parse(metadata.startedAt) >
					Date.parse(asDate(options.startedBefore, new Date()).toISOString())
			)
				return false;
			return true;
		});
		return [...entries].sort(
			(left, right) =>
				Date.parse(right.metadata.startedAt) -
				Date.parse(left.metadata.startedAt),
		);
	}
	...
	private resolveEntry(recordingId: string): RecordingStorageEntry {
		if (
			...
			throw new RecordingServiceError('invalid_id', 'Recording id is invalid.');
		const entry = this.storage
			.list()
			.find((candidate) => candidate.metadata.recordingId === recordingId);
		if (entry === undefined)
			throw new RecordingServiceError('not_found', 'Recording does not exist.');
		return entry;
	}
	...
	private toListItem(
		...
		try {
			const stats = statSync(castPath) as unknown as {
	...
	private emitState(
		...
		this.emitStateValue(this.toState(active, status));
	}
	...
	private toState(
		active: ActiveRecording,
		status: RecordingState['status'] = active.metadata.recordingState ===
		'failed'
			? 'failed'
			: 'recording',
	): RecordingState {
		return {
			...
			eventCount: active.metadata.eventCount,
			startedAt: active.metadata.startedAt,
			errorMessage: active.metadata.errorMessage,
		};
	}
	private resolveInputSetting(): boolean {
		const value = this.options.captureInput;
		return typeof value === 'function' ? value() : value === true;
	}
	private resolveSensitivePolicy(): SensitiveInputPolicy {
		const value = this.options.sensitiveInputPolicy;
	...
	private resolveDefaultRecording(): boolean {
		const value = this.options.defaultRecordNewTerminals;
		return typeof value === 'function' ? value() : value === true;
	}
	private validateSessionId(sessionId: string): void {
		if (
			...
			sessionId.length === 0 ||
			utf8ByteLength(sessionId) > MAX_SESSION_ID_BYTES ||
...
function boundedListLimit(value: number | undefined): number {
	...
	if (!Number.isSafeInteger(value) || value < 1 || value > 200)
		throw new RangeError('recording list limit is invalid');
	return value;
}
function boundedOffset(value: number | undefined): number {
	if (value === undefined) return 0;
	if (!Number.isSafeInteger(value) || value < 0)
		throw new RangeError('recording list offset is invalid');
	return value;
}
...
function baseName(value: string | null): string | null {
	return value === null || value.length === 0 ? null : path.basename(value);
}
