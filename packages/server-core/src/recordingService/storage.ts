...
interface StorageOptions {
	...
	readonly migrateStoredMetadata?: (
...
function normalizeMetadata(
	...
	const recordingId =
		typeof value.recordingId === 'string' ? value.recordingId : fallbackId;
	if (recordingId === undefined || !RECORDING_ID_PATTERN.test(recordingId))
		return null;
	const state =
		value.recordingState === 'recording' ||
		value.recordingState === 'completed' ||
		value.recordingState === 'interrupted' ||
		value.recordingState === 'failed'
	...
	const endedAt =
		...
		!Number.isNaN(Date.parse(value.endedAt))
			? new Date(value.endedAt).toISOString()
			: null;
	const relativeCastPath =
		typeof value.relativeCastPath === 'string' &&
		!path.isAbsolute(value.relativeCastPath)
			? value.relativeCastPath
			: `${recordingId}.cast`;
	const candidateCast = path.resolve(root, relativeCastPath);
	if (
		!isWithin(path.resolve(root), candidateCast) ||
		!candidateCast.endsWith('.cast')
	)
		return null;
	const castSize = safeInteger(value.castSize ?? value.bytesWritten, 0);
	const inputPolicy =
		value.inputPolicy === 'record-with-sensitive-filter'
	...
	return {
		...
		title:
			safeString(value.title, 'Terminal Recording') ?? 'Terminal Recording',
		...
		finalCols:
			value.finalCols === null
				? null
				: Math.max(
						2,
						safeInteger(value.finalCols, safeInteger(value.cols, 80, 2), 2),
					),
		finalRows:
			value.finalRows === null
				? null
				: Math.max(
						1,
						safeInteger(value.finalRows, safeInteger(value.rows, 24, 1), 1),
					),
		startedAt,
		endedAt,
		durationMs:
		...
		errorMessage:
			value.errorMessage === null
				? null
				: value.errorMessage === 'The recording could not be written.'
					? value.errorMessage
					: null,
	};
}
...
function metadataFromCast(
	...
	try {
		stat = statSync(castPath);
	} catch {
		throw new RecordingServiceError(
			'not_found',
			'Recording cast does not exist.',
		);
	}
	const id = path.basename(castPath, '.cast');
	const header = readHeader(castPath);
	const term =
		typeof header?.term === 'object' && header.term !== null
			? (header.term as Record<string, unknown>)
			: {};
	const timestamp =
	...
	return {
		...
		title:
			typeof header?.title === 'string'
				? header.title.slice(0, 512)
				: 'Incomplete terminal recording',
		note: null,
		color: null,
		emoji: null,
		cwd: null,
...
export class NodeRecordingStorage implements RecordingStorage {
	...
	private readonly migrateStoredMetadata:
	...
	constructor(options: StorageOptions) {
		...
		this.indexPath =
			options.libraryIndexPath ??
			path.join(options.homeDirectory, '.terminay', 'recording-roots.json');
	...
	registerRoot(rawRoot: string, create = true): string {
		...
		const canonical = existsSync(canonicalCandidate)
	...
	describeRoot(rawRoot: string, create = false): RecordingRootReference {
		...
		try {
			...
			/* retain the root reference while a volume is unavailable */
		}
		const recordingIds = new Set<string>();
		if (available) {
			for (const entry of this.list()) {
				if (isWithin(canonical, path.resolve(entry.castPath)))
					recordingIds.add(entry.metadata.recordingId);
			}
		}
		return {
			rootId: opaqueRootId(canonical),
			available,
	...
	createRecording(
		metadata: RecordingMetadata,
		header: string,
	): RecordingStorageEntry {
		if (!RECORDING_ID_PATTERN.test(metadata.recordingId))
		...
		const directory = path.join(
			root,
			/^\d{4}-\d{2}-\d{2}$/.test(date) ? date : 'unknown-date',
		);
		...
		if (existsSync(castPath) || existsSync(metadataPath))
			throw new RecordingServiceError(
				'storage_failed',
		...
		try {
			...
			throw new RecordingServiceError(
				...
				error instanceof Error
					? error.message
					: 'Unable to create recording cast.',
			);
		}
	...
	append(entry: RecordingStorageEntry, text: string): void {
		...
		try {
			...
			throw new RecordingServiceError(
				'storage_failed',
				error instanceof Error ? error.message : 'Unable to append recording.',
			);
		} finally {
			if (descriptor !== undefined) closeSync(descriptor);
		}
		...
		try {
			...
			const metadata =
				parsed === null
					...
					: normalizeMetadata(
							parsed,
							this.authorizedRoot(castPath),
							path.basename(castPath, '.cast'),
						);
			if (metadata === null || this.migrateStoredMetadata === undefined)
				return metadata;
			const migrated = this.migrateStoredMetadata(metadata);
			if (migrated !== metadata)
				this.writeMetadata({ metadata, castPath }, migrated);
			return migrated;
		} catch {
			return null;
		}
		...
		for (const root of this.rootSet) {
			...
			for (const filePath of this.walk(root)) {
				...
				if (metadata === null) {
					...
					try {
						this.writeMetadata({ metadata, castPath: filePath }, metadata);
					} catch {
						/* read-only recovery remains usable */
					}
				}
				const canonical = this.authorizeCastPath(filePath);
				if (!found.has(metadata.recordingId))
					found.set(metadata.recordingId, { metadata, castPath: canonical });
			}
			for (const filePath of this.walk(root)) {
				...
				try {
					...
					const castPath = path.join(
						path.dirname(filePath),
						`${path.basename(filePath, '.json')}.cast`,
					);
						...
						if (metadata !== null && !found.has(metadata.recordingId)) {
							const unavailable =
								metadata.recordingState === 'recording'
									? {
											...metadata,
											recordingState: 'interrupted' as const,
											endedAt: metadata.endedAt ?? new Date().toISOString(),
										}
									: metadata;
							found.set(metadata.recordingId, {
					...
					/* malformed sidecars do not take down the timeline */
				}
			}
		}
		return [...found.values()];
	...
	readChunk(
		...
		if (!Number.isSafeInteger(start) || start < 0)
			throw new RecordingServiceError(
				'invalid_offset',
				'Recording chunk offset is invalid.',
			);
		if (
			...
			throw new RecordingServiceError(
				'chunk_limit',
				`Recording chunk size must be between 1 and ${maxChunkBytes} bytes.`,
			);
		const castPath = this.authorizeCastPath(entry.castPath);
		let descriptor: number | undefined;
		try {
			...
			if (start > totalSize)
				throw new RecordingServiceError(
					'invalid_offset',
					'Recording chunk offset is beyond the end.',
				);
			if (start > 0) {
				const preceding = Buffer.allocUnsafe(1);
				const result = readSync(descriptor, preceding, 0, 1, start - 1);
				if (result !== 1 || preceding[0] !== 0x0a)
					throw new RecordingServiceError(
						'invalid_offset',
						'Recording chunk offset is not a record boundary.',
					);
			}
			if (start === totalSize)
				return {
			...
			if (newline < 0) {
				if (start + count < totalSize)
					throw new RecordingServiceError(
						'chunk_limit',
						`An asciicast record exceeds the ${maxBytes} byte chunk limit.`,
					);
			...
			let content: string;
			try {
				content = new TextDecoder('utf-8', { fatal: true }).decode(complete);
			} catch {
				throw new RecordingServiceError(
					'malformed_recording',
	...
	delete(entry: RecordingStorageEntry): void {
		...
		try {
			...
			throw new RecordingServiceError(
				'storage_failed',
				error instanceof Error ? error.message : 'Unable to delete recording.',
			);
		}
	}
	...
	recover(now: Date): void {
		for (const entry of this.list()) {
			...
			const metadata: RecordingMetadata = {
				...
				durationMs: Math.max(
					0,
					now.getTime() - Date.parse(entry.metadata.startedAt),
				),
				castSize: this.castSize(entry.castPath),
				bytesWritten: this.castSize(entry.castPath),
			};
			try {
				this.writeMetadata(entry, metadata);
			} catch {
				/* remain discoverable with the last valid sidecar */
			}
		}
	}

	close(): void {
		/* writes are synchronous; kept for lifecycle composition */
	}
	...
	private rootForNewRecording(): string {
		...
		if (first === undefined)
			throw new RecordingServiceError(
				'invalid_root',
				'No recording root is configured.',
			);
		return first;
	}
	...
	private expandRoot(rawRoot: string, homeDirectory: string): string {
		...
		return path.isAbsolute(trimmed)
			? trimmed
			: path.join(homeDirectory, trimmed);
	}
	...
	private persistRoots(): void {
		try {
			atomicJsonWrite(this.indexPath, { version: 1, roots: [...this.rootSet] });
		} catch {
			/* recording itself remains usable */
		}
	}
	...
	private walk(root: string): string[] {
		...
		for (const entry of entries) {
			...
			if (entry.isDirectory()) output.push(...this.walk(entryPath));
				...
				entry.isFile() &&
				(entry.name.endsWith('.cast') || entry.name.endsWith('.json'))
			)
	...
	private authorizedRoot(castPath: string): string {
		...
		const root = [...this.rootSet].find((candidate) =>
			isWithin(path.resolve(candidate), resolved),
		);
		if (root === undefined)
			throw new RecordingServiceError(
				'path_escape',
				'Recording file is outside the recordings library.',
			);
		return path.resolve(root);
	}

	private authorizeCastPath(castPath: string): string {
		if (typeof castPath !== 'string' || !castPath.endsWith('.cast'))
			throw new RecordingServiceError(
				'path_escape',
				'Recording file is not authorized.',
			);
