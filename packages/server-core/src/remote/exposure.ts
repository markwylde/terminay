...
export class RemoteExposureController {
	...
	constructor(options: RemoteExposureControllerOptions) {
		...
		this.defaultLifetimeMs = positive(
			options.defaultLifetimeMs ?? 5 * 60 * 1000,
			'defaultLifetimeMs',
		);
		this.audit =
			options.audit ??
			new RemoteAuditLog({ serverId: this.manager.serverId, now: this.now });
		this.pairingRateLimiter =
			options.pairingRateLimiter ?? new RemoteRateLimiter({ now: this.now });
		if (
			this.manager.serverId !== this.pairing.serverId ||
			this.manager.sessionOrigin !== this.pairing.sessionOrigin
		) {
			throw new TypeError(
				'remote exposure identity does not match pairing identity',
			);
		}
	}
	...
	get status(): RemoteExposureStatus {
		...
		const activePairings = this.pairing
			.list()
			.filter((room) => room.state === 'active');
		const pairing = activePairings.sort(
			(left, right) => right.expiresAt - left.expiresAt,
		)[0];
	...
	rotate(
		expiresAt = this.now() + this.defaultLifetimeMs,
	...
	createPairing(
		expiresAt = this.manager.exposure.expiresAt,
		rotate = false,
	): RemotePairingHandoff {
		if (this.manager.exposure.state !== 'exposed' || expiresAt === undefined)
			throw new Error('remote exposure is not active');
		if (expiresAt > (this.manager.exposure.expiresAt ?? expiresAt))
			throw new RangeError('pairing expiry exceeds exposure expiry');
		const room = rotate
			? this.pairing.rotate(expiresAt)
			: this.pairing.create(expiresAt);
