...
export type RemotePairingRoomState = "active" | "consumed" | "expired" | "locked" | "rotated";
...
export class RemotePairingStore {
  ...
  constructor(private readonly options: RemotePairingStoreOptions) {
    if (!validId(options.serverId) || !validOrigin(options.sessionOrigin)) throw new TypeError("pairing server identity/origin is invalid");
    this.now = options.now ?? (() => Date.now());
    this.maxRooms = positive(options.maxRooms ?? DEFAULT_MAX_ROOMS, "maxRooms");
    this.maxFailedAttempts = positive(options.maxFailedAttempts ?? DEFAULT_MAX_FAILED_ATTEMPTS, "maxFailedAttempts");
    this.defaultLifetimeMs = positive(options.defaultLifetimeMs ?? DEFAULT_LIFETIME_MS, "defaultLifetimeMs");
    this.maxLifetimeMs = positive(options.maxLifetimeMs ?? DEFAULT_MAX_LIFETIME_MS, "maxLifetimeMs");
    if (this.defaultLifetimeMs > this.maxLifetimeMs) throw new RangeError("defaultLifetimeMs cannot exceed maxLifetimeMs");
    this.randomBytes = options.randomBytes ?? ((size) => new Uint8Array(nodeRandomBytes(size)));
  }
  ...
  create(expiresAt = this.now() + this.defaultLifetimeMs): RemotePairingRoom {
    ...
    if (this.rooms.size >= this.maxRooms) throw new Error("pairing room limit reached");
    ...
    return Object.freeze({ roomId, serverId: state.serverId, sessionOrigin: state.sessionOrigin, expiresAt, secret });
  }
  ...
  rotate(expiresAt = this.now() + this.defaultLifetimeMs): RemotePairingRoom {
    this.validateExpiry(expiresAt);
    this.expireRooms();
    for (const room of this.rooms.values()) if (room.state === "active") room.state = "rotated";
    return this.create(expiresAt);
  }

  consume(attempt: RemotePairingAttempt): RemotePairingAdmission {
    this.expireRooms();
    if (!validId(attempt.roomId) || attempt.serverId !== this.options.serverId || attempt.sessionOrigin !== this.options.sessionOrigin) throw new Error("pairing room is unavailable");
    const room = this.rooms.get(attempt.roomId);
    if (room === undefined || room.state !== "active") throw new Error("pairing room is unavailable");
    if (typeof attempt.secret !== "string" || attempt.secret.length > 256) throw new Error("pairing secret is invalid");
    ...
    room.state = "consumed";
    return Object.freeze({ roomId: room.roomId, serverId: room.serverId, sessionOrigin: room.sessionOrigin, consumedAt: this.now() });
  }
  ...
  list(): readonly RemotePairingRoomMetadata[] {
    this.expireRooms();
    return Object.freeze([...this.rooms.values()].map((room) => this.snapshot(room)));
  }

  /** Disable active rooms without retaining their one-time secrets. */
  disable(): void {
    this.expireRooms();
    for (const room of this.rooms.values()) if (room.state === "active") room.state = "expired";
  }

  private validateExpiry(expiresAt: number): void {
    const now = this.now();
    if (!Number.isSafeInteger(expiresAt) || expiresAt <= now || expiresAt - now > this.maxLifetimeMs) throw new RangeError("pairing room expiry is invalid");
  }

  private expireRooms(): void {
    const now = this.now();
    for (const room of this.rooms.values()) if (room.state === "active" && room.expiresAt <= now) room.state = "expired";
  }

  private pruneTerminalRooms(): void {
    this.expireRooms();
    for (const [roomId, room] of this.rooms) if (room.state !== "active") this.rooms.delete(roomId);
  }

  private snapshot(room: PairingRoomState): RemotePairingRoomMetadata {
    return Object.freeze({ roomId: room.roomId, serverId: room.serverId, sessionOrigin: room.sessionOrigin, expiresAt: room.expiresAt, state: room.state, failedAttempts: room.failedAttempts });
  }

  private entropy(size: number): Uint8Array {
    const bytes = this.randomBytes(size);
    if (!(bytes instanceof Uint8Array) || bytes.byteLength !== size) throw new TypeError("pairing entropy generator returned an invalid length");
    return bytes;
  }
}
...
function toToken(bytes: Uint8Array): string { return Buffer.from(bytes).toString("base64url"); }
...
function validOrigin(value: string): boolean {
  try {
    ...
    const allowedProtocol = url.protocol === "https:" || (url.protocol === "http:" && (url.hostname === "127.0.0.1" || url.hostname === "localhost"));
