...
export type RemoteReconnectGrantLifetime = "1h" | "24h" | "7d" | "until-revoked";
export type RemoteReconnectGrantStatus = "none" | "valid" | "expired" | "revoked";
...
const LIFETIME_MS: Readonly<Record<Exclude<RemoteReconnectGrantLifetime, "until-revoked">, number>> = {
	"1h": 60 * 60 * 1000,
	"24h": 24 * 60 * 60 * 1000,
	"7d": 7 * 24 * 60 * 60 * 1000,
};
...
export class RemoteReconnectGrantStore {
	...
	issue(options: {
		...
		if (!validId(options.deviceId)) throw new TypeError("reconnect device identity is invalid");
		this.prune();
	...
	rotate(options: {
		...
		if (options.deviceId !== undefined && options.deviceId !== existing.deviceId)
	...
	createChallenge(options: {
		...
		readonly origin: string;
		readonly clientNonce: string;
	}): { readonly challenge: RemoteReconnectChallenge; readonly signingInput: string } {
		const record = this.requireByHandle(options.handle, options.origin);
		if (!validNonce(options.clientNonce)) throw new TypeError("reconnect client nonce is invalid");
		this.prune();
	...
	verifyProof(options: {
		...
		readonly verifyDeviceProof?: (deviceId: ProtocolId, signingInput: string) => boolean;
	}): RemoteReconnectGrantRecord {
		this.prune();
		const pending = this.challenges.get(options.attemptId);
		if (pending === undefined) throw new Error("reconnect challenge is unavailable");
		...
		const expected = createHmac("sha256", Buffer.from(record.proofVerifier, "base64url"))
			.update(pending.signingInput)
			.digest("base64url");
		if (!equalToken(expected, options.proof)) throw new Error("reconnect proof is invalid");
		if (options.verifyDeviceProof !== undefined && !options.verifyDeviceProof(record.deviceId, pending.signingInput))
	...
	revokeDevice(deviceId: ProtocolId): number {
		if (!validId(deviceId)) throw new TypeError("reconnect device identity is invalid");
	...
	summary(deviceId: ProtocolId): RemoteReconnectGrantSummary {
		...
		if (record === undefined)
			return Object.freeze({ deviceId, handle: null, expiresAt: null, lastUsedAt: null, status: "none" });
		return Object.freeze({
			deviceId,
	...
	private requireByHandle(handle: string, origin: string): RemoteReconnectGrantRecord {
		if (!validToken(handle) || origin !== this.options.sessionOrigin)
			throw new Error("reconnect grant is unavailable for this origin");
		this.prune();
		const record = [...this.grants.values()].find((candidate) => candidate.handle === handle);
		if (record === undefined || !this.isUsable(record)) throw new Error("reconnect grant is no longer valid");
		return record;
	}

	private isUsable(record: RemoteReconnectGrantRecord): boolean {
		return record.revokedAt === null && (record.expiresAt === null || record.expiresAt > this.now());
	}
	...
	private entropy(size: number): Uint8Array {
		...
		if (!(bytes instanceof Uint8Array) || bytes.byteLength !== size)
			throw new TypeError("reconnect entropy generator returned an invalid length");
		return bytes;
	}
...
export function serializeRemoteReconnectChallenge(challenge: RemoteReconnectChallenge): string {
...
export function createRemoteReconnectProof(grant: string, signingInput: string): string {
...
function resolveExpiry(now: number, lifetime: RemoteReconnectGrantLifetime | string | null | undefined): number | null {
	if (lifetime === "until-revoked") return null;
	const selected = lifetime === "1h" || lifetime === "7d" || lifetime === "24h" ? lifetime : "24h";
	return now + LIFETIME_MS[selected];
}

function statusOf(record: RemoteReconnectGrantRecord, now: number): RemoteReconnectGrantStatus {
	if (record.revokedAt !== null) return "revoked";
	if (record.expiresAt !== null && record.expiresAt <= now) return "expired";
	return "valid";
}

function snapshotRecord(record: RemoteReconnectGrantRecord): RemoteReconnectGrantRecord {
...
function validOrigin(value: string): boolean {
	try {
		...
		return protocolOk && !url.username && !url.password && !url.search && !url.hash;
	} catch {
		return false;
	}
}
