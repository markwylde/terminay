...
export type RemoteTrafficChannel =
	| 'control'
	| 'application'
	| 'terminal'
	| 'assets';
...
interface PeerState {
	...
	readonly connectedAt: number;
	lastSeenAt: number;
	readonly queues: Record<RemoteTrafficChannel, QueueState>;
}
...
export class RemoteConnectionManager {
	...
	constructor(private readonly options: RemoteConnectionManagerOptions) {
		...
		this.maxAssetQueuedBytes = positive(
			options.maxAssetQueuedBytes ?? 16 * 1024 * 1024,
			'maxAssetQueuedBytes',
		);
		this.maxUsedTickets = positive(
			options.maxUsedTickets ?? 4096,
			'maxUsedTickets',
		);
	}
	...
	expose(expiresAt: number): RemoteExposure {
		const now = this.requireCurrentTime();
		if (!Number.isSafeInteger(expiresAt) || expiresAt <= now)
			throw new RangeError('exposure expiry is invalid');
	...
	/**
	 * Reject a replayed or currently unadmittable ticket before callers perform
	 * expensive device-key or approval verification. This is intentionally an
	 * assertion rather than a reservation: `admit` remains the atomic
	 * consume-and-connect operation, so concurrent handshakes cannot both use a
	 * ticket.
	 */
	assertTicketAvailable(ticketId: ProtocolId): void {
		this.pruneUsedTickets();
		if (!validId(ticketId))
			throw new Error('remote ticket is invalid or expired');
		if (this.usedTickets.has(ticketId))
			throw new Error('remote ticket has already been used');
		if (this.usedTickets.size >= this.maxUsedTickets)
			throw new Error('remote ticket replay ledger is full');
	}
	...
	admit(proof: RemoteAuthProof): RemotePeerSnapshot {
		...
		this.expireExposureIfNeeded();
		if (
			this.exposureValue.state !== 'exposed' ||
			this.exposureValue.expiresAt === undefined
		)
			throw new Error('remote exposure is unavailable');
		if (!proof.authenticated)
			throw new Error('remote peer is not authenticated');
		if (
			proof.serverId !== this.options.serverId ||
			proof.sessionOrigin !== this.options.sessionOrigin
		)
			throw new Error('remote peer identity mismatch');
		if (
			!validId(proof.deviceId) ||
			!validId(proof.ticketId) ||
			!Number.isSafeInteger(proof.expiresAt) ||
			proof.expiresAt <= now
		)
			throw new Error('remote ticket is invalid or expired');
		if (this.revokedDevices.has(proof.deviceId))
			throw new Error('remote device is revoked');
		this.assertTicketAvailable(proof.ticketId);
		// A revoked peer remains observable until the next independent admission so
		// callers can report the terminal state. It must not, however, turn into an
		// unbounded historical ledger when devices subsequently connect again.
		this.pruneTerminalPeers();
		const connectedPeers = [...this.peers.values()].filter(
			(peer) => peer.state === 'connected',
		).length;
		if (connectedPeers >= this.maxPeers)
			throw new Error('remote peer limit reached');
	...
	send(
		...
		if (!(bytes instanceof Uint8Array) || bytes.byteLength > this.maxFrameBytes)
			throw new RangeError('remote frame exceeds limit');
		const queue = peer.queues[channel];
		const limit =
			channel === 'assets' ? this.maxAssetQueuedBytes : this.maxQueuedBytes;
		if (queue.bytes + bytes.byteLength > limit)
			throw new Error(`remote ${channel} queue limit reached`);
	...
	drain(
		...
		if (
			...
			throw new RangeError('drain limit is invalid');
		const queue = peer.queues[channel];
	...
	snapshot(): RemoteTransportSnapshot {
		return Object.freeze({
			...
			peers: Object.freeze(
				[...this.peers.values()].map((peer) => this.snapshotPeer(peer)),
			),
		});
	}

	/** Reclaim expired ticket replay entries and refresh exposure state. */
	cleanup(): number {
		const before = this.usedTickets.size;
		this.expireExposureIfNeeded();
		this.pruneUsedTickets();
		return before - this.usedTickets.size;
	}
	...
	private snapshotPeer(peer: PeerState): RemotePeerSnapshot {
		return Object.freeze({
			...
			queuedBytes: Object.values(peer.queues).reduce(
				(total, queue) => total + queue.bytes,
				0,
			),
		});
	}
	...
	private requireConnectedPeer(peerId: ProtocolId): PeerState {
		...
		if (peer.state !== 'connected')
			throw new Error('remote peer is not connected');
		return peer;
	}
	...
	private pruneUsedTickets(): void {
		...
		for (const [ticketId, expiresAt] of this.usedTickets)
			if (expiresAt <= now) this.usedTickets.delete(ticketId);
	}

	private pruneTerminalPeers(): void {
		for (const [peerId, peer] of this.peers) {
			if (peer.state !== 'connected') this.peers.delete(peerId);
		}
	}

	private nextId(): string {
		this.sequence += 1;
		return `${this.requireCurrentTime().toString(36)}-${this.sequence.toString(36)}`;
	}
	...
	private requireCurrentTime(): number {
		const now = this.currentTime();
		if (now === undefined) throw new Error('remote runtime clock is invalid');
		return now;
	}
}
...
function createQueues(): Record<RemoteTrafficChannel, QueueState> {
	return {
		control: { chunks: [], bytes: 0 },
		application: { chunks: [], bytes: 0 },
		terminal: { chunks: [], bytes: 0 },
		assets: { chunks: [], bytes: 0 },
	};
}

function clearQueues(peer: PeerState): void {
	for (const queue of Object.values(peer.queues)) {
