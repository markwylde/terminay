...
export interface ServerRuntimeHooks {
	readonly startServices?: (
		config: ServerRuntimeConfig,
		services?: ServerRuntimeServices,
	) => void | Promise<void>;
	readonly stopServices?: (
		deadline: number,
		services?: ServerRuntimeServices,
	) => void | Promise<void>;
}
...
export class ServerRuntime {
	...
	constructor(
		config: ServerRuntimeConfig,
		private readonly hooks: ServerRuntimeHooks = {},
	) {
		if (!/^[-A-Za-z0-9._:]{1,128}$/.test(config.serverId))
			throw new TypeError('invalid server id');
		if (config.dataRoot.length === 0 || config.dataRoot.length > 4096)
			throw new TypeError('invalid data root');
		if (
			config.shutdownTimeoutMs !== undefined &&
			(!Number.isSafeInteger(config.shutdownTimeoutMs) ||
				config.shutdownTimeoutMs < 0)
		)
			throw new RangeError('invalid shutdown timeout');
		const platformPaths =
			config.platformPaths === undefined
				? undefined
				: validateServerPlatformPaths(config.platformPaths, config.dataRoot);
		const normalizedConfig: ServerRuntimeConfig =
			platformPaths === undefined ? config : { ...config, platformPaths };
		this.services = Object.freeze(
			normalizedConfig.services === undefined
				? {}
	...
	get state(): RuntimePhase {
		return this.phase;
	}
	async start(): Promise<RuntimeHealth> {
		if (this.phase === 'ready') return this.health();
		if (this.phase === 'starting' && this.startPromise !== undefined)
			return this.startPromise;
		if (this.phase !== 'created')
			throw new Error(`server runtime is ${this.phase}`);
		this.phase = 'starting';
			...
			try {
				...
				try {
					...
					throw new AggregateError(
						[error, lockError],
						'server startup and vault fencing failed',
					);
				}
				throw error;
			}
	...
	async stop(): Promise<void> {
		...
		this.phase = 'stopping';
		const deadline = Date.now() + (this.config.shutdownTimeoutMs ?? 5_000);
		const stopWork = (async (): Promise<void> => {
			...
			if (failures.length > 0)
				throw cleanupFailure('server runtime shutdown failed', failures);
		})();
	...
	health(): RuntimeHealth {
		return {
			...
			uptimeMs:
				this.startedAt === 0 ? 0 : Math.max(0, Date.now() - this.startedAt),
		};
	}
		...
		const remoteExposure =
			remoteStatus === undefined
				...
				: {
						...
						expiresAt:
							remoteStatus.exposure.expiresAt === undefined
								? null
								: new Date(remoteStatus.exposure.expiresAt).toISOString(),
						connectedPeers: remoteStatus.peers.filter(
		...
		const terminal =
			terminalSessions === undefined
				...
				: {
						...
						runningSessions: terminalSessions.filter(
	...
	withSecret<T>(
		...
		if (vault === undefined)
			return Promise.reject(new Error('server vault is unavailable'));
...
function readSettingsRevision(
	repository: ServerSettingsRepository | undefined,
): number | undefined {
