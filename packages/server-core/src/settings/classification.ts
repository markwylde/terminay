 ...
 */
export function resolveServerSettingsForDevice(
	...
	for (const [path, value] of flattened) {
		if (
			classifySetting(path) !== 'device-override' ||
			!DEVICE_OVERRIDE_PATHS.has(normalizePath(path))
		)
			continue;
...
function partitionValue(
	...
	else if (authority === 'connection-host')
		result.connectionHost[key] = cloneJson(value);
	else if (authority === 'device-override')
		result.deviceOverrides[key] = cloneJson(value);
...
function partitionNested(
	...
	if (authority === parentAuthority) target[key] = cloneJson(value);
		...
		result.deviceOverrides[path] = cloneJson(value);
	else if (authority === 'connection-host')
		result.connectionHost[path] = cloneJson(value);
...
function isSecretKey(key: string): boolean {
	return /(?:secret|token|password|passphrase|credential|privatekey|api[_-]?key|plaintext|pairingpinhash)/i.test(
		key,
	);
}
