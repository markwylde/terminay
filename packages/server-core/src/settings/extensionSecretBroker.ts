...
export class ExtensionSecretBroker {
	...
	replaceBindings(bindings: readonly ExtensionSecretBinding[]): void {
		if (!Array.isArray(bindings))
			throw new TypeError('secret bindings must be an array');
		...
		for (const candidate of bindings) {
			...
			if (next.has(key))
				throw new TypeError('duplicate extension secret binding');
	...
	upsertBinding(binding: ExtensionSecretBinding): void {
	...
	removeBinding(
		extensionId: ProtocolId,
		profileId: ProtocolId,
		fieldId: ProtocolId,
	): void {
		this.bindings.delete(bindingKey(extensionId, profileId, fieldId));
	}
	...
	async withSecret<T>(
		...
		const binding = this.bindings.get(
			bindingKey(principal.extensionId, request.profileId, request.fieldId),
		);
		if (binding === undefined) throw denied();
		let authorized = false;
		try {
			authorized = await this.authorize(
				principal,
				Object.freeze({
...
function normalizeBinding(
	value: ExtensionSecretBinding,
): ExtensionSecretBinding {
	if (!value || typeof value !== 'object')
		throw new TypeError('secret binding is required');
...
function normalizePrincipal(
	value: ExtensionSecretPrincipal,
): ExtensionSecretPrincipal {
	if (
		!value ||
		typeof value !== 'object' ||
		!(value.permissions instanceof Set)
	) {
	...
	return Object.freeze({
		...
		...(value.sessionId === undefined
			? {}
			: { sessionId: normalizeId(value.sessionId, 'session id') }),
	});
}

function normalizeRequest(
	value: ExtensionSecretRequest,
): ExtensionSecretRequest {
	if (!value || typeof value !== 'object')
		throw new TypeError('extension secret request is invalid');
	return Object.freeze({
		profileId: normalizeId(value.profileId, 'profile id'),
		fieldId: normalizeId(value.fieldId, 'field id'),
	});
...
function denied(): ExtensionSecretBrokerError {
	return new ExtensionSecretBrokerError(
		'denied',
		'extension secret access is denied',
	);
}
...
function mapVaultFailure(error: unknown): ExtensionSecretBrokerError {
	...
	if (code === 'locked')
		return new ExtensionSecretBrokerError('locked', 'server vault is locked');
	if (code === 'unavailable')
		return new ExtensionSecretBrokerError(
			'unavailable',
	...
	if (code === 'missing')
		return new ExtensionSecretBrokerError(
			'missing',
			'extension secret is not configured',
		);
	return new ExtensionSecretBrokerError(
		'failed',
		'extension secret resolution failed',
	);
}
