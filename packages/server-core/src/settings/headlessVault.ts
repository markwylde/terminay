...
export class FileVaultEnvelopeStorage implements VaultEnvelopeStorage {
	...
	async write(serialized: string): Promise<void> {
		if (
			...
			Buffer.byteLength(serialized) > HEADLESS_VAULT_MAX_ENVELOPE_BYTES
		) {
			throw new Error('vault envelope exceeds its size limit');
		}
...
export class HeadlessPassphraseVaultAdapter implements SecretVaultAdapter {
	...
	static async open(
		options: HeadlessPassphraseVaultOptions,
	): Promise<HeadlessPassphraseVaultAdapter> {
		if (!options || typeof options !== 'object' || !options.storage)
			throw new TypeError('vault storage is required');
		...
		if (serialized === undefined) {
			try {
				...
				const adapter = new HeadlessPassphraseVaultAdapter(
					options.serverId,
					options.storage,
					undefined,
					now,
		...
		return new HeadlessPassphraseVaultAdapter(
			options.serverId,
			options.storage,
			serialized,
			now,
	...
	list(): readonly SecretReference[] {
		...
		return Object.freeze(
			this.envelope.entries.map((entry) =>
				Object.freeze({
					id: entry.id,
					configured: true,
	...
	async unlock(request: { readonly secret: Uint8Array }): Promise<void> {
		...
		if (!(request?.secret instanceof Uint8Array))
			throw new TypeError('unlock secret must be bytes');
		if (
			request.secret.byteLength < HEADLESS_VAULT_MIN_PASSPHRASE_BYTES ||
			request.secret.byteLength > MAX_VAULT_UNLOCK_BYTES
		) {
			...
			if (!this.envelope) {
				...
				try {
					...
					const wrapped = encryptAesGcm(
						dataKey,
						candidateProtectorKey,
						keyAad(this.serverId),
					);
				...
				const salt = decodeBase64(
					this.envelope.keyEnvelope.salt,
					SCRYPT.saltBytes,
				);
				try {
					candidateProtectorKey = await deriveKey(passphrase, salt);
					candidateKey = decryptAesGcm(
	...
	async rotate(): Promise<void> {
		...
		if (!protectorKey || !this.envelope)
			throw new HeadlessVaultError('vault is locked');
		if (this.envelope.revision >= HEADLESS_VAULT_MAX_REVISION)
			throw new HeadlessVaultError('vault revision limit reached');
		...
		try {
			...
			for (const entry of this.envelope.entries) {
				...
				try {
					plaintext = decryptAesGcm(
						entry,
						currentKey,
						entryAad(this.serverId, entry.id),
					);
					entries.push({
						id: entry.id,
						label: entry.label,
			...
			nextKey.fill(0);
			throw error instanceof HeadlessVaultError
				? error
				: new HeadlessVaultError();
		}
	}
	...
	private async writeEntry(
		...
		const key = this.requireKey();
		if (typeof input?.id !== 'string' || !VAULT_ID_PATTERN.test(input.id))
			throw new TypeError('vault secret id is invalid');
		if (
			!(input.value instanceof Uint8Array) ||
			input.value.byteLength > MAX_VAULT_SECRET_BYTES
		)
			throw new RangeError('secret exceeds the limit');
		const current = this.envelope;
		if (!current) throw new HeadlessVaultError('vault is locked');
		const index = current.entries.findIndex((entry) => entry.id === input.id);
		if (!replace && index >= 0)
			throw new HeadlessVaultError('secret already exists');
		if (replace && index < 0) throw new HeadlessVaultError('missing secret');
		const label = input.label ?? '';
		if (
			typeof label !== 'string' ||
			textEncoder.encode(label).byteLength > MAX_VAULT_LABEL_BYTES
		)
			throw new RangeError('secret label exceeds the limit');
		...
		try {
			...
			const entry: EnvelopeEntry = {
				...
				...encryptAesGcm(
					plaintext,
					key,
					entryAad(this.serverId, input.id),
					usedNonces,
				),
			};
			const entries = current.entries.slice();
			if (replace) entries[index] = entry;
				...
				if (entries.length >= HEADLESS_VAULT_MAX_ENTRIES)
					throw new RangeError('vault entry count exceeds the limit');
				entries.push(entry);
			}
			const candidate = createEnvelope(
				this.serverId,
	...
	private async persist(candidate: VaultEnvelope): Promise<void> {
		...
		if (Buffer.byteLength(serialized) > HEADLESS_VAULT_MAX_ENVELOPE_BYTES)
			throw new HeadlessVaultError('vault envelope exceeds its size limit');
		try {
...
function deriveKey(passphrase: Buffer, salt: Buffer): Promise<Buffer> {
	return scryptAsync(passphrase, salt, SCRYPT.keyLength, {
		N: SCRYPT.N,
		r: SCRYPT.r,
		p: SCRYPT.p,
...
function entryAad(serverId: string, id: string): Uint8Array {
	return textEncoder.encode(
		JSON.stringify(['terminay-vault-entry', 1, serverId, id]),
	);
}

function keyAad(serverId: string): Uint8Array {
	return textEncoder.encode(
		JSON.stringify(['terminay-vault-data-key', 1, serverId]),
	);
}
...
function createEnvelope(
	...
	revision: number,
	_usedNonces?: Set<string>,
): VaultEnvelope {
	if (
		!Number.isSafeInteger(revision) ||
		revision < 0 ||
		revision > HEADLESS_VAULT_MAX_REVISION
	)
		throw new HeadlessVaultError('vault revision limit reached');
	const withoutManifest = {
		format: HEADLESS_VAULT_FORMAT,
		version: 1 as const,
		serverId,
		revision,
		keyEnvelope,
...
function parseEnvelope(
	...
	if (
		...
		throw new HeadlessVaultError('vault envelope is invalid');
	let parsed: unknown;
	try {
	...
	if (
		value.format !== HEADLESS_VAULT_FORMAT ||
		value.version !== 1 ||
		value.serverId !== expectedServerId ||
		!Array.isArray(value.entries) ||
		value.entries.length > HEADLESS_VAULT_MAX_ENTRIES ||
		!Number.isSafeInteger(revision) ||
		revision < 0 ||
		revision > HEADLESS_VAULT_MAX_REVISION
	)
	...
	if (
		!keyEnvelope ||
		keyEnvelope.protector !== HEADLESS_VAULT_PROTECTOR ||
		keyEnvelope.version !== 1 ||
		keyEnvelope.cipher !== HEADLESS_VAULT_CIPHER ||
		typeof keyEnvelope.salt !== 'string' ||
		typeof keyEnvelope.nonce !== 'string' ||
		typeof keyEnvelope.tag !== 'string' ||
		typeof keyEnvelope.ciphertext !== 'string' ||
		!keyEnvelope.kdf ||
		JSON.stringify(keyEnvelope.kdf) !== JSON.stringify(SCRYPT)
	)
		throw new HeadlessVaultError('vault envelope is invalid');
	if (
		manifest?.version !== 1 ||
		manifest.cipher !== HEADLESS_VAULT_CIPHER ||
	...
	const entries = (value.entries as unknown[]).map((raw) => {
		...
		if (
			...
			textEncoder.encode(entry.label).byteLength > MAX_VAULT_LABEL_BYTES ||
			entry.version !== 1 ||
			entry.cipher !== HEADLESS_VAULT_CIPHER ||
		...
		if (ciphertext.length > MAX_VAULT_SECRET_BYTES)
			throw new HeadlessVaultError('vault envelope is invalid');
		ciphertext.fill(0);
		return {
			id: entry.id,
			...
			version: 1 as const,
			updatedAt,
			cipher: HEADLESS_VAULT_CIPHER as typeof HEADLESS_VAULT_CIPHER,
			nonce: entry.nonce,
			tag: entry.tag,
	...
	return {
		...
		revision,
		keyEnvelope: keyEnvelope as unknown as KeyEnvelope,
		entries,
...
function encryptAesGcm(
	...
	for (let attempt = 0; attempt < 8; attempt += 1) {
		...
		if (!usedNonces.has(candidate.toString('base64url'))) {
			nonce = candidate;
			break;
		}
		candidate.fill(0);
	...
	const result: CipherRecord = {
		cipher: HEADLESS_VAULT_CIPHER,
