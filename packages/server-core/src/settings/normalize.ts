...
const SECRET_KEY =
	/(?:secret|token|password|passphrase|credential|privatekey|api[_-]?key|plaintext|pairingpinhash)/i;
...
export function normalizeSettingsAndSecrets(
	input: unknown,
): NormalizedSettings {
...
function migrateLegacyShellSettings(
	input: unknown,
	...
	if (
		...
		settings.shellProfiles !== undefined
	)
		return settings;
	const legacy = isSettingsObject(settings.shell) ? settings.shell : {};
	const program =
		typeof legacy.program === 'string' ? legacy.program.trim() : '';
	const mode =
		legacy.startupMode === 'login' || legacy.startupMode === 'non-login'
			? legacy.startupMode
			: 'default';
	const extraArgs =
		typeof legacy.extraArgs === 'string' ? legacy.extraArgs : '';
	if (program === '' && mode === 'default' && extraArgs.trim() === '') {
		return {
			...settings,
			shellProfiles: shellProfilesSettingsAsJson(
				normalizeShellProfilesSettings(DEFAULT_SERVER_SETTINGS.shellProfiles),
			),
		};
	}
	const parsed = parseLegacyShellArguments(extraArgs);
	const invalidProgram =
		program.length > 4096 || program.includes('\0') || /[\r\n]/.test(program);
	const profile: ShellProfileDefinition = {
		...
		target:
			program === '' || invalidProgram
				? { kind: 'system' }
				: { kind: 'executable', executable: program },
		args: parsed.args,
		startupMode: mode,
		environment: {},
		...(parsed.requiresReview || invalidProgram
			? { requiresReview: true }
			: {}),
	};
	return {
...
export function normalizeSecretReference(
	...
	if (typeof object.label === 'string' && object.label.length <= 256)
		result.label = object.label;
	if (Number.isSafeInteger(object.version) && (object.version as number) >= 0)
		result.version = object.version as number;
	if (
		Number.isSafeInteger(object.updatedAt) &&
		(object.updatedAt as number) >= 0
	)
		result.updatedAt = object.updatedAt as number;
	return result;
}
...
function collectSecretReferences(
	...
	if (isSettingsObject(input)) {
		for (const key of ['secretReferences', 'secrets', 'vaultReferences']) {
			...
			if (isSettingsObject(refs))
				Object.assign(output, normalizeSecretReferences(refs));
		}
	}
	for (const [key, value] of Object.entries(rawSettings))
		collectSuspiciousSecrets(output, key, value, key);
	return output;
}

function collectSuspiciousSecrets(
	output: Record<string, SecretReference>,
	key: string,
	value: unknown,
	path: string,
): void {
...
function mergeKnownDefaults(
	defaults: SettingsObject,
	candidate: SettingsObject,
): SettingsObject {
