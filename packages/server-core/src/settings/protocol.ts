...
export function createSettingsOperationRegistry(
	...
	if (!(repository instanceof ServerSettingsRepository))
		throw new TypeError('settings repository is required');
	return {
		operations: {
	...
	async function update(
		...
		if (!isSettingsObject(payload.settings))
			throw protocolError('validation', 'settings update payload is invalid');
		if (Reflect.has(payload.settings, 'shellProfiles'))
			throw protocolError(
				'validation',
				'shell profiles require the dedicated profile operations',
			);
	...
	async function reset(
		...
		if (path === 'shellProfiles' || path?.startsWith('shellProfiles.'))
			throw protocolError(
				'validation',
				'shell profiles require the dedicated profile reset operation',
			);
		const result =
			path === undefined
				? await repository.update(
						defaultSettingsWithoutShellProfiles(),
						request.envelope.expectedRevision,
						request.envelope.commandId,
					)
	...
	function applied(
		result: Awaited<ReturnType<ServerSettingsRepository['update']>>,
	): { result: JsonValue; revision: number } {
		if (!result.ok) {
			throw protocolError(
				'conflict',
...
function defaultSettingsWithoutShellProfiles() {
	return Object.fromEntries(
		Object.entries(cloneDefaultServerSettings()).filter(
			([key]) => key !== 'shellProfiles',
		),
	);
}
...
function publicSettingsState(
	state: Awaited<ReturnType<ServerSettingsRepository['load']>>,
) {
	const cloned = structuredClone(state);
	const shellProfiles = cloned.settings.shellProfiles;
	if (
		isSettingsObject(shellProfiles) &&
		Array.isArray(shellProfiles.profiles)
	) {
		(cloned.settings as Record<string, JsonValue>).shellProfiles = {
			...shellProfiles,
			profiles: shellProfiles.profiles.map((profile) =>
				isSettingsObject(profile) ? { ...profile, environment: {} } : profile,
			),
		};
	}
	return cloned;
