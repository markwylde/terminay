...
export class ServerSettingsRepository {
	...
	async load(): Promise<ServerSettingsState> {
		...
		if (raw !== undefined && !sameJson(raw, state)) {
			if (this.backend.backup === undefined) {
				...
				this.loaded = false;
				throw new Error(
					'settings migration requires recoverable backup support',
				);
			}
			const fromSchema =
				typeof raw === 'object' &&
				raw !== null &&
				!Array.isArray(raw) &&
				Number.isSafeInteger((raw as Record<string, unknown>).schemaVersion)
					? ((raw as Record<string, number>).schemaVersion as number)
					: null;
			try {
				await this.backend.backup(cloneSettings(raw));
				await this.backend.commit(state);
				await this.backend.audit?.({
					action: 'migration',
					ok: true,
					fromSchema,
	...
	get state(): ServerSettingsState {
		if (!this.loaded || this.current === undefined)
			throw new Error('settings repository is not loaded');
		return cloneSettings(this.current);
	}
	...
	get settings(): SettingsObject {
		return this.state.settings;
	}

	snapshot(): ServerSettingsState {
		return this.state;
	}

	commandOutcome(
		commandId: string | undefined,
	): SettingsApplyResult | undefined {
	...
	onChange(listener: (state: ServerSettingsState) => void): () => void {
		if (typeof listener !== 'function')
			throw new TypeError('settings listener is required');
	...
	async apply(envelope: SettingsCommandEnvelope): Promise<SettingsApplyResult> {
		...
		if (
			...
			const result: SettingsApplyResult = {
				ok: false,
				conflict: {
					code: 'conflict',
		...
		try {
			if (envelope.command.type === 'set') {
				...
				nextSettings = setPath(
					current.settings,
					envelope.command.path,
					envelope.command.value,
				);
			} else if (
				envelope.command.type === 'merge' ||
				envelope.command.type === 'replace'
			) {
				nextSettings =
					envelope.command.type === 'replace'
						? normalizeServerSettings(envelope.command.settings)
						: mergePatch(current.settings, envelope.command.settings);
			} else {
			...
			throw error instanceof Error
				? error
				: new Error('settings command rejected');
		}
		const next: ServerSettingsState = {
			schemaVersion: SETTINGS_SCHEMA_VERSION,
			revision: current.revision + 1,
			cursor: String(current.revision + 1),
			settings: nextSettings,
			secretReferences: nextReferences,
		};
	...
	async set(
		...
		return this.apply({
			commandId,
			expectedRevision,
			command: { type: 'set', path, value },
		});
	}

	async reset(
		options: SettingsResetOptions | number = {},
	): Promise<SettingsApplyResult> {
		const normalized =
			typeof options === 'number' ? { expectedRevision: options } : options;
		return this.apply({
...
function setPath(
	...
	for (const part of parts.slice(0, -1)) {
		...
		if (typeof child !== 'object' || child === null || Array.isArray(child))
			throw new Error('setting path does not address an object');
...
function resetPath(
	...
	if (path === undefined || path.trim() === '')
		return cloneDefaultServerSettings();
...
function assertServerPath(path: string): void {
	if (!isServerSettingPath(path))
		throw new Error(
			`setting path is ${classifySetting(path)}, not server-owned`,
		);
}
