...
export type VaultTestResult =
	...
	| {
			readonly ok: false;
			readonly status: VaultStatus;
			readonly code: 'missing' | 'locked' | 'unavailable' | 'failed';
	  };
...
export class ServerVaultService {
	...
	async put(input: VaultSecretInput): Promise<VaultMutationResult> {
		...
		return {
			...
			reference: sanitizeReference(reference, normalized.id),
		};
	}
	...
	async replace(input: VaultSecretInput): Promise<VaultMutationResult> {
		...
		try {
			...
			throw sanitizeVaultError(
				'replace',
				error,
				readAdapterState(this.adapter),
			);
		} finally {
			normalized.value.fill(0);
		}
	...
	withSecret<T>(
		...
		if (typeof callback !== 'function')
			throw new TypeError('secret callback is required');
		return this.adapter
			.withSecret(normalizedId, async (secret) => {
...
function normalizeInput(input: VaultSecretInput): VaultSecretInput {
	if (!input || typeof input !== 'object')
		throw new TypeError('vault input is required');
...
function normalizeId(value: ProtocolId): ProtocolId {
	if (typeof value !== 'string' || !VAULT_ID_PATTERN.test(value))
...
function normalizeLabel(value: string | undefined): string | undefined {
	...
	if (
		typeof value !== 'string' ||
		new TextEncoder().encode(value).byteLength > MAX_VAULT_LABEL_BYTES
	)
		throw new RangeError('vault secret label exceeds the limit');
	return value;
}
...
function assertBytes(value: Uint8Array, name: string, maxBytes: number): void {
	...
	if (value.byteLength > maxBytes)
		throw new RangeError(`${name} exceeds the limit`);
}

function sanitizeReference(
	reference: SecretReference,
	fallbackId?: ProtocolId,
): SecretReference {
	if (!reference || typeof reference !== 'object')
		throw new Error('vault adapter returned an invalid reference');
	const id =
		reference &&
		typeof reference.id === 'string' &&
		VAULT_ID_PATTERN.test(reference.id)
			? reference.id
			: fallbackId;
	if (id === undefined)
		throw new Error('vault adapter returned an invalid reference');
	...
	if (typeof reference.label === 'string')
		output.label = normalizeLabel(reference.label);
	if (
		Number.isSafeInteger(reference.version) &&
		(reference.version as number) >= 0
	)
		output.version = reference.version;
	if (
		Number.isSafeInteger(reference.updatedAt) &&
		(reference.updatedAt as number) >= 0
	)
		output.updatedAt = reference.updatedAt;
	return Object.freeze(output);
}

function normalizeState(value: VaultState): VaultState {
	if (value === 'unavailable' || value === 'locked' || value === 'unlocked')
		return value;
	return 'unavailable';
}
...
function classifyTestFailure(
	error: unknown,
	state: VaultState,
): 'missing' | 'locked' | 'unavailable' | 'failed' {
	if (state === 'locked') return 'locked';
	if (state === 'unavailable') return 'unavailable';
	if (
		error instanceof Error &&
		/missing|not found|unknown/iu.test(error.message)
	)
		return 'missing';
	return 'failed';
}
...
function sanitizeVaultError(
	...
	if (
		...
		return new VaultServiceError(operation, 'missing', 'missing secret');
	if (state === 'locked')
		return new VaultServiceError(operation, 'locked', 'vault is locked');
