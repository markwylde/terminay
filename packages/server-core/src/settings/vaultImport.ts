...
export type EmbeddedVaultImportErrorCode =
	| 'unavailable'
	| 'locked'
	| 'invalid_source'
	| 'failed';
...
export class EmbeddedVaultImportCoordinator {
	private readonly inFlight = new Map<
		string,
		Promise<EmbeddedVaultImportResult>
	>();
	...
	constructor(
		...
		if (safeStorage.backend !== 'embedded-safe-storage')
			throw new TypeError('embedded safe-storage adapter is required');
		this.limits = Object.freeze({
			...
			maxCiphertextBytes: boundedLimit(
				options.maxCiphertextBytes,
				DEFAULT_MAX_CIPHERTEXT_BYTES,
				1,
				MAX_VAULT_SECRET_BYTES * 4,
			),
			maxTotalCiphertextBytes: boundedLimit(
				options.maxTotalCiphertextBytes,
				DEFAULT_MAX_TOTAL_CIPHERTEXT_BYTES,
				1,
				64 * 1024 * 1024,
			),
		});
	...
	importOnce(
		source: EmbeddedVaultImportSource,
	): Promise<EmbeddedVaultImportResult> {
		const normalized = validateSource(source, this.limits);
		const existing = this.inFlight.get(normalized.sourceId);
		if (existing !== undefined) return existing;
		const operation = this.run(normalized).finally(() =>
			this.inFlight.delete(normalized.sourceId),
		);
	...
	private async run(
		...
		try {
			...
			throw new EmbeddedVaultImportError(
				'failed',
				'embedded import marker could not be read',
			);
		}
		if (complete) {
			return Object.freeze({
				...
				imported: 0,
				skippedExisting: source.entries.length,
		...
		try {
			...
			throw new EmbeddedVaultImportError(
				'unavailable',
				'embedded safe storage is unavailable',
			);
		}
		if (!available)
			throw new EmbeddedVaultImportError(
		...
		if (status.state === 'locked')
			throw new EmbeddedVaultImportError('locked', 'server vault is locked');
		if (status.state === 'unavailable')
			throw new EmbeddedVaultImportError(
				'unavailable',
				'server vault is unavailable',
			);
		...
		for (const entry of source.entries) {
			...
			if (existing.code === 'locked')
				throw new EmbeddedVaultImportError(
					'locked',
					'server vault became locked during import',
				);
			if (existing.code === 'unavailable')
				throw new EmbeddedVaultImportError(
					'unavailable',
					'server vault became unavailable during import',
				);
			if (existing.code !== 'missing')
				throw new EmbeddedVaultImportError(
					'failed',
					'server vault entry could not be inspected',
				);
			let plaintext: Uint8Array | undefined;
			try {
				plaintext = await this.safeStorage.decrypt(
					new Uint8Array(entry.encryptedValue),
				);
				if (
					!(plaintext instanceof Uint8Array) ||
					plaintext.byteLength > MAX_VAULT_SECRET_BYTES
				)
					throw new EmbeddedVaultImportError(
						'invalid_source',
						'decrypted legacy secret exceeds the limit',
					);
		...
		try {
			...
			throw new EmbeddedVaultImportError(
				'failed',
				'embedded import marker could not be committed',
			);
		}
		return Object.freeze({
			sourceId: source.sourceId,
...
function validateSource(
	source: EmbeddedVaultImportSource,
	limits: Required<EmbeddedVaultImportOptions>,
): EmbeddedVaultImportSource {
	if (
		!source ||
		...
		throw new EmbeddedVaultImportError(
			'invalid_source',
			'embedded import source id is invalid',
		);
	}
	if (
		!Array.isArray(source.entries) ||
		source.entries.length > limits.maxEntries
	)
		throw new EmbeddedVaultImportError(
			'invalid_source',
			'embedded import entry count exceeds the limit',
		);
	let total = 0;
	for (const entry of source.entries) {
		if (
			...
			!(entry.encryptedValue instanceof Uint8Array)
		)
			throw new EmbeddedVaultImportError(
				'invalid_source',
				'embedded import entry is invalid',
			);
		if (entry.encryptedValue.byteLength > limits.maxCiphertextBytes)
			throw new EmbeddedVaultImportError(
				'invalid_source',
				'embedded import ciphertext exceeds the limit',
			);
		total += entry.encryptedValue.byteLength;
		if (total > limits.maxTotalCiphertextBytes)
			throw new EmbeddedVaultImportError(
				'invalid_source',
				'embedded import payload exceeds the limit',
			);
	}
	return {
		sourceId: source.sourceId,
