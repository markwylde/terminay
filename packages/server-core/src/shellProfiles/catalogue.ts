...
export class ShellProfileCatalogueService {
	...
	async catalogue(): Promise<ShellProfileCatalogue> {
		try {
			...
			entries.push(
				withReferences(
					discovery.systemProfile,
					this.references(systemDefinition.id),
				),
			);
			for (const discovered of discovery.discoveredProfiles) {
				...
				entries.push(
					withReferences(discovered, this.references(definition.id)),
				);
			}
			const definitionsById = new Map(
				settings.profiles.map((definition) => [definition.id, definition]),
			);
			...
			if (
				new TextEncoder().encode(JSON.stringify(catalogue)).byteLength >
				60 * 1024
			) {
				throw new Error(
					'shell profile catalogue exceeds the protocol response budget',
				);
			}
	...
	async resolveProfile(
		...
		if (snapshot === undefined)
			throw new Error('shell profile catalogue snapshot is unavailable');
		...
		if (profile === undefined || definition === undefined)
			throw new Error('shell profile does not exist');
		if (definition.requiresReview === true || !profile.availability.available)
			throw new Error('shell profile is unavailable');
		const target = await this.options.discovery.resolveTarget(
			definition,
			snapshot.discovery,
		);
	...
	async detail(profileId: ProtocolId): Promise<ShellProfileDefinition> {
		...
		if (profile === undefined)
			throw new ShellProfileMutationError(
				'not-found',
				'custom shell profile not found',
			);
	...
	async validate(profile: unknown) {
		try {
			...
			const discoveredValidation = this.options.discovery.validate(
				normalized,
				await this.options.discovery.discover(),
			);
			const duplicateName = this.currentSettings().profiles.some(
				(candidate) =>
			...
			this.audit({
				...
				...(result.valid ? {} : { code: result.issues[0]?.code ?? 'invalid' }),
			});
			return result;
		} catch (error) {
			this.audit({ action: 'validate', ok: false, code: 'validation' });
			throw error;
		}
	}
	...
	create(profile: unknown, expectedRevision?: number, commandId?: string) {
		return this.mutate('create', commandId, async () => {
			...
			const normalized = normalizeShellProfile(profile);
			if (current.profiles.some((entry) => entry.id === normalized.id))
				throw new ShellProfileMutationError(
					'conflict',
			...
			return this.commit(
				{
					...current,
					profiles: [...current.profiles, normalized],
					order: [...current.order, normalized.id],
				},
				expectedRevision,
				commandId,
			);
	...
	reorder(
		...
		return this.mutate('reorder', commandId, () =>
			this.commit(
				{ ...this.currentSettings(), order: profileIds },
				expectedRevision,
				commandId,
			),
		);
	...
	delete(profileId: string, expectedRevision?: number, commandId?: string) {
		return this.mutate('delete', commandId, async () => {
			const current = this.currentSettings();
			if (current.defaultProfileId === profileId)
				throw new ShellProfileMutationError(
					'referenced',
					'reassign the server default before deleting this profile',
				);
			const references = this.references(profileId);
			if (references.length > 0)
				throw new ShellProfileMutationError(
					'referenced',
					'clear or reassign project defaults before deleting this profile',
					references,
				);
			if (!current.profiles.some((entry) => entry.id === profileId))
				throw new ShellProfileMutationError(
					'not-found',
					'custom shell profile not found',
				);
			return this.commit(
				{
					...
					order: current.order.filter((id) => id !== profileId),
				},
				expectedRevision,
				commandId,
			);
	...
	setDefault(profileId: string, expectedRevision?: number, commandId?: string) {
		return this.mutate('set-default', commandId, async () => {
			...
			if (
				profileId !== 'system' &&
				!current.profiles.some((entry) => entry.id === profileId)
			) {
				throw new ShellProfileMutationError(
					'not-found',
					'only System default or a custom profile can be the server default',
				);
			}
			return this.commit(
	...
	setCwdPolicy(
		cwdPolicy: ShellProfilesSettings['cwdPolicy'],
		expectedRevision?: number,
		commandId?: string,
	) {
		return this.mutate('set-cwd-policy', commandId, () =>
			this.commit(
				{ ...this.currentSettings(), cwdPolicy },
				expectedRevision,
				commandId,
			),
		);
	...
	reset(expectedRevision?: number, commandId?: string) {
		return this.mutate('reset', commandId, async () => {
			const referenced = this.currentSettings().profiles.flatMap((profile) =>
				this.references(profile.id),
			);
			if (referenced.length > 0)
				throw new ShellProfileMutationError(
					'referenced',
					'clear project shell defaults before resetting profiles',
					referenced,
				);
			return this.commit(
				{
	...
	private references(profileId: ProtocolId): readonly ProtocolId[] {
		return Object.freeze([
			...(this.options.projectReferences?.(profileId) ?? []),
		]);
	}

	private currentSettings(): ShellProfilesSettings {
		return normalizeShellProfilesSettings(
			this.options.settings.snapshot().settings.shellProfiles,
		);
	}

	private async assertValid(profile: ShellProfileDefinition): Promise<void> {
		const result = this.options.discovery.validate(
			profile,
			await this.options.discovery.discover(),
		);
		if (!result.valid)
			throw new ShellProfileMutationError(
				'validation',
				result.issues[0]?.message ?? 'shell profile is invalid',
			);
	}

	private assertUniqueName(
		profile: ShellProfileDefinition,
		settings: ShellProfilesSettings,
	): void {
		if (
			...
			throw new ShellProfileMutationError(
				'validation',
				'a shell profile with this name already exists',
			);
		}
	}
	...
	private commit(
		...
		return this.options.settings.set(
			'shellProfiles',
			normalized as unknown as import('@terminay/protocol').JsonValue,
			expectedRevision,
			commandId,
		);
	}
	...
	private async mutate<T>(
		action: ShellProfileAuditOutcome['action'],
		...
		try {
			...
			this.audit({
				...
				ok: false,
				code:
					error instanceof ShellProfileMutationError ? error.code : 'internal',
			});
			throw error;
		} finally {
			release();
	...
	private audit(outcome: ShellProfileAuditOutcome): void {
		this.options.audit?.(Object.freeze(outcome));
	}
	...
	private async customEntry(
		...
		let availability: ShellProfileCatalogueEntry['availability'] = {
			available: true,
		};
		if (definition.requiresReview === true) {
			availability = {
				available: false,
				reason: 'This migrated profile requires review.',
			};
		} else {
			try {
				...
				availability = {
					available: false,
					reason:
						error instanceof ShellProfileDiscoveryError &&
						error.code === 'unsupported-startup-mode'
...
export class ShellProfileMutationError extends Error {
	constructor(
		...
		super(message);
		this.name = 'ShellProfileMutationError';
	}
}

function definitionFromEntry(
	entry: ShellProfileCatalogueEntry,
): ShellProfileDefinition {
	return Object.freeze({
		id: entry.id,
		name: entry.name,
...
function redactDefinition(
	...
	ShellProfileCatalogueEntry,
	'kind' | 'readOnly' | 'source' | 'availability' | 'projectReferences'
> {
	return {
		id: definition.id,
...
function withReferences(
	entry: ShellProfileCatalogueEntry,
	projectReferences: readonly ProtocolId[],
): ShellProfileCatalogueEntry {
