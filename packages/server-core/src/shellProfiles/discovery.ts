...
export interface ShellDiscoveryHost {
	...
	readonly readEtcShells?: (
		signal?: AbortSignal,
	) => MaybePromise<string | null>;
	readonly listWslDistributions?: (
		signal?: AbortSignal,
...
export interface ShellProfileCollectionValidationResult
	extends ShellProfileValidationResult {
	readonly profileIssues: Readonly<
		Record<string, readonly ShellProfileValidationIssue[]>
	>;
}
...
const COLOR =
	/^(?:#[0-9A-Fa-f]{6}|#[0-9A-Fa-f]{8}|[A-Za-z][A-Za-z0-9-]{0,63})$/u;
const SECRET_LIKE_ENVIRONMENT_KEY =
	/(?:^|_)(?:PASSWORD|PASSWD|SECRET|TOKEN|API_KEY|PRIVATE_KEY|CREDENTIALS?)(?:_|$)/iu;
...
export class ShellProfileDiscoveryService {
	...
	constructor(
		host: ShellDiscoveryHost,
		options: ShellDiscoveryServiceOptions = {},
	) {
		if (
			typeof host !== 'object' ||
			host === null ||
			typeof host.probeExecutable !== 'function'
		) {
		...
		if (!Number.isSafeInteger(timeout) || timeout < 10 || timeout > 30_000)
			throw new RangeError('shell discovery host timeout is invalid');
	...
	validate(
		...
		if (isStructurallyRecognizableProfile(profile)) {
			...
			if (
				typed.startupMode !== 'default' &&
				(executable === null || !supportsShellStartupMode(executable))
			) {
				issues.push(
					issue(
						'unsupported-startup-mode',
						'startupMode',
						'The selected target does not support this startup mode.',
					),
				);
			}
			if (
				typed.target.kind === 'system' &&
				discovery !== undefined &&
				discovery.systemExecutable === null
			) {
				issues.push(
					issue(
						'target-unavailable',
						'target',
						'The System default shell is unavailable on this server.',
					),
				);
			}
			if (typed.target.kind === 'wsl' && this.host.platform !== 'win32') {
				issues.push(
					issue(
						'unsupported-target',
						'target.kind',
						'WSL profiles are available only on Windows servers.',
					),
				);
			}
			if (typed.requiresReview === true) {
				issues.push(
					issue(
						'review-required',
						'requiresReview',
						'This migrated shell profile must be reviewed before it can be launched.',
					),
				);
			}
		}
	...
	validateProfiles(
		profiles: unknown,
		discovery?: ShellDiscoveryResult,
	): ShellProfileCollectionValidationResult {
		...
		if (!Array.isArray(profiles)) {
			issues.push(
				issue(
					'invalid-profiles',
					'profiles',
					'Shell profiles must be an array.',
				),
			);
			return Object.freeze({
				valid: false,
				issues: Object.freeze(issues),
				profileIssues: Object.freeze(byProfile),
			});
		}
		if (profiles.length > MAX_SHELL_PROFILES) {
			issues.push(
				issue(
					'profile-limit',
					'profiles',
					`At most ${MAX_SHELL_PROFILES} shell profiles are allowed.`,
				),
			);
		}
		...
		for (
			let index = 0;
			index < Math.min(profiles.length, MAX_SHELL_PROFILES + 1);
			index += 1
		) {
			...
			if (typeof record?.id === 'string') {
				if (ids.has(record.id))
					local.push(
						issue('duplicate-id', 'id', 'Shell profile ids must be unique.'),
					);
				ids.add(record.id);
			}
				...
				if (names.has(key))
					local.push(
						issue(
							'duplicate-name',
							'name',
							'Shell profile names must be unique.',
						),
					);
				names.add(key);
			}
	...
	async resolveTarget(
		profile: ShellProfileDefinition,
		discovery?: ShellDiscoveryResult,
	): Promise<ResolvedShellLaunchTarget> {
		const discovered = discovery ?? (await this.discover());
		const validation = this.validate(profile, discovered);
		if (!validation.valid) {
			const unsupported = validation.issues.some(
				(entry) => entry.code === 'unsupported-startup-mode',
			);
			throw new ShellProfileDiscoveryError(
				unsupported ? 'unsupported-startup-mode' : 'invalid-profile',
				'The shell profile is invalid.',
			);
		}
		...
		if (profile.target.kind === 'executable') {
			...
			if (executable === null)
				throw new ShellProfileDiscoveryError(
					'target-unavailable',
					'The shell profile executable is unavailable on this server.',
				);
			return Object.freeze({ kind: 'executable', executable });
		}
		...
		const distribution = distributions.find((entry) =>
			sameHostIdentity(entry, requestedDistribution, true),
		);
		if (wslExecutable === null || distribution === undefined) {
			throw new ShellProfileDiscoveryError(
				'target-unavailable',
				'The shell profile WSL distribution is unavailable on this server.',
			);
		}
		return Object.freeze({
			kind: 'wsl',
			...
			distribution,
			...(profile.target.shellPath === undefined
				? {}
				: { shellPath: profile.target.shellPath }),
		});
	}
	...
	private async discoverPosix(): Promise<ShellDiscoveryResult> {
		...
		if (isAbsoluteSafePath(this.host.accountShell))
			addCandidate(candidates, this.host.accountShell, 'account', true);
		if (isAbsoluteSafePath(this.host.environmentShell))
			addCandidate(candidates, this.host.environmentShell, 'environment', true);

		const rawShellsText =
			this.host.readEtcShells === undefined
				? null
				: await this.callHost(
						(signal) => this.host.readEtcShells!(signal),
						null,
					);
		const shellsText = typeof rawShellsText === 'string' ? rawShellsText : null;
		for (const path of parseEtcShells(shellsText))
			addCandidate(candidates, path, 'etc-shells', true);
		for (const path of POSIX_FALLBACKS[platform])
			addCandidate(candidates, path, 'fallback', true);
	...
	private async discoverWindows(): Promise<ShellDiscoveryResult> {
		...
		addCandidate(
			systemCandidates,
			this.host.environmentShell,
			'environment',
			true,
		);
		addCandidate(
			systemCandidates,
			this.host.openSshDefaultShell,
			'fallback',
			true,
		);
		for (const value of boundedCandidates(
			this.host.windowsPowerShellCandidates ?? DEFAULT_WINDOWS_POWERSHELL,
		)) {
			addCandidate(
				systemCandidates,
				value,
		...
		for (const value of boundedCandidates(
			this.host.powerShell7Candidates ?? DEFAULT_WINDOWS_PWSH,
		))
			addCandidate(
				discoveryCandidates,
				value,
		...
		for (const value of boundedCandidates(
			this.host.commandPromptCandidates ?? DEFAULT_WINDOWS_CMD,
		))
			addCandidate(
				discoveryCandidates,
				value,
		...
		for (const value of boundedCandidates(
			this.host.gitBashCandidates ?? DEFAULT_GIT_BASH,
		))
			addCandidate(discoveryCandidates, value, 'git-bash', false, 'Git Bash');

		const executableResult = await this.buildExecutableResult(
			discoveryCandidates,
			systemCandidates,
		);
		...
		return boundedDiscoveryResult(
			executableResult.systemProfile,
			executableResult.systemExecutable,
			uniqueProfileNames(discoveredProfiles),
		);
	}

	private async buildExecutableResult(
		candidates: readonly Candidate[],
		systemCandidates: readonly Candidate[] = candidates,
		...
		const resolvedValues = await Promise.all(
			uniqueCandidates.map((candidate) => this.probe(candidate)),
		);
	...
	private async probe(candidate: string): Promise<string | null> {
		...
		const canonical = await this.callHost(
			(signal) => this.host.probeExecutable(candidate, signal),
			null,
		);
	...
	private async wslDistributions(): Promise<readonly string[]> {
		...
		const raw = await this.callHost(
			(signal) => this.host.listWslDistributions!(signal),
			[] as readonly string[],
		);
		...
		for (
			let index = 0;
			index < Math.min(raw.length, MAX_WSL_DISTRIBUTIONS);
			index += 1
		) {
...
function validateProfileShape(profile: unknown): ShellProfileValidationIssue[] {
	...
	if (!isRecord(profile))
		return [
			issue(
				'invalid-profile',
				'profile',
				'The shell profile must be an object.',
			),
		];

	rejectUnknownSecretFields(profile, '', issues);
	if (!safeText(profile.id, MAX_ID_LENGTH))
		issues.push(issue('invalid-id', 'id', 'The shell profile id is invalid.'));
	if (!safeTrimmedText(profile.name, MAX_NAME_LENGTH))
		issues.push(
			issue(
				'invalid-name',
				'name',
				`The shell profile name must be between 1 and ${MAX_NAME_LENGTH} characters.`,
			),
		);
	validateTarget(profile.target, issues);
	if (
		profile.startupMode !== 'default' &&
		profile.startupMode !== 'login' &&
		profile.startupMode !== 'non-login'
	) {
		issues.push(
			issue(
				'invalid-startup-mode',
				'startupMode',
				'The shell profile startup mode is invalid.',
			),
		);
	}
	if (
		!Array.isArray(profile.args) ||
		profile.args.length > MAX_SHELL_PROFILE_ARGS
	) {
		issues.push(
			issue(
				'argument-limit',
				'args',
				`A shell profile accepts at most ${MAX_SHELL_PROFILE_ARGS} arguments.`,
			),
		);
	} else {
		for (let index = 0; index < profile.args.length; index += 1) {
			if (!safeText(profile.args[index], MAX_ARGUMENT_LENGTH))
				issues.push(
					issue(
						'invalid-argument',
						`args.${index}`,
						'The shell profile argument is invalid.',
					),
				);
		}
	}
	...
	if (
		...
		!safeTrimmedText(profile.icon, MAX_PRESENTATION_LENGTH)
	)
		issues.push(
			issue('invalid-icon', 'icon', 'The shell profile icon is invalid.'),
		);
	if (
		profile.color !== undefined &&
		(typeof profile.color !== 'string' || !COLOR.test(profile.color))
	)
		issues.push(
			issue('invalid-color', 'color', 'The shell profile colour is invalid.'),
		);
	return issues;
}

function isStructurallyRecognizableProfile(
	value: unknown,
): value is ShellProfileDefinition {
	...
	return (
		...
		(value.startupMode === 'default' ||
			value.startupMode === 'login' ||
			value.startupMode === 'non-login')
	);
}

function validateTarget(
	target: unknown,
	issues: ShellProfileValidationIssue[],
): void {
	if (!isRecord(target)) {
		issues.push(
			issue('invalid-target', 'target', 'The shell profile target is invalid.'),
		);
		return;
	}
	if (target.kind === 'system') return;
	if (target.kind === 'executable') {
		if (!safeTrimmedText(target.executable, MAX_PATH_LENGTH))
			issues.push(
				issue(
					'invalid-executable',
					'target.executable',
					'The shell profile executable is invalid.',
				),
			);
		return;
	}
	if (target.kind === 'wsl') {
		if (!validDistribution(target.distribution))
			issues.push(
				issue(
					'invalid-distribution',
					'target.distribution',
					'The WSL distribution is invalid.',
				),
			);
		if (
			target.shellPath !== undefined &&
			(!safeTrimmedText(target.shellPath, MAX_PATH_LENGTH) ||
				!isAbsolute(target.shellPath))
		) {
			issues.push(
				issue(
					'invalid-wsl-shell',
					'target.shellPath',
					'The WSL shell path must be an absolute path.',
				),
			);
		}
		return;
	...
	issues.push(
		issue(
			'invalid-target',
			'target.kind',
			'The shell profile target kind is invalid.',
		),
	);
}
...
function validateEnvironment(
	...
	if (
		!isRecord(value) ||
		Object.keys(value).length > MAX_SHELL_PROFILE_ENVIRONMENT
	) {
		issues.push(
			issue(
				'environment-limit',
				'environment',
				`A shell profile accepts at most ${MAX_SHELL_PROFILE_ENVIRONMENT} environment entries.`,
			),
		);
		return;
	}
		...
		if (isProtectedTerminalEnvironmentName(key)) {
			issues.push(
				issue(
					...
					`environment.${key}`,
					'This server-managed environment variable cannot be changed by a shell profile.',
				),
			);
			...
			issues.push(
				issue(
					'secret-environment',
					`environment.${key}`,
					'Secret material cannot be stored in a shell profile.',
				),
			);
		}
		if (
			environmentValue !== null &&
			(!safeText(environmentValue, MAX_ENVIRONMENT_VALUE_LENGTH) ||
				new TextEncoder().encode(environmentValue).byteLength >
			...
			issues.push(
				issue(
					...
					`environment.${key}`,
					'The environment variable value is invalid.',
				),
			);
		}
	}
...
function rejectUnknownSecretFields(
	...
	for (const [key, fieldValue] of Object.entries(value)) {
		...
		if (
			/(?:secret|password|credential|token)/iu.test(key) &&
			prefix !== 'environment'
		) {
			issues.push(
				issue(
					'secret-field',
					field,
					'Secret fields are not supported by shell profiles.',
				),
			);
		}
		if ((key === 'target' || key === 'presentation') && isRecord(fieldValue))
			rejectUnknownSecretFields(fieldValue, field, issues);
	}
}

function executableForStartupValidation(
	profile: ShellProfileDefinition,
	discovery?: ShellDiscoveryResult,
): string | null {
	if (profile.target.kind === 'executable') return profile.target.executable;
	if (profile.target.kind === 'system')
		return discovery?.systemExecutable ?? null;
...
function parseEtcShells(value: string | null): readonly string[] {
	...
	if (
		value.length > MAX_ETC_SHELLS_BYTES ||
		new TextEncoder().encode(value).byteLength > MAX_ETC_SHELLS_BYTES
	)
		return [];
	const result: string[] = [];
	...
	for (
		let index = 0;
		index < Math.min(lines.length, MAX_ETC_SHELLS_LINES);
		index += 1
	) {
		const rawLine = lines[index] ?? '';
		const line = rawLine.trim();
		if (line.length === 0 || line.startsWith('#') || !isAbsoluteSafePath(line))
			continue;
		result.push(line);
...
function systemDefaultProfile(available: boolean): ShellProfileCatalogueEntry {
	return Object.freeze({
		...
		availability: Object.freeze({
			available,
			...(available ? {} : { reason: 'No usable system shell was found.' }),
		}),
...
function discoveredProfile(
	target: ShellProfileDefinition['target'],
	name: string,
...
function uniqueProfileNames(
	...
	return profiles.map((profile) => {
		...
		return Object.freeze({
			...profile,
			name: `${baseName.slice(0, MAX_NAME_LENGTH - suffix.length)}${suffix}`,
		});
	});
}
...
function addCandidate(
	...
	if (
		list.length >= MAX_DISCOVERY_CANDIDATES ||
		!safeTrimmedText(value, MAX_PATH_LENGTH)
	)
		return;
	list.push({
		candidate: value,
...
function boundedCandidates(values: readonly string[]): readonly string[] {
	...
	for (
		let index = 0;
		index < Math.min(values.length, MAX_DISCOVERY_SOURCE_CANDIDATES);
		index += 1
	) {
...
function boundedDiscoveryResult(
	...
	for (const profile of profiles) {
		...
		const candidate = {
			systemProfile,
			discoveredProfiles: [...discoveredProfiles, profile],
			systemExecutable,
		};
		if (
	...
	return Object.freeze({
		systemProfile,
		discoveredProfiles: Object.freeze(discoveredProfiles),
		systemExecutable,
	});
}
...
function safeTrimmedText(value: unknown, max: number): value is string {
	return (
		safeText(value, max) && value.trim().length > 0 && value === value.trim()
	);
}
