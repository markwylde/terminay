...
export async function createNodeShellDiscoveryHost(
	...
	return {
		...
		comSpec: windows
			? (environment.ComSpec ?? environment.COMSPEC ?? null)
			: null,
		powerShell7Candidates: windows
			? [join(programFiles, 'PowerShell', '7', 'pwsh.exe'), 'pwsh.exe']
			: [],
		...
		probeExecutable: (candidate) =>
			probeExecutable(candidate, environment, windows),
		...(windows
			? { listWslDistributions }
			: { readEtcShells: () => readBoundedEtcShells() }),
	};
}

export async function readBoundedEtcShells(
	path = '/etc/shells',
): Promise<string> {
	...
	try {
		...
		return buffer
			.subarray(0, Math.min(bytesRead, MAX_ETC_SHELLS_READ_BYTES))
			.toString('utf8');
	} finally {
		await handle.close();
	}
...
async function probeExecutable(
	...
	const pathEntries = (environment.PATH ?? environment.Path ?? '')
		...
		.filter(Boolean);
	const suffixes =
		windows && !/\.[A-Za-z0-9]+$/u.test(candidate)
			? (environment.PATHEXT ?? '.COM;.EXE;.BAT;.CMD').split(';')
			: [''];
	for (const directory of pathEntries) {
		for (const suffix of suffixes) {
			const found = await executablePath(
				join(directory, `${candidate}${suffix}`),
			);
