...
const SECRET_LIKE =
	/(?:secret|token|password|passphrase|credential|private[_-]?key|api[_-]?key|plaintext|pairing[_-]?pin)/i;
...
const STARTUP_MODES = new Set<ShellStartupMode>([
	'default',
	'login',
	'non-login',
]);
const CWD_POLICIES = new Set<NewTerminalCwdPolicy>([
	'current',
	'project',
	'home',
]);

export const DEFAULT_SHELL_PROFILES_SETTINGS: ShellProfilesSettings =
	Object.freeze({
...
export function shellProfilesSettingsAsJson(
...
export function normalizeShellProfilesSettings(
	input: unknown,
	...
	if (!Array.isArray(profilesRaw) || profilesRaw.length > MAX_SHELL_PROFILES) {
		throw new TypeError(
			`shell profiles must contain at most ${MAX_SHELL_PROFILES} entries`,
		);
	}
	const profiles = profilesRaw.map((profile, index) =>
		normalizeShellProfile(profile, index),
	);
	if (utf8Bytes(JSON.stringify(profiles)) > MAX_SHELL_PROFILES_BYTES)
		throw new TypeError('shell profiles exceed the aggregate storage limit');
	const ids = new Set<string>();
	const names = new Set<string>();
	for (const profile of profiles) {
		if (ids.has(profile.id))
			throw new TypeError('shell profile ids must be unique');
		const name = profile.name.toLocaleLowerCase();
		if (names.has(name))
			throw new TypeError('shell profile names must be unique');
		ids.add(profile.id);
		names.add(name);
	}
	const defaultProfileId = id(
		raw.defaultProfileId ?? SYSTEM_SHELL_PROFILE_ID,
		'defaultProfileId',
	);
	if (
		defaultProfileId !== SYSTEM_SHELL_PROFILE_ID &&
		!ids.has(defaultProfileId)
	) {
		throw new TypeError('default shell profile does not exist');
	}
	const cwdPolicy = raw.cwdPolicy ?? 'current';
	if (
		typeof cwdPolicy !== 'string' ||
		!CWD_POLICIES.has(cwdPolicy as NewTerminalCwdPolicy)
	) {
	...
	if (
		...
		orderRaw.some((entry) => typeof entry !== 'string')
	) {
		throw new TypeError('shell profile order is invalid');
	}
	const order = orderRaw.map((entry) => id(entry, 'order id'));
	if (
		order.length !== profiles.length ||
		new Set(order).size !== order.length ||
		order.some((entry) => !ids.has(entry))
	) {
		throw new TypeError(
			'shell profile order must contain every custom profile exactly once',
		);
	}
	return {
		defaultProfileId,
		cwdPolicy: cwdPolicy as NewTerminalCwdPolicy,
		profiles,
		order,
	};
}

export function normalizeShellProfile(
	input: unknown,
	index = 0,
): ShellProfileDefinition {
	const raw = object(input, `profile ${index}`);
	const profileId = id(raw.id, 'profile id');
	if (profileId === SYSTEM_SHELL_PROFILE_ID)
		throw new TypeError('the System default profile id is reserved');
	const name = boundedString(raw.name, 'profile name', 1, 128);
	const startupMode = raw.startupMode ?? 'default';
	if (
		typeof startupMode !== 'string' ||
		!STARTUP_MODES.has(startupMode as ShellStartupMode)
	) {
	...
	if (color !== undefined && !/^#[0-9a-f]{6}(?:[0-9a-f]{2})?$/i.test(color))
		throw new TypeError('profile color is invalid');
	...
	if (utf8Bytes(JSON.stringify(result)) > MAX_SHELL_PROFILE_BYTES)
		throw new TypeError('shell profile exceeds the storage limit');
	return result;
}
...
export function parseLegacyShellArguments(input: string): {
	...
	if (escaping || quote !== undefined)
		return { args: [input], requiresReview: true };
	if (started) args.push(value);
	const unsafe =
		args.length > MAX_SHELL_PROFILE_ARGS ||
		args.some((argument) =>
			COMMAND_ARGUMENTS.has(argument.toLocaleLowerCase()),
		);
	return unsafe
		? { args: [input], requiresReview: true }
		: { args, requiresReview: false };
}
...
function normalizeTarget(input: unknown): ShellProfileTarget {
	...
	if (raw.kind === 'system') {
		if (Object.keys(raw).some((key) => key !== 'kind'))
			throw new TypeError('System target has unsupported fields');
		return { kind: 'system' };
	}
	if (raw.kind === 'executable') {
		...
		if (executable.includes('\0') || /[\r\n]/.test(executable))
			throw new TypeError('shell executable is invalid');
		return { kind: 'executable', executable };
	}
	if (raw.kind === 'wsl') {
		const distribution = boundedString(
			raw.distribution,
			'WSL distribution',
			1,
			128,
		...
		if (/^[.-]|[\0\r\n]/.test(distribution))
			throw new TypeError('WSL distribution is invalid');
		const shellPath = optionalBoundedString(
			raw.shellPath,
			'WSL shell path',
			4096,
		);
...
function normalizeArgs(input: unknown): readonly string[] {
	if (!Array.isArray(input) || input.length > MAX_SHELL_PROFILE_ARGS)
		throw new TypeError('shell profile arguments are invalid');
	return input.map((value) => {
		const argument = boundedString(value, 'shell argument', 0, 4096);
		if (argument.includes('\0') || /[\r\n]/.test(argument))
			throw new TypeError('shell argument is invalid');
		if (COMMAND_ARGUMENTS.has(argument.toLocaleLowerCase()))
			throw new TypeError('shell command arguments are not supported');
		return argument;
	});
}
...
function normalizeEnvironment(
	...
	if (entries.length > MAX_SHELL_PROFILE_ENVIRONMENT)
		throw new TypeError('shell profile environment has too many entries');
	const result: Record<string, string | null> = {};
	for (const [key, value] of entries) {
		if (!ENVIRONMENT_KEY_PATTERN.test(key))
			throw new TypeError('shell profile environment key is invalid');
		if (isProtectedTerminalEnvironmentName(key))
			throw new TypeError('shell profile environment key is server protected');
		if (SECRET_LIKE.test(key))
			throw new TypeError(
				'shell profiles cannot contain secret-like environment fields',
			);
		if (
			value !== null &&
			(typeof value !== 'string' ||
