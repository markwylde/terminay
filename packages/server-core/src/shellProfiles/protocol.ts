...
export function createShellProfileOperationRegistry(
	service: ShellProfileCatalogueService,
): { readonly operations: OperationRegistries } {
	return {
		operations: {
			queries: {
				[SHELL_PROFILE_OPERATIONS.catalogue]: async () =>
					json(await service.catalogue()),
				[SHELL_PROFILE_OPERATIONS.detail]: (request: QueryRequest) =>
					invoke(() =>
						service.detail(
							field(object(request.envelope.payload).profileId, 'profileId'),
						),
					),
				[SHELL_PROFILE_OPERATIONS.validate]: (request: QueryRequest) =>
					invoke(() =>
						service.validate(object(request.envelope.payload).profile),
					),
			},
			commands: {
				[SHELL_PROFILE_OPERATIONS.refresh]: async () =>
					json(await service.catalogue()),
				[SHELL_PROFILE_OPERATIONS.create]: (request: CommandRequest) =>
					mutation(request, service, 'create'),
				[SHELL_PROFILE_OPERATIONS.update]: (request: CommandRequest) =>
					mutation(request, service, 'update'),
				[SHELL_PROFILE_OPERATIONS.reorder]: (request: CommandRequest) =>
					mutation(request, service, 'reorder'),
				[SHELL_PROFILE_OPERATIONS.delete]: (request: CommandRequest) =>
					mutation(request, service, 'delete'),
				[SHELL_PROFILE_OPERATIONS.setDefault]: (request: CommandRequest) =>
					mutation(request, service, 'setDefault'),
				[SHELL_PROFILE_OPERATIONS.setCwdPolicy]: (request: CommandRequest) =>
					mutation(request, service, 'setCwdPolicy'),
				[SHELL_PROFILE_OPERATIONS.reset]: (request: CommandRequest) =>
					mutation(request, service, 'reset'),
			},
			policies: {
	...
	try {
		...
		if (action === 'create') {
			...
			result = await service.create(
				{ ...profile, id: deterministicProfileId(request.envelope.commandId) },
			...
			result = await service.reorder(
				stringArray(payload.profileIds, 'profileIds'),
				request.envelope.expectedRevision,
				request.envelope.commandId,
			);
		else if (action === 'delete')
			result = await service.delete(
				field(payload.profileId, 'profileId'),
				request.envelope.expectedRevision,
				request.envelope.commandId,
			);
		else if (action === 'setDefault')
			result = await service.setDefault(
				field(payload.profileId, 'profileId'),
				request.envelope.expectedRevision,
				request.envelope.commandId,
			);
		else if (action === 'setCwdPolicy')
			result = await service.setCwdPolicy(
				field(payload.cwdPolicy, 'cwdPolicy') as 'current' | 'project' | 'home',
				request.envelope.expectedRevision,
				request.envelope.commandId,
			);
		else
...
function object(value: unknown): Record<string, JsonValue> {
	if (typeof value !== 'object' || value === null || Array.isArray(value))
		throw protocolError(
			'validation',
			'shell profile payload must be an object',
		);
...
function mapError(error: unknown): ProtocolError {
	...
	if (error instanceof ShellProfileMutationError) {
		const code =
			error.code === 'not-found'
				? 'unavailable'
				: error.code === 'conflict' || error.code === 'referenced'
					? 'conflict'
					: 'validation';
		return protocolError(
			code,
			error.message,
			error.projectIds.length === 0
				? undefined
	...
	return protocolError(
		'validation',
		error instanceof Error ? error.message : 'shell profile operation failed',
	);
}
