...
import { type TerminalService, TerminalSubscription } from './service.js';
import type {
...
export class TerminalServiceAdapter {
	...
	attach(
		...
		if (typeof sink?.onEvent !== 'function')
			throw new TypeError('terminal attachment sink must provide onEvent');
		...
		const onEvent = (event: TerminalEvent): void => {
			...
			if (event.position !== deliveredPosition) {
				...
				if (mutable !== undefined)
					queueMicrotask(() => this.closeAttachment(id, 'slow_consumer'));
				return;
			}
			started = true;
	...
	detach(attachment: string | TerminalAttachment): void {
		const id =
			typeof attachment === 'string' ? attachment : attachment.attachmentId;
		const mutable = this.attachments.get(id);
		if (mutable === undefined) return;
		mutable.closed = true;
		this.attachments.delete(id);
		if (this.byClientSession.get(mutable.key) === id)
			this.byClientSession.delete(mutable.key);
	...
	private nextAttachmentId(clientId: string): string {
		...
		return `terminal-attachment:${clientId}:${this.attachmentCounter}`.slice(
			0,
			128,
		);
	}
...
export class TerminalAttachment {
	...
	constructor(
		adapter: TerminalServiceAdapter,
	...
	get clientId(): string {
		return this.mutable.clientId;
	}
	get sessionId(): string {
		return this.mutable.identity.sessionId;
	}
	get closed(): boolean {
	...
	snapshot(): TerminalAttachmentSnapshot {
		return this.adapter.snapshot(this.mutable);
	}
	ack(position: number): void {
		this.adapter.acknowledge(this.mutable, position);
	}
	detach(): void {
...
function validateClientId(value: string): void {
	if (
		...
		throw new TerminalServiceError('invalid_identity', 'clientId is invalid');
}
