...
export class DetachableTerminalConsumerRegistry {
	...
	attach(
		...
		this.subscriptions.set(key(identity, consumerId), {
			consumerId,
			identity,
			subscription,
		});
	...
	detach(
		identity: TerminalIdentity,
		consumerId: string,
		expectedSubscription?: TerminalSubscription,
	): boolean {
		assertConsumerId(consumerId);
		...
		if (
			...
			subscription.subscription !== expectedSubscription
		)
			return false;
...
function assertConsumerId(value: string): asserts value is string {
	if (typeof value !== 'string' || !CONSUMER_ID_PATTERN.test(value))
		throw new TypeError('terminal consumer id is invalid');
}
