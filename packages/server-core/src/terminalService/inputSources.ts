...
import type { TerminalAuthorization, TerminalIdentity } from './types.js';
...
const SOURCES: readonly TerminalInputSource[] = [
	'keyboard',
	'paste',
	'macro',
	'dictation',
...
export class TerminalInputSourceAdapter {
	...
	private readonly onInputAccepted:
		| ((event: TerminalAcceptedInput) => void | Promise<void>)
		| undefined;
	...
	constructor(
		...
		options: TerminalInputSourceAdapterOptions = {},
	) {
		this.service = service;
		this.maxQueuedInputBytes = positiveLimit(
			options.maxQueuedInputBytes ?? DEFAULT_MAX_QUEUED_INPUT_BYTES,
			'maxQueuedInputBytes',
		);
		this.maxResizeLeaseMs = positiveLimit(
			options.maxResizeLeaseMs ?? DEFAULT_MAX_RESIZE_LEASE_MS,
			'maxResizeLeaseMs',
		);
		this.resizeLeaseMs = boundedLease(
			options.resizeLeaseMs ?? DEFAULT_RESIZE_LEASE_MS,
			this.maxResizeLeaseMs,
		);
	...
	input(
		request: TerminalInputSourceRequest,
	): Promise<TerminalInputSourceResult> {
	...
	write(
		...
		if (queue.pendingBytes + bytes.byteLength > this.maxQueuedInputBytes) {
			throw new TerminalServiceError(
				...
				{
					max: this.maxQueuedInputBytes,
					actual: queue.pendingBytes + bytes.byteLength,
				},
			);
		}
		if (normalized.sequence !== undefined) {
			...
			if (previous !== undefined && normalized.sequence <= previous)
				throw new TerminalServiceError(
					'invalid_position',
					'terminal input sequence is stale',
					{ expected: previous + 1, actual: normalized.sequence },
				);
	...
	async resize(request: TerminalResizeRequest): Promise<TerminalResizeResult> {
		...
		if (normalized.mode === 'release') {
			...
			if (owner.clientId !== normalized.clientId)
				throw resizeOwnershipError(owner);
			this.resizeOwners.delete(key);
		...
		if (
			...
			(owner === undefined || owner.clientId !== normalized.clientId)
		) {
			throw owner === undefined
				? new TerminalServiceError(
						'forbidden',
						'terminal resize ownership has expired',
						{ reason: 'resize_owner' },
					)
				: resizeOwnershipError(owner);
		}
		if (
			normalized.mode === 'claim' &&
			owner !== undefined &&
			owner.clientId !== normalized.clientId
		)
			throw resizeOwnershipError(owner);
		...
		if (snapshot === undefined)
			throw new TerminalServiceError(
				...
				{ sessionId: normalized.identity.sessionId },
			);
		const leaseExpiresAt =
			now +
			boundedLease(
				normalized.leaseMs ?? this.resizeLeaseMs,
				this.maxResizeLeaseMs,
			);
		...
		return Object.freeze({
			...
			ownership: freezeOwnership(next),
		});
	}
	...
	private async pump(queue: InputQueue): Promise<void> {
		...
		try {
			...
			queue.active = false;
			if (queue.items.length === 0 && queue.pendingBytes === 0)
	...
	private assertWritable(
		...
		if (
			...
			throw new TerminalServiceError(
				'forbidden',
				'terminal identity is outside its source boundary',
				{ reason: 'identity' },
			);
		...
		if (authorization !== undefined) {
			if (
				...
				throw new TerminalServiceError(
					'forbidden',
					'terminal authorization is outside its source boundary',
					{ reason: 'authorization' },
				);
			if (authorization.scope !== 'write' && authorization.scope !== 'admin')
				throw new TerminalServiceError(
					'forbidden',
					'terminal source is not authorized to write',
					{ reason: 'write' },
				);
		}
	}
...
function normalizeInputRequest(
	request: TerminalInputSourceRequest,
): TerminalInputSourceRequest {
	validateIdentity(request.identity);
	validateClientId(request.clientId);
	validateSource(request.source);
	if (
		request.authorization?.clientId !== undefined &&
		request.authorization.clientId !== request.clientId
	)
		throw new TerminalServiceError(
			'forbidden',
			'terminal authorization client does not match input source',
			{ reason: 'client' },
		);
	if (
		...
		throw new TerminalServiceError(
			'invalid_position',
			'terminal input sequence is invalid',
		);
	return request;
}
...
function normalizeResizeRequest(
	...
	validateClientId(request.clientId);
	validateSource(request.source);
	if (!VIEWPORTS.includes(request.viewport))
		throw new TerminalServiceError(
			'invalid_identity',
			'terminal viewport is invalid',
		);
	if (
		request.mode !== 'claim' &&
		request.mode !== 'update' &&
		...
		throw new TerminalServiceError(
			'invalid_identity',
			'terminal resize mode is invalid',
		);
	if (
		request.mode !== 'release' &&
		(!Number.isSafeInteger(request.cols) || !Number.isSafeInteger(request.rows))
	)
		throw new TerminalServiceError(
			'invalid_dimensions',
			'terminal resize dimensions are invalid',
		);
	if (
		request.mode === 'release' &&
		...
		throw new TerminalServiceError(
			'invalid_dimensions',
			'terminal release cannot carry dimensions',
		);
	return request;
}
...
function validateIdentity(identity: TerminalIdentity): void {
	for (const name of Object.keys(identity))
		if (name !== 'serverId' && name !== 'projectId' && name !== 'sessionId')
			throw new TerminalServiceError(
				'invalid_identity',
				'terminal identity contains unknown fields',
			);
	for (const value of [
		identity.serverId,
		identity.projectId,
...
function toBytes(value: Uint8Array | string): Uint8Array {
	...
	if (!(bytes instanceof Uint8Array) || bytes.byteLength === 0)
		throw new TerminalServiceError(
			'invalid_bytes',
			'terminal input bytes are invalid',
		);
	return bytes.slice();
}
...
function resizeOwnershipError(
	owner: MutableResizeOwnership,
): TerminalServiceError {
	return new TerminalServiceError(
		'forbidden',
		'terminal resize is owned by another client',
		{ reason: 'resize_owner', actual: owner.clientId },
	);
}

function freezeOwnership(
	value: MutableResizeOwnership,
...
function boundedLease(value: number, max: number): number {
	if (!Number.isSafeInteger(value) || value <= 0 || value > max)
		throw new RangeError('resize lease is out of bounds');
	return value;
}
