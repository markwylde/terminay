...
export type NodePtyForegroundListener = (
	event: NodePtyForegroundProcess,
) => void;
...
export interface NodePtyProcess extends PtyProcess {
	readonly onForegroundProcess: (
		listener: NodePtyForegroundListener,
	) => Unsubscribe;
}
...
export interface NodePtyFactoryOptions {
	...
	readonly resolveForegroundProcess?: (
		pid: number,
		signal?: AbortSignal,
	) => Promise<string | null>;
}
...
export function createNodePtyFactory(
	...
	factoryOptions: NodePtyFactoryOptions = {},
): { readonly spawn: (options: PtySpawnOptions) => NodePtyProcess } {
	if (module === undefined || typeof module.spawn !== 'function')
		throw new TypeError('node-pty module must provide spawn');
	const foregroundPolling = createForegroundPolling(
		factoryOptions.foregroundPolling,
	);
	return {
		spawn(options: PtySpawnOptions): NodePtyProcess {
			...
			return {
				...
				...(typeof child.pause === 'function'
					? { pause: () => child.pause?.() }
					: {}),
				...(typeof child.resume === 'function'
					? { resume: () => child.resume?.() }
					: {}),
				...
				factoryOptions.resolveCwd !== undefined
					? {
							getCwd: (signal?: AbortSignal) =>
								factoryOptions.resolveCwd!(child.pid!, signal),
						}
					: {}),
...
function createForegroundPolling(
	...
	if (!Number.isFinite(intervalMs) || intervalMs <= 0)
		throw new RangeError(
			'foreground polling intervalMs must be a positive finite number',
		);
	const setTimer =
		options?.setInterval ??
...
function createForegroundObserver(
	...
	const publish = (
		...
		if (observation === 'available') {
			...
			if (processName === lastProcess && lastObservation === 'available')
				return;
		...
		const event = Object.freeze({
			...
			shellForeground:
				...
				isConfiguredShellProcess(publishedName, shellProcess),
			observation,
		});
	...
	const startSample = (): void => {
		...
		const onLatestAbort = (): void => {
			controller.abort(
				latestSignal?.reason ?? new Error('foreground observation aborted'),
			);
		};
	...
	const observeFresh = (signal?: AbortSignal): Promise<void> => {
		...
		return new Promise((resolve, reject) => {
			...
			sampleAbort?.abort(
				signal?.reason ?? new Error('foreground observation replaced'),
			);
			pending = false;
			queueMicrotask(() => {
	...
	return {
		...
		subscribe(listener: NodePtyForegroundListener): Unsubscribe {
			if (typeof listener !== 'function')
				throw new TypeError('foreground listener must be a function');
			if (disposed) return () => {};
...
function isConfiguredShellProcess(
	processName: string,
	configuredShell: string,
): boolean {
	const normalized = processName.startsWith('-')
		? processName.slice(1)
		: processName;
