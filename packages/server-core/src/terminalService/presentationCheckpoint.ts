...
export interface TerminalPresentationCheckpointAuthorityOptions
	extends TerminalPresentationCheckpointLimits {
	readonly now?: () => number;
	readonly generateCheckpointId?: () => string;
}

export interface TerminalPresentationCheckpointPrepared
	extends TerminalIdentity {
...
export interface TerminalPresentationCheckpointMetadata
	extends TerminalPresentationCheckpointPrepared {
	readonly attachmentId: string;
}
...
export class TerminalPresentationCheckpointAuthority {
	...
	createSession(
		...
		if (existing !== undefined) {
			...
			throw error(
				'checkpoint_invalid',
				'terminal checkpoint session already exists with different dimensions',
			);
		}
		if (this.sessions.size >= this.limits.maxSessions)
			throw error(
				'checkpoint_limit',
				'terminal checkpoint session limit reached',
			);
		...
		const terminal = new Terminal({
			...
			scrollback: this.limits.maxScrollback,
			allowProposedApi: true,
		});
		const serializer = new SerializeAddon();
		terminal.loadAddon(serializer);
		const initial = new TextEncoder().encode(
			serializer.serialize({ scrollback: this.limits.maxScrollback }),
		);
		if (initial.byteLength > this.limits.maxSnapshotBytes)
			throw error(
				'checkpoint_limit',
				'initial terminal checkpoint exceeds its serialized size limit',
			);
		this.sessions.set(key, {
			...
			tailBytes: 0,
			nextTailSequence: 0,
			processedTailSequence: 0,
			unretainedThroughSequence: 0,
			queuedBytes: 0,
			pinnedBytes: 0,
			unavailable: false,
			tailOverflowed: false,
			lastCheckpointAt: this.now(),
			safeScanner: new SafeBoundaryScanner(),
	...
	ingestOutput(
		...
		if (!(bytes instanceof Uint8Array) || bytes.byteLength === 0)
			throw error(
				'checkpoint_invalid',
				'terminal checkpoint output bytes are invalid',
			);
		if (bytes.byteLength > this.limits.maxOutputChunkBytes)
			return this.fail(
				session,
				error(
					'checkpoint_limit',
					'terminal checkpoint output chunk exceeds its limit',
				),
			);
		...
		if (position !== session.outputPosition)
			return this.fail(
				session,
				error(
					'checkpoint_unavailable',
					'terminal checkpoint output position is not contiguous',
				),
			);
		if (session.queuedBytes + bytes.byteLength > this.limits.maxQueuedBytes)
			return this.fail(
				session,
				error(
					'checkpoint_limit',
					'terminal checkpoint parser backlog exceeds its limit',
				),
			);
		...
		const sequence = this.appendTail(
			session,
			Object.freeze({ type: 'output', position, nextPosition, bytes: copy }),
			copy.byteLength,
		);
		return this.enqueue(session, async () => {
			try {
				...
				await writeTerminal(
					session.terminal,
					session.decoder.decode(copy, { stream: true }),
				);
				session.processedPosition = nextPosition;
				session.processedTailSequence = sequence;
				if (session.safeScanner.safe && this.shouldCheckpoint(session))
					this.snapshot(session, sequence);
			} finally {
				session.queuedBytes -= copy.byteLength;
			}
		});
	...
	ingestResize(
		...
		const sequence = this.appendTail(
			...
			Object.freeze({
				type: 'resize',
				position: session.outputPosition,
				dimensions: next,
			}),
			0,
		);
	...
	async prepare(
	...
	): Promise<TerminalPresentationCheckpointPrepared> {
		const session = this.requireSession(identity);
		...
		if (session.pins.size >= this.limits.maxPinsPerSession)
			throw error('checkpoint_limit', 'terminal checkpoint pin limit reached');
		await session.queue;
		if (session.unavailable)
			throw error(
				'checkpoint_unavailable',
		...
		if (session.safeScanner.safe)
			this.snapshot(session, session.processedTailSequence);
		if (session.tailOverflowed)
			throw error(
				'checkpoint_unavailable',
				'terminal checkpoint tail exceeds its recovery limit',
			);
		...
		const tailBytes = tail.reduce(
			(sum, event) =>
				sum + (event.type === 'output' ? event.bytes.byteLength : 0),
			0,
		);
		const byteLength = checkedAdd(state.byteLength, tailBytes);
		if (session.pinnedBytes + byteLength > this.limits.maxPinnedBytesPerSession)
			throw error(
				'checkpoint_limit',
				'terminal checkpoint pinned bytes exceed their limit',
			);
		const pin: MutablePin = {
			checkpointId: this.nextCheckpointId(),
			...session.identity,
			clientId: options.clientId,
			position: session.checkpointPosition,
			headPosition: session.outputPosition,
			...
			tailByteLength: tailBytes,
			byteLength,
			expiresAt: checkedAdd(this.now(), this.limits.maxPinLifetimeMs),
			state,
			tail,
			consumed: false,
		};
	...
	bind(
		checkpointId: string,
		scope: TerminalPresentationCheckpointScope,
		...
		if (
			...
			throw error(
				'checkpoint_forbidden',
				'terminal checkpoint belongs to another attachment',
			);
		pin.attachmentId = scope.attachmentId;
		return metadata(pin);
	}
	...
	async pin(
		scope: TerminalPresentationCheckpointScope,
	): Promise<TerminalPresentationCheckpointMetadata> {
	...
	read(
		...
		return Object.freeze({
			...metadata(pin),
			state: new Uint8Array(pin.state),
			tail: cloneTail(pin.tail),
		});
	}
	...
	release(
		checkpointId: string,
		scope?: Partial<TerminalPresentationCheckpointScope>,
	): void {
		if (typeof checkpointId !== 'string' || checkpointId.length === 0) return;
		for (const session of this.sessions.values()) {
			const pin = session.pins.get(checkpointId);
			if (pin === undefined) continue;
			if (scope !== undefined && !matchesScope(pin, scope, false))
				throw error(
					'checkpoint_forbidden',
					'terminal checkpoint is outside its attachment boundary',
				);
	...
	releaseAttachment(scope: TerminalPresentationCheckpointScope): void {
		for (const session of this.sessions.values()) {
			...
			for (const pin of [...session.pins.values()])
				if (
					pin.clientId === scope.clientId &&
					pin.attachmentId === scope.attachmentId
				)
					this.release(pin.checkpointId, scope);
		}
	}
	...
	releaseClient(clientId: string): void {
		for (const session of this.sessions.values())
			for (const pin of [...session.pins.values()])
				if (pin.clientId === clientId) this.release(pin.checkpointId);
	}
	...
	close(): void {
		for (const session of [...this.sessions.values()])
			this.closeSession(session.identity);
	}

	session(
		identity: TerminalIdentity,
	): TerminalPresentationCheckpointSessionSnapshot | undefined {
		const session = this.sessions.get(sessionKey(identity));
		...
		return Object.freeze({
			...
			checkpointPosition: session.checkpointPosition,
			queuedBytes: session.queuedBytes,
			tailBytes: session.tailBytes,
			pins: session.pins.size,
			pinnedBytes: session.pinnedBytes,
			unavailable: session.unavailable || session.tailOverflowed,
		});
	}
	...
	async readPresentation(
		...
		const maxRows =
			options.maxRows === undefined
				? this.limits.maxScrollback + session.dimensions.rows
				: positive(options.maxRows, 'maxRows');
		const format: TerminalPresentationFormat = options.format ?? 'text';
		if (format !== 'text' && format !== 'ansi')
			throw error(
				'checkpoint_invalid',
				'terminal presentation format is invalid',
			);
		...
		if (format === 'ansi') {
			...
			const serialized = session.serializer.serialize({
				scrollback: retainedRows - session.dimensions.rows,
			});
		...
		return Object.freeze({
			...
			droppedBytes: Math.max(
				0,
				byteLength(fullText) - byteLength(returnedText),
			),
	...
	private appendTail(
		...
		event: TerminalPresentationCheckpointTailEvent,
		bytes: number,
	): number {
		...
		if (
			!session.tailOverflowed &&
			session.tail.length < this.limits.maxTailEvents &&
			session.tailBytes + bytes <= this.limits.maxTailBytes
		) {
	...
	private shouldCheckpoint(session: MutableSession): boolean {
		return (
			session.tailOverflowed ||
			session.tailBytes >= this.limits.checkpointIntervalBytes ||
			this.now() - session.lastCheckpointAt >= this.limits.checkpointIntervalMs
		);
	}
	...
	private snapshot(session: MutableSession, throughSequence: number): void {
		...
		const state = new TextEncoder().encode(
			session.serializer.serialize({ scrollback: this.limits.maxScrollback }),
		);
		if (state.byteLength > this.limits.maxSnapshotBytes)
			throw error(
				'checkpoint_limit',
				'terminal checkpoint exceeds its serialized size limit',
			);
		...
		session.tail = session.tail.filter(
			(record) => record.sequence > throughSequence,
		);
		session.tailBytes = session.tail.reduce(
			(sum, record) => sum + record.byteLength,
			0,
		);
		if (session.unretainedThroughSequence <= throughSequence)
			session.unretainedThroughSequence = 0;
		session.tailOverflowed =
			session.unretainedThroughSequence > throughSequence ||
			session.tailBytes > this.limits.maxTailBytes ||
			session.tail.length > this.limits.maxTailEvents;
	...
	private requireSession(identity: TerminalIdentity): MutableSession {
		...
		if (session === undefined)
			throw error(
				'checkpoint_not_found',
				'terminal checkpoint session is unavailable',
			);
		return session;
	}
	...
	private requirePin(
		...
		const pin = session.pins.get(checkpointId);
		if (pin === undefined) {
			if (this.expiredCheckpointIds.has(checkpointId))
				throw error('checkpoint_expired', 'terminal checkpoint expired');
			if (this.consumedCheckpointIds.has(checkpointId))
				throw error(
					'checkpoint_consumed',
					'terminal checkpoint was already consumed',
				);
			throw error('checkpoint_not_found', 'terminal checkpoint is unavailable');
		}
		if (pin.consumed)
		...
		if (!matchesScope(pin, scope, requireBound, ignoreAttachment))
			throw error(
				'checkpoint_forbidden',
				'terminal checkpoint is outside its attachment boundary',
			);
	...
	private fail(
		session: MutableSession,
		reason: TerminalPresentationCheckpointError,
	): Promise<never> {
		session.unavailable = true;
	...
	private rememberTombstone(
		...
		if (
			!tombstones.has(checkpointId) &&
			tombstones.size >= MAX_PIN_TOMBSTONES
		) {
	...
	private nextCheckpointId(): string {
		const id =
			this.generateCheckpointIdHook?.() ??
			`terminal-checkpoint:${++this.checkpointCounter}`;
		validateId(id, 'checkpointId');
		for (const session of this.sessions.values())
			if (session.pins.has(id))
				throw error(
					'checkpoint_invalid',
					'terminal checkpoint id already exists',
				);
		return id;
	}
}
...
class SafeBoundaryScanner {
	private state: 'ground' | 'escape' | 'csi' | 'string' | 'stringEscape' =
		'ground';
	...
	get safe(): boolean {
		return (
			this.state === 'ground' && this.utf8Remaining === 0 && !this.hyperlinkOpen
		);
	}

	ingest(bytes: Uint8Array): void {
		for (const byte of bytes) this.consume(byte);
	}
	...
	private consume(byte: number): void {
		...
		if (byte === 0x90 || byte === 0x98 || byte === 0x9e || byte === 0x9f) {
			this.startString(false);
			return;
		}
	...
	private consumeStringByte(byte: number): void {
		...
		if (this.oscSemicolons === 0) {
			...
			else if (this.oscCode.length < 8 && byte >= 0x30 && byte <= 0x39)
				this.oscCode += String.fromCharCode(byte);
			return;
		}
...
function boundedRows(
	...
	return {
		rows: selected,
		droppedRows,
		truncated: truncated || droppedRows > 0,
	};
}
...
function truncateUtf8(
	value: string,
	maxBytes: number,
): { readonly value: string; readonly droppedBytes: number } {
...
function preparedMetadata(
	pin: MutablePin,
): TerminalPresentationCheckpointPrepared {
	return Object.freeze({
		...
		headPosition: pin.headPosition,
		checkpointDimensions: Object.freeze({ ...pin.checkpointDimensions }),
		dimensions: Object.freeze({ ...pin.dimensions }),
		formatVersion: pin.formatVersion,
		stateByteLength: pin.stateByteLength,
		tailByteLength: pin.tailByteLength,
		byteLength: pin.byteLength,
		expiresAt: pin.expiresAt,
	});
}
function metadata(pin: MutablePin): TerminalPresentationCheckpointMetadata {
	if (pin.attachmentId === undefined)
		throw error(
			'checkpoint_forbidden',
			'terminal checkpoint is not bound to an attachment',
		);
	return Object.freeze({
		...preparedMetadata(pin),
		attachmentId: pin.attachmentId,
	});
}
function cloneTail(
	tail: readonly TerminalPresentationCheckpointTailEvent[],
): readonly TerminalPresentationCheckpointTailEvent[] {
	return Object.freeze(
		tail.map((event) =>
			event.type === 'output'
				...
				: Object.freeze({
						...event,
						dimensions: Object.freeze({ ...event.dimensions }),
					}),
		),
	);
}
function matchesScope(
	...
	if (
		(scope.serverId !== undefined && pin.serverId !== scope.serverId) ||
		(scope.projectId !== undefined && pin.projectId !== scope.projectId) ||
		(scope.sessionId !== undefined && pin.sessionId !== scope.sessionId) ||
		(scope.clientId !== undefined && pin.clientId !== scope.clientId)
	)
		return false;
	return (
		(!requireBound || pin.attachmentId !== undefined) &&
		(ignoreAttachment ||
...
function sameIdentity(
	left: TerminalIdentity,
	right: TerminalIdentity,
): boolean {
	return (
...
function validateScope(scope: TerminalPresentationCheckpointScope): void {
	validateIdentity(scope);
	validateId(scope.clientId, 'clientId');
	validateId(scope.attachmentId, 'attachmentId');
}
function validateIdentity(identity: TerminalIdentity): void {
	validateId(identity.serverId, 'serverId');
	validateId(identity.projectId, 'projectId');
	validateId(identity.sessionId, 'sessionId');
}
function validateId(value: string, name: string): void {
	if (typeof value !== 'string' || !ID_PATTERN.test(value))
		throw error('checkpoint_invalid', `${name} is invalid`);
}
function validatePosition(value: number): void {
	if (!Number.isSafeInteger(value) || value < 0)
		throw error(
			'checkpoint_invalid',
			'terminal checkpoint position is invalid',
		);
}
function validateDimensions(value: TerminalDimensions, limits: Limits): void {
	if (
		...
		throw error(
			'checkpoint_invalid',
			'terminal checkpoint dimensions are invalid',
		);
}
function normalizeLimits(
	options: TerminalPresentationCheckpointLimits,
): Limits {
	return {
		...
		maxCols: positive(options.maxCols ?? DEFAULT_MAX_COLS, 'maxCols'),
		maxRows: positive(options.maxRows ?? DEFAULT_MAX_ROWS, 'maxRows'),
		maxScrollback: nonNegative(
			options.maxScrollback ?? DEFAULT_MAX_SCROLLBACK,
			'maxScrollback',
		),
		maxOutputChunkBytes: positive(
			options.maxOutputChunkBytes ?? DEFAULT_MAX_OUTPUT_CHUNK_BYTES,
			'maxOutputChunkBytes',
		),
		maxQueuedBytes: positive(
		...
		maxTailBytes: positive(
			options.maxTailBytes ?? DEFAULT_MAX_TAIL_BYTES,
			'maxTailBytes',
		),
		maxTailEvents: positive(
			options.maxTailEvents ?? DEFAULT_MAX_TAIL_EVENTS,
			'maxTailEvents',
		),
		checkpointIntervalBytes: positive(
			options.checkpointIntervalBytes ?? DEFAULT_CHECKPOINT_INTERVAL_BYTES,
			'checkpointIntervalBytes',
		),
		checkpointIntervalMs: positive(
			options.checkpointIntervalMs ?? DEFAULT_CHECKPOINT_INTERVAL_MS,
			'checkpointIntervalMs',
		),
		maxPinsPerSession: positive(
			options.maxPinsPerSession ?? DEFAULT_MAX_PINS_PER_SESSION,
			'maxPinsPerSession',
		),
		maxPinnedBytesPerSession: positive(
			options.maxPinnedBytesPerSession ?? DEFAULT_MAX_PINNED_BYTES_PER_SESSION,
			'maxPinnedBytesPerSession',
		),
		maxPinLifetimeMs: positive(
			options.maxPinLifetimeMs ?? DEFAULT_MAX_PIN_LIFETIME_MS,
			'maxPinLifetimeMs',
		),
	};
}
...
function checkedAdd(left: number, right: number): number {
	...
	if (!Number.isSafeInteger(sum))
		throw error(
			'checkpoint_limit',
			'terminal checkpoint size exceeds its limit',
		);
	return sum;
}
...
function error(
	code: TerminalPresentationCheckpointErrorCode,
	message: string,
): TerminalPresentationCheckpointError {
	return new TerminalPresentationCheckpointError(code, message);
}
