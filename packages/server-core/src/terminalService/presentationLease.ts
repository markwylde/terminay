...
export type TerminalPresentationLeaseMode =
	| 'acquire'
	| 'renew'
	| 'takeover'
	| 'release'
...
export interface TerminalPresentationLeaseOptions {
	...
	readonly onChanged?: (
		...
		action: TerminalPresentationLeaseMode,
	) => void;
}
...
export class TerminalPresentationLeaseAuthority {
	...
	constructor(options: TerminalPresentationLeaseOptions = {}) {
		this.maxLeaseMs = positiveLease(
			options.maxLeaseMs ?? DEFAULT_MAX_LEASE_MS,
			Number.MAX_SAFE_INTEGER,
		);
		this.leaseMs = positiveLease(
	...
	change(
		mode: TerminalPresentationLeaseMode,
		identity: TerminalPresentationLeaseIdentity,
		options: { leaseMs?: number; admin?: boolean } = {},
	): TerminalPresentationLeaseState {
		const key = sessionKey(identity);
		...
		if (mode === 'revoke') {
			if (options.admin !== true)
				throw new TerminalServiceError(
					'forbidden',
					'terminal presentation revocation requires admin access',
					{ reason: 'presentation_admin' },
				);
			if (current === undefined) return this.state(identity);
			return this.commit(key, identity, undefined, mode);
		}
		...
		const leaseExpiresAt =
			this.now() +
			positiveLease(options.leaseMs ?? this.leaseMs, this.maxLeaseMs);
		return this.commit(
			key,
			identity,
			{ identity: { ...identity }, leaseExpiresAt },
			mode,
		);
	}
	...
	assertHolder(
		...
		if (current === undefined)
			throw new TerminalServiceError(
				'forbidden',
				'terminal presentation is read-only',
				{ reason: 'presentation_owner' },
			);
		this.assertSameHolder(current, identity);
	...
	releaseAttachment(identity: TerminalPresentationLeaseIdentity): boolean {
		const key = sessionKey(identity);
		const current = this.current(key);
		if (current === undefined || !sameHolder(current.identity, identity))
			return false;
	...
	releaseClient(identity: TerminalIdentity, clientId: string): boolean {
		const key = sessionKey(identity);
		const current = this.current(key);
		if (current === undefined || current.identity.clientId !== clientId)
			return false;
		this.commit(key, identity, undefined, 'release');
		return true;
	}
	...
	private assertSameHolder(
		current: MutableLease,
		identity: TerminalPresentationLeaseIdentity,
	): void {
		if (!sameHolder(current.identity, identity))
			throw new TerminalServiceError(
				'forbidden',
				'terminal presentation is controlled by another attachment',
				{ reason: 'presentation_owner', actual: current.identity.clientId },
			);
	}
	...
	private commit(
		...
		lease: MutableLease | undefined,
		action: TerminalPresentationLeaseMode,
	): TerminalPresentationLeaseState {
		if (lease === undefined) this.leases.delete(key);
		else this.leases.set(key, lease);
		const revision = (this.revisions.get(key) ?? 0) + 1;
		this.revisions.set(key, revision);
...
function sameHolder(
	left: TerminalPresentationLeaseIdentity,
	right: TerminalPresentationLeaseIdentity,
): boolean {
	return (
...
function positiveLease(value: number, max: number): number {
	if (!Number.isSafeInteger(value) || value <= 0 || value > max)
		throw new RangeError('terminal presentation lease is out of bounds');
	return value;
}
		...
		...(lease === undefined
			...
			: {
					holder: Object.freeze({
						clientId: lease.identity.clientId,
						attachmentId: lease.identity.attachmentId,
