...
export class TerminalSubscription {
	...
	get closeReason(): TerminalCloseReason | undefined {
	...
	drain(): readonly TerminalEvent[] {
		...
		const last = [...events]
			.reverse()
			.find((event): event is TerminalOutputEvent => event.type === 'output');
		if (last !== undefined)
			this.cursorValue = Math.max(this.cursorValue, last.nextPosition);
		return events;
	}

	poll(): readonly TerminalEvent[] {
		return this.drain();
	}
	...
	ack(position: number): void {
		...
		if (position > this.session.outputPosition)
			throw new TerminalServiceError(
				'invalid_position',
				'acknowledged position is ahead of terminal output',
				{ expected: this.session.outputPosition, actual: position },
			);
		this.cursorValue = position;
		this.queue = this.queue.filter(
			(event) => event.type !== 'output' || event.nextPosition > position,
		);
		this.queuedBytesValue = this.queue.reduce(
	...
	closeForOverflow(): void {
		...
		if (this.listener !== undefined) {
			try {
				this.listener(copyEvent(event));
			} catch {
				/* observer cannot affect PTY supervision */
			}
		}
	...
	private deliver(event: TerminalEvent): void {
		...
		try {
			...
			this.service.reportListenerFault(this, cause);
		}
		if (event.type === 'output')
			this.cursorValue = Math.max(this.cursorValue, event.nextPosition);
	}
...
export class TerminalService {
	...
	private readonly generateSessionIdHook:
	...
	constructor(
		...
		limits?: TerminalServiceLimits,
	);
	constructor(
		optionsOrServerId: TerminalServiceOptions | string,
		factory?: PtyFactory,
		limits: TerminalServiceLimits = {},
	) {
		const options: TerminalServiceOptions =
			typeof optionsOrServerId === 'string'
				? {
						serverId: optionsOrServerId,
						ptyFactory: factory as PtyFactory,
						...limits,
					}
				: optionsOrServerId;
		...
		if (
			!options.ptyFactory ||
			(typeof options.ptyFactory !== 'function' &&
				typeof options.ptyFactory.spawn !== 'function')
		)
			throw new TypeError('ptyFactory must provide spawn');
		...
		if (this.eventListener !== undefined)
			this.listeners.add(this.eventListener);
	}
	...
	onInput(
		listener: (identity: TerminalIdentity, bytes: Uint8Array) => void,
	): Unsubscribe {
		if (typeof listener !== 'function')
			throw new TypeError('terminal input listener must be a function');
	...
	async observeForegroundProcess(
		...
		const lastBusy =
			mutable.lastForeground === undefined
				? false
				: !mutable.lastForeground.shellForeground;
		const result = (
		...
		const onOuterAbort = (): void => {
			controller.abort(
				signal?.reason ?? new Error('foreground observation aborted'),
			);
		};
		...
		try {
			...
			const refreshPromise = Promise.resolve(
				mutable.process.refreshForegroundProcess(controller.signal),
			);
			refreshPromise.catch(() => undefined);
			await Promise.race([refreshPromise, timeoutPromise]);
			return observationFromLastForeground(
				mutable.lastForeground,
				lastBusy,
				result,
			);
		} catch (error) {
			return result(
				...
				error instanceof Error &&
					error.message === 'foreground observation timeout'
					? 'timeout'
					: 'failed',
			);
		} finally {
	...
	listSessions(): readonly TerminalSessionSnapshot[] {
		return [...this.sessionsById.values()].map(snapshotOf);
	}
	...
	async createSession(
		...
		if (options.serverId !== undefined && options.serverId !== this.serverId)
			throw new TerminalServiceError(
				'forbidden',
				'terminal belongs to another server',
				{ serverId: this.serverId, actual: options.serverId },
			);
		assertId(options.projectId, 'projectId');
		const sessionId =
			options.sessionId ?? this.nextSessionId(options.projectId);
		assertId(sessionId, 'sessionId');
		...
		const identity: TerminalIdentity = Object.freeze({
		...
		if (typeof shellPath !== 'string' || shellPath.trim().length === 0)
			throw new TypeError('default shell path is invalid');
		...
		const spawnOptions: PtySpawnOptions = {
			...
			lifecycleEnvironment === undefined
				? {}
				: {
						env: {
		...
		try {
			...
			if (
				...
				try {
					this.sessionLifecycle?.terminalStarted?.(identity, process.pid);
				} catch {
					/* observers cannot fail PTY creation */
				}
			}
	...
	async createResolvedSession(
		...
		if (launch.identity.serverId !== this.serverId)
			throw new TerminalServiceError(
				'forbidden',
				'terminal belongs to another server',
			);
		...
		if (this.sessionsById.has(launch.identity.sessionId))
			throw new TerminalServiceError(
				'session_exists',
				'terminal session already exists',
		...
		if (!Number.isSafeInteger(launch.createdAt) || launch.createdAt < 0)
			throw new TypeError('createdAt must be a non-negative safe integer');
		if (
			typeof launch.shellPath !== 'string' ||
			launch.shellPath.trim().length === 0
		)
			throw new TypeError('resolved shell path is invalid');
		if (typeof launch.cwd !== 'string' || launch.cwd.length === 0)
			throw new TypeError('resolved terminal cwd is invalid');
		...
		const mutable: MutableSession = {
			...
			launch: Object.freeze({
				...
				...(launch.profile.icon === undefined
					? {}
					: { icon: launch.profile.icon }),
				...(launch.profile.color === undefined
					? {}
					: { color: launch.profile.color }),
		...
		try {
			...
			console.error(
				'[terminay-terminal-spawn]',
				boundedSpawnFailureReason(error),
			);
			...
			try {
				...
				/* lifecycle cleanup cannot replace the bounded spawn error */
			}
			throw new TerminalServiceError(
				'spawn_failed',
	...
	create(options: TerminalCreateOptions): Promise<TerminalSessionHandle> {
	...
	subscribe(
		...
		if (mutable.subscribers.size >= this.limits.maxSubscribersPerSession)
			throw new TerminalServiceError(
				'subscriber_limit',
				'terminal subscriber limit reached',
				{ max: this.limits.maxSubscribersPerSession },
			);
		const from = options.fromPosition ?? 0;
		validatePosition(from);
		if (from > mutable.outputPosition)
			throw new TerminalServiceError(
				'invalid_position',
				'subscription position is ahead of terminal output',
				{ expected: mutable.outputPosition, actual: from },
			);
		if (from < mutable.replayFrom)
			throw new TerminalServiceError(
				'replay_gap',
				'requested terminal output is no longer retained',
				{
					fromPosition: from,
		...
		for (const chunk of mutable.replay) {
			...
			initial.push(
				outputEvent(mutable, chunk, Math.max(from, chunk.position), true),
			);
		}
	...
	attach(
		...
		return this.subscribe(session, options);
	}
	...
	readRetainedOutput(
		...
		const mutable = this.requireSession(session);
		this.authorize(mutable, options.authorization, 'read');
		const maxBytes = boundedReadBytes(
			options.maxBytes,
		...
		if (requestedFromPosition > mutable.outputPosition) {
			throw new TerminalServiceError(
				'invalid_position',
				'requested terminal output is ahead of the stream',
				{
		...
		const fromPosition = historyLost
			? mutable.replayFrom
		...
		return Object.freeze({
			...
			outputPosition: mutable.outputPosition,
			historyLost,
			droppedBytes: historyLost
				? mutable.replayFrom - requestedFromPosition
				: 0,
	...
	readPresentation(
		...
		if (this.presentationCheckpoints === undefined) {
			throw new TerminalServiceError(
				...
				'canonical terminal presentation is unavailable',
			);
		}
		return this.waitForPresentationDrain(mutable).then(() =>
			this.presentationCheckpoints!.readPresentation(mutable.identity, options),
		);
	}
	...
	async input(
		...
		if (bytes.byteLength > this.limits.maxInputBytes)
			throw new TerminalServiceError(
				...
				{ max: this.limits.maxInputBytes, actual: bytes.byteLength },
			);
	...
	write(
		...
		return this.input(session, data, authorization);
	}
	...
	waitForInactivity(
		...
		if (!Number.isFinite(durationMs) || durationMs < 0) {
			return Promise.reject(
				new RangeError('durationMs must be a finite non-negative number'),
			);
		}
		...
		return new Promise<void>((resolve, reject) => {
			...
			if (signal !== undefined) {
				waiter.abortListener = () =>
					this.rejectInactivityWaiter(mutable, waiter, abortReason(signal));
	...
	async kill(
		...
		try {
			await mutable.process.kill(signal);
		} catch (error) {
	...
	terminate(
		...
		return this.kill(session, authorization, signal);
	}
	...
	async interrupt(
		...
		try {
			await mutable.process?.kill('SIGTERM');
		} finally {
	...
	async shutdown(
		...
		await Promise.all(
			[...before].map(async (mutable) => {
				mutable.pendingExitReason = reason;
				try {
					await mutable.process?.kill(options.signal ?? 'SIGTERM');
				} catch {
					/* still publish one interruption for a process that cannot be reattached */
				}
				this.finish(
					mutable,
					{ exitCode: reason === 'shutdown' ? 143 : 130, signal: 15 },
					reason,
					at,
				);
			}),
	...
	markInterrupted(at = this.now()): readonly TerminalExitEvent[] {
		...
		for (const mutable of this.sessionsById.values()) {
			...
			mutable.pendingExitReason = 'interrupted';
			try {
				void mutable.process?.kill('SIGTERM');
			} catch {
				/* best effort */
			}
			this.finish(mutable, { exitCode: 130, signal: 15 }, 'interrupted', at);
	...
	interruptAll(at = this.now()): readonly TerminalExitEvent[] {
		return this.markInterrupted(at);
	}
	...
	private attachProcess(mutable: MutableSession, process: PtyProcess): void {
		const onData: PtyDataListener = (value) => {
			try {
				this.appendOutput(mutable, toBytes(value));
			} catch {
		...
		const onForegroundProcess = (event: PtyForegroundProcess) => {
			...
			try {
				this.sessionLifecycle?.foregroundProcessChanged?.(
					mutable.identity,
					event,
				);
			} catch {
		...
		if (process.onForegroundProcess !== undefined) {
			mutable.foregroundProcessUnsubscribe = normalizeUnsubscribe(
				process.onForegroundProcess(onForegroundProcess),
			);
		}
	}
	...
	private appendOutput(mutable: MutableSession, bytes: Uint8Array): void {
		...
		for (
			...
			const chunkBytes = copyBytes(
				bytes.slice(
					offset,
					Math.min(bytes.byteLength, offset + this.limits.maxOutputChunkBytes),
				),
			);
	...
	private observePresentationOutput(
	...
	private waitForPresentationDrain(mutable: MutableSession): Promise<void> {
		if (!mutable.checkpointDraining && mutable.checkpointQueue.length === 0)
			return Promise.resolve();
		return new Promise((resolve) =>
			mutable.checkpointDrainWaiters.add(resolve),
		);
	}
	...
	private observePresentationResize(
		mutable: MutableSession,
		dimensions: TerminalDimensions,
	): void {
		try {
			void this.presentationCheckpoints
				?.ingestResize(mutable.identity, dimensions)
				.catch(() => undefined);
		} catch {
	...
	private finish(
		...
		at: number,
		_spawnMessage?: string,
	): void {
		if (mutable.exit !== undefined) return;
		const metadata: TerminalExitMetadata = Object.freeze({
			...normalizeExit(rawExit),
			reason,
			at,
		});
		mutable.exit = metadata;
		mutable.status =
			reason === 'interrupted' || reason === 'shutdown'
				? 'interrupted'
				: 'exited';
		mutable.pendingExitReason = undefined;
	...
	private assertIdentity(
		...
		if (
			identity.serverId !== mutable.identity.serverId ||
			identity.projectId !== mutable.identity.projectId ||
			identity.sessionId !== mutable.identity.sessionId
		)
			throw new TerminalServiceError(
				'forbidden',
				'terminal identity is outside its server/project/session boundary',
				{
					serverId: mutable.identity.serverId,
	...
	private authorize(
		mutable: MutableSession,
		authorization: TerminalAuthorization | undefined,
		required: 'read' | 'write',
	): void {
		if (authorization === undefined) return;
		if (
			authorization.serverId !== mutable.identity.serverId ||
			authorization.projectId !== mutable.identity.projectId ||
			(authorization.sessionId !== undefined &&
				authorization.sessionId !== mutable.identity.sessionId)
		)
			throw new TerminalServiceError(
				'forbidden',
				'terminal authorization is outside its server/project/session boundary',
				{
					serverId: mutable.identity.serverId,
					projectId: mutable.identity.projectId,
					sessionId: mutable.identity.sessionId,
		...
		if (
			...
			throw new TerminalServiceError(
				'forbidden',
				'terminal operation is not authorized for this client',
				{ reason: required },
			);
	}
	...
	private emit(event: TerminalEvent): void {
		for (const listener of this.listeners) {
			try {
				...
				/* observers cannot affect the PTY */
			}
		}
	}
	...
	private armInactivityWaiter(
		mutable: MutableSession,
		waiter: InactivityWaiter,
	): void {
		if (
			waiter.settled ||
	...
	private rejectInactivityWaiter(
		mutable: MutableSession,
		waiter: InactivityWaiter,
		reason: unknown,
	): void {
	...
	private disposeInactivityWaiter(
		mutable: MutableSession,
		waiter: InactivityWaiter,
	): boolean {
	...
	private nextSessionId(projectId: string): string {
		...
		do {
			...
			this.sessionsById.has(`${this.serverId}:session:${this.sessionCounter}`)
		);
...
export class TerminalSessionHandle {
	...
	get identity(): TerminalIdentity {
		return this.session.identity;
	}
	get serverId(): string {
		return this.session.identity.serverId;
	}
	get projectId(): string {
		return this.session.identity.projectId;
	}
	get sessionId(): string {
		return this.session.identity.sessionId;
	}
	get status(): TerminalSessionStatus {
		return this.session.status;
	}
	get outputPosition(): number {
		return this.session.outputPosition;
	}
	get exit(): TerminalExitMetadata | undefined {
	...
	subscribe(options: TerminalSubscriptionOptions = {}): TerminalSubscription {
		return this.service.subscribe(this.identity, options);
	}
	attach(options: TerminalSubscriptionOptions = {}): TerminalSubscription {
	...
	input(
		...
		return this.service.input(this.identity, data, authorization);
	}
	write(
		data: Uint8Array | string,
		authorization?: TerminalAuthorization,
	): Promise<void> {
		return this.input(data, authorization);
	}
	resize(
		dimensions: TerminalDimensions,
		authorization?: TerminalAuthorization,
	): Promise<void> {
		return this.service.resize(this.identity, dimensions, authorization);
	}
	kill(
		authorization?: TerminalAuthorization,
		signal?: number | string,
	): Promise<void> {
		return this.service.kill(this.identity, authorization, signal);
	}
	interrupt(authorization?: TerminalAuthorization, at?: number): Promise<void> {
		return this.service.interrupt(this.identity, authorization, at);
	}
	waitForInactivity(
		durationMs: number,
		options: TerminalInactivityOptions = {},
	): Promise<void> {
		return this.service.waitForInactivity(this.identity, durationMs, options);
	}
}
...
function normalizeLimits(options: TerminalServiceOptions): NormalizedLimits {
	return {
		...
		maxOutputChunkBytes: positiveLimit(
			options.maxOutputChunkBytes ?? DEFAULT_MAX_OUTPUT_CHUNK_BYTES,
			'maxOutputChunkBytes',
		),
		maxReplayBytes: positiveLimit(
			options.maxReplayBytes ?? DEFAULT_MAX_REPLAY_BYTES,
			'maxReplayBytes',
		),
		maxQueuedOutputBytes: positiveLimit(
			options.maxQueuedOutputBytes ?? DEFAULT_MAX_QUEUED_OUTPUT_BYTES,
			'maxQueuedOutputBytes',
		),
		maxSubscribersPerSession: positiveLimit(
			options.maxSubscribersPerSession ?? DEFAULT_MAX_SUBSCRIBERS,
...
function validateDimensions(
	...
	limits: Pick<NormalizedLimits, 'maxCols' | 'maxRows'>,
): TerminalDimensions {
	if (
		!Number.isSafeInteger(value.cols) ||
		value.cols <= 0 ||
		value.cols > limits.maxCols ||
		!Number.isSafeInteger(value.rows) ||
		value.rows <= 0 ||
		value.rows > limits.maxRows
	)
		throw new TerminalServiceError(
			'invalid_dimensions',
			'terminal dimensions exceed the configured bounds',
			{ max: Math.max(limits.maxCols, limits.maxRows) },
		);
...
function toBytes(value: Uint8Array | string): Uint8Array {
	...
	throw new TerminalServiceError(
		'invalid_bytes',
		'terminal bytes must be a Uint8Array or string',
	);
}
...
function boundedReadBytes(value: number, maximum: number): number {
	if (!Number.isSafeInteger(value) || value <= 0 || value > maximum) {
		throw new TerminalServiceError(
			'invalid_bytes',
			'terminal read byte limit is invalid',
			{ max: maximum, actual: value },
		);
	}
	return value;
}
...
function normalizeExit(exit: PtyExit): {
	exitCode: number;
	signal: number | null;
} {
	const exitCode =
		typeof exit.exitCode === 'number' && Number.isSafeInteger(exit.exitCode)
			? exit.exitCode
			: 0;
	const signal =
		typeof exit.signal === 'number' &&
		Number.isSafeInteger(exit.signal) &&
		exit.signal > 0
			? exit.signal
			: null;
...
function outputEvent(
	...
	return Object.freeze({
		...
		...session.identity,
		position: start,
		nextPosition: chunk.nextPosition,
		bytes,
		data: bytes,
		replay,
	});
...
function exitEvent(session: MutableSession): TerminalExitEvent {
	if (session.exit === undefined)
		throw new Error('terminal exit metadata is missing');
	return Object.freeze({
		type: 'exit',
		...session.identity,
		metadata: session.exit,
...
function checkedPositionAdd(position: number, amount: number): number {
	...
	if (!Number.isSafeInteger(next))
		throw new TerminalServiceError(
			'output_too_large',
			'terminal output position exhausted',
		);
	return next;
}
...
function busyFromLastForeground(
	current: PtyForegroundProcess | undefined,
	lastBusy: boolean,
): boolean {
	if (
		current !== undefined &&
		current.processName.length > 0 &&
		!current.shellForeground
	)
		return true;
	return lastBusy;
}
...
function observationFromLastForeground(
	...
	if (current?.observation === 'limited') {
		return result(
			'limited',
			busyFromLastForeground(current, lastBusy),
			'failed',
		);
	}
	return result(
		'available',
		current === undefined ? false : !current.shellForeground,
	);
}

function normalizeUnsubscribe(
	value: Unsubscribe | undefined,
...
async function spawn(
	...
	const process =
		typeof factory === 'function'
			...
			: await factory.spawn(options);
	if (
		process === undefined ||
		typeof process.write !== 'function' ||
		typeof process.resize !== 'function' ||
		typeof process.kill !== 'function' ||
		typeof process.onData !== 'function' ||
		typeof process.onExit !== 'function'
	)
		throw new Error('PTY adapter returned an invalid process');
	return process;
}
