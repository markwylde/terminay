 ...
 */
export async function reportWorkspaceRecovery(
	state: WorkspaceState,
	probe: WorkspaceRootProbe,
): Promise<WorkspaceRecoveryReport> {
	validateWorkspace(state);
	if (typeof probe?.exists !== 'function')
		throw new TypeError('workspace root probe is required');
	const missingRoots: WorkspaceRootStatus[] = [];
	for (const project of Object.values(state.projects)) {
		if (!(await probe.exists(project.root, project.id)))
			missingRoots.push({
