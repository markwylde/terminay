...
export async function buildUiBundleManifest({
  ...
  const applicationFiles = files.filter((path) => path !== manifestRelativePath);
  if (!applicationFiles.includes(entryFile)) throw new Error(`UI bundle entry is missing: ${entryFile}`);
  if (applicationFiles.length === 0) throw new Error("UI bundle contains no application files");

  const provisionalAssets = await Promise.all(applicationFiles.map(async (path) => {
  ...
  const canonical = provisionalAssets
    .map((asset) => `${asset.path.slice("/remote-app/provisional/".length)}:${asset.hash}`)
    .sort()
    .join("\n");
  const bundleId = createHash("sha256").update(canonical).digest("base64url").slice(0, 32);
  const assets = provisionalAssets.map((asset) => ({
    ...asset,
    path: asset.path.replace("/remote-app/provisional/", `/remote-app/${bundleId}/`),
  }));
  const manifest = {
    schemaVersion: 1,
    bundleId,
...
async function walkRegularFiles(root) {
  ...
  async function visit(directory) {
    ...
    for (const entry of entries) {
      ...
      if (metadata.isSymbolicLink()) throw new Error(`UI bundle contains a symbolic link: ${relative(root, absolute)}`);
      if (metadata.isDirectory()) {
        ...
        throw new Error(`UI bundle contains a non-regular entry: ${relative(root, absolute)}`);
      }
    }
  }
  await visit(root);
...
if (process.argv[1] !== undefined && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  if (serverVersion === undefined || serverVersion.length === 0) {
    throw new Error("usage: build-ui-bundle-manifest.mjs [root-directory] <server-version> [protocol-version]");
  }
