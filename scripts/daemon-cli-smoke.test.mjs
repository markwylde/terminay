...
import { CONTAINER_DRIVER, SMOKE_STEPS, isSmokeEnabled, runDaemonSmoke } from './daemon-cli-smoke.mjs'
...
test('the smoke drives the whole documented lifecycle', () => {
  assert.deepEqual([...SMOKE_STEPS], ['install', 'status', 'upgrade', 'qr-code --no-wait', 'uninstall'])
  for (const step of SMOKE_STEPS) {
    assert.ok(CONTAINER_DRIVER.includes(`--- ${step} ---`), `the driver must run ${step}`)
  }
})
...
test('the smoke asserts against systemd itself, not against the CLI it just ran', () => {
  ...
  assert.match(CONTAINER_DRIVER, /systemctl\('cat', 'terminay-server\.service'\)/u)
  assert.match(CONTAINER_DRIVER, /systemctl\('is-enabled', 'terminay-server\.service'\)/u)
  assert.match(CONTAINER_DRIVER, /systemctl\('is-active', 'terminay-server\.service'\)/u)
...
test('the local-archive seam is a source install, so it is never a signature bypass', async () => {
  const source = await readFile(resolve('apps/terminay-cli/src/commands/install.ts'), 'utf8')
  ...
  assert.doesNotMatch(cli, /localArchivePath/u, 'the shipped binary must never install an unsigned local archive')
})

test('the container smoke passes when it is opted into', { skip: !isSmokeEnabled() }, async () => {
