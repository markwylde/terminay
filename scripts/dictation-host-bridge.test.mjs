...
const [app, settingsWindow, preload, declarations] = await Promise.all([
...
test('Desktop dictation routes use one narrow validated host', () => {
  ...
  for (const method of [
    ...
    assert.match(rendererSource, new RegExp(`dictationHost(?:\\?|)\\.${method}\\(`, 'u'))
  }
  assert.doesNotMatch(
    rendererSource,
  ...
  assert.match(preload, /exposeInMainWorld\('terminayDictationHost'/u)
