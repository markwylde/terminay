...
test('permits only the bounded vulnerabilities inside the pinned npm installer', () => {
  const result = evaluateProductionAudit(report({
    ...
    npm: { severity: 'high', nodes: ['node_modules/npm'] },
    tar: { severity: 'high', nodes: ['node_modules/npm/node_modules/tar'] },
  }), packageJson)
  assert.deepEqual(result.exceptions.map((item) => item.name), ['brace-expansion', 'ip-address', 'npm', 'tar'])
})
...
test('fails closed for critical, unrelated, relocated, or unpinned vulnerabilities', () => {
  ...
  assert.throws(() => evaluateProductionAudit(report({ tar: { severity: 'high', nodes: ['node_modules/tar'] } }), packageJson))
  assert.throws(() => evaluateProductionAudit(report({ npm: { severity: 'high', nodes: ['node_modules/npm/node_modules/npm'] } }), packageJson))
