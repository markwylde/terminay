...
test("pull-request CI selects one provider-specific E2E image and five-shard pair", () => {
  ...
  const macSmoke = job("packaged-macos-smoke");
  assert.match(macSmoke, /name: Packaged macOS startup smoke/u);
  assert.match(macSmoke, /runs-on: \$\{\{ github\.server_url == 'https:\/\/github\.com' && 'macos-latest' \|\| 'ubuntu-latest' \}\}/u);
  assert.match(macSmoke, /Record unavailable macOS runner/u);
  assert.match(macSmoke, /run: npm run test:packaged-startup-macos/u);
  assert.doesNotMatch(macSmoke, /(?:ea165f8d65b6e75b540449e92b4886f43607fa02|d3f86a106a0bac45b974a628896c90dbdf5c8093|ff15f0306b3f739f7b6fd43fb5d26cd321bd4de5|9bc31d5ccc31df68ecc42ccf4149144866c47d8a)/u);
...
test("provider-specific E2E shards wait only for their own image and use declared runners", () => {
  ...
  assert.match(job("packaged-macos-smoke"), /runs-on: \$\{\{ github\.server_url == 'https:\/\/github\.com' && 'macos-latest' \|\| 'ubuntu-latest' \}\}/u);
