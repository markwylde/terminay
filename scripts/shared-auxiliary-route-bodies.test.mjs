...
const routes = [
  ...
  ['edit-tab', 'src/shared/SharedEditTabRouteBody.tsx', 'src/components/EditTabWindow.tsx'],
]

test('all four auxiliary routes consume host-neutral shared bodies', async () => {
  for (const [route, sharedPath, desktopPath] of routes) {
    const [shared, desktop] = await Promise.all([readFile(sharedPath, 'utf8'), readFile(desktopPath, 'utf8')])
    ...
    assert.ok(component)
    assert.match(desktop, new RegExp(`<${component}`, 'u'))
  }
})
