...
export async function stageBuiltInExtensions(options = {}) {
	...
	const output = resolve(
		root,
		options.outputDirectory ?? 'build/built-in-extensions',
	);
	const catalogue = await loadCatalogue(
		join(root, 'extensions', 'builtins.json'),
	);
	...
	try {
		...
		for (const entry of catalogue) {
			...
			await unpack(
				archive.path,
				join(artifactDirectory, 'node_modules'),
				entry.packageName,
			);
			...
			artifacts.push(
				Object.freeze({
					...
					manifestMetadata: packageJson.terminay,
					directory: `artifacts/${entry.extensionId}`,
					inventoryHash: sha256(
						JSON.stringify(
							tree.map(({ path, size, sha256: hash }) => ({
								path,
								size,
								hash,
							})),
...
async function pack(root, workspace, destination) {
	...
	if (
		...
		throw new Error(`npm pack result for ${workspace} is invalid`);
	return Object.freeze({
		path: join(destination, result.filename),
...
function lockForArtifact(rootLock, packageJson, pack, sdk, copied) {
	...
	if (sdkPackage === undefined || typeof sdkPackage.version !== 'string')
		throw new Error(
			'public Extension API workspace package is missing from lockfile',
		);
	packages[`node_modules/${SDK}`] = {
		version: sdkPackage.version,
		resolved: 'file:extension-api.tgz',
	...
	for (const name of copied) {
		...
		if (
			...
			throw new Error(`staged production dependency is not locked: ${name}`);
	...
	return {
		name: 'terminay-built-in-extension-stage',
		lockfileVersion: 3,
		packages,
	};
}
	...
	async function walk(directory) {
		for (const entry of await readdir(directory, { withFileTypes: true })) {
			...
			if (entry.isSymbolicLink())
				throw new Error(`built-in artifact contains symlink: ${path}`);
			if (entry.isDirectory()) await walk(absolute);
				...
				throw new Error(
					`built-in artifact contains non-regular entry: ${path}`,
				);
		}
	}
	await walk(root);
...
async function loadCatalogue(path) {
	...
	if (
		...
		throw new Error(
			'built-in extension catalogue must name exactly five extensions',
		);
	const entries = value.extensions.map((entry) => {
		if (
			...
			!/^[a-z0-9][a-z0-9._-]{0,126}$/u.test(entry.extensionId) ||
			entry.directory.includes('/') ||
			entry.directory.includes('\\') ||
			entry.directory === '.' ||
			entry.directory === '..'
		)
			throw new Error('built-in extension catalogue entry is invalid');
		return Object.freeze({
	...
	if (
		new Set(entries.map((entry) => entry.extensionId)).size !== 5 ||
		new Set(entries.map((entry) => entry.packageName)).size !== 5 ||
		!entries.every((entry) => expectedIds.has(entry.extensionId))
	)
		throw new Error(
			'built-in extension catalogue does not match the official inventory',
		);
	return entries;
}
...
function assertPackage(entry, packageJson) {
	if (
		...
		packageJson.terminay?.id !== entry.extensionId
	)
		throw new Error(
			`packed built-in package identity differs from catalogue: ${entry.packageName}`,
		);
	if (!Array.isArray(packageJson.files) || packageJson.files.includes('src'))
		throw new Error(
			`built-in package has an unsafe pack policy: ${entry.packageName}`,
		);
}
function sha256(value) {
	return createHash('sha256').update(value).digest('hex');
}
function npm(cwd, argumentsValue) {
	return runFile(process.execPath, [npmCli, ...argumentsValue], {
		cwd,
...
if (import.meta.url === `file://${process.argv[1]}`) {
	...
	const result = await stageBuiltInExtensions({
		skipChecks,
		...(index === -1 ? {} : { outputDirectory: process.argv[index + 1] }),
	});
	process.stdout.write(
		`${JSON.stringify({ outputDirectory: result.outputDirectory, extensions: result.inventory.artifacts.map((artifact) => `${artifact.packageName}@${artifact.version}`) }, null, 2)}\n`,
	);
}
