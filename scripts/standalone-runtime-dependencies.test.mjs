...
async function writePackage(root, name, manifest) {
	...
	await writeFile(join(directory, 'index.js'), `export const name = ${JSON.stringify(name)}\n`);
	return directory;
}

test('standalone dependency staging closes production dependencies without workspace links', async () => {
	const root = await mkdtemp(join(tmpdir(), 'terminay-standalone-runtime-deps-'));
	try {
		const runtime = join(root, 'runtime');
		const workspace = join(root, 'workspace-core');
		await writePackage(runtime, 'external-root', { dependencies: { 'nested.value': '1.0.0' } });
		await writePackage(runtime, 'nested.value', {});
		await mkdir(workspace, { recursive: true });
		await writeFile(join(workspace, 'package.json'), JSON.stringify({ name: '@terminay/server-core', dependencies: { 'external-root': '1.0.0' } }));
		...
		assert.deepEqual(staged, ['@terminay/server-core', 'external-root', 'nested.value']);
		assert.equal((await readFile(join(root, 'artifact/node_modules/external-root/index.js'), 'utf8')).includes('external-root'), true);
		assert.equal((await readFile(join(root, 'artifact/node_modules/nested.value/index.js'), 'utf8')).includes('nested.value'), true);
	} finally {
...
test('standalone dependency staging dereferences a workspace package source', async () => {
	const root = await mkdtemp(join(tmpdir(), 'terminay-standalone-runtime-link-'));
	try {
		const runtime = join(root, 'runtime');
		const actual = join(root, 'actual-linked-package');
		await mkdir(actual, { recursive: true });
		await writeFile(join(actual, 'package.json'), JSON.stringify({ name: 'linked' }));
		...
		const info = await (await import('node:fs/promises')).lstat(join(root, 'artifact/node_modules/linked'));
