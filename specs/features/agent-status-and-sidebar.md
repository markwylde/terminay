...
The `terminay-agent-cursor` package binds a Cursor chat only from the exact
writable `store.db` held by the registered PTY process tree. It validates the
shared session UUID and bounded adjacent metadata before selecting the matching
JSONL transcript below Cursor's project root. CWD, nearest-file, timestamp, and
active-tab guesses never establish identity.

The extension observes bounded title and model metadata while bound, so rename
and model changes update the canonical entry without changing lifecycle state.
A missing title falls back to bounded user-query text with Cursor's timestamp
wrapper removed. User, assistant, and turn-ended records map root work and
completion without projecting assistant content, reasoning, tool arguments, or
tool output. A Cursor `Task` call becomes a child only when the durable provider
format supplies a stable child identity and matching completion evidence; an
unpaired tool call never creates a phantom child.
