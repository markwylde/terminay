...
- [x] Replace opaque cross-window adopted-project payloads with server move
  commands while preserving current drag UX. The canonical ownership boundary
  is covered by a write-scoped `project.move` operation and
  `WorkspaceClient.moveProject`; the compatibility drag/adoption adapter stays
  intact, and the real framed client/server test preserves panel and session
  ids. Broad UI drag/adoption E2E remains open below.
- [x] Remove PTY ownership from `webContentsId`; renderer subscriptions become
  detachable consumers for the server-owned PTY path. `ServerTerminalAuthority`
  now uses bounded client consumer tokens and keeps numeric renderer ids only
  as a compatibility subscription alias.
...
- [x] Preserve current project drag, split, rename, close, popout, and adoption
  E2E coverage through the compatibility adapter. Real Electron coverage now
  verifies project-tab tear-off/adoption, edit/rename/close, terminal splits,
  panel popout, root adoption, and terminal/project close behavior
  (`e2e/project-tabs.spec.ts` 2/2; `e2e/workspace.spec.ts` 7/7).
