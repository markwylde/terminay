...
export function ShellProfilesSettings({ client, serverIdentity }: ShellProfilesSettingsProps) {
	...
	if (!catalogue) {
		return <div className="settings-group shell-profiles-loading" role={loadError ? 'alert' : 'status'}>{loadError ?? 'Loading shell profiles…'}</div>;
	}
	...
	return <div className="shell-profiles" aria-busy={busy}>
		...
		<div className="settings-group shell-profiles-overview">
			<div className="settings-row shell-profiles-defaults"><label className="settings-row-info" htmlFor="shell-profile-default"><span className="settings-row-label">Default shell profile</span><span className="settings-row-description">Used by new terminals unless a project or one-time choice overrides it.</span></label><div className="settings-row-control shell-profiles-defaults__control"><select id="shell-profile-default" className="settings-select" value={catalogue.defaultProfileId} disabled={busy} onChange={(event) => void runMutation((revision) => client.setDefault(event.target.value, { expectedRevision: revision }), 'Server default profile changed.')}>
			...
			<div className="settings-row shell-profiles-defaults"><label className="settings-row-info" htmlFor="shell-profile-cwd-policy"><span className="settings-row-label">New terminals start in</span><span className="settings-row-description">Current terminal or panel falls back safely to the project folder.</span></label><div className="settings-row-control shell-profiles-defaults__control"><select id="shell-profile-cwd-policy" className="settings-select" value={catalogue.cwdPolicy} disabled={busy} onChange={(event) => void runMutation((revision) => client.setCwdPolicy(event.target.value as ShellProfileCwdPolicy, { expectedRevision: revision }), 'Working-directory policy changed.')}>
		...
		<div className="shell-profiles-toolbar">
			<label className="shell-profiles-search"><span className="sr-only">Search shell profiles</span><input className="settings-input-text" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search profiles" /></label>
			<div className="shell-profiles-toolbar__actions"><button type="button" className="settings-secondary-button settings-secondary-button--small" disabled={busy} onClick={() => void runMutation((revision) => client.refresh({ expectedRevision: revision }), 'Shell discovery refreshed.')}>Refresh discovery</button>
				...
				<button type="button" className="settings-primary-button settings-primary-button--small" disabled={busy || catalogue.entries.filter((profile) => profile.kind === 'custom').length >= 64} title={catalogue.entries.filter((profile) => profile.kind === 'custom').length >= 64 ? 'This server already has the maximum of 64 custom profiles.' : 'Create a custom shell profile'} onClick={() => { setEditingExisting(false); setEditing(editableProfile()); }}>New profile</button></div>
		</div>
		<div className="shell-profile-groups">
			{groups.map((group) => group.entries.length ? <section key={group.kind} className="shell-profile-group" aria-labelledby={`shell-profile-group-${group.kind}`}><h4 className="settings-section-title" id={`shell-profile-group-${group.kind}`}>{group.title}</h4><div className="settings-group shell-profile-list">
				{group.entries.map((profile) => <article key={profile.id} className={`shell-profile-card${profile.availability.available ? '' : ' shell-profile-card--unavailable'}`}>
					...
					<div className="shell-profile-card__actions">{profile.kind === 'custom' ? <><button type="button" className="settings-secondary-button settings-secondary-button--small shell-profile-card__move" disabled={busy} onClick={() => void moveProfile(profile, -1)} aria-label={`Move ${profile.name} up`}>↑</button><button type="button" className="settings-secondary-button settings-secondary-button--small shell-profile-card__move" disabled={busy} onClick={() => void moveProfile(profile, 1)} aria-label={`Move ${profile.name} down`}>↓</button><button type="button" className="settings-secondary-button settings-secondary-button--small" disabled={busy} onClick={() => void editCustomProfile(profile)}>Edit</button><button type="button" className="settings-secondary-button settings-secondary-button--small" disabled={busy || catalogue.defaultProfileId === profile.id || profile.projectReferences.length > 0} title={catalogue.defaultProfileId === profile.id ? 'Choose another server default before deleting this profile.' : profile.projectReferences.length > 0 ? 'Choose another default in the referenced projects before deleting this profile.' : 'Delete profile'} onClick={() => void deleteProfile(profile)}>Delete</button></> : <button type="button" className="settings-secondary-button settings-secondary-button--small" disabled={busy} onClick={() => { setEditingExisting(false); setEditing(editableProfile(profile)); }}>Copy</button>}</div>
				</article>)}
			</div></section> : null)}
		...
		{editing ? <div className="shell-profile-editor-backdrop" role="presentation"><form ref={editorRef} className="shell-profile-editor" role="dialog" aria-modal="true" aria-labelledby="shell-profile-editor-title" onSubmit={(event) => void saveProfile(event)}>
			...
			<div className="shell-profile-editor__fields"><label><span>Name</span><input value={editing.name} maxLength={128} autoComplete="off" onChange={(event) => setEditing({ ...editing, name: event.target.value })} aria-invalid={Boolean(fieldErrors.name)} aria-describedby={fieldErrors.name ? 'shell-profile-error-name' : undefined} />{fieldErrors.name ? <small id="shell-profile-error-name" className="shell-profile-field-error">{fieldErrors.name}</small> : null}</label>
				<div className="shell-profile-editor__row"><label><span>Target type</span><select aria-label="Target type" value={editing.targetKind} onChange={(event) => setEditing({ ...editing, targetKind: event.target.value as EditableProfile['targetKind'] })}><option value="executable">Executable</option><option value="wsl">Windows Subsystem for Linux</option><option value="system">System default</option></select></label><label><span>Startup mode</span><select aria-label="Startup mode" value={editing.startupMode} onChange={(event) => setEditing({ ...editing, startupMode: event.target.value as EditableProfile['startupMode'] })}><option value="default">Shell default</option><option value="login">Login</option><option value="non-login">Non-login</option></select></label></div>
				{editing.targetKind === 'executable' ? <label><span>Executable</span><input aria-label="Executable" value={editing.executable} placeholder="/bin/zsh" onChange={(event) => setEditing({ ...editing, executable: event.target.value })} />{fieldErrors['target.executable'] ? <small className="shell-profile-field-error">{fieldErrors['target.executable']}</small> : null}</label> : editing.targetKind === 'wsl' ? <div className="shell-profile-editor__row"><label><span>WSL distribution</span><input value={editing.distribution} onChange={(event) => setEditing({ ...editing, distribution: event.target.value })} /></label><label><span>Shell path (optional)</span><input value={editing.shellPath} placeholder="/bin/bash" onChange={(event) => setEditing({ ...editing, shellPath: event.target.value })} /></label></div> : <p className="shell-profile-editor__hint">Terminay resolves the account shell on this server each time a new terminal is created.</p>}
