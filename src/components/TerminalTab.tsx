...
export function TerminalTab(
	...
	const displayedActivityState =
		...
		(terminalActivityState === 'recent' &&
			params?.showActiveTabActivityIndicator !== true) ||
		(terminalActivityState === 'unviewed' &&
			params?.showFinishedTabActivityIndicator === false)
			? 'viewed'
	...
	const hasCustomColor =
		typeof color === 'string' && color !== DEFAULT_TERMINAL_TAB_COLOR;
	...
	const activeMacroCount = macroRuns.filter(
		(run) => run.status === 'running' || run.status === 'canceling',
	).length;
	const finishedMacroCount = macroRuns.filter(
		(run) => run.status !== 'running' && run.status !== 'canceling',
	).length;
	...
	const dispatchNewTerminalWithProfileEvent = (target: HTMLElement) => {
		target.dispatchEvent(
			new CustomEvent('terminay-new-terminal-with-profile', { bubbles: true }),
		);
	};
	...
	const contextMenuItems: ContextMenuItem[] = [
		{
			...
			onClick: () =>
				dispatchNewTerminalWithProfileEvent(
					contextMenuTargetRef.current ?? document.body,
				),
		},
		{
			label: 'Close',
	...
	const renderStepIcon = (status: TerminalTabMacroRunStep['status']) => {
		switch (status) {
			case 'completed':
				return (
					<CheckCircle2
						className="terminal-tab-macro-step__icon terminal-tab-macro-step__icon--completed"
						aria-hidden="true"
					/>
				);
			case 'running':
				return (
					<LoaderCircle
						className="terminal-tab-macro-step__icon terminal-tab-macro-step__icon--running"
						aria-hidden="true"
					/>
				);
			case 'failed':
				return (
					<AlertCircle
						className="terminal-tab-macro-step__icon terminal-tab-macro-step__icon--failed"
						aria-hidden="true"
					/>
				);
			case 'canceled':
				return (
					<XCircle
						className="terminal-tab-macro-step__icon terminal-tab-macro-step__icon--canceled"
						aria-hidden="true"
					/>
				);
			case 'pending':
				return (
					<Circle
						className="terminal-tab-macro-step__icon terminal-tab-macro-step__icon--pending"
						aria-hidden="true"
					/>
				);
		}
	...
	const macroTrigger = hasMacroHistory ? (
		...
		>
			<button
				...
				title={
					activeMacroCount > 0
						? `${activeMacroCount} running macro${activeMacroCount === 1 ? '' : 's'}`
						: `${macroRuns.length} recent macro run${macroRuns.length === 1 ? '' : 's'}`
				}
			>
				<span
			...
			<span
				...
				title={
					recordingStatus === 'recording'
						...
						: (params?.recordingError ?? 'Recording failed')
				}
				role="img"
				aria-label={
					recordingStatus === 'recording'
				...
				? createPortal(
						...
						>
							<div className="terminal-tab-macro-popover__header">
								<div className="terminal-tab-macro-popover__label">
									Macro queue
								</div>
									...
									return (
										<div key={run.id} className="terminal-tab-macro-run">
											...
											>
												{isExpanded ? (
													<ChevronDown
														className="terminal-tab-macro-run__expand"
														aria-hidden="true"
													/>
												) : (
													<ChevronRight
												...
												<span className="terminal-tab-macro-run__title">
													{run.title}
												</span>
												{!isActive ? (
													<button
													...
													>
														<Trash2
															className="terminal-tab-macro-run__clear-icon"
															aria-hidden="true"
														/>
													</button>
												) : null}
												<span
													className={`terminal-tab-macro-run__status terminal-tab-macro-run__status--${run.status}`}
												>
													{run.status}
												</span>
											</button>
												...
												<div className="terminal-tab-macro-run__body">
													<div className="terminal-tab-macro-steps">
														{run.steps.map((step) => (
															<div
																key={step.id}
																className="terminal-tab-macro-step"
															>
																<span
																	className="terminal-tab-macro-step__marker"
																	aria-hidden="true"
																>
																	{renderStepIcon(step.status)}
																</span>
																<span
																	className={`terminal-tab-macro-step__title terminal-tab-macro-step__title--${step.status}`}
																>
																	{step.title}
																</span>
															</div>
