...
type PerformantTextViewerProps = {
	canSwitchToMonaco: boolean;
	fileViewerClient: FileViewerClient;
	filePath: string;
	onSparseEditChange: (
		owner: string,
		edit: FileViewerSparseFileEdit | null,
		lineDelta?: number,
	) => void;
...
const MAX_WINDOW_INDEX_ATTEMPTS = 32;
...
function projectPageText(
	...
	return new TextDecoder('utf-8', { fatal: true, ignoreBOM: true }).decode(
		applySparseEdits(original, localEdits),
	);
}
...
function toTextPage(
	...
	if (new TextEncoder().encode(originalText).byteLength !== end - first.start) {
		throw new Error(
			'The ranged UTF-8 page does not match its original byte boundaries.',
		);
	}
	return {
		end,
...
async function readCanonicalTextPage(
	client: FileViewerClient,
	filePath: string,
	projectId: string,
	page: number,
	signal: AbortSignal,
): Promise<{ lines: readonly FileViewerTextLine[] }> {
	const requestedStartLine = page * PAGE_LINES;
	for (let attempt = 0; attempt < MAX_WINDOW_INDEX_ATTEMPTS; attempt += 1) {
		const result = await client.readServerTextLines(
			filePath,
			requestedStartLine,
			PAGE_LINES,
			projectId,
...
export function PerformantTextViewer({
	...
	const findPageAtLogicalLine = useCallback(
		(logicalLine: number) => {
			const pageCount = Math.max(
				1,
				Math.ceil((metadata?.lineCount ?? 1) / PAGE_LINES),
			);
			let page = Math.max(
				0,
				Math.min(pageCount - 1, Math.floor(logicalLine / PAGE_LINES)),
			);
			...
			while (
				page + 1 < pageCount &&
				getLogicalPageStart(page + 1) <= logicalLine
			) {
				page += 1;
			}
			return page;
	...
	}, [canonicalFilePath, filePath, fileViewerClient, projectId, projectRoot]);

	const visibleRange = useMemo(() => {
		const lineCount = Math.max(1, (metadata?.lineCount ?? 0) + totalLineDelta);
		const startLine = Math.max(
			0,
			Math.floor(scrollTop / ROW_HEIGHT) - OVERSCAN_LINES,
		);
		const endLine = Math.min(
			lineCount,
			Math.ceil(
				(scrollTop + Math.max(viewportHeight, ROW_HEIGHT)) / ROW_HEIGHT,
			) + OVERSCAN_LINES,
		);
	...
	useEffect(() => {
		...
		const reservedPages = new Map<number, symbol>();
		for (const page of missingPages) {
			...
			void pageRequest
				.then((result) => {
					...
					setPages((current) => {
						...
						const retained = [...loadedPagesRef.current].sort(
							(left, right) =>
								Math.abs(left - nearestPage) - Math.abs(right - nearestPage),
						);
				...
				.finally(() => {
					if (loadingPagesRef.current.get(page) === reservation) {
					...
					if (
						...
						setPageRequestVersion((version) => version + 1);
					}
				});
		}
		return () => {
			// React runs this cleanup before the replacement effect. Release the
			// old effect's reservations synchronously so that replacement can
			// immediately request the same visible page with the updated sparse
			// line projection. Waiting for the aborted promise's `finally` leaves
			// the page marked as loading with no render scheduled to retry it.
			for (const [page, reservation] of reservedPages) {
				if (loadingPagesRef.current.get(page) === reservation) {
					loadingPagesRef.current.delete(page);
				}
			}
	...
	if (error && !metadata) {
		return (
			<div className="file-preview-unsupported">
				Unable to open Performant text mode: {error}
			</div>
		);
	}
	...
	if (!metadata) {
		return (
			<div className="file-panel file-panel--loading">Indexing text file…</div>
		);
	}
		...
		<div className="file-performant-text-viewer">
			<div className="file-performant-text-viewer__toolbar">
				...
				{canSwitchToMonaco ? (
					...
					<span>Ranged mode is required above 128 MB</span>
				)}
			</div>
			...
			>
				<div
					...
					style={{
						height:
							Math.max(1, metadata.lineCount + totalLineDelta) * ROW_HEIGHT,
					}}
				>
					{visiblePages.map((pageNumber) => {
						...
						const originalPageLineCount = Math.max(
							...
							Math.min(
								PAGE_LINES,
								metadata.lineCount - pageNumber * PAGE_LINES,
							),
						);
						const pageLineDelta = pageLineDeltas.get(pageNumber) ?? 0;
						const height =
							Math.max(1, originalPageLineCount + pageLineDelta) * ROW_HEIGHT;
						...
						return (
							...
							>
								{page ? (
									<>
										<div
											className="file-performant-text-page__numbers"
											aria-hidden="true"
										>
											...
											onChange={(event) => {
												...
												if (nextText === page.originalText) {
													...
													onSparseEditChange(
														...
														countNewlines(nextText) -
															countNewlines(page.originalText),
													);
												}
											}}
										/>
									...
									<span className="file-performant-text-page__loading">
										Loading lines {firstLineNumber.toLocaleString()}–
										{lastLineNumber.toLocaleString()}…
									</span>
								)}
							</div>
						);
