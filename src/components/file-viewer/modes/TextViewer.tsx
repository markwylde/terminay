...
>) {
	...
	const [estimatedBytesPerLine, setEstimatedBytesPerLine] = useState(
		ESTIMATED_BYTES_PER_LINE,
	);
	const pageBytes = Math.max(
		1024,
		Math.min(64 * 1024, estimatedBytesPerLine * PERFORMANT_PAGE_LINES),
	);
	...
	const firstRow = Math.max(
		0,
		Math.floor(localScrollTop / PERFORMANT_ROW_HEIGHT) -
			PERFORMANT_OVERSCAN_ROWS,
	);
	const lastRow = Math.min(
		PERFORMANT_PAGE_LINES,
		Math.ceil((localScrollTop + viewportHeight) / PERFORMANT_ROW_HEIGHT) +
			PERFORMANT_OVERSCAN_ROWS,
	);
				...
				if (requestOffset > 0) {
					...
					nextText =
						firstLineEnd === -1
							? ''
							: nextText.slice(
									firstLineEnd + (nextText[firstLineEnd] === '\r' ? 2 : 1),
								);
				}
				const rawLines = splitLines(nextText);
				if (pageIndex === 0 && rawLines.length > 1) {
					setEstimatedBytesPerLine(
						Math.max(
							8,
							Math.min(
								2048,
								Math.round(response.text.length / rawLines.length),
							),
						),
					);
				}
	...
	return (
		...
		>
			<div
				className="file-text-virtual-surface__content"
				style={{ height: totalHeight }}
			>
				{page ? (
					<div
						className="file-text-virtual-page"
						style={{
							height: pageHeight,
							transform: `translateY(${pageTop}px)`,
						}}
					>
						<div className="file-text-virtual-page__gutter" aria-hidden="true">
							{visibleLines.map((_, index) => (
								<span
									...
									style={{
										transform: `translateY(${(firstRow + index) * PERFORMANT_ROW_HEIGHT}px)`,
									}}
								>
			...
			{loadingPage !== null ? (
				<div className="file-text-viewer__loading">Loading text window…</div>
			) : null}
			{loadError ? (
				<div className="file-text-viewer__error">
					Unable to load this text window: {loadError}
				</div>
			) : null}
			<div className="file-text-viewer__selection" aria-live="polite">
				{selectionLength > 0
					? `Selected ${selectionLength.toLocaleString()} characters in the loaded window.`
					: 'Selection is limited to the loaded window.'}
			</div>
		</div>
	);
}
