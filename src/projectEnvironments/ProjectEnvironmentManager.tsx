	...
	return (
		<div className="environment-manager">
			<header className="management-route-header">
				<div>
					<p className="management-route-eyebrow">Selected Terminay Server</p>
					<h2>Project Environments</h2>
					<p>
						Profiles and credentials are stored by <strong>{serverName}</strong>
						.
					</p>
				</div>
			...
			<div className="management-route-grid">
				<aside
					className="management-route-list"
					aria-label="Project environments"
				>
					<label>
						<span className="sr-only">Search project environments</span>
						<input
							type="search"
							value={query}
					...
					{filtered.map((item) => (
						...
						>
							<span>
								...
								<small>
									{item.providerLabel} · {item.endpointSummary}
								</small>
							</span>
							<span
								className={`environment-status environment-status--${item.status}`}
							>
				...
				<section className="management-route-detail" aria-live="polite">
					{selected ? (
						<>
							<div className="management-route-title">
								<span className="management-route-icon" aria-hidden="true">
									{selected.isThisServer
										? '›_'
										: selected.providerLabel === 'SSH'
											? '⇄'
											: '⬡'}
								</span>
								<div>
									<h3>{selected.name}</h3>
									<p>{selected.providerLabel}</p>
								</div>
							</div>
							<dl>
								<div>
								...
								<div>
									<dt>Endpoint</dt>
									<dd>{selected.endpointSummary}</dd>
								</div>
								<div>
									<dt>Default root</dt>
									<dd>{selected.defaultRoot ?? 'Environment home'}</dd>
								</div>
								<div>
									<dt>Projects</dt>
									<dd>{selected.referencedProjectCount}</dd>
								</div>
							</dl>
							{selected.statusCard ? (
								<section
									className={`environment-trust-card environment-trust-card--${selected.statusCard.tone ?? 'neutral'}`}
									aria-label={selected.statusCard.title}
								>
									<h4>{selected.statusCard.title}</h4>
									<p>{selected.statusCard.summary}</p>
									<dl>
									...
									<div className="management-route-actions">
										{selected.statusCard.actions.map((action) => (
											<button
												...
												type="button"
												className={
													action.kind === 'destructive' ? 'danger' : undefined
												}
							...
							<div className="management-route-actions">
								<button
									type="button"
									disabled={
										selected.isThisServer || selected.profileId === undefined
									}
									onClick={() =>
										selected.profileId && onTest(selected.profileId)
									}
								>
									Test
								</button>
								...
								<button
									...
									onClick={() =>
										selected.profileId && onRemove(selected.profileId)
									}
								>
									Remove
								</button>
							...
							{selected.referencedProjectCount > 0 ? (
								<p className="management-route-note">
									Removal is blocked while projects reference this environment.
								</p>
							) : null}
						</>
					) : (
