...
import type { OutboundClientMessage, RemoteMessageSocket, RemoteSocketState } from './socket'
...
type WebRtcBootstrap = {
  ...
  getChannel?: (name: 'api' | 'terminal') => RTCDataChannel | Promise<RTCDataChannel>
  relayOrigin?: string
  sessionId?: string
...
function getQueryTransportMode(searchParams: URLSearchParams): RemoteTransportMode | null {
...
function persistRuntimeHints(mode: RemoteTransportMode, sessionId?: string): void {
  try {
...
function getSessionId(searchParams: URLSearchParams, config: RemoteRuntimeConfig, bridge?: WebRtcBootstrap): string | undefined {
  const querySessionId = searchParams.get('sessionId') ?? searchParams.get('webrtcSessionId') ?? undefined
  if (querySessionId) {
...
function createPairingOrigin(mode: RemoteTransportMode, relayOrigin?: string): string {
  if (mode === 'local') {
...
async function resolveWebRtcChannel(name: 'api' | 'terminal'): Promise<RTCDataChannel> {
  const bridge = getRemoteWindow().__TERMINAY_REMOTE_WEBRTC__
  const directChannel = name === 'api' ? bridge?.apiChannel : bridge?.terminalChannel
  ...
  throw new Error('WebRTC transport is not ready. Reopen this page from the Terminay WebRTC QR code.')
}
...
class WebRtcApiTransport implements RemoteApiTransport {
  ...
  async postJson<TResponse>(pathname: string, body: unknown): Promise<TResponse> {
    ...
    return new Promise<TResponse>((resolve, reject) => {
      this.pending.set(id, { reject, resolve: (value) => resolve(value as TResponse) })
...
class WebRtcTerminalSocket implements RemoteMessageSocket {
  ...
  async connect(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const handleMessage = (event: MessageEvent<string>) => {
        const message = parseJsonMessage(event.data) as RemoteServerMessage | null
      ...
      const handleClose = () => {
        if (!settled) {
          settled = true
          reject(new Error('WebRTC terminal channel closed before initialization completed.'))
        }
        this.hasHandshake = false
        this.connectionId = ''
      ...
      channel.addEventListener('close', handleClose)
      channel.addEventListener('error', handleError)
      channel.send(JSON.stringify({ ticket: this.ticket, type: 'terminal-auth' }))
    })
  }
  ...
  send(message: OutboundClientMessage): void {
    if (!this.channel || this.channel.readyState !== 'open' || !this.hasHandshake || !this.connectionId) {
...
export function createRemoteTransportRuntime(): RemoteTransportRuntime {
  ...
  const queryMode = getQueryTransportMode(searchParams)
  const configMode = normalizeTransportMode(config.transport)
  const sessionMode = getSessionTransportMode()
  const hostedWebRtcMode = window.location.hostname === 'app.terminay.com' && Boolean(bridge ?? config.sessionId)
  const mode = configMode ?? queryMode ?? sessionMode ?? (hostedWebRtcMode ? 'webrtc' : 'local')
