...
export function SharedEditTabRouteBody({ state, onCancel, onSubmit }: SharedEditTabRouteBodyProps) {
  ...
  return <div className="edit-window-shell" data-shared-route-body="edit-tab">
    <form className="edit-window-card" onSubmit={save} onKeyDown={saveOnEnter}>
      ...
      <div className="edit-window-preview-section"><div className="edit-window-preview-label">Preview</div><div className={`edit-window-preview-container edit-window-preview-container--${state.kind}`} style={{ '--project-color': previewColor } as CSSProperties}>
        {state.kind === 'project' ? <div className="tab-preview-project"><span className="tab-preview-project-main">{previewEmoji ? <span className="tab-preview-project-emoji" aria-hidden="true">{previewEmoji}</span> : null}<span className="tab-preview-project-title">{previewTitle}</span></span><PreviewClose className="tab-preview-project-close" /></div>
          : <div className="tab-preview-terminal">{previewEmoji ? <span className="tab-preview-terminal-emoji" aria-hidden="true">{previewEmoji}</span> : null}<span className="tab-preview-terminal-title">{previewTitle}</span><PreviewClose className="tab-preview-terminal-close" /></div>}
      </div></div>
