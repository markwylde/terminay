import type { DocumentationCatalog, DocumentationClient, FileObservationClient, FileWatchHandle } from '@terminay/client-core';
...
export function useDocumentationController(options: {
	...
	const [expandedFolders, setExpandedFolders] = useState<ReadonlySet<string>>(() => new Set(expandedFolderIds));
	const requestRef = useRef(0);
	const timerRef = useRef<number | undefined>(undefined);
	const catalogRef = useRef<DocumentationCatalog | undefined>(undefined);
	catalogRef.current = catalog;
	const refresh = useCallback((immediate = true) => {
		if (client === undefined) return;
		const run = () => {
			const request = ++requestRef.current;
			setLoading(true);
			void client.catalog(projectId, catalogRef.current?.revision).then((next) => {
	...
	useEffect(() => {
		...
		void observationClient.startWatch(projectId, '').then(async (next) => {
			if (disposed) { await observationClient.stopWatch(next.subscriptionId); return; }
			handle = next;
